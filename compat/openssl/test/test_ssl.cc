...
TEST(SSLTest, test_SSL_CTX_set_select_certificate_cb) {
  ...
  std::thread server([&]() {
    ...
    SSL_CTX_set_select_certificate_cb(
        ctx.get(), [](const SSL_CLIENT_HELLO* client_hello) -> enum ssl_select_cert_result_t {
          std::set<uint16_t>* received_ext_types = static_cast<std::set<uint16_t>*>(
              SSL_CTX_get_ex_data(SSL_get_SSL_CTX(client_hello->ssl), 0));
  ...
  std::set<uint16_t> expected_ext_types{
      TLSEXT_TYPE_extended_master_secret, TLSEXT_TYPE_supported_groups,
      TLSEXT_TYPE_ec_point_formats,       TLSEXT_TYPE_session_ticket,
      TLSEXT_TYPE_signature_algorithms,   TLSEXT_TYPE_key_share,
      TLSEXT_TYPE_psk_key_exchange_modes, TLSEXT_TYPE_supported_versions};
...
TEST(SSLTest, SSL_CIPHER_get_min_version) {
  ...
  for (int i = 0; i < sk_SSL_CIPHER_num(ciphers); i++) {
    ...
    if (boring_cipher_versions.find(name) != boring_cipher_versions.end()) {
      EXPECT_STREQ(version_str(boring_cipher_versions[name]), version_str(min_version))
          << " for " << name;
    }
  }
}
...
TEST(SSLTest, test_SSL_get_peer_full_cert_chain) {
...

  {
    ...
    ASSERT_EQ(1, SSL_CTX_use_certificate_chain_file(ctx.get(), client_2_cert_chain_pem.path()))
        << (ERR_print_errors_fp(stderr), "");
...
TEST(SSLTest, test_SSL_CTX_set_verify_algorithm_prefs) {
  ...
  uint16_t prefs[]{SSL_SIGN_RSA_PSS_RSAE_SHA256, SSL_SIGN_ECDSA_SECP256R1_SHA256};

  ASSERT_EQ(
      1, SSL_CTX_set_verify_algorithm_prefs(ctx.get(), prefs, (sizeof(prefs) / sizeof(prefs[0]))));
}
...
TEST(SSLTest, test_SSL_early_callback_ctx_extension_get) {
  ...
  std::thread server([&]() {
    ...
    SSL_CTX_set_select_certificate_cb(
        ctx.get(), [](const SSL_CLIENT_HELLO* client_hello) -> ssl_select_cert_result_t {
          const uint8_t* tlsext_data;
          size_t tlsext_len;

          if (SSL_early_callback_ctx_extension_get(client_hello, TLSEXT_TYPE_server_name,
                                                   &tlsext_data, &tlsext_len)) {
            ...
            if (CBS_len(&server_name_extension) == (2 + 1 + 2 + strlen(SERVERNAME)) &&
                ...
                CBS_get_u16_length_prefixed(&server_name_extension_bytes,
                                            &server_name_extension_name) &&
                CBS_len(&server_name_extension_name) == strlen(SERVERNAME) &&
                memcmp(CBS_data(&server_name_extension_name), SERVERNAME, strlen(SERVERNAME)) ==
                    0) {
              return ssl_select_cert_success;
            }
          }
...
TEST_P(SSLTestWithSigalgs, test_SSL_get_peer_signature_algorithm) {
...

  {
    ...
    if (sigalgs.client_sigalgs) {
      ASSERT_EQ(1, SSL_CTX_set1_sigalgs_list(ctx.get(), sigalgs.client_sigalgs))
          << ERR_error_string(ERR_get_error(), nullptr);
    }
    bssl::UniquePtr<SSL> ssl(SSL_new(ctx.get()));
...
INSTANTIATE_TEST_SUITE_P(SSLTestWithSigalgs, SSLTestWithSigalgs,
                         ::testing::Values(Sigalgs{"rsa_pss_rsae_sha256", "rsa_pss_rsae_sha256",
                                                   SSL_SIGN_RSA_PSS_RSAE_SHA256},
                                           Sigalgs{"rsa_pss_rsae_sha384", "rsa_pss_rsae_sha384",
                                                   SSL_SIGN_RSA_PSS_RSAE_SHA384},
                                           Sigalgs{"rsa_pss_rsae_sha512", "rsa_pss_rsae_sha512",
...
TEST(SSLTest, test_SSL_get_signature_algorithm_name) {
  ...
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_ECDSA_SECP256R1_SHA256, 0),
               "ecdsa_sha256");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_ECDSA_SECP256R1_SHA256, 1),
               "ecdsa_secp256r1_sha256");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_ECDSA_SECP384R1_SHA384, 0),
               "ecdsa_sha384");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_ECDSA_SECP384R1_SHA384, 1),
               "ecdsa_secp384r1_sha384");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_ECDSA_SECP521R1_SHA512, 0),
               "ecdsa_sha512");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_ECDSA_SECP521R1_SHA512, 1),
               "ecdsa_secp521r1_sha512");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_RSA_PSS_RSAE_SHA256, 0),
               "rsa_pss_rsae_sha256");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_RSA_PSS_RSAE_SHA384, 0),
               "rsa_pss_rsae_sha384");
  EXPECT_STREQ(SSL_get_signature_algorithm_name(SSL_SIGN_RSA_PSS_RSAE_SHA512, 0),
               "rsa_pss_rsae_sha512");
...
TEST(SSLTest, test_SSL_set_chain_and_key) {
  ...
  std::thread server([&]() {
    ...
    for (const char* pem_str :
         {server_2_cert_pem_str, intermediate_ca_2_cert_pem_str, intermediate_ca_1_cert_pem_str}) {
      bssl::UniquePtr<BIO> bio(BIO_new_mem_buf(pem_str, strlen(pem_str)));
      long der_len = 0;
      uint8_t* der_data = nullptr;
      ASSERT_EQ(1, PEM_bytes_read_bio(&der_data, &der_len, nullptr, PEM_STRING_X509, bio.get(),
                                      nullptr, nullptr));
...
TEST(SSLTest, test_SSL_SESSION_from_bytes) {
...

  {
    ...
    bssl::UniquePtr<SSL_SESSION> session2{
        SSL_SESSION_from_bytes(session1_buf, session1_len, ctx.get())};
...
TEST(SSLTest, test_SSL_ex_data) {
  ...
  auto free_func = [](void* parent, void* ptr, CRYPTO_EX_DATA* ad, int index, long argl,
                      void* argp) {
...
TEST(SSLTest, test_SSL_set_alpn_protos) {
  ...
  static const uint8_t server_protos[]{
      2, 'h', '2', 8, 'h', 't', 't', 'p', '/', '1', '.', '1', 6, 's', 'p', 'd', 'y', '/', '1',
  };

  static const uint8_t client_protos[]{6,   's', 'p', 'd', 'y', '/', '1', 8,
                                       'h', 't', 't', 'p', '/', '1', '.', '1'};
  ...
  SSL_CTX_set_alpn_select_cb(
      server_ctx.get(),
      [](SSL* ssl, const uint8_t** out, uint8_t* out_len, const uint8_t* in, unsigned in_len,
         void* arg) -> int {
        ...
        if (SSL_select_next_proto(const_cast<uint8_t**>(out), out_len, in, in_len, server_protos,
                                  sizeof(server_protos)) == OPENSSL_NPN_NEGOTIATED) {
          return SSL_TLSEXT_ERR_OK;
        }
...
TEST(SSLTest, test_SSL_get_signature_algorithm_key_type) {
  ...
  for (int i = 0; i < (sizeof(keytypes) / sizeof(keytypes[0])); i++) {
    EXPECT_EQ(keytypes[i].keytype, SSL_get_signature_algorithm_key_type(keytypes[i].sigalg))
        << "keytypes[" << i << "]";
  }
}
...
TEST(SSLTest, test_SSL_is_signature_algorithm_rsa_pss) {
  ...
  for (int i = 0; i < (sizeof(sigalgs) / sizeof(sigalgs[0])); i++) {
    EXPECT_EQ(sigalgs[i].is_rsa_pss, SSL_is_signature_algorithm_rsa_pss(sigalgs[i].sigalg))
        << "sigalgs[" << i << "]";
  }
}
...
TEST(SSLTest, test_SSL_get0_peer_verify_algorithms) {
  ...
  ASSERT_EQ(
      1, SSL_CTX_set1_sigalgs_list(client_ctx.get(),
...
TEST(SSLTest, test_SSL_get_servername_inside_select_certificate_cb) {
  ...
  SSL_CTX_set_select_certificate_cb(
      server_ctx.get(), [](const SSL_CLIENT_HELLO* client_hello) -> ssl_select_cert_result_t {
...
TEST(SSLTest, test_SSL_get_servername_null_inside_select_certificate_cb) {
  ...
  SSL_CTX_set_select_certificate_cb(
      server_ctx.get(), [](const SSL_CLIENT_HELLO* client_hello) -> ssl_select_cert_result_t {
        return SSL_get_servername(client_hello->ssl, TLSEXT_NAMETYPE_host_name)
...
TEST(SSLTest, test_SSL_CTX_set_select_certificate_cb_leak_from_callback_exception) {
  ...
  EXPECT_THROW(CompleteHandshakes(client_ssl.get(), server_ssl.get()), std::runtime_error);
}
...
TEST(SSLTest, test_SSL_CTX_set_custom_verify_alert_codes) {
  ...
  for (auto const& [alert_code, alert_string] : alerts) {
    ...
    ASSERT_TRUE(
        SSL_CTX_use_certificate_chain_file(server_ctx.get(), server_2_cert_chain_pem.path()));
    ASSERT_TRUE(
        SSL_CTX_use_PrivateKey_file(server_ctx.get(), server_2_key_pem.path(), SSL_FILETYPE_PEM));
    bssl::UniquePtr<SSL> server_ssl(SSL_new(server_ctx.get()));
    ASSERT_TRUE(SSL_set_fd(server_ssl.get(), sockets[0]));
    ...
    SSL_CTX_set_custom_verify(
        ...
        [](SSL * ssl, uint8_t * out_alert) -> enum ssl_verify_result_t {
