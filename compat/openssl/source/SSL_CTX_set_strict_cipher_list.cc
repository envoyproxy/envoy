...
extern "C" int SSL_CTX_set_strict_cipher_list(SSL_CTX* ctx, const char* str) {
  ...
  // The Envoy code uses BoringSSL's Equal Preference Group syntax when
  // specifying default cipher lists. However, OpenSSL doesn't support this
  // syntax so we need to preprocess osslstr before passing it to OpenSSL. There
  // is no equivalent mechanism in OpenSSL, so the best we can do is flatten
  // the syntax into a regular : separated list i.e. we simply remove all square
  // brackets, and swap | for :
  size_t length = 0;
  for (size_t i = 0; i < osslstr.size(); ++i) {
    if (osslstr[i] == '[' || osslstr[i] == ']') {
      continue; // remove square brackets
    }
    osslstr[length++] = (osslstr[i] == '|') ? ':' : osslstr[i];
  }
  osslstr.resize(length);
