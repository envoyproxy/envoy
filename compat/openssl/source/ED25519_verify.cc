...
/*
 * bssl:
 * https://github.com/google/boringssl/blob/098695591f3a2665fccef83a3732ecfc99acdcdd/src/include/openssl/curve25519.h#L90
 * ossl: None
 *
...
extern "C" int ED25519_verify(const uint8_t* message, size_t message_len,
  ...
  // bssl::UniquePtr<EVP_PKEY> pkey(EVP_PKEY_new_raw_public_key(EVP_PKEY_ED25519, NULL,
  // castToUChar(key), key.length()));
  //
  ...
  ossl_EVP_PKEY* pkey = ossl.ossl_EVP_PKEY_new_raw_public_key(
      ossl_EVP_PKEY_ED25519, NULL, reinterpret_cast<const unsigned char*>(public_key), 32);
  ...
  if (res == 1) {
    ...
    res = ossl.ossl_EVP_DigestVerify(ctx, reinterpret_cast<const unsigned char*>(signature), 64,
