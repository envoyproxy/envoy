...
struct OverrideResultBase {
  template <typename CTX> static int index();

  template <typename CTX> static void* get_ex_data(CTX ctx, int index);

  template <typename CTX> static void set_ex_data(CTX ctx, int index, void* data);
};
