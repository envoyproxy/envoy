...
# TODO(phlax): Remove WORKSPACE compat
_LEGACY_WORKSPACE_REPOS = [
    "liburing",
    "elfutils",
    "libbpf",
    "icu",
...
_STUB_BUILD = """\
# Generated by //bazel:extensions.bzl (legacy_workspace_compat_extension).
#
# This is a placeholder so that WORKSPACE-era labels (@{name}_workspace//:all)
# resolve under bzlmod. It carries no sources and cannot be built -- the
# bzlmod build must consume the @{name} module instead.

load("@envoy//bazel:legacy_workspace_compat.bzl", "legacy_workspace_stub")

legacy_workspace_stub(
    name = "all",
    dep = "{name}",
...
def _legacy_workspace_repo_impl(rctx):
    rctx.file("WORKSPACE", "")
    rctx.file("BUILD.bazel", _STUB_BUILD.format(name = rctx.attr.dep))

_legacy_workspace_repo = repository_rule(
    implementation = _legacy_workspace_repo_impl,
    attrs = {"dep": attr.string(mandatory = True)},
)

def _legacy_workspace_compat_impl(_):
    for dep in _LEGACY_WORKSPACE_REPOS:
        _legacy_workspace_repo(
            name = "%s_workspace" % dep,
            dep = dep,
        )

legacy_workspace_compat_extension = module_extension(
    implementation = _legacy_workspace_compat_impl,
)
