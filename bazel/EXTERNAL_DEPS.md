...
1. Add the source Bazel module to [`MODULE.bazel`](../MODULE.bazel) and its registry entry.
...
2. Update the corresponding metadata in [`bazel/deps.yaml`](deps.yaml).
3. `bazel test //test/...`
...
the dependency's module name in `MODULE.bazel`.
...
To prefetch `boringssl`, use the module archive selected by the registry:
