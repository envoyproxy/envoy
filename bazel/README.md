...
> **Note**: To avoid rate-limiting by GitHub API, you can provide a valid GitHub token to `GITHUB_TOKEN` environment variable.
> Example: `GITHUB_TOKEN=<GITHUB_TOKEN> python3 ./tools/github/write_current_source_version.py`.
> The environment variable name that holds the token can also be customized by setting `--github_api_token_env_name`.
> As an example: `MY_TOKEN=<GITHUB_TOKEN> python3 ./tools/github/write_current_source_version.py --github_api_token_env_name=MY_TOKEN`.
