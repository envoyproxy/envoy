...
#                Possible values are:
#                build - dependencies used in build process;
#                test - dependencies used for unit tests;
#                api - dependencies used in API protos;
#                dataplane - dependencies used in processing downstream or upstream requests;
#                controlplane - dependencies used to process xDS requests;
#                other - dependencies that do no handle untrusted data and are used for various utility purposes.

REPOSITORY_LOCATIONS_ANNOTATED = dict(
    bazel_compdb = dict(
...
def _strip_annotations():
    stripped = dict(REPOSITORY_LOCATIONS_ANNOTATED)
    for key, value in stripped.items():
        value.pop("use_category", None)

    return stripped

# REPOSITORY_LOCATIONS is used to initialize http_archive which produces errors for annotation
# attributes.
REPOSITORY_LOCATIONS = _strip_annotations()
