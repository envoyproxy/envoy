...
StringErrorCollector::StringErrorCollector(std::string& error_text,
    ...
    : error_text_(error_text),
      index_offset_(one_indexing ? 1 : 0),
      ignore_warnings_(ignore_warnings) {}

void StringErrorCollector::AddError(int line, ColumnNumber column,
                                    const std::string& message) {
  absl::SubstituteAndAppend(&error_text_, "$0($1): $2\n", line + index_offset_,
                            column + index_offset_, message);
}

void StringErrorCollector::AddWarning(int line, ColumnNumber column,
                                      const std::string& message) {
...
TEST(TextToBinaryReserializerTest, TextFormatWithExtensionWorks) {
  ...
  const auto& file_descriptor_info = protobuf::reflection::
      bazel_cc_proto_descriptor_library_testdata_test_extension::
          kFileDescriptorInfo;
...
[testdata.dynamic_descriptors.number]: 20
  ...
  ASSERT_THAT(concrete_message.bar(), Eq("hello world"));
  ASSERT_THAT(concrete_message.GetExtension(
                  testdata::dynamic_descriptors::number),
              Eq(20));
}
