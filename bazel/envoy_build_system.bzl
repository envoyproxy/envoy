...
def envoy_proto_descriptor(name, out, srcs = [], external_deps = []):
    ...
    # Track whether we need to compute paths dynamically
    ...
    if "well_known_protos" in external_deps:
        ...
        # Use any.proto as a marker to compute the include path dynamically
    ...
    if has_protobuf_deps or has_googleapis_deps:
        ...
        # Compute googleapis include path if needed
        if has_googleapis_deps and googleapis_include_marker:
            # Extract the directory path from googleapis proto location
            # e.g., external/com_google_googleapis/google/api/httpbody.proto -> external/com_google_googleapis
            cmd_parts.append("GOOGLEAPIS_PROTO_PATH=$(location " + googleapis_include_marker + ")")
            cmd_parts.append("GOOGLEAPIS_INCLUDE_PATH=$${GOOGLEAPIS_PROTO_PATH%/google/api/*}")

        # Compute protobuf include path if needed
        if has_protobuf_deps and protobuf_include_marker:
            # Extract the directory containing the protobuf proto files at build time
            # e.g., external/com_google_protobuf~/src/google/protobuf/any.proto -> external/com_google_protobuf~/src
            cmd_parts.append("ANY_PROTO_PATH=$(location " + protobuf_include_marker + ")")
            cmd_parts.append("PROTOBUF_INCLUDE_PATH=$${ANY_PROTO_PATH%/google/protobuf/*}")

        cmd_parts.append("INCLUDE_OPTS=\"--include_imports\"")

        # Add all static include paths
        for include_path in include_paths:
            cmd_parts.append("INCLUDE_OPTS=\"$$INCLUDE_OPTS -I" + include_path + "\"")

        # Add dynamic include paths
        ...
        cmd_parts.append("$(location @com_google_protobuf//:protoc) $$INCLUDE_OPTS --descriptor_set_out=$@ " + " ".join(input_files))
        cmd = " && ".join(cmd_parts)
    else:
