...
public:
  ...
  void initializeFilter(const std::string& yaml) {
    ...
    conn_manager_ = std::make_unique<ConnectionManager>(
        *config_, factory_context_.getServerFactoryContext().mainThreadDispatcher().timeSource());
    conn_manager_->initializeReadFilterCallbacks(filter_callbacks_);
    conn_manager_->onNewConnection();
    current_ = factory_context_.getServerFactoryContext()
  ...
  void initializeCluster() {
    ...
    factory_context_.mock_server_context_.cluster_manager_.initializeThreadLocalClusters(
        {"fake_cluster"});
    ON_CALL(factory_context_.mock_server_context_.cluster_manager_.thread_local_cluster_,
            prioritySet())
