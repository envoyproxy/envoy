...
public:
  ...
  MOCK_METHOD(SmtpSession&, getSession, (), (override));
};

// Fixture class.
class SmtpFilterTest
    : public ::testing::TestWithParam<
          std::tuple<std::function<void(SmtpFilter*, Buffer::Instance&, bool)>,
                     std::function<uint32_t(const SmtpFilter*)>>> {
public:
  SmtpFilterTest() {
...
TEST_F(SmtpFilterTest, NewConnection) {
...
TEST_P(SmtpFilterTest, ReadData) {
...
INSTANTIATE_TEST_SUITE_P(ProcessDataTests, SmtpFilterTest,
                         ::testing::Values(std::make_tuple(&SmtpFilter::onData,
                                                           &SmtpFilter::getFrontendBufLength),
                                           std::make_tuple(&SmtpFilter::onWrite,
                                                           &SmtpFilter::getBackendBufLength)));

// Test generates various smtp payloads and feeds them into filter.
// It expects that certain statistics are updated.
TEST_F(SmtpFilterTest, BackendMsgsStats) {
  ...
  createSmtpMsg(data_, "C", "INSERT 123");
  filter_->onWrite(data_, false);
  ...
  createSmtpMsg(data_, "C", "DELETE 123");
  filter_->onWrite(data_, false);
...
TEST_F(SmtpFilterTest, EncryptedSessionStats) {
...
// Smtp metadata.
TEST_F(SmtpFilterTest, MetadataIncorrectSQL) {
  // Pretend that startup message has been received.
  static_cast<DecoderImpl*>(filter_->getDecoder())->state(DecoderImpl::State::InSyncState);
  setMetadata();

  createSmtpMsg(data_, "Q", "BLAH blah blah");
  ASSERT_THAT(Network::FilterStatus::Continue, filter_->onData(data_, true));
...
// Decoder::Stopped.
TEST_F(SmtpFilterTest, TerminateSSL) {
...
TEST_F(SmtpFilterTest, UpstreamSSL) {
  EXPECT_CALL(filter_callbacks_, connection()).WillRepeatedly(ReturnRef(connection_));
