...
class SmtpProxyFrontendDecoderTest : public SmtpProxyDecoderTestBase,
                                         public ::testing::TestWithParam<std::string> {};

// Class is used for parameterized tests for encrypted messages.
class SmtpProxyFrontendEncrDecoderTest : public SmtpProxyDecoderTestBase,
                                             public ::testing::TestWithParam<uint32_t> {};

// Class is used for parameterized tests for backend messages.
class SmtpProxyBackendDecoderTest : public SmtpProxyDecoderTestBase,
                                        public ::testing::TestWithParam<std::string> {};

class SmtpProxyBackendStatementTest
    : public SmtpProxyDecoderTestBase,
      public ::testing::TestWithParam<std::pair<std::string, bool>> {};

class SmtpProxyErrorTest
    : public SmtpProxyDecoderTestBase,
      public ::testing::TestWithParam<std::tuple<std::string, DecoderCallbacks::ErrorType>> {};

class SmtpProxyNoticeTest
    : public SmtpProxyDecoderTestBase,
...
TEST_F(SmtpProxyDecoderTest, StartupMessage) {
  ...
  ASSERT_THAT(decoder_->getAttributes().at("user"), "smtp");
...
TEST_F(SmtpProxyDecoderTest, StartupMessageNoAttr) {
  decoder_->state(DecoderImpl::State::InitState);

  createInitialSmtpRequest(data_);
  ...
  // Verify parsing attributes
  ASSERT_THAT(decoder_->getAttributes().at("user"), "smtp");
  ASSERT_THAT(decoder_->getAttributes().at("database"), "smtp");
  // This attribute should not be found
  ASSERT_THAT(decoder_->getAttributes().find("no"), decoder_->getAttributes().end());
}

TEST_F(SmtpProxyDecoderTest, InvalidStartupMessage) {
  decoder_->state(DecoderImpl::State::InitState);
...
TEST_F(SmtpProxyDecoderTest, StartupMessageRandomData) {
  srand(time(nullptr));
...
// nothing more.
TEST_F(SmtpProxyDecoderTest, ReadingBufferSingleMessages) {
  decoder_->state(DecoderImpl::State::InSyncState);
...
// with missing bytes.
TEST_F(SmtpProxyDecoderTest, ReadingBufferLargeMessages) {
  decoder_->state(DecoderImpl::State::InSyncState);
...
TEST_F(SmtpProxyDecoderTest, TwoMessagesInOneBuffer) {
  decoder_->state(DecoderImpl::State::InSyncState);
...
TEST_F(SmtpProxyDecoderTest, Unknown) {
  decoder_->state(DecoderImpl::State::InSyncState);
  ...
  createSmtpMsg(data_, "=", "some not important string which will be ignored anyways");
  ASSERT_THAT(decoder_->onData(data_, true), Decoder::Result::ReadyForNext);
  ASSERT_THAT(data_.length(), 0);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
}
...
TEST_F(SmtpProxyDecoderTest, IncorrectMessages) {
  decoder_->state(DecoderImpl::State::InSyncState);
...
TEST_F(SmtpProxyFrontendDecoderTest, FrontendInc) {
  decoder_->state(DecoderImpl::State::InSyncState);
  EXPECT_CALL(callbacks_, incMessagesFrontend());
  createSmtpMsg(data_, "f", "some text");
  ASSERT_THAT(decoder_->onData(data_, true), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
...
TEST_F(SmtpProxyFrontendDecoderTest, TerminateMessage) {
  decoder_->state(DecoderImpl::State::InSyncState);
...
TEST_F(SmtpProxyFrontendDecoderTest, QueryMessage) {
  EXPECT_CALL(callbacks_, processQuery);
  createSmtpMsg(data_, "Q", "SELECT * FROM whatever;");
  ASSERT_THAT(decoder_->onData(data_, true), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
}
...
// correctly.
TEST_F(SmtpProxyFrontendDecoderTest, ParseMessage) {
  ...
  query_name += '\0';
  createSmtpMsg(data_, "P", query_name + query + query_params);
  ASSERT_THAT(decoder_->onData(data_, true), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
...
TEST_F(SmtpProxyBackendDecoderTest, BackendInc) {
  EXPECT_CALL(callbacks_, incMessagesBackend());
  createSmtpMsg(data_, "I");
...
TEST_F(SmtpProxyBackendDecoderTest, ParseStatement) {
  // Payload contains a space after the keyword
  // Rollback counter should be bumped up.
  EXPECT_CALL(callbacks_, incTransactionsRollback());
  createSmtpMsg(data_, "C", "ROLLBACK 123");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  data_.drain(data_.length());
  ...
  EXPECT_CALL(callbacks_, incTransactionsRollback());
  createSmtpMsg(data_, "C", "ROLLBACK");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  data_.drain(data_.length());
  ...
  EXPECT_CALL(callbacks_, incTransactionsRollback()).Times(0);
  EXPECT_CALL(callbacks_, incStatements(DecoderCallbacks::StatementType::Other));
  createSmtpMsg(data_, "C", "ROLLBACK123");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  data_.drain(data_.length());
}
...
TEST_F(SmtpProxyDecoderTest, Backend) {
  decoder_->state(DecoderImpl::State::InSyncState);
  // C message
  EXPECT_CALL(callbacks_, incStatements(DecoderCallbacks::StatementType::Other));
  createSmtpMsg(data_, "C", "BEGIN 123");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  ASSERT_THAT(data_.length(), 0);
  ...
  EXPECT_CALL(callbacks_, incStatements(DecoderCallbacks::StatementType::Other));
  createSmtpMsg(data_, "C", "START TR");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  ASSERT_THAT(data_.length(), 0);
  ...
  EXPECT_CALL(callbacks_, incTransactionsCommit());
  createSmtpMsg(data_, "C", "INSERT 1");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  ASSERT_THAT(data_.length(), 0);
  ...
  EXPECT_CALL(callbacks_, incTransactionsCommit());
  createSmtpMsg(data_, "C", "UPDATE 123");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  ASSERT_THAT(data_.length(), 0);
  ...
  EXPECT_CALL(callbacks_, incTransactionsCommit());
  createSmtpMsg(data_, "C", "DELETE 88");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  ASSERT_THAT(data_.length(), 0);
}
...
TEST_F(SmtpProxyBackendDecoderTest, AuthenticationMsg) {
  ...
  createSmtpMsg(data_, "R", "blah blah");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
  data_.drain(data_.length());
...
// indicates error.
TEST_P(SmtpProxyErrorTest, ParseErrorMsgs) {
  EXPECT_CALL(callbacks_, incErrors(std::get<1>(GetParam())));
  createSmtpMsg(data_, "E", std::get<0>(GetParam()));
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
}

INSTANTIATE_TEST_SUITE_P(
    SmtpProxyErrorTestSuite, SmtpProxyErrorTest,
    ::testing::Values(
        ...
        // This is the real German message in Smtp > 9.6. It contains keyword
        ...
        // This situation can only happen in Smtp < 9.6. Starting with version 9.6
...
TEST_P(SmtpProxyNoticeTest, ParseNoticeMsgs) {
  EXPECT_CALL(callbacks_, incNotices(std::get<1>(GetParam())));
  createSmtpMsg(data_, "N", std::get<0>(GetParam()));
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::InSyncState);
}

INSTANTIATE_TEST_SUITE_P(
    SmtpProxyNoticeTestSuite, SmtpProxyNoticeTest,
...
TEST_P(SmtpProxyFrontendEncrDecoderTest, EncyptedTraffic) {
  ...
  createSmtpMsg(data_, "P", "Some message just to fill the payload.");
  ASSERT_THAT(decoder_->onData(data_, false), Decoder::Result::ReadyForNext);
  ASSERT_THAT(decoder_->state(), DecoderImpl::State::EncryptedState);
  // Decoder should drain data.
  ASSERT_THAT(data_.length(), 0);
...
INSTANTIATE_TEST_SUITE_P(FrontendEncryptedMessagesTests, SmtpProxyFrontendEncrDecoderTest,
                         ::testing::Values(80877103, 80877104));

// Test onSSLRequest callback.
TEST_F(SmtpProxyDecoderTest, TerminateSSL) {
  // Set decoder to wait for initial message.
  decoder_->state(DecoderImpl::State::InitState);
...
class SmtpProxyUpstreamSSLTest
    : public SmtpProxyDecoderTestBase,
      public ::testing::TestWithParam<std::tuple<std::string, bool, DecoderImpl::State>> {};

TEST_F(SmtpProxyDecoderTest, UpstreamSSLDisabled) {
  // Set decoder to wait for initial message.
  decoder_->state(DecoderImpl::State::InitState);
...
TEST_P(SmtpProxyUpstreamSSLTest, UpstreamSSLEnabled) {
  // Set decoder to wait for initial message.
  decoder_->state(DecoderImpl::State::InitState);
...
INSTANTIATE_TEST_SUITE_P(BackendEncryptedMessagesTests, SmtpProxyUpstreamSSLTest,
                         ::testing::Values(
...
TEST_F(SmtpProxyDecoderTest, Linearize) {
  decoder_->state(DecoderImpl::State::InSyncState);
