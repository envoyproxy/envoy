...
// Test verifies that Smtp filter replies with correct code upon
...
TEST_P(DownstreamSSLSmtpIntegrationTest, TerminateSSL) {
  ...
  // Message will be processed by Smtp filter which
  ...
  // Make sure that the successful login bumped up the number of sessions.
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_terminated_ssl", 1);
}
...
public:
  DownstreamSSLWrongConfigSmtpIntegrationTest()
      ...
      : SmtpBaseIntegrationTest(std::make_tuple("terminate_ssl: true", ""), NoUpstreamSSL) {}
};
...
TEST_P(DownstreamSSLWrongConfigSmtpIntegrationTest, TerminateSSLNoStartTlsTransportSocket) {
  ...
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_terminated_ssl", 0);
}
...
// Tests do not use the real smtp server and concentrate only on initial exchange.
// The initial packet
// sent by the downstream client, SSL request sent to fake upstream and SSL response sent back by
// fake client  are valid smtp payloads, because they must be parsed by smtp filter.
...
TEST_P(UpstreamSSLDisabledSmtpIntegrationTest, BasicConnectivityTest) {
  ...
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_upstream_ssl_success", 0);
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_upstream_ssl_failed", 0);
}
...
// Test verifies that smtp filter starts upstream SSL negotiation with
// fake upstream upon receiving initial smtp packet. When server agrees
// to use SSL, TLS transport socket is attached to fake upstream and
// fake upstream receives initial smtp packet over encrypted connection.
TEST_P(UpstreamSSLRequireSmtpIntegrationTest, ServerAgreesForSSLTest) {
  IntegrationTcpClientPtr tcp_client = makeTcpConnection(lookupPort("listener_0"));
  ASSERT_TRUE(fake_upstreams_[0]->waitForRawConnection(fake_upstream_connection_));
  ...
  createInitialSmtpRequest(data);
  ASSERT_TRUE(tcp_client->write(data.toString()));
  // Smtp filter should buffer the original message and negotiate SSL upstream.
  ...
  // Make sure that upstream received initial smtp request, which
  ...
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_upstream_ssl_success", 1);
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_upstream_ssl_failed", 0);
}

// Test verifies that smtp filter will not continue when upstream SSL
// is required and fake upstream does not agree for SSL.
TEST_P(UpstreamSSLRequireSmtpIntegrationTest, ServerDeniesSSLTest) {
  IntegrationTcpClientPtr tcp_client = makeTcpConnection(lookupPort("listener_0"));
  ASSERT_TRUE(fake_upstreams_[0]->waitForRawConnection(fake_upstream_connection_));
  ...
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_upstream_ssl_success", 0);
  test_server_->waitForCounterEq("smtp.smtp_stats.sessions_upstream_ssl_failed", 1);
}
