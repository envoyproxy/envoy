...
Decoder::Result DecoderImpl::onDataInit(Buffer::Instance& data, bool) {
  ...
  if (code >= 0x04d20000) {
    ...
    if (encrypted_) {
      ENVOY_LOG(trace, "smtp_proxy: detected encrypted traffic.");
    ...
    ENVOY_LOG(debug, "Detected version {}.{} of Smtp", code >> 16, code & 0x0000FFFF);
...
void DecoderImpl::processMessageBody(Buffer::Instance& data, absl::string_view direction,
  ...
  ENVOY_LOG(trace, "smtp_proxy: {} bytes remaining in buffer", data.length());
...
Decoder::Result DecoderImpl::onDataInSync(Buffer::Instance& data, bool frontend) {
  ...
  ENVOY_LOG(trace, "smtp_proxy: parsing message, len {}", data.length());
  ...
  ENVOY_LOG(trace, "smtp_proxy: command is {}", command_);
  ...
  if (validationResult == Message::ValidationNeedMoreData) {
    ENVOY_LOG(trace, "smtp_proxy: cannot parse message. Not enough bytes in the buffer.");
    return Decoder::Result::NeedMoreData;
  }
...
void DecoderImpl::decodeErrorNotice(MsgParserDict& types) {
  ...
  for (const auto& it : types.keywords_) {
    ...
    // Smtp versions prior to 9.6 use only S prefix while
