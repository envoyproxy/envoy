      ...
      [&singleton_manager](Http::HttpConnPoolImplBase* pool) {
        return std::make_unique<ActiveClient>(
            *pool, absl::nullopt, [&singleton_manager](Http::HttpConnPoolImplBase& pool) {
                ...
                if (!(cluster.clusterType().has_value() && cluster.clusterType()->name() == "envoy.clusters.reverse_connection")) {
                ...
                std::shared_ptr<Envoy::Extensions::Bootstrap::ReverseConnection::ReverseConnRegistry> reverse_conn_registry =
                        singleton_manager.getTyped<Envoy::Extensions::Bootstrap::ReverseConnection::ReverseConnRegistry>("reverse_conn_registry_singleton");
