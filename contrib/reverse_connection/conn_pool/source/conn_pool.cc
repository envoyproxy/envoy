#include "contrib/reverse_connection/conn_pool/source/conn_pool.h"
...
Http::ConnectionPool::InstancePtr ReverseConnPoolFactoryImpl::allocateConnPool(
      ...
      [&singleton_manager](Http::HttpConnPoolImplBase* pool) {
        return std::make_unique<ActiveClient>(
            *pool, absl::nullopt, [&singleton_manager](Http::HttpConnPoolImplBase& pool) {
              ...
              if (!(cluster.clusterType().has_value() &&
                    cluster.clusterType()->name() == "envoy.clusters.reverse_connection")) {
              ...
              std::shared_ptr<Envoy::Extensions::Bootstrap::ReverseConnection::ReverseConnRegistry>
                  reverse_conn_registry = singleton_manager.getTyped<
                      Envoy::Extensions::Bootstrap::ReverseConnection::ReverseConnRegistry>(
                      "reverse_conn_registry_singleton");
              if (reverse_conn_registry == nullptr) {
                throw EnvoyException("Cannot access cached reverse connection socket. Reverse "
                                     "connection registry not found");
              }
              Envoy::Extensions::Bootstrap::ReverseConnection::RCThreadLocalRegistry*
                  thread_local_registry = reverse_conn_registry->getLocalRegistry();
              if (thread_local_registry == nullptr) {
                throw EnvoyException("Cannot access cached reverse connection socket. Thread local "
                                     "reverse connection registry is null");
              }
              ...
              Network::ClientConnectionPtr connection =
                  std::make_unique<ReversedClientConnectionImpl>(
                      ...
                      std::move(transport_socket), std::move(host_socket), *thread_local_registry,
                      expects_proxy_protocol);
