...
func (f *sslFilter) DecodeHeaders(headers api.RequestHeaderMap, endStream bool) api.StatusType {
	...
	cipherSuite := ssl.CiphersuiteString()
	headers.Set("x-cipher-suite", cipherSuite)

	cipherID := ssl.CiphersuiteId()
	if cipherID != 0xffff {
