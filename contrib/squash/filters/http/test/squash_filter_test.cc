...
SquashFilterConfig constructSquashFilterConfigFromYaml(
  ...
  return SquashFilterConfig(proto_config, context.mock_server_context_.cluster_manager_);
}
...
TEST(SquashFilterConfigTest, V2ApiConversion) {
  ...
  factory_context.mock_server_context_.cluster_manager_.initializeClusters({"fake_cluster"}, {});
...
TEST(SquashFilterConfigTest, ParsesEnvironment) {
  ...
  NiceMock<Envoy::Server::Configuration::MockFactoryContext> factory_context;
  factory_context.mock_server_context_.cluster_manager_.initializeClusters({"squash"}, {});
...
public:
  SquashFilterTest()
      : request_(&factory_context_.mock_server_context_.cluster_manager_.thread_local_cluster_
                      .async_client_) {}
...
protected:
  ...
  void initFilter() {
    ...
    factory_context_.mock_server_context_.cluster_manager_.initializeClusters({"squash"}, {});
    ...
    config_ = std::make_shared<SquashFilterConfig>(
        p, factory_context_.mock_server_context_.cluster_manager_);

    filter_ = std::make_shared<SquashFilter>(
        config_, factory_context_.mock_server_context_.cluster_manager_);
  ...
  void startDownstreamRequest() {
    ...
    EXPECT_CALL(factory_context_.mock_server_context_.cluster_manager_.thread_local_cluster_,
                httpAsyncClient())
        .WillRepeatedly(ReturnRef(factory_context_.mock_server_context_.cluster_manager_
  ...
  void expectAsyncClientSend() {
    EXPECT_CALL(
        factory_context_.mock_server_context_.cluster_manager_.thread_local_cluster_.async_client_,
        send_(_, _, _))
        .WillOnce(Invoke(
...
TEST_F(SquashFilterTest, DecodeHeaderContinuesOnClientFail) {
  ...
  EXPECT_CALL(factory_context_.mock_server_context_.cluster_manager_.thread_local_cluster_,
              httpAsyncClient())
      .WillOnce(ReturnRef(factory_context_.mock_server_context_.cluster_manager_
                              .thread_local_cluster_.async_client_));
...
TEST_F(SquashFilterTest, PollingAttachmentNoCluster) {
  ...
  // Expect the second get attachment request
  ON_CALL(factory_context_.mock_server_context_.cluster_manager_, getThreadLocalCluster("squash"))
