...
curl -s "http://localhost:9090/stats" | grep -q "cluster.hello.health_check.healthy: 2"

run_log "Query healthy instances for envoy: World"
curl -s "http://localhost:9091/stats" | grep -q "cluster.world.health_check.healthy: 2"
...
curl -s "http://localhost:9090/stats" | grep -q "cluster.hello.health_check.healthy: 1"

run_log "Ensure that we now only have one healthy instance of World"
curl -s "http://localhost:9091/stats" | grep -q "cluster.world.health_check.healthy: 1"
