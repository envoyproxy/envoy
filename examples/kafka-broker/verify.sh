...
docker-compose exec -T kafka /bin/bash -c " \
...
docker-compose exec -T kafka kafka-console-consumer --bootstrap-server localhost:19092 --topic $TOPIC --from-beginning --max-messages 1
