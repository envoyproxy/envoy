...
DrainManagerImpl::DrainManagerImpl(Instance& server, envoy::api::v2::listener::Listener::DrainType drain_type)
...
bool DrainManagerImpl::drainClose() const {
  ...
  if (drain_type_ == envoy::api::v2::listener::Listener_DrainType_DEFAULT && server_.healthCheckFailed()) {
    return true;
  }
