...
void FilterChainManagerImpl::addFilterChain(
    absl::Span<const envoy::api::v3alpha::listener::FilterChain* const> filter_chain_span,
    FilterChainFactoryBuilder& filter_chain_factory_builder) {
  std::unordered_set<envoy::api::v3alpha::listener::FilterChainMatch, MessageUtil, MessageUtil>
      filter_chains;
...
void FilterChainManagerImpl::addFilterChainForDestinationPorts(
    ...
    const envoy::api::v3alpha::listener::FilterChainMatch::ConnectionSourceType source_type,
...
const Network::FilterChain* FilterChainManagerImpl::findFilterChainForSourceTypes(
  ...
  const auto& filter_chain_local =
      source_types[envoy::api::v3alpha::listener::FilterChainMatch::LOCAL];

  const auto& filter_chain_external =
      source_types[envoy::api::v3alpha::listener::FilterChainMatch::EXTERNAL];
  ...
  const auto& filter_chain_any = source_types[envoy::api::v3alpha::listener::FilterChainMatch::ANY];
