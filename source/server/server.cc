...
collectYamlFilesInDirectory(const std::string& directory_path,
                            absl::string_view wildcard_pattern) {
...
absl::StatusOr<std::vector<std::string>>
expandIncludePath(const std::string& path, const std::string& including_file_path,
                  Api::Api& api) {
  const std::string include_base_directory =
      std::filesystem::path(including_file_path).parent_path().string();

  const std::string resolved_path = isAbsolutePath(path) ? normalizePath(path)
                                                         : joinPath(include_base_directory, path);
  if (hasWildcard(resolved_path)) {
    const std::filesystem::path resolved_fs_path(resolved_path);
    std::string pattern_directory = resolved_fs_path.parent_path().string();
