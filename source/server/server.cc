...
void InstanceImpl::drainListeners() {
  log().info("closing and draining listeners");
...
void InstanceImpl::initialize(Options& options, TestHooks& hooks,
                              ComponentFactory& component_factory) {
  log().info("initializing epoch {} (hot restart version={})", options.restartEpoch(),
             restarter_.version());
  ...
  sig_hup_ = handler_.dispatcher().listenForSignal(SIGHUP, [this]() -> void {
    log().info("caught and eating SIGHUP. See documentation for how to hot restart.");
  });
  ...
  clusterManager().setInitializedCb([this, &hooks]() -> void {
    log().info("all clusters initialized. starting workers");
...
void InstanceImpl::loadServerFlags(const Optional<std::string>& flags_path) {
  ...
  if (handler_.api().fileExists(flags_path.value() + "/drain")) {
    log().info("starting server in drain mode");
...
void InstanceImpl::shutdown() {
  log().info("shutdown invoked. sending SIGTERM to self");
  kill(getpid(), SIGTERM);
}

void InstanceImpl::shutdownAdmin() {
  log().info("shutting down admin due to child startup");
  stat_flush_timer_.reset();
  handler_.closeListeners();
  admin_->socket().close();

  log().info("terminating parent process");
  restarter_.terminateParent();
}
