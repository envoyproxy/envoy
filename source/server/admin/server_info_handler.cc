...
Http::Code ServerInfoHandler::handlerCerts(absl::string_view,
  ...
  response.add(MessageUtil::getJsonStringFromMessageOrDie(certificates, true, true));
  return Http::Code::OK;
}
...
Http::Code ServerInfoHandler::handlerMemory(absl::string_view,
  ...
  response.add(MessageUtil::getJsonStringFromMessageOrDie(memory, true, true)); // pretty-print
  return Http::Code::OK;
}
...
Http::Code ServerInfoHandler::handlerServerInfo(absl::string_view, Http::ResponseHeaderMap& headers,
  ...
  *command_line_options = *server_.options().toCommandLineOptions();
  ...
  response.add(MessageUtil::getJsonStringFromMessageOrDie(server_info, true, true));
