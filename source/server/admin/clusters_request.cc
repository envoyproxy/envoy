...
TextClustersRequest::TextClustersRequest(uint64_t chunk_limit, Instance& server,
                                         const ClustersParams& params)
    : ClustersRequest(chunk_limit, server, params), idx_{0} {}

bool TextClustersRequest::nextChunk(Buffer::Instance& response) {
...
void TextClustersRequest::render(std::reference_wrapper<const Upstream::Cluster> cluster,
...
void TextClustersRequest::addOutlierInfo(const std::string& cluster_name,
...
void TextClustersRequest::addCircuitBreakerSettings(const std::string& cluster_name,
...
JsonClustersRequest::JsonClustersRequest(uint64_t chunk_limit, Instance& server,
                                         const ClustersParams& params)
    : ClustersRequest(chunk_limit, server, params), idx_(0) {
...
Http::Code JsonClustersRequest::start(Http::ResponseHeaderMap& response_headers) {
  response_headers.setReferenceContentType(Http::Headers::get().ContentTypeValues.Json);
  return Http::Code::OK;
}

bool JsonClustersRequest::nextChunk(Buffer::Instance& response) {
  const uint64_t original_request_size = response.length();
  for (; idx_ < clusters_.size() && response.length() - original_request_size < chunk_limit_;
       idx_++) {
...
void JsonClustersRequest::render(std::reference_wrapper<const Upstream::Cluster> cluster,
                                 Buffer::Instance& response) {
...
void JsonClustersRequest::setHostname(const Upstream::HostSharedPtr& host,
                                      std::vector<Json::Streamer::Map::NameValue>& host_config) {
...
void JsonClustersRequest::drainBufferIntoResponse(Buffer::Instance& response) {
  if (&response != &buffer_) {
...
void JsonClustersRequest::finalize(Buffer::Instance& response) {
