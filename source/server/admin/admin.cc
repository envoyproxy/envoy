...
AdminImpl::AdminImpl(const std::string& profile_path, Server::Instance& server,
    ...
    : server_(server), listener_info_(std::make_shared<ListenerInfoImpl>()),
      ...
      server_info_handler_(server), cpu_info_handler_(server),
      ...
      handlers_{
          ...
          makeHandler("/cpu/workers", "print CPU utilization for each worker thread in percentage",
                      MAKE_ADMIN_HANDLER(cpu_info_handler_.handlerWorkersCpu), false, false,
                      {{Admin::ParamDescriptor::Type::String, "sampling_interval_ms",
                        "Sampling interval in ms (default: 1000)"},
                       {Admin::ParamDescriptor::Type::Enum, "format",
