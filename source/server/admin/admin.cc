...
void setHealthFlag(Upstream::Host::HealthFlag flag, const Upstream::Host& host,
  ...
  switch (flag) {
    ...
    case Upstream::Host::HealthFlag::DEGRADED_EDS_HEALTH:
      ...
      } else if (host.healthFlagGet(
                     Upstream::Host::HealthFlag::DEGRADED_EDS_HEALTH)) {
    ...
    case Upstream::Host::HealthFlag::PENDING_DYNAMIC_REMOVAL:
      health_status.set_pending_dynamic_removal(host.healthFlagGet(
          Upstream::Host::HealthFlag::PENDING_DYNAMIC_REMOVAL));
      break;
...
void trimResourceMessage(const Protobuf::FieldMask& field_mask,
  ...
  for (int i = 0; i < field_mask.paths().size(); ++i) {
    ...
    const Protobuf::FieldDescriptor* field =
        descriptor->FindFieldByName(frags[0]);
  ...
  if (!any_field_name.empty()) {
    const Protobuf::FieldDescriptor* any_field =
        descriptor->FindFieldByName(any_field_name);
    if (reflection->HasField(message, any_field)) {
      ...
      // Trim message.
      ProtobufUtil::FieldMaskUtil::TrimMessage(inner_field_mask,
                                               inner_message.get());
...
void AdminImpl::addOutlierInfo(
    const std::string& cluster_name,
    const Upstream::Outlier::Detector* outlier_detector,
    Buffer::Instance& response) {
  if (outlier_detector) {
    response.add(fmt::format("{}::outlier::success_rate_average::{:g}\n",
                             cluster_name,
                             outlier_detector->successRateAverage(
                                 Upstream::Outlier::DetectorHostMonitor::
                                     SuccessRateMonitorType::ExternalOrigin)));
    response.add(fmt::format(
        "{}::outlier::success_rate_ejection_threshold::{:g}\n", cluster_name,
        outlier_detector->successRateEjectionThreshold(
            Upstream::Outlier::DetectorHostMonitor::SuccessRateMonitorType::
                ExternalOrigin)));
    response.add(fmt::format(
...
void AdminImpl::addCircuitSettings(const std::string& cluster_name,
  ...
  response.add(fmt::format("{}::{}_priority::max_requests::{}\n", cluster_name,
                           priority_str, resource_manager.requests().max()));
  response.add(fmt::format("{}::{}_priority::max_retries::{}\n", cluster_name,
                           priority_str, resource_manager.retries().max()));
}
...
void AdminImpl::writeClustersAsJson(Buffer::Instance& response) {
  ...
  for (auto& cluster_pair : server_.clusterManager().clusters()) {
    ...
    envoy::admin::v3::ClusterStatus& cluster_status =
        *clusters.add_cluster_statuses();
    cluster_status.set_name(cluster_info->name());

    const Upstream::Outlier::Detector* outlier_detector =
        cluster.outlierDetector();
    ...
    if (outlier_detector != nullptr &&
        outlier_detector->successRateEjectionThreshold(
            Upstream::Outlier::DetectorHostMonitor::SuccessRateMonitorType::
                LocalOrigin) > 0.0) {
      cluster_status.mutable_local_origin_success_rate_ejection_threshold()
          ->set_value(outlier_detector->successRateEjectionThreshold(
              Upstream::Outlier::DetectorHostMonitor::SuccessRateMonitorType::
                  LocalOrigin));
    }
    ...
    for (auto& host_set : cluster.prioritySet().hostSetsPerPriority()) {
      for (auto& host : host_set->hosts()) {
        envoy::admin::v3::HostStatus& host_status =
            *cluster_status.add_host_statuses();
        Network::Utility::addressToProtobufAddress(
            *host->address(), *host_status.mutable_address());
...
#undef SET_HEALTH_FLAG
        ...
        success_rate = host->outlierDetector().successRate(
            Upstream::Outlier::DetectorHostMonitor::SuccessRateMonitorType::
                LocalOrigin);
        if (success_rate >= 0.0) {
          host_status.mutable_local_origin_success_rate()->set_value(
              success_rate);
        }
      }
    }
  ...
  response.add(
      MessageUtil::getJsonStringFromMessage(clusters, true));  // pretty-print
}

// TODO(efimki): Add support of text readouts stats.
void AdminImpl::writeClustersAsText(Buffer::Instance& response) {
  for (auto& cluster : server_.clusterManager().clusters()) {
    addOutlierInfo(cluster.second.get().info()->name(),
                   cluster.second.get().outlierDetector(), response);

    addCircuitSettings(cluster.second.get().info()->name(), "default",
                       cluster.second.get().info()->resourceManager(
                           Upstream::ResourcePriority::Default),
                       response);
    addCircuitSettings(cluster.second.get().info()->name(), "high",
                       cluster.second.get().info()->resourceManager(
    ...
    for (auto& host_set :
         cluster.second.get().prioritySet().hostSetsPerPriority()) {
      for (auto& host : host_set->hosts()) {
        ...
        response.add(fmt::format("{}::{}::local_origin_success_rate::{}\n",
                                 ...
                                 host->outlierDetector().successRate(
                                     Upstream::Outlier::DetectorHostMonitor::
                                         SuccessRateMonitorType::LocalOrigin)));
      }
    }
  }
}
...
Http::Code AdminImpl::handlerClusters(absl::string_view url,
  ...
  Http::Utility::QueryParams query_params =
      Http::Utility::parseAndDecodeQueryString(url);
...
void AdminImpl::addAllConfigToDump(envoy::admin::v3::ConfigDump& dump,
  ...
  Envoy::Server::ConfigTracker::CbsMap callbacks_map =
      config_tracker_.getCallbacksMap();
  if (include_eds) {
    if (!server_.clusterManager().clusters().empty()) {
      callbacks_map.emplace("endpoint",
                            [this] { return dumpEndpointConfigs(); });
    }
  }
...
absl::optional<std::pair<Http::Code, std::string>> AdminImpl::addResourceToDump(
  ...
  for (const auto& key_callback_pair : callbacks_map) {
    ...
    if (!field_descriptor) {
      ...
      return absl::optional<std::pair<Http::Code, std::string>>{std::make_pair(
          ...
          fmt::format(
              "{} is not a repeated field. Use ?mask={} to get only this field",
              field_descriptor->name(), field_descriptor->name()))};
    }

    auto repeated = reflection->GetRepeatedPtrField<Protobuf::Message>(
  ...
  return absl::optional<std::pair<Http::Code, std::string>>{
      std::make_pair(Http::Code::NotFound,
                     fmt::format("{} not found in config dump", resource))};
}

void AdminImpl::addLbEndpoint(const Upstream::HostSharedPtr& host,
                              envoy::config::endpoint::v3::LocalityLbEndpoints&
                                  locality_lb_endpoint) const {
  ...
  if (host->healthCheckAddress()->asString() != host->address()->asString()) {
    health_check_config.set_port_value(
        host->healthCheckAddress()->ip()->port());
  }
}

ProtobufTypes::MessagePtr AdminImpl::dumpEndpointConfigs() const {
  auto endpoint_config_dump =
      std::make_unique<envoy::admin::v3::EndpointsConfigDump>();
  ...
  for (auto& cluster_pair : server_.clusterManager().clusters()) {
    ...
    if (cluster_info->edsServiceName().has_value()) {
      cluster_load_assignment.set_cluster_name(
          cluster_info->edsServiceName().value());
    } else {
    ...
    for (auto& host_set : cluster.prioritySet().hostSetsPerPriority()) {
      policy.mutable_overprovisioning_factor()->set_value(
          host_set->overprovisioningFactor());

      if (!host_set->hostsPerLocality().get().empty()) {
        for (int index = 0;
             index <
             static_cast<int>(host_set->hostsPerLocality().get().size());
             index++) {
          auto locality_host_set = host_set->hostsPerLocality().get()[index];

          if (!locality_host_set.empty()) {
            auto& locality_lb_endpoint =
                *cluster_load_assignment.mutable_endpoints()->Add();
            locality_lb_endpoint.mutable_locality()->MergeFrom(
                locality_host_set[0]->locality());
            locality_lb_endpoint.set_priority(locality_host_set[0]->priority());
            if (host_set->localityWeights() != nullptr &&
    ...
    if (cluster_info->addedViaApi()) {
      auto& dynamic_endpoint =
          *endpoint_config_dump->mutable_dynamic_endpoint_configs()->Add();
      dynamic_endpoint.mutable_endpoint_config()->PackFrom(
          cluster_load_assignment);
    } else {
      auto& static_endpoint =
          *endpoint_config_dump->mutable_static_endpoint_configs()->Add();
      static_endpoint.mutable_endpoint_config()->PackFrom(
          cluster_load_assignment);
    }
  }
...
Http::Code AdminImpl::handlerConfigDump(
  ...
  response_headers.setReferenceContentType(
      Http::Headers::get().ContentTypeValues.Json);
  response.add(
      MessageUtil::getJsonStringFromMessage(dump, true));  // pretty-print
  return Http::Code::OK;
}
...
void AdminImpl::startHttpListener(
  ...
  socket_ =
      std::make_shared<Network::TcpListenSocket>(address, socket_options, true);
  ...
  if (!address_out_path.empty()) {
    ...
    if (!address_out_file) {
      ENVOY_LOG(critical,
                "cannot open admin address output file {} for writing.",
                address_out_path);
    } else {
...
AdminImpl::AdminImpl(const std::string& profile_path, Server::Instance& server)
    : server_(server),
      request_id_extension_(
          Http::RequestIDExtensionFactory::defaultInstance(server_.random())),
      profile_path_(profile_path),
      stats_(Http::ConnectionManagerImpl::generateStats("http.admin.",
                                                        server_.stats())),
      null_overload_manager_(server_.threadLocal()),
      tracing_stats_(Http::ConnectionManagerImpl::generateTracingStats(
          "http.admin.", no_op_store_)),
      route_config_provider_(server.timeSource()),
      scoped_route_config_provider_(server.timeSource()),
      stats_handler_(server),
      logs_handler_(server),
      profiling_handler_(profile_path),
      runtime_handler_(server),
      listeners_handler_(server),
      server_cmd_handler_(server),
      ...
      handlers_{
          ...
          {"/clusters", "upstream cluster status",
           MAKE_ADMIN_HANDLER(handlerClusters), false, false},
          ...
          {"/cpuprofiler", "enable/disable the CPU profiler",
           MAKE_ADMIN_HANDLER(profiling_handler_.handlerCpuProfiler), false,
           true},
          {"/heapprofiler", "enable/disable the heap profiler",
           MAKE_ADMIN_HANDLER(profiling_handler_.handlerHeapProfiler), false,
           true},
          {"/healthcheck/fail", "cause the server to fail health checks",
           MAKE_ADMIN_HANDLER(server_cmd_handler_.handlerHealthcheckFail),
           false, true},
          {"/healthcheck/ok", "cause the server to pass health checks",
           MAKE_ADMIN_HANDLER(server_cmd_handler_.handlerHealthcheckOk), false,
           true},
          {"/help", "print out list of admin commands",
           MAKE_ADMIN_HANDLER(handlerHelp), false, false},
          {"/hot_restart_version",
          ...
          {"/memory", "print current allocation/heap usage",
           MAKE_ADMIN_HANDLER(server_info_handler_.handlerMemory), false,
           false},
          {"/quitquitquit", "exit the server",
           MAKE_ADMIN_HANDLER(server_cmd_handler_.handlerQuitQuitQuit), false,
           true},
          {"/reset_counters", "reset all counters to zero",
           MAKE_ADMIN_HANDLER(stats_handler_.handlerResetCounters), false,
           true},
          {"/drain_listeners", "drain listeners",
           MAKE_ADMIN_HANDLER(listeners_handler_.handlerDrainListeners), false,
           true},
          {"/server_info", "print server version/status information",
           MAKE_ADMIN_HANDLER(server_info_handler_.handlerServerInfo), false,
           false},
          {"/ready",
          ...
          {"/stats/prometheus", "print server stats in prometheus format",
           MAKE_ADMIN_HANDLER(stats_handler_.handlerPrometheusStats), false,
           false},
          {"/stats/recentlookups", "Show recent stat-name lookups",
           MAKE_ADMIN_HANDLER(stats_handler_.handlerStatsRecentLookups), false,
           false},
          ...
          {"/listeners", "print listener info",
           MAKE_ADMIN_HANDLER(listeners_handler_.handlerListenerInfo), false,
           false},
          {"/runtime", "print runtime values",
           MAKE_ADMIN_HANDLER(runtime_handler_.handlerRuntime), false, false},
          {"/runtime_modify", "modify runtime values",
           MAKE_ADMIN_HANDLER(runtime_handler_.handlerRuntimeModify), false,
           true},
...
Http::ServerConnectionPtr AdminImpl::createCodec(
  ...
  return Http::ConnectionManagerUtility::autoCreateCodec(
      connection, data, callbacks, server_.stats(), http1_codec_stats_,
...
bool AdminImpl::createNetworkFilterChain(
  ...
  connection.addReadFilter(
      Network::ReadFilterSharedPtr{new Http::ConnectionManagerImpl(
          *this, server_.drainManager(), server_.random(),
          server_.httpContext(), server_.runtime(), server_.localInfo(),
          server_.clusterManager(), null_overload_manager_,
          server_.timeSource())});
  return true;
}
...
void AdminImpl::createFilterChain(
  ...
  callbacks.addStreamFilter(
      std::make_shared<AdminFilter>(createCallbackFunction()));
}
...
Http::Code AdminImpl::runCallback(absl::string_view path_and_query,
    ...
    if (path_and_query.compare(0, query_index, handler.prefix_) == 0) {
      ...
      if (handler.mutates_server_state_) {
        const absl::string_view method =
            admin_stream.getRequestHeaders().getMethodValue();
        if (method != Http::Headers::get().MethodValues.Post) {
          ENVOY_LOG(
              error,
              "admin path \"{}\" mutates state, method={} rather than POST",
  ...
  Memory::Utils::tryShrinkHeap(
      server_.runtime().threadsafeSnapshot()->getInteger(
          "envoy.memory.heap_shrink_threshold",
          static_cast<uint64_t>(100 * 1024 * 1024UL)));
  return code;
}
...
std::vector<const AdminImpl::UrlHandler*> AdminImpl::sortedHandlers() const {
  ...
  std::sort(sorted_handlers.begin(), sorted_handlers.end(),
            [](const UrlHandler* h1, const UrlHandler* h2) {
              return h1->prefix_ < h2->prefix_;
            });
...
Http::Code AdminImpl::handlerAdminHome(
  ...
  response.add(
      absl::StrReplaceAll(AdminHtmlStart, {{"@FAVICON@", EnvoyFavicon}}));
  ...
  for (const UrlHandler* handler : sortedHandlers()) {
    ...
    response.add(
        fmt::format("<tr class='home-row'><td class='home-data'>{}</td>"
...
bool AdminImpl::addHandler(const std::string& prefix,
  ...
  if (pos != std::string::npos) {
    ENVOY_LOG(error, "filter \"{}\" contains invalid character '{}'", prefix,
              prefix[pos]);
    return false;
  }
  ...
  if (it == handlers_.end()) {
    handlers_.push_back(
        {prefix, help_text, callback, removable, mutates_state});
    return true;
  }
  return false;
}
...
bool AdminImpl::removeHandler(const std::string& prefix) {
  ...
  handlers_.remove_if([&prefix](const UrlHandler& entry) {
...
Http::Code AdminImpl::request(absl::string_view path_and_query,
  ...
  Http::Code code =
      runCallback(path_and_query, response_headers, response, filter);
