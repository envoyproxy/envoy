...
class ApiListenerImpl : public Configuration::FactoryContext,
                        public Network::DrainDecision,
                        Logger::Loggable<Logger::Id::config> {
public:
  ApiListenerImpl(const envoy::api::v2::Listener& config, ListenerManagerImpl& parent,
  ...
  Http::ServerConnectionCallbacks* apiHandle() {
    if (!http_connection_manager_) {
      ENVOY_LOG_MISC(error, "There was no hcm creating one");
      http_connection_manager_ = http_connection_manager_factory_(read_callbacks_);
      ENVOY_LOG(error, "Exercised lambda");
    }
