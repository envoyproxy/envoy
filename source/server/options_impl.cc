...
OptionsImpl::OptionsImpl(std::vector<std::string> args,
  ...
  TCLAP::ValueArg<uint32_t> mongo_bson_max_depth("", "mongo-bson-max-depth",
                                                 "Max depth of BSON documents the Mongo proxy "
                                                 "filter will decode",
                                                 false, 128, "uint32_t", cmd);
  ...
  mongo_bson_max_depth_ = mongo_bson_max_depth.getValue();
...
Server::CommandLineOptionsPtr OptionsImpl::toCommandLineOptions() const {
  ...
  command_line_options->set_mongo_bson_max_depth(mongoBsonMaxDepth());
