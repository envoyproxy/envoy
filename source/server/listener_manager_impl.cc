...
bool ListenerManagerImpl::addOrUpdateListener(const envoy::api::v3alpha::Listener& config,
...
bool ListenerManagerImpl::addOrUpdateListenerInternal(const envoy::api::v3alpha::Listener& config,
  ...
  if (listenersStopped(config)) {
    ENVOY_LOG(
        ...
        name, envoy::api::v3alpha::core::TrafficDirection_Name(config.traffic_direction()));
    return false;
  }
...
void ListenerManagerImpl::stopListeners(StopListenersType stop_listeners_type) {
  ...
  for (Network::ListenerConfig& listener : listeners()) {
    if (stop_listeners_type != StopListenersType::InboundOnly ||
        listener.direction() == envoy::api::v3alpha::core::INBOUND) {
...
std::unique_ptr<Network::FilterChain> ListenerFilterChainFactoryBuilder::buildFilterChain(
    const envoy::api::v3alpha::listener::FilterChain& filter_chain) const {
  ...
  if (!filter_chain.has_transport_socket()) {
    if (filter_chain.has_hidden_envoy_deprecated_tls_context()) {
      ...
      MessageUtil::jsonConvert(filter_chain.hidden_envoy_deprecated_tls_context(),
...
Network::ListenSocketFactorySharedPtr ListenerManagerImpl::createListenSocketFactory(
    const envoy::api::v3alpha::core::Address& proto_address, ListenerImpl& listener,
    bool reuse_port) {
  Network::Address::SocketType socket_type =
