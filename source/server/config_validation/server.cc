...
void ValidationInstance::initialize(const Options& options,
  ...
  cluster_manager_factory_ = std::make_unique<Upstream::ValidationClusterManagerFactory>(
      admin(), runtime(), stats(), loadReportingServiceStats(), threadLocal(), random(),
      dnsResolver(), sslContextManager(), dispatcher(), localInfo(), *secret_manager_,
      messageValidationContext(), *api_, http_context_, grpc_context_, accessLogManager(),
