...
ProdComponentFactory::createDrainManager(Server::Instance &server) {
  // The global drain manager only triggers on listener modification, which
  // effectively is hot restart at the global level. The per-listener drain
  // managers decide whether to to include /healthcheck/fail status.
  return std::make_unique<Server::DrainManagerImpl>(
      server, envoy::api::v2::Listener_DrainType_MODIFY_ONLY);
}

Runtime::LoaderPtr
ProdComponentFactory::createRuntime(Server::Instance &server,
...
MainCommonBase::MainCommonBase(
  ...
  try {
    ...
    case Server::Mode::Serve: {
      ...
      auto local_address =
          Network::Utility::getLocalAddress(options_.localAddressIpVersion());
      logging_context_ = std::make_unique<Logger::Context>(
          options_.logLevel(), options_.logFormat(), log_lock);

      configureComponentLogLevels();

      stats_store_ = std::make_unique<Stats::ThreadLocalStoreImpl>(
          options_.statsOptions(), restarter_->statsAllocator());

      server_ = std::make_unique<Server::InstanceImpl>(
          options_, time_system, local_address, test_hooks, *restarter_,
          *stats_store_, access_log_lock, component_factory,
          std::move(random_generator), *tls_, thread_factory);
...
void MainCommonBase::configureComponentLogLevels() {
  for (auto &component_log_level : options_.componentLogLevels()) {
    Logger::Logger *logger_to_change =
        Logger::Registry::logger(component_log_level.first);
...
void MainCommonBase::adminRequest(absl::string_view path_and_query,
  ...
  server_->dispatcher().post([this, path_and_query_buf, method_buf, handler]() {
    ...
    std::string body;
    server_->admin().request(path_and_query_buf, method_buf, response_headers,
                             body);
...
MainCommon::MainCommon(int argc, const char *const *argv)
    : options_(argc, argv, &MainCommon::hotRestartVersion, spdlog::level::info),
      base_(options_, real_time_system_, default_test_hooks_,
            prod_component_factory_,
            std::make_unique<Runtime::RandomGeneratorImpl>(),
            platform_impl_.threadFactory()) {}

std::string MainCommon::hotRestartVersion(uint64_t max_num_stats,
                                          uint64_t max_stat_name_len,
                                          bool hot_restart_enabled) {
#ifdef ENVOY_HOT_RESTART
  if (hot_restart_enabled) {
    return Server::HotRestartImpl::hotRestartVersion(max_num_stats,
                                                     max_stat_name_len);
  }
#else
