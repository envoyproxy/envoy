...
MainCommonBase::MainCommonBase(OptionsImpl& options, Event::TimeSystem& time_system,
    ...
    : options_(options), component_factory_(component_factory), thread_system_(thread_system) {
  ...
  case Server::Mode::Serve: {
    ...
    server_ = std::make_unique<Server::InstanceImpl>(
        ...
        access_log_lock, component_factory, std::move(random_generator), *tls_, thread_system);
...
bool MainCommonBase::run() {
  ...
  case Server::Mode::Validate: {
    auto local_address = Network::Utility::getLocalAddress(options_.localAddressIpVersion());
    return Server::validateConfig(options_, local_address, component_factory_, thread_system_);
  }
  case Server::Mode::InitOnly:
    PERF_DUMP();
    return true;
...
MainCommon::MainCommon(int argc, const char* const* argv)
    : options_(argc, argv, &MainCommon::hotRestartVersion, spdlog::level::info),
      base_(options_, real_time_system_, default_test_hooks_, prod_component_factory_,
            std::make_unique<Runtime::RandomGeneratorImpl>(), thread_system_) {}
