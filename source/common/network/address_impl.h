...
protected:
  ...
  InstanceBase(Type type, const std::string sock_interface) : type_(type) {
...
public:
   ...
   */
  explicit Ipv4Instance(const std::string& address, const std::string& sock_interface = "");

  /**
   * Construct from a string IPv4 address such as "1.2.3.4" as well as a port.
   */
  Ipv4Instance(const std::string& address, uint32_t port, const std::string& sock_interface = "");

  /**
   * Construct from a port. The IPv4 address will be set to "any" and is suitable for binding
   * a port to any available address.
   */
  explicit Ipv4Instance(uint32_t port, const std::string& sock_interface = "");
...
public:
   ...
   */
  explicit Ipv6Instance(const std::string& address, const std::string& sock_interface = "");

  /**
   * Construct from a string IPv6 address such as "12:34::5" as well as a port.
   */
  Ipv6Instance(const std::string& address, uint32_t port, const std::string& sock_interface = "");

  /**
   * Construct from a port. The IPv6 address will be set to "any" and is suitable for binding
   * a port to any available address.
   */
  explicit Ipv6Instance(uint32_t port, const std::string& sock_interface = "");
