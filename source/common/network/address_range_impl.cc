...
uint32_t portToTry(uint32_t start, uint32_t end, Runtime::RandomGenerator& random) {
  ...
  uint32_t port_to_try =
      start + static_cast<uint32_t>((end + 1 - start) * unitary_scaled_random_value);
  ...
  resolve(const envoy::api::v2::core::SocketAddress& socket_address) override {
    ...
    case envoy::api::v2::core::SocketAddress::kNamedPort:
      return std::make_shared<IpInstanceRange>(
          ...
          socket_address.named_port(), random_);
...
IpInstanceRange::IpInstanceRange(InstanceConstSharedPtr&& address_instance,
  ...
  if (address_instance_->ip()->port() != 0u) {
    throw EnvoyException(
        fmt::format("IpInstanceRange initialized with address_instance with non-zero port \"{}\".",
                    address_instance_->ip()->port()));
  }
  if (ports.size() != 2u) {
    throw EnvoyException(fmt::format(
        "Named port value \"{}\" does not have \"<start>-<end>\" format", range_as_named_port));
  }
  if (!absl::SimpleAtoi(ports[0], &start_port_)) {
    throw EnvoyException(fmt::format(
        "Start port of named port value \"{}\" does not convert to integer.", range_as_named_port));
  }
  ...
  friendly_name_ = address_instance_->ip()->addressAsString() + ":" + std::to_string(start_port_) +
                   "-" + std::to_string(end_port_);
}
...
int IpInstanceRange::bind(int fd) const {
  ...
  while (tries--) {
    ...
    case IpVersion::v4: {
      ...
      ret = ::bind(fd, reinterpret_cast<const sockaddr*>(&socket_address), sizeof(socket_address));
    } break;
    case IpVersion::v6: {
      sockaddr_in6 socket_address;
      absl::uint128 result(address_instance_->ip()->ipv6()->address());
      memcpy(static_cast<void*>(&socket_address.sin6_addr.s6_addr), static_cast<void*>(&result),
             sizeof(absl::uint128));
