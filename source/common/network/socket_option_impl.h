...
public:
  ...
  SocketOptionImpl(Network::SocketOptionName optname, absl::string_view value,
                   std::optional<Network::Socket::Type> socket_type = std::nullopt,
                   std::optional<Network::Address::IpVersion> socket_ip_version = std::nullopt)
      : in_state_(std::nullopt), optname_(optname), value_(value.begin(), value.end()),
        socket_type_(socket_type), socket_ip_version_(socket_ip_version) {
    ASSERT(reinterpret_cast<uintptr_t>(value_.data()) % alignof(void*) == 0);
  }
   ...
   */
  std::optional<Network::Socket::Type> socketType() const;
   ...
   */
  std::optional<Network::Address::IpVersion> socketIpVersion() const;
...
private:
  ...
  std::optional<const envoy::config::core::v3::SocketOption::SocketState> in_state_;
  ...
  std::optional<Network::Socket::Type> socket_type_;
  ...
  std::optional<Network::Address::IpVersion> socket_ip_version_;
};
