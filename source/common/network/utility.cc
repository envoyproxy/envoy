...
Address::InstanceConstSharedPtr Utility::fromProtoAddress(const envoy::api::v2::Address& address) {
  ...
  Address::InstanceConstSharedPtr Utility::getLocalAddress(const Address::IpVersion version) {
    ...
    for (ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
      ...
      if ((ifa->ifa_addr->sa_family == AF_INET && version == Address::IpVersion::v4) ||
        ...
        ret = Address::addressFromSockAddr(*addr, (version == Address::IpVersion::v4)
                                                      ? sizeof(sockaddr_in)
                                                      : sizeof(sockaddr_in6));
  ...
  bool Utility::portInRangeList(const Address::Instance& address,
                                const std::list<PortRange>& list) {
    if (address.type() != Address::Type::Ip) {
      return false;
    }
