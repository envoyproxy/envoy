...
public:
  AlpnDecoratingTransportSocketOptions(std::vector<std::string>&& alpn,
      ...
      : alpn_fallback_list_(std::move(alpn)), inner_options_(std::move(inner_options)) {}
  ...
  const std::vector<std::string>& applicationProtocolListFallback() const override {
    return alpn_fallback_list_;
  }
  void hashKey(std::vector<uint8_t>& key) const override;

private:
  const std::vector<std::string> alpn_fallback_list_;
...
public:
  TransportSocketOptionsImpl(absl::string_view override_server_name = "",
                             ...
                             std::vector<std::string>&& fallback_alpn = {})
