...
void DnsResolverImpl::PendingResolution::onAresGetAddrInfoCallback(int status, int timeouts,
  ...
  if (status == ARES_EDESTRUCTION) {
    ...
    if (!cancelled_) {
      ENVOY_LOG_STABLE("dns_resolution_destroyed", "dns resolution for {} destroyed", dns_name_);
  ...
  if (completed_) {
    if (!cancelled_) {
      ...
      ENVOY_LOG_STABLE("dns_resolution_complete", "dns resolution for {} completed with status {}",
...
ActiveDnsQuery* DnsResolverImpl::resolve(const std::string& dns_name,
  ...
  ENVOY_LOG_STABLE("cares_dns_resolution_started", "dns resolution for {} started", dns_name_);
