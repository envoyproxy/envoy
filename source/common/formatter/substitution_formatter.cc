...
absl::optional<std::string> MetadataFormatter::extract( const StreamInfo::StreamInfo& stream_info) const {
  auto metadata = get_func_(stream_info);
  return (metadata != nullptr) ? formatMetadata(*metadata) : absl::nullopt;
}

ProtobufWkt::Value MetadataFormatter::extractValue(const StreamInfo::StreamInfo& stream_info) const {
  auto metadata = get_func_(stream_info);
...
absl::optional<std::string> FilterStateFormatter::extract(
...
ProtobufWkt::Value FilterStateFormatter::extractValue(
                                                     const StreamInfo::StreamInfo& stream_info) const {
  const Envoy::StreamInfo::FilterState::Object* state = filterState(stream_info);
  if (!state) {
    return unspecifiedValue();
...
absl::optional<std::string> SystemTimeFormatter::extract(
                                                        const StreamInfo::StreamInfo& stream_info) const {
...
ProtobufWkt::Value SystemTimeFormatter::extractValue(
    const StreamInfo::StreamInfo& stream_info) const {
