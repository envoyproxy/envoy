...
private:
  ...
  using OpenTelemetryFormatValue =
      absl::variant<const std::vector<FormatterProviderPtr>, const OpenTelemetryFormatMapWrapper,
                    const OpenTelemetryFormatListWrapper>;
  ...
  using OpenTelemetryFormatMapVisitor = StructFormatMapVisitorHelper<
      const std::function<::opentelemetry::proto::common::v1::AnyValue(
          const std::vector<FormatterProviderPtr>&)>,
      const std::function<::opentelemetry::proto::common::v1::AnyValue(
  ...
  providersCallback(const std::vector<FormatterProviderPtr>& providers,
                    const Http::RequestHeaderMap& request_headers,
                    const Http::ResponseHeaderMap& response_headers,
                    const Http::ResponseTrailerMap& response_trailers,
                    const StreamInfo::StreamInfo& stream_info,
