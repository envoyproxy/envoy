...
Event::DispatcherPtr Impl::allocateDispatcher(const std::string& name, absl::optional<uint32_t> worker_thread_index) {
  return std::make_unique<Event::DispatcherImpl>(name, worker_thread_index, *this, time_system_, watermark_factory_);
}
...
Impl::allocateDispatcher(const std::string& name, absl::optional<uint32_t> worker_thread_index,
  ...
  return std::make_unique<Event::DispatcherImpl>(name, worker_thread_index, *this, time_system_, scaled_timer_factory,
                                                 watermark_factory_);
}

Event::DispatcherPtr Impl::allocateDispatcher(const std::string& name, absl::optional<uint32_t> worker_thread_index,
                                              Buffer::WatermarkFactoryPtr&& factory) {
  return std::make_unique<Event::DispatcherImpl>(name, worker_thread_index, *this, time_system_, std::move(factory));
}
