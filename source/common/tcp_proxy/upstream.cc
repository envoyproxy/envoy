...
bool TcpUpstream::readDisable(bool disable) {
  if (upstream_conn_data_ == nullptr ||
    ...
    // Because we flush write downstream, we can have a case where upstream has already
    // disconnected and we are waiting to flush. If we had a watermark event during this time we
    // should no longer touch the upstream connection.
    return false;
  }
