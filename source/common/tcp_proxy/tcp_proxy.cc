...
void Filter::initializeUpstreamConnection() {
  ...
  while (true) {
    ...
    UpstreamStatus status = maybeTunnel(cluster_name);
    ...
    case UpstreamStatus::Error:
      ...
      return;
    case UpstreamStatus::Retry:
      // An immediate failure during creating connection. An example is overflow which could be
      // fixed by other threads. Will retry initialization.
      break;
    default:
