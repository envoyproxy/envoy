...
ConnPoolImplBase::tryCreateNewConnection(float global_preconnect_ratio) {
  ...
  if (!can_create_connection) {
    host_->cluster().upstreamStats()->upstream_cx_overflow_.inc();
  }
...
void ConnPoolImplBase::attachStreamToClient(Envoy::ConnectionPool::ActiveClient& client,
  ...
  if (client.state() == Envoy::ConnectionPool::ActiveClient::State::ReadyForEarlyData) {
    host_->cluster().upstreamStats()->upstream_rq_0rtt_.inc();
  }
  ...
  if (enforceMaxRequests() && !host_->cluster().resourceManager(priority_).requests().canCreate()) {
    ...
    host_->cluster().upstreamStats()->upstream_rq_pending_overflow_.inc();
    return;
  }
  ...
  if (client.remaining_streams_ == 0) {
    ...
    host_->cluster().upstreamStats()->upstream_cx_max_requests_.inc();
  ...
  host_->cluster().upstreamStats()->upstream_rq_total_.inc();
  host_->cluster().upstreamStats()->upstream_rq_active_.inc();
...
void ConnPoolImplBase::onStreamClosed(Envoy::ConnectionPool::ActiveClient& client,
  ...
  host_->cluster().upstreamStats()->upstream_rq_active_.dec();
...
void ConnPoolImplBase::onConnectionEvent(ActiveClient& client, absl::string_view failure_reason,
  ...
  case Network::ConnectionEvent::LocalClose: {
    ...
    if (!client.hasHandshakeCompleted()) {
      ...
      host_->cluster().upstreamStats()->upstream_cx_connect_fail_.inc();
  ...
  case Network::ConnectionEvent::ConnectedZeroRtt: {
    ...
    ASSERT(client.state() == ActiveClient::State::Connecting);
    host()->cluster().upstreamStats()->upstream_cx_connect_with_0_rtt_.inc();
...
PendingStream::PendingStream(ConnPoolImplBase& parent, bool can_send_early_data)
  ...
  parent_.host()->cluster().upstreamStats()->upstream_rq_pending_total_.inc();
  parent_.host()->cluster().upstreamStats()->upstream_rq_pending_active_.inc();
  parent_.host()->cluster().resourceManager(parent_.priority()).pendingRequests().inc();
}

PendingStream::~PendingStream() {
  parent_.host()->cluster().upstreamStats()->upstream_rq_pending_active_.dec();
...
void ConnPoolImplBase::purgePendingStreams(
  ...
  while (!pending_streams_to_purge_.empty()) {
    ...
    host_->cluster().upstreamStats()->upstream_rq_pending_failure_eject_.inc();
...
void ConnPoolImplBase::onPendingStreamCancel(PendingStream& stream,
  ...
  host_->cluster().upstreamStats()->upstream_rq_cancelled_.inc();
  checkForIdleAndCloseIdleConnsIfDraining();
}
...
ActiveClient::ActiveClient(ConnPoolImplBase& parent, uint32_t lifetime_stream_limit,
  ...
  conn_connect_ms_ = std::make_unique<Stats::HistogramCompletableTimespanImpl>(
      parent_.host()->cluster().upstreamStats()->upstream_cx_connect_ms_, parent_.dispatcher().timeSource());
  conn_length_ = std::make_unique<Stats::HistogramCompletableTimespanImpl>(
      parent_.host()->cluster().upstreamStats()->upstream_cx_length_ms_, parent_.dispatcher().timeSource());
  ...
  parent_.host()->cluster().upstreamStats()->upstream_cx_total_.inc();
  parent_.host()->cluster().upstreamStats()->upstream_cx_active_.inc();
...
void ActiveClient::releaseResourcesBase() {
  if (!resources_released_) {
    ...
    parent_.host()->cluster().upstreamStats()->upstream_cx_active_.dec();
...
void ActiveClient::onConnectTimeout() {
  ...
  parent_.host()->cluster().upstreamStats()->upstream_cx_connect_timeout_.inc();
  timed_out_ = true;
  close();
}
...
void ActiveClient::onConnectionDurationTimeout() {
  ...
  parent_.host()->cluster().upstreamStats()->upstream_cx_max_duration_reached_.inc();
