...
SendMsgRequest::SendMsgRequest(IoUringSocket& socket, const struct msghdr* msg, int flags)
  ...
  *msg_ = *msg;
  ...
  if (msg->msg_name && msg->msg_namelen > 0) {
    ...
    memcpy(name_buf_.get(), msg->msg_name, msg->msg_namelen);
  ...
  if (msg->msg_control && msg->msg_controllen > 0) {
    ...
    memcpy(control_buf_.get(), msg->msg_control, msg->msg_controllen);
...
void IoUringServerSocket::onRecvmsg(Request* req, int32_t result, bool injected) {
  ...
  // Continue with recvmsg if needed for connectionless sockets.
