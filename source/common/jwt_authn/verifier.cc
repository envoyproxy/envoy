...
bool Verifier::Verify(const Jwt& jwt, const Jwks& jwks) {
  ...
  for (auto& jwk : jwks.keys()) {
    ...
    if (jwk->kty == "EC" && VerifySignatureEC(jwk->ec_key.get(), jwt.signature, signed_data)) {
      ...
      return true;
               ...
               VerifySignatureRSA(jwk->evp_pkey.get(), EVP_sha256(), jwt.signature,
                                  signed_data)) {
      // Verification succeeded.
      return true;
    }
...
bool Verifier::VerifySignatureRSA(EVP_PKEY* key, const EVP_MD* md, const uint8_t* signature,
...
bool Verifier::VerifySignatureRSA(EVP_PKEY* key, const EVP_MD* md, const std::string& signature,
                                  const std::string& signed_data) {
  return VerifySignatureRSA(key, md, CastToUChar(signature), signature.length(),
                            CastToUChar(signed_data), signed_data.length());
}

bool Verifier::VerifySignatureEC(EC_KEY* key, const uint8_t* signature, size_t signature_len,
