...
public:
  ...
  void addChild(std::string value, OnMatch<DataType>&& on_match) override {
    if (Runtime::runtimeFeatureEnabled(USE_RADIX_TREE_FOR_TRIE_LOOKUP)) {
      radix_tree_.insert(value, std::make_shared<OnMatch<DataType>>(std::move(on_match)));
    } else {
      trie_lookup_table_.add(value, std::make_shared<OnMatch<DataType>>(std::move(on_match)));
    }
  }
...
protected:
  ...
  MatchResult doMatch(const DataType& data, absl::string_view key,
    ...
    if (Runtime::runtimeFeatureEnabled(USE_RADIX_TREE_FOR_TRIE_LOOKUP)) {
      auto result = radix_tree_.LongestPrefix(std::string(key));
      if (result.found) {
        const std::shared_ptr<OnMatch<DataType>>& on_match = result.val;
      ...
      const absl::InlinedVector<std::shared_ptr<OnMatch<DataType>>, 4> results =
          trie_lookup_table_.findMatchingPrefixes(key);
...
private:
  ...
  Tree<std::string, std::shared_ptr<OnMatch<DataType>>> radix_tree_;
  TrieLookupTable<std::shared_ptr<OnMatch<DataType>>> trie_lookup_table_;
};
