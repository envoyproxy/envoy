...
namespace Compressor {
  ...
  // Not calling deflateEnd() will cause leaks.
  ZlibCompressorImpl::~ZlibCompressorImpl() {  
    deflateEnd(ZlibPtr_.get());
  }

  uint64_t ZlibCompressorImpl::getTotalIn() {
    return ZlibPtr_->total_in;
  }

  uint64_t ZlibCompressorImpl::getTotalOut() {
    return ZlibPtr_->total_out;
  }

  void ZlibCompressorImpl::setMemoryLevel(uint mem_level) {
    memory_level_ = mem_level;
  }

  void ZlibCompressorImpl::setWindowBits(int win_bits) {
    window_bits_ = win_bits;
  }

  void ZlibCompressorImpl::setChunk(uint64_t chunk) {
    chunk_ = chunk;
  }
  ...
  bool ZlibCompressorImpl::init(CompressionLevel level, CompressionStrategy strategy) {
    ...
    int result = deflateInit2(ZlibPtr_.get(), level, Z_DEFLATED, 
                              window_bits_, memory_level_, strategy);
