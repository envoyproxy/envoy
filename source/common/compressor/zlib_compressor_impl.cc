...
void ZlibCompressorImpl::updateOutput(Buffer::Instance& output_buffer, bool require_finish) {
  ...
  if (require_finish) {
    ...
    // output stream. However depending on how much output space is provided, deflate() may have to
    // be called several times until it has provided the complete compressed stream, even after it
    // has consumed all of the input. The flush parameter must continue to be Z_FINISH for those
    // subsequent calls. Ref: https://zlib.net/zlib_how.html.
    int result = Z_OK;
    while (result != Z_STREAM_END) {
      result = deflate(zstream_ptr_.get(), Z_FINISH);
      RELEASE_ASSERT(result != Z_STREAM_ERROR);
      updateOutput(output_buffer);
    }
  }
}
