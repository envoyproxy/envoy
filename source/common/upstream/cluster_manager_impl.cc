...
void ClusterManagerImpl::postThreadLocalClusterUpdate(const Cluster& cluster, uint32_t priority,
  ...
  HealthyHostVectorConstSharedPtr healthy_hosts_copy(new HealthyHostVector(host_set->healthyHosts()));
  DegradedHostVectorConstSharedPtr degraded_hosts_copy(new DegradedHostVector(host_set->degradedHosts()));
