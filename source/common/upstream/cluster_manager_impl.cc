...
ClusterManagerImpl::ThreadLocalClusterManagerImpl::ClusterEntry::connPool(
  ...
  // The Connection Pool tracking is a WIP. We plan for it to eventually have the ability to fail,
  // but until we add upper layer handling for failures, it should not. So, assert that we don't
  // accidentally add conditions that could allow it to fail.
