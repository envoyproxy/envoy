...
public:
  ...
  create(const envoy::config::cluster::v3alpha::Cluster& cluster, ClusterManager& cluster_manager,
  ...
  selectDnsResolver(const envoy::config::cluster::v3alpha::Cluster& cluster,
...
private:
  ...
  virtual std::pair<ClusterImplBaseSharedPtr, ThreadAwareLoadBalancerPtr> createClusterWithConfig(
      const envoy::config::cluster::v3alpha::Cluster& cluster, const ConfigProto& proto_config,
      ClusterFactoryContext& context,
      Server::Configuration::TransportSocketFactoryContext& socket_factory_context,
      Stats::ScopePtr&& stats_scope) PURE;
};
