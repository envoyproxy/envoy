...
void HealthCheckerImplBase::ActiveHealthCheckSession::onIntervalBase() {
  ...
  if (parent_.disable_health_check_if_active_traffic_ &&
    ...
    std::cout << "current time:" << time_source_.monotonicTime().time_since_epoch().count()
              << std::endl;
    std::cout << "traffic successful time:" << traffic_successful_time.time_since_epoch().count()
              << std::endl;
