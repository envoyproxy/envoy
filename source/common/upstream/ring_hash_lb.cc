...
RingHashLoadBalancer::Ring::Ring(const absl::optional<envoy::api::v2::Cluster::RingHashLbConfig>& config,
  ...
  const uint64_t min_ring_size =
      config ? PROTOBUF_GET_WRAPPED_OR_DEFAULT(config.value(), minimum_ring_size, 1024)
                     : 1024;
