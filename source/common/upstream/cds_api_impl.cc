...
CdsApiPtr CdsApiImpl::create(const envoy::api::v3alpha::core::ConfigSource& cds_config,
                             ClusterManager& cm, Stats::Scope& scope,
                             ProtobufMessage::ValidationVisitor& validation_visitor) {
  return CdsApiPtr{new CdsApiImpl(cds_config, cm, scope, validation_visitor)};
}

CdsApiImpl::CdsApiImpl(const envoy::api::v3alpha::core::ConfigSource& cds_config,
                       ClusterManager& cm, Stats::Scope& scope,
                       ProtobufMessage::ValidationVisitor& validation_visitor)
...
void CdsApiImpl::onConfigUpdate(const Protobuf::RepeatedPtrField<ProtobufWkt::Any>& resources,
  ...
  std::vector<envoy::api::v3alpha::Cluster> clusters;
  for (const auto& cluster_blob : resources) {
    clusters.push_back(MessageUtil::anyConvert<envoy::api::v3alpha::Cluster>(cluster_blob));
...
void CdsApiImpl::onConfigUpdate(
  ...
  for (const auto& resource : added_resources) {
    ...
    try {
      cluster = MessageUtil::anyConvert<envoy::api::v3alpha::Cluster>(resource.resource());
