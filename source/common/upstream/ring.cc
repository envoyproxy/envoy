...
Ring::Ring(const NormalizedHostWeightVector& normalized_host_weights, double min_normalized_weight,
  ...
  for (const auto& entry : normalized_host_weights) {
    ...
    while (current_hashes < target_hashes) {
      ...
      const uint64_t hash = (hash_function == HashFunction::MURMUR_HASH_2)
  ...
  std::sort(ring_.begin(), ring_.end(),
            [](const Entry& lhs, const Entry& rhs) -> bool { return lhs.hash_ < rhs.hash_; });
  ...
  stats.size_.set(ring_size_);
  stats.min_hashes_per_host_.set(min_hashes_per_host);
  stats.max_hashes_per_host_.set(max_hashes_per_host);
}

HostConstSharedPtr Ring::chooseHost(uint64_t h, uint32_t attempt) const {
  if (ring_.empty()) {
    return nullptr;
  }
...
RingStats Ring::generateStats(Stats::Scope& scope) { return {ALL_RING_STATS(POOL_GAUGE(scope))}; }
