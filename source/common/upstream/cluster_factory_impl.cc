...
ClusterSharedPtr ClusterFactoryImplBase::create(const envoy::api::v2::Cluster& cluster, ClusterManager& cluster_manager, Stats::Store& stats,
  ...
  ClusterFactoryContextImpl context(cluster_manager, stats, tls, dns_resolver,
                                    ssl_context_manager, runtime, random, dispatcher, log_manager, local_info, admin, singleton_manager,
                                    outlier_event_logger, added_via_api, api);
  return factory->create(cluster, context);
}

Network::DnsResolverSharedPtr ClusterFactoryImplBase::selectDnsResolver(const envoy::api::v2::Cluster& cluster, ClusterFactoryContext& context) {
...
ClusterSharedPtr ClusterFactoryImplBase::create(const envoy::api::v2::Cluster& cluster, ClusterFactoryContext& context) {

  auto stats_scope = generateStatsScope(cluster, context.stats());
  Server::Configuration::TransportSocketFactoryContextImpl factory_context(
    context.admin(), context.sslContextManager(), *stats_scope, context.clusterManager(), context.localInfo(),
    context.dispatcher(), context.random(), context.stats(), context.singletonManager(), context.tls(), context.api());

  ClusterImplBaseSharedPtr new_cluster = createClusterImpl(cluster, context, factory_context, std::move(stats_scope));
  ...
  if (!cluster.health_checks().empty()) {
    ...
    if (cluster.health_checks().size() != 1) {
      ...
      new_cluster->setHealthChecker(HealthCheckerFactory::create(
        cluster.health_checks()[0], *new_cluster, context.runtime(), context.random(), context.dispatcher(),
        context.logManager()));
    }
  }
...
ClusterImplBaseSharedPtr StaticClusterFactory::createClusterImpl(const envoy::api::v2::Cluster& cluster,
...
ClusterImplBaseSharedPtr StrictDnsClusterFactory::createClusterImpl(const envoy::api::v2::Cluster& cluster,
                                                                 ClusterFactoryContext& context, Server::Configuration::TransportSocketFactoryContext& socket_factory_context,
                                                                 Stats::ScopePtr&& stats_scope) {
