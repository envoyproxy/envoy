...
HealthCheckerFactory::create(const envoy::api::v2::core::HealthCheck& hc_config,
  ...
  case envoy::api::v2::core::HealthCheck::HealthCheckerCase::kGrpcHealthCheck:
    ...
    return std::make_shared<ProdGrpcHealthCheckerImpl>(cluster, hc_config, dispatcher, runtime,
                                                       random);
  case envoy::api::v2::core::HealthCheck::HealthCheckerCase::kExtensionHealthCheck: {
    auto& factory = Config::Utility::getAndCheckFactory<Upstream::ExtensionHealthCheckerFactory>(
        hc_config.extension_health_check().name());
    return factory.createExtensionHealthChecker(hc_config, cluster, runtime, random, dispatcher);
  }
  default:
    // Checked by schema.
    NOT_REACHED;
