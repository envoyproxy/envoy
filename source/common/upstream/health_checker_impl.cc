...
void GrpcHealthCheckerImpl::GrpcActiveHealthCheckSession::decodeHeaders(
  ...
  if (!Grpc::Common::hasGrpcContentType(*headers)) {
    onRpcComplete(Grpc::Status::GrpcStatusMapping::Internal, "invalid gRPC content-type", false);
    return;
  }
  if (end_stream) {
    ...
    onRpcComplete(Grpc::Status::GrpcStatusMapping::Internal,
...
void GrpcHealthCheckerImpl::GrpcActiveHealthCheckSession::decodeData(Buffer::Instance& data,
  ...
  if (!decoder_.decode(data, decoded_frames)) {
    onRpcComplete(Grpc::Status::GrpcStatusMapping::Internal, "gRPC wire protocol decode error",
                  false);
  }
  for (auto& frame : decoded_frames) {
    if (frame.length_ > 0) {
      if (health_check_response_) {
        ...
        onRpcComplete(Grpc::Status::GrpcStatusMapping::Internal, "unexpected streaming", false);
        return;
      }
...
void GrpcHealthCheckerImpl::GrpcActiveHealthCheckSession::decodeTrailers(
  ...
  auto grpc_status =
      maybe_grpc_status ? maybe_grpc_status.value() : Grpc::Status::GrpcStatusMapping::Internal;
...
void GrpcHealthCheckerImpl::GrpcActiveHealthCheckSession::logHealthCheckStatus(
  ...
  if (grpc_status != Grpc::Status::GrpcStatusMapping::Ok && !grpc_message.empty()) {
