...
private:
  ...
  public:
    HostSubsetImpl(const HostSet& original_host_set, bool locality_weight_aware)
        : HostSetImpl(original_host_set.priority(), original_host_set.over_provisioning_factor()),
  ...
  protected:
    HostSetImplPtr createHostSet(uint32_t priority, uint32_t over_provisioning_factor) override;
