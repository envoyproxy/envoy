...
HdsCluster::HdsCluster(Server::Configuration::ServerFactoryContext& server_context,
  ...
  for (const auto& locality_endpoints : cluster_.load_assignment().endpoints()) {
    ...
    for (const auto& host : locality_endpoints.lb_endpoints()) {
      ...
      HostSharedPtr endpoint = std::shared_ptr<HostImpl>(THROW_OR_RETURN_VALUE(HostImpl::create(
          info_, "", std::move(address_or_error.value()), nullptr, nullptr, 1,
          locality_endpoints.locality(), host.endpoint().health_check_config(), 0,
          envoy::config::core::v3::UNKNOWN, time_source_), std::unique_ptr<HostImpl>));
  ...
  for (auto& endpoints : locality_endpoints) {
    ...
    for (auto& endpoint : endpoints.lb_endpoints()) {
      ...
      if (!update_cluster_info && host_pair != hosts_map_.end()) {
        ...
        THROW_IF_STATUS_NOT_OK(address_or_error, throw);
        host = std::shared_ptr<HostImpl>(THROW_OR_RETURN_VALUE(HostImpl::create(info_, "", std::move(address_or_error.value()), nullptr,
