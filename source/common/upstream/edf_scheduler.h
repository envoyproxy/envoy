...
public:
  ...
  static EdfScheduler<C> createWithPicks(const std::vector<std::shared_ptr<C>>& entries,
    ...
    double weights_sum =
        std::accumulate(entries.cbegin(), entries.cend(), 0.0,
                        [&calculate_weight](double sum_so_far, const std::shared_ptr<C>& entry) {
                          return sum_so_far + calculate_weight(*entry);
                        });
    ...
    std::transform(entries.cbegin(), entries.cend(), std::back_inserter(floor_picks),
                   [picks, weights_sum, &calculate_weight](const std::shared_ptr<C>& entry) {
                     // Getting the lower-bound by casting to an integer.
                     return static_cast<uint32_t>(calculate_weight(*entry) / weights_sum * picks);
                   });
    ...
    for (size_t i = 0; i < entries.size(); ++i) {
      ...
      const double weight = calculate_weight(*entries[i]);
