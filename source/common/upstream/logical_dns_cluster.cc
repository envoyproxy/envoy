...
void LogicalDnsCluster::updateHosts(
    std::string dns_address, const std::list<Network::DnsResponse>&& response,
    ...
    active_dns_query_ = dns_resolver_->resolveSrv(
        ...
        [this, dns_address](std::list<Network::DnsResponse>&& response) -> void {
          updateHosts(
              ...
              [](const Network::DnsResponse& dns_response) { return dns_response.address_; });
        });
  } else {
    active_dns_query_ = dns_resolver_->resolve(
        ...
        [this, dns_address](std::list<Network::DnsResponse>&& response) -> void {
          updateHosts(dns_address, std::move(response),
                      [this](const Network::DnsResponse& dns_response) {
