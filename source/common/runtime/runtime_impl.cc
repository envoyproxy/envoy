...
DiskLayer::DiskLayer(const std::string& name, const std::string& path,
  ...
  walkDirectory(path, "", 1, file_system);
}
...
void DiskLayer::walkDirectory(const std::string& path, const std::string& prefix, uint32_t depth,
  ...
  if (file_system.illegalPath(path)) {
  ...
  for (const Filesystem::DirectoryEntry& entry : directory) {
    ...
    if (entry.type_ == Filesystem::FileType::Directory && entry.name_ != "." &&
        entry.name_ != "..") {
      walkDirectory(full_path, full_prefix, depth + 1, file_system);
      ...
      const std::string text_file{file_system.fileReadToEnd(full_path)};
...
DiskBackedLoaderImpl::DiskBackedLoaderImpl(Event::Dispatcher& dispatcher,
    ...
    : LoaderImpl(DoNotLoadSnapshot{}, generator, store, tls),
      ...
      override_path_(root_symlink_path + "/" + override_dir), file_system_(file_system) {
...
std::unique_ptr<SnapshotImpl> DiskBackedLoaderImpl::createNewSnapshot() {
  std::vector<Snapshot::OverrideLayerConstPtr> layers;
  try {
    layers.push_back(std::make_unique<DiskLayer>("root", root_path_, file_system_));
    if (file_system_.directoryExists(override_path_)) {
      layers.push_back(std::make_unique<DiskLayer>("override", override_path_, file_system_));
