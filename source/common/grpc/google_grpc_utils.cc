...
GoogleGrpcUtils::channelArgsFromConfig(const envoy::config::core::v3::GrpcService& config) {
  ...
  for (const auto& channel_arg : config.google_grpc().channel_args()) {
    switch (channel_arg.value_specifier_case()) {
    case envoy::config::core::v3::GrpcService::GoogleGrpc::ChannelArgs::kStringValue: {
      args.SetString(channel_arg.key(), channel_arg.string_value());
      break;
    }
    case envoy::config::core::v3::GrpcService::GoogleGrpc::ChannelArgs::kIntValue: {
      args.SetInt(channel_arg.key(), channel_arg.int_value());
      break;
    }
    default:
