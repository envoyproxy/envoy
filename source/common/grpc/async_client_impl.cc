...
void AsyncStreamImpl::initialize(bool buffer_body_for_retry) {
  if (parent_.cm_.get(parent_.remote_cluster_name_) == nullptr) {
    callbacks_.onRemoteClose(Status::GrpcStatusMapping::Unavailable, "Cluster not available");
    http_reset_ = true;
    return;
  }
  ...
  if (stream_ == nullptr) {
    callbacks_.onRemoteClose(Status::GrpcStatusMapping::Unavailable, EMPTY_STRING);
    http_reset_ = true;
    return;
  }
...
void AsyncStreamImpl::onHeaders(Http::HeaderMapPtr&& headers, bool end_stream) {
  ...
  if (http_response_status != enumToInt(Http::Code::OK)) {
    ...
    streamError(Status::GrpcStatusMapping::Canceled);
    return;
  }
  if (end_stream) {
...
void AsyncStreamImpl::onData(Buffer::Instance& data, bool end_stream) {
  ...
  if (!decoder_.decode(data, decoded_frames_)) {
    streamError(Status::GrpcStatusMapping::Internal);
    return;
  }
  ...
  if (end_stream) {
    streamError(Status::GrpcStatusMapping::Unknown);
  }
}
...
void AsyncStreamImpl::onTrailers(Http::HeaderMapPtr&& trailers) {
  ...
  if (!grpc_status) {
    grpc_status = Status::GrpcStatusMapping::Unknown;
  }
