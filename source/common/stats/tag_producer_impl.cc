...
TagProducerImpl::TagProducerImpl(const envoy::config::metrics::v3alpha::StatsConfig& config) {
  ...
  for (const auto& tag_specifier : config.stats_tags()) {
    ...
    if (tag_specifier.tag_value_case() ==
            envoy::config::metrics::v3alpha::TagSpecifier::TagValueCase::TAG_VALUE_NOT_SET ||
        tag_specifier.tag_value_case() ==
            envoy::config::metrics::v3alpha::TagSpecifier::TagValueCase::kRegex) {
      ...
      if (tag_specifier.regex().empty()) {
        ...
        addExtractor(Stats::TagExtractorImpl::createTagExtractor(name, tag_specifier.regex()));
               ...
               envoy::config::metrics::v3alpha::TagSpecifier::TagValueCase::kFixedValue) {
...
void TagProducerImpl::reserveResources(const envoy::config::metrics::v3alpha::StatsConfig& config) {
  default_tags_.reserve(config.stats_tags().size());
}

std::unordered_set<std::string>
TagProducerImpl::addDefaultExtractors(const envoy::config::metrics::v3alpha::StatsConfig& config) {
