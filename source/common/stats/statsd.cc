...
UdpStatsdSink::UdpStatsdSink(ThreadLocal::SlotAllocator& tls,
    ...
    : BaseSink(tls.allocateSlot()), tls_(tls.allocateSlot()), server_address_(std::move(address)),
      use_tag_(use_tag) {
...
TcpStatsdSink::TcpStatsdSink(const LocalInfo::LocalInfo& local_info,
    ...
    : BaseSink(tls.allocateSlot()), tls_(tls.allocateSlot()), cluster_manager_(cluster_manager),
