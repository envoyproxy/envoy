...
void SecretManagerImpl::addStaticSecret(const envoy::api::v3alpha::auth::Secret& secret) {
  switch (secret.type_case()) {
  case envoy::api::v3alpha::auth::Secret::TypeCase::kTlsCertificate: {
  ...
  case envoy::api::v3alpha::auth::Secret::TypeCase::kValidationContext: {
  ...
  case envoy::api::v3alpha::auth::Secret::TypeCase::kSessionTicketKeys: {
    auto secret_provider =
...
TlsCertificateConfigProviderSharedPtr SecretManagerImpl::findOrCreateTlsCertificateProvider(
    const envoy::api::v3alpha::core::ConfigSource& sds_config_source,
...
void redactSecret(envoy::api::v3alpha::auth::Secret* secret) {
  if (secret &&
      secret->type_case() == envoy::api::v3alpha::auth::Secret::TypeCase::kTlsCertificate) {
    auto tls_certificate = secret->mutable_tls_certificate();
    if (tls_certificate->has_private_key() &&
        tls_certificate->private_key().specifier_case() !=
            envoy::api::v3alpha::core::DataSource::SpecifierCase::kFilename) {
  ...
  if (secret &&
      secret->type_case() == envoy::api::v3alpha::auth::Secret::TypeCase::kSessionTicketKeys) {
