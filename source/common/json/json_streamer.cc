...
Streamer<StringBuffer>::Level::Level(Streamer& streamer, absl::string_view opener,
...
template <class StringBuffer> Streamer<StringBuffer>::Level::~Level() {
...
template <class StringBuffer>
typename Streamer<StringBuffer>::MapPtr Streamer<StringBuffer>::makeRootMap() {
  ASSERT_LEVELS_EMPTY;
  return std::make_unique<Map>(*this);
}

template <class StringBuffer>
typename Streamer<StringBuffer>::ArrayPtr Streamer<StringBuffer>::makeRootArray() {
  ASSERT_LEVELS_EMPTY;
  return std::make_unique<Array>(*this);
}

template <class StringBuffer>
typename Streamer<StringBuffer>::MapPtr Streamer<StringBuffer>::Level::addMap() {
...
template <class StringBuffer>
typename Streamer<StringBuffer>::ArrayPtr Streamer<StringBuffer>::Level::addArray() {
  ASSERT_THIS_IS_TOP_LEVEL;
  nextField();
  return std::make_unique<Array>(streamer_);
}

template <class StringBuffer> void Streamer<StringBuffer>::Level::addNumber(double number) {
  ASSERT_THIS_IS_TOP_LEVEL;
  nextField();
  streamer_.addNumber(number);
}

template <class StringBuffer> void Streamer<StringBuffer>::Level::addNumber(uint64_t number) {
  ASSERT_THIS_IS_TOP_LEVEL;
  nextField();
  streamer_.addNumber(number);
}

template <class StringBuffer> void Streamer<StringBuffer>::Level::addNumber(int64_t number) {
  ASSERT_THIS_IS_TOP_LEVEL;
  nextField();
  streamer_.addNumber(number);
}

template <class StringBuffer> void Streamer<StringBuffer>::Level::addBool(bool b) {
  ASSERT_THIS_IS_TOP_LEVEL;
  nextField();
  streamer_.addBool(b);
}

template <class StringBuffer> void Streamer<StringBuffer>::Level::addString(absl::string_view str) {
  ASSERT_THIS_IS_TOP_LEVEL;
  nextField();
...
#ifndef NDEBUG
template <class StringBuffer> void Streamer<StringBuffer>::pop(Level* level) {
  ASSERT(levels_.top() == level);
  levels_.pop();
}

template <class StringBuffer> void Streamer<StringBuffer>::push(Level* level) {
  levels_.push(level);
}
#endif

template <class StringBuffer> void Streamer<StringBuffer>::Level::nextField() {
  if (is_first_) {
    is_first_ = false;
  } else {
...
template <class StringBuffer> void Streamer<StringBuffer>::Map::nextField() {
  if (expecting_value_) {
...
template <class StringBuffer> void Streamer<StringBuffer>::Map::addKey(absl::string_view key) {
  ASSERT_THIS_IS_TOP_LEVEL;
...
template <class StringBuffer> void Streamer<StringBuffer>::Map::addEntries(const Entries& entries) {
...
template <class StringBuffer> void Streamer<StringBuffer>::Level::addValue(const Value& value) {
  switch (value.index()) {
  case 0:
...
template <class StringBuffer>
void Streamer<StringBuffer>::Array::addEntries(const Entries& values) {
  for (const Value& value : values) {
    this->addValue(value);
  }
}

template <class StringBuffer> void Streamer<StringBuffer>::addNumber(double number) {
  if (std::isnan(number)) {
    response_.add("null");
  } else {
...
template <class StringBuffer> void Streamer<StringBuffer>::addNumber(uint64_t number) {
  response_.add(absl::StrCat(number));
}

template <class StringBuffer> void Streamer<StringBuffer>::addNumber(int64_t number) {
  response_.add(absl::StrCat(number));
}

template <class StringBuffer> void Streamer<StringBuffer>::addBool(bool b) {
  response_.add(b ? "true" : "false");
}

template <class StringBuffer>
void Streamer<StringBuffer>::addSanitized(absl::string_view prefix, absl::string_view str,
