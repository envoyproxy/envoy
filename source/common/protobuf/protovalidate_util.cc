...
#ifdef ENVOY_ENABLE_PROTOVALIDATE
  try {
    // Create a validator factory. This is done per-call for thread safety,
    // though it could be optimized with a thread-local cache in the future.
    ...
    // Create an arena for efficient memory management of validation objects.
    google::protobuf::Arena arena;
    auto validator = factory_result.value()->NewValidator(&arena);

    // Execute validation rules defined in the message's .proto file
    // using buf.validate field options.
    ...
    if (!result.success()) {
      // Collect violation messages for detailed error reporting
...
#else
  // Protovalidate library not available - this is a no-op stub
...
bool ProtovalidateUtil::isAvailable() {
  // Return true if protovalidate library was compiled in, false otherwise.
  // This allows callers to check feature availability at runtime.
#ifdef ENVOY_ENABLE_PROTOVALIDATE
  return true;
#else
  return false;
