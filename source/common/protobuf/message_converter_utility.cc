...
absl::StatusOr<MaybeParseGrpcMessageOutput> MaybeParseGrpcMessage(
  ...
  if (request_in.length() < kGrpcDelimiterByteSize) {
    ENVOY_LOG_MISC(info, "Need more data for gRPC frame header parsing. Current size={}",  request_in.length());
  ...
  if (request_in.length() < total_size) {
    ENVOY_LOG_MISC(info, "Need more data for gRPC frame. Current size={}, needed={}",request_in.length() ,total_size);
    MaybeParseGrpcMessageOutput ret;
    ret.needs_more_data = true;
    return ret;
  }
  ...
  const Envoy::Buffer::RawSliceVector slices =
      output.owned_bytes->getRawSlices();
  for (auto slice : slices) {
    ...
    output.message->AppendExternalMemory(static_cast<const char*>(slice_mem),
                                         slice_length);
...
absl::StatusOr<std::string> SizeToDelimiter(size_t size) {
  if (size > std::numeric_limits<uint32_t>::max()) {
    return absl::FailedPreconditionError(
        absl::StrCat("Current input message size ", size,
                     " is larger than max allowed gRPC message length of ",
...
absl::StatusOr<uint64_t> DelimiterToSize(const unsigned char* delimiter) {
  if (delimiter[0] != 0) {
    ...
    return absl::InvalidArgumentError(
        absl::StrCat("Unsupported gRPC frame 'Compressed-Flag'",
                     std::to_string(delimiter[0])));
  }
