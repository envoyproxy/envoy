...
MessageConverter::MessageConverter(
    std::unique_ptr<CreateMessageDataFunc> factory, uint32_t buffer_limit)
    : factory_(std::move(factory)),
      buffer_limit_(buffer_limit),
      parsed_bytes_usage_(std::make_shared<uint64_t>(0)) {}

absl::StatusOr<std::unique_ptr<StreamMessage>>
MessageConverter::AccumulateMessage(Envoy::Buffer::Instance& data,
                                    bool end_stream) {
  // Append the input data to buffer that will be parsed.
  parsing_buffer_.move(data);

  ENVOY_LOG_MISC(info,"Checking buffer limits: actual {} > limit {}?", BytesBuffered(), buffer_limit_);
  ...
  auto parsed_output =
                   MaybeParseGrpcMessage(*factory_, parsing_buffer_);
  if (!parsed_output.ok()) {
  ...
  if (parsed_output->needs_more_data) {
    if (is_final_message) {
      ...
      return absl::InvalidArgumentError(
          "Did not receive enough data for gRPC message parsing.");
    } else {
  ...
  auto message_data = std::make_unique<StreamMessage>(
      std::move(parsed_output->message), std::move(parsed_output->owned_bytes),
      parsed_bytes_usage_);
  ...
  if (parsed_output->owned_bytes != nullptr) {
    ...
    ENVOY_LOG_MISC(info,"len(parsed owned_bytes)={}",
            parsed_output->owned_bytes->length());
  }
  return message_data;
}

absl::StatusOr<std::vector<std::unique_ptr<StreamMessage>>>
MessageConverter::AccumulateMessages(Envoy::Buffer::Instance& data,
                                     bool end_stream) {
...
MessageConverter::ConvertBackToBuffer(std::unique_ptr<StreamMessage> message) {
  ...
  if (conversions_to_envoy_buffer_ > conversions_to_message_data_) {
    return absl::FailedPreconditionError(absl::StrCat(
        "Data corruption! Number of conversions to StreamMessage = ",
        conversions_to_message_data_, ", but this is the ",
        conversions_to_envoy_buffer_,
  ...
  Envoy::Buffer::InstancePtr output_message =
 ...
 auto delimiter = SizeToDelimiter(esf_out_message_size);
  ...
  while (input_stream->Next(&data, &size)) {
    last_fragment =
        new Envoy::Buffer::BufferFragmentImpl(data, size, fragment_releasor);
  ...
  if (last_fragment == nullptr) {
    ...
    ABSL_DCHECK_EQ(output_message->length(),
              google::grpc::transcoding::kGrpcDelimiterByteSize);
    return output_message;
  }

  ENVOY_LOG_MISC(info,"converted back len(raw_message)={}, len(output_message)={}" , message_lifetime->size()
