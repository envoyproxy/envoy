...
//
// For implementation details, see the following docs:
// 1) go/cloud-esf-convergence-data-conversion-message-lifetime
 ...
 public:
  MessageConverter(
      std::unique_ptr<CreateMessageDataFunc>
          factory)
      : MessageConverter(std::move(factory),
  ...
  absl::StatusOr<std::unique_ptr<StreamMessage>> AccumulateMessage(
  ...
  absl::StatusOr<Envoy::Buffer::InstancePtr> ConvertBackToBuffer(
      std::unique_ptr<StreamMessage> message);
