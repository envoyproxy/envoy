...
DeltaSubscriptionImpl::DeltaSubscriptionImpl(std::shared_ptr<GrpcMux> context, absl::string_view type_url,
  ...
  void DeltaSubscriptionImpl::start(const std::set<std::string>& resources, SubscriptionCallbacks& callbacks) override {
    context_->addSubscription(resources, type_url_, callbacks, stats_, init_fetch_timeout_);
  }

  void DeltaSubscriptionImpl::updateResources(const std::set<std::string>& update_to_these_names) override {
