...
void Utility::translateApiConfigSource(
  ...
  if (api_type == ApiType::get().Grpc) {
    ...
    envoy::api::v3alpha::core::GrpcService* grpc_service = api_config_source.add_grpc_services();
    grpc_service->mutable_envoy_grpc()->set_cluster_name(cluster);
  } else {
    if (api_type == ApiType::get().UnsupportedRestLegacy) {
      api_config_source.set_api_type(envoy::api::v3alpha::core::ApiConfigSource::
...
void Utility::checkApiConfigSourceNames(
  ...
  const bool is_grpc =
      (api_config_source.api_type() == envoy::api::v3alpha::core::ApiConfigSource::GRPC ||
       api_config_source.api_type() == envoy::api::v3alpha::core::ApiConfigSource::DELTA_GRPC);
...
void Utility::validateClusterName(const Upstream::ClusterManager::ClusterInfoMap& clusters,
  ...
  if (it == clusters.end() || it->second.get().info()->addedViaApi() ||
      it->second.get().info()->type() == envoy::api::v3alpha::Cluster::EDS) {
    throw EnvoyException(fmt::format(
...
RateLimitSettings Utility::parseRateLimitSettings(
    const envoy::api::v3alpha::core::ApiConfigSource& api_config_source) {
...
Utility::createTagProducer(const envoy::config::bootstrap::v3alpha::Bootstrap& bootstrap) {
  return std::make_unique<Stats::TagProducerImpl>(bootstrap.stats_config());
}

Stats::StatsMatcherPtr
Utility::createStatsMatcher(const envoy::config::bootstrap::v3alpha::Bootstrap& bootstrap) {
...
Grpc::AsyncClientFactoryPtr Utility::factoryForGrpcApiConfigSource(
    ...
    const envoy::api::v3alpha::core::ApiConfigSource& api_config_source, Stats::Scope& scope) {
  Utility::checkApiConfigSourceNames(api_config_source);

  if (api_config_source.api_type() != envoy::api::v3alpha::core::ApiConfigSource::GRPC &&
      api_config_source.api_type() != envoy::api::v3alpha::core::ApiConfigSource::DELTA_GRPC) {
...
envoy::api::v3alpha::ClusterLoadAssignment Utility::translateClusterHosts(
  ...
  envoy::api::v3alpha::endpoint::LocalityLbEndpoints* locality_lb_endpoints =
  ...
  for (const envoy::api::v3alpha::core::Address& host : hosts) {
    envoy::api::v3alpha::endpoint::LbEndpoint* lb_endpoint =
...
BackOffStrategyPtr Utility::prepareDnsRefreshStrategy(const envoy::api::v3alpha::Cluster& cluster,
