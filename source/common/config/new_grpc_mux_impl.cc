...
NewGrpcMuxImpl::NewGrpcMuxImpl(Event::Dispatcher& dispatcher,
...
void NewGrpcMuxImpl::onDiscoveryResponse(
  ...
  pausable_ack_queue_.push(std::move(sub->second->sub_state_.handleResponse(*message)));
  trySendDiscoveryRequests();
}
...
void NewGrpcMuxImpl::trySendDiscoveryRequests() {
  while (true) {
    ...
    if (!pausable_ack_queue_.empty()) {
      ...
      sendGrpcMessage(sub->second->sub_state_.getNextRequestWithAck(pausable_ack_queue_.front()));
      pausable_ack_queue_.pop();
    } else {
      sendGrpcMessage(sub->second->sub_state_.getNextRequestAckless());
    }
  }
