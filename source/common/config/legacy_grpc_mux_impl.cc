...
void LegacyGrpcMuxImpl::start() { grpc_stream_.establishNewStream(); }

void LegacyGrpcMuxImpl::sendDiscoveryRequest(const std::string& type_url) {
  ...
  for (auto& watch : api_state.watches_) {
    for (const std::string& resource : watch->resource_names_) {
...
Watch* LegacyGrpcMuxImpl::addWatch(const std::string& type_url,
  ...
  watches_for_type.insert(watches_for_type.begin(), std::move(watch));
  ...
  return watches_for_type.front().get();
}

void LegacyGrpcMuxImpl::removeWatch(const std::string& type_url, Watch* watch) {
  const bool needs_notification = !watch->resource_names_.empty();
  auto& api_state = apiStateFor(type_url);
  for (auto it = api_state.watches_.begin(); it != api_state.watches_.end(); it++) {
    if (it->get() == watch) {
      api_state.watches_.erase(it);
    }
  }
...
ScopedResume LegacyGrpcMuxImpl::pause(const std::string& type_url) {
  return pause(std::vector<std::string>{type_url});
}

ScopedResume LegacyGrpcMuxImpl::pause(const std::vector<std::string> type_urls) {
...
void LegacyGrpcMuxImpl::onDiscoveryResponse(
...
void LegacyGrpcMuxImpl::onWriteable() { drainRequests(); }

void LegacyGrpcMuxImpl::onStreamEstablished() {
...
void LegacyGrpcMuxImpl::onEstablishmentFailure() {
...
void LegacyGrpcMuxImpl::queueDiscoveryRequest(const std::string& queue_item) {
...
LegacyGrpcMuxImpl::ApiState& LegacyGrpcMuxImpl::apiStateFor(const std::string& type_url) {
...
void LegacyGrpcMuxImpl::drainRequests() {
