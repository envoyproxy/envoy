...
public:
  ...
  static std::unique_ptr<IncrementalSubscription<ResourceType>> subscriptionFromConfigSource(
    ...
    // We allow the non-INCREMENTAL "GRPC" enum value because non-incremental xDS over
    // gRPC is now implemented as a layer over incremental, and therefore is expected to
    // use this factory despite specifying GRPC.
