...
public:
  template <typename T>
  ResponseMapper(const T& config, Server::Configuration::CommonFactoryContext& context,
...
public:
  ...
  template <typename T>
  ResponseMapImpl(const T& config, Server::Configuration::CommonFactoryContext& context,
                  ProtobufMessage::ValidationVisitor& validationVisitor)
...
template <>
ResponseMapPtr Factory::create(const envoy::config::response_map::v3::ResponseMap& config,
