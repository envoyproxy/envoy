...
void QuicStatsGatherer::maybeDoDeferredLog(bool record_ack_timing) {
  ...
  Formatter::HttpFormatterContext log_context{request_header_map_.get(), response_header_map_.get(),
                                              response_trailer_map_.get()};
  log_context.setAccessLogType(AccessLog::AccessLogType::DownstreamEnd);
