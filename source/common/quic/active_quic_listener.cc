...
size_t ActiveQuicListener::numPacketsExpectedPerEventLoop() const {
  // Expect each session to read packets_received_to_connection_count_ratio_ number of packets in
  // this read event.
...
ActiveQuicListenerFactory::ActiveQuicListenerFactory(
    ...
    : concurrency_(concurrency), enabled_(config.enabled()),
      packets_received_to_connection_count_ratio_(PROTOBUF_GET_WRAPPED_OR_DEFAULT(
