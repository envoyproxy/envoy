...
quic::QuicAsyncStatus EnvoyQuicServerProofVerifier::VerifyCertChain(
  ...
  // since the actual client address may not be fully established at this point.
  ...
  // Get the server SSL context to perform client certificate validation.
  ...
  // Client certificates are required - validate the provided certificate chain.
  if (certs.empty()) {
  ...
  // Parse the certificate chain from DER format and build the X509 stack.
