...
void FilterManager::decodeHeaders(ActiveStreamDecoderFilter* filter, RequestHeaderMap& headers,
  ...
  for (; entry != decoder_filters_.end(); entry++) {
    ...
    ENVOY_BUG(!state_.local_complete_ || status == FilterHeadersStatus::StopIteration ||
                  ...
                  status == FilterHeadersStatus::StopAllIterationAndWatermark,
              "Filters should return "
              "FilterHeadersStatus::{StopIteration,StopAllIterationAndBuffer,"
              "StopAllIterationAndWatermark} after sending a local reply.");
