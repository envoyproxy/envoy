...
public:
  ...
  void initialize(Upstream::Host::CreateConnectionData& data, HttpConnPoolImplBase& parent) {
    ...
    codec_client_->setConnectionStats(
        {parent_.host()->cluster().upstreamStats()->upstream_cx_rx_bytes_total_,
         parent_.host()->cluster().upstreamStats()->upstream_cx_rx_bytes_buffered_,
         parent_.host()->cluster().upstreamStats()->upstream_cx_tx_bytes_total_,
         parent_.host()->cluster().upstreamStats()->upstream_cx_tx_bytes_buffered_,
         &parent_.host()->cluster().upstreamStats()->bind_errors_, nullptr});
  }
