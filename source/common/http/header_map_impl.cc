...
HeaderMapImpl::HeaderEntryImpl::HeaderEntryImpl(const LowerCaseString& key, HeaderString&& value,
                                                uint32_t index, bool coalescable)
    : key_(key), value_(std::move(value)), index_(index), coalescable_(coalescable) {}

HeaderMapImpl::HeaderEntryImpl::HeaderEntryImpl(HeaderString&& key, HeaderString&& value,
                                                uint32_t index, bool coalescable)
    : key_(std::move(key)), value_(std::move(value)), index_(index), coalescable_(coalescable) {}
...
void HeaderMapImpl::addViaMove(HeaderString&& key, HeaderString&& value) {
  ...
  if (entry != headers_map_.end()) {
    if (entry->second.front().coalescable_) {
      // A coalescable header, append using delimiter
      const uint64_t added_size =
    ...
    // Create a new entry
    const bool coalescable = canCoalesce(key);
    addSize(key.size() + value.size());
    std::list<HeaderEntryImpl> header_entry;
    header_entry.emplace_back(std::move(key), std::move(value), next_header_index_++, coalescable);
