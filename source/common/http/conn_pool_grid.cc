...
ConnectivityGrid::ConnectivityGrid(
    ...
    OptRef<AlternateProtocols> alternate_protocols,
...
bool ConnectivityGrid::shouldAttemptHttp3() {
  ...
  AlternateProtocols::Origin origin("https", host_->hostname(), port);
  ...
  for (const AlternateProtocols::AlternateProtocol& protocol : protocols.ref()) {
