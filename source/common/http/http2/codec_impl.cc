...
Status ConnectionImpl::trackInboundFrames(int32_t stream_id, size_t length, uint8_t type,
  ...
  const bool end_stream = (type == NGHTTP2_DATA || type == NGHTTP2_HEADERS) && (flags & NGHTTP2_END_STREAM);
  const bool is_empty = (length - padding_length) == 0;
  result = protocol_constraints_.trackInboundFrames(type, end_stream, is_empty);
  if (!result.ok()) {
