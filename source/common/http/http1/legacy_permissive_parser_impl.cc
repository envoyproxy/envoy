...
class LegacyPermissiveHttpParserImpl::Impl {
public:
  Impl(http_parser_permissive::http_parser_type type) {
    http_parser_init(&parser_, type);
    parser_.allow_chunked_length = 1;
  }

  Impl(http_parser_permissive::http_parser_type type, void* data) : Impl(type) {
    parser_.data = data;
    settings_ = {
        [](http_parser_permissive::http_parser* parser) -> int {
        ...
        [](http_parser_permissive::http_parser* parser, const char* at, size_t length) -> int {
          auto* conn_impl = static_cast<ParserCallbacks*>(parser->data);
  ...
  RcVal execute(const char* slice, int len) {
    return {http_parser_permissive::http_parser_execute(&parser_, &settings_, slice, len),
            HTTP_PARSER_ERRNO(&parser_)};
  }

  void resume() { http_parser_permissive::http_parser_pause(&parser_, 0); }

  ParserStatus pause() {
    http_parser_permissive::http_parser_pause(&parser_, 1);
  ...
  bool isChunked() const { return parser_.flags & http_parser_permissive::F_CHUNKED; }

  absl::string_view methodName() const {
    return http_parser_permissive::http_method_str(
        static_cast<http_parser_permissive::http_method>(parser_.method));
  }

  int hasTransferEncoding() const { return parser_.uses_transfer_encoding; }

private:
  http_parser_permissive::http_parser parser_;
  http_parser_permissive::http_parser_settings settings_;
};

LegacyPermissiveHttpParserImpl::LegacyPermissiveHttpParserImpl(MessageType type,
                                                               ParserCallbacks* data) {
  http_parser_permissive::http_parser_type parser_type;
  switch (type) {
  case MessageType::Request:
    parser_type = http_parser_permissive::HTTP_REQUEST;
    break;
  case MessageType::Response:
    parser_type = http_parser_permissive::HTTP_RESPONSE;
    break;
  }
...
LegacyPermissiveHttpParserImpl::~LegacyPermissiveHttpParserImpl() = default;

LegacyPermissiveHttpParserImpl::RcVal LegacyPermissiveHttpParserImpl::execute(const char* slice,
                                                                              int len) {
  return impl_->execute(slice, len);
}

void LegacyPermissiveHttpParserImpl::resume() { impl_->resume(); }

ParserStatus LegacyPermissiveHttpParserImpl::pause() { return impl_->pause(); }

ParserStatus LegacyPermissiveHttpParserImpl::getStatus() { return intToStatus(impl_->getErrno()); }

uint16_t LegacyPermissiveHttpParserImpl::statusCode() const { return impl_->statusCode(); }

int LegacyPermissiveHttpParserImpl::httpMajor() const { return impl_->httpMajor(); }

int LegacyPermissiveHttpParserImpl::httpMinor() const { return impl_->httpMinor(); }

absl::optional<uint64_t> LegacyPermissiveHttpParserImpl::contentLength() const {
  return impl_->contentLength();
}

bool LegacyPermissiveHttpParserImpl::isChunked() const { return impl_->isChunked(); }

absl::string_view LegacyPermissiveHttpParserImpl::methodName() const { return impl_->methodName(); }

absl::string_view LegacyPermissiveHttpParserImpl::errnoName(int rc) const {
  return http_parser_permissive::http_errno_name(
      static_cast<http_parser_permissive::http_errno>(rc));
}

int LegacyPermissiveHttpParserImpl::hasTransferEncoding() const {
  return impl_->hasTransferEncoding();
}

int LegacyPermissiveHttpParserImpl::statusToInt(const ParserStatus code) const {
  // See
  // https://github.com/nodejs/http-parser/blob/5c5b3ac62662736de9e71640a8dc16da45b32503/http_parser.h#L72.
  switch (code) {
