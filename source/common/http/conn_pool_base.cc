...
void ConnPoolImplBase::attachRequestToClient(ActiveClient& client,
  ...
  if (!host_->cluster().resourceManager(priority_).requests().canCreate()) {
    ...
    RequestStreamEncoder& new_encoder = client.newStreamEncoder(response_decoder);
...
ConnectionPool::Cancellable* ConnPoolImplBase::newStream(ResponseStreamDecoder& response_decoder,
                                                         ConnectionPool::Callbacks& callbacks) {
...
ConnPoolImplBase::PendingRequest::PendingRequest(ConnPoolImplBase& parent,
                                                 ResponseStreamDecoder& decoder,
                                                 ConnectionPool::Callbacks& callbacks)
...
ConnPoolImplBase::newPendingRequest(ResponseStreamDecoder& decoder,
                                    ConnectionPool::Callbacks& callbacks) {
