...
void MultiplexedActiveClientBase::onGoAway(Http::GoAwayErrorCode) {
  ...
  parent_.host()->cluster().upstreamStats()->upstream_cx_close_notify_.inc();
...
void MultiplexedActiveClientBase::onStreamReset(Http::StreamResetReason reason) {
  ...
  case StreamResetReason::ConnectionFailure:
    parent_.host()->cluster().upstreamStats()->upstream_rq_pending_failure_eject_.inc();
  ...
  case StreamResetReason::OverloadManager:
    parent_.host()->cluster().upstreamStats()->upstream_rq_tx_reset_.inc();
    break;
  case StreamResetReason::RemoteReset:
    parent_.host()->cluster().upstreamStats()->upstream_rq_rx_reset_.inc();
    break;
