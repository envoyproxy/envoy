...
tcpProxyConfig(const envoy::api::v2::route::RouteAction& route_config,
  ...
  return std::make_shared<TcpProxy::TcpProxyConfig>(tcp_config, factory_context);
}
...
WsHandlerImpl::WsHandlerImpl(HeaderMap& request_headers,
    ...
    : TcpProxy::TcpProxyFilter(config, cluster_manager), request_headers_(request_headers),
...
Network::FilterStatus WsHandlerImpl::onData(Buffer::Instance& data, bool end_stream) {
  ...
  case ConnectState::Connected:
    ...
    return TcpProxy::TcpProxyFilter::onData(data, end_stream);
...
void WsHandlerImpl::onConnectionSuccess() {
  ...
  if (queued_data_.length() > 0 || queued_end_stream_) {
    ...
    TcpProxy::TcpProxyFilter::onData(queued_data_, queued_end_stream_);
    ASSERT(queued_data_.length() == 0);
  }
}
