...
double DefaultsProfile::getNumber(const ConfigContext& config, const std::string& field,
  ...
  const absl::optional<ProtobufWkt::Value> value =
      getProtoValue(std::string(config.getContext()), field);
...
std::chrono::milliseconds DefaultsProfile::getMs(const ConfigContext& config,
  ...
  if (value && value->has_string_value()) {
    ...
    const auto result = ProtobufUtil::JsonStringToMessage(
        absl::StrCat("\"", value->string_value(), "\""), &duration);
...
absl::optional<ProtobufWkt::Value> DefaultsProfile::getProtoValue(const std::string& config_name,
  ...
  const auto config_it = fields_map.find(config_name);
  ...
  if (config_it != fields_map.end()) {
    ...
    const auto value_it = fields_submap.find(field);
  ...
  const auto value_it = fields_map.find(full_field_name);
