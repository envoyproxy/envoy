...
public:
  class Options {
  public:
    Options(std::chrono::milliseconds interval, std::chrono::milliseconds target_delay)
        : interval_(interval), target_delay_(target_delay) {}

    std::chrono::milliseconds interval() const { return interval_; }
    std::chrono::milliseconds targetDelay() const { return target_delay_; }
  ...
  Codel(TimeSource& time_source);

  /// Preferable construction method
  explicit Codel(TimeSource& time_source, const Options& options);
  ...
  bool overloaded(std::chrono::nanoseconds delay) {
    return overloadedExplicitNow(delay, time_source_.monotonicTime());
  }
  bool overloadedExplicitNow(std::chrono::nanoseconds delay,
  ...
  absl::StatusOr<bool> setOptions(Options const& options);
  ...
  std::chrono::milliseconds getSloughTimeout(std::chrono::milliseconds delay) const;

private:
  std::atomic<uint64_t> codel_min_delay_ns_;
  std::atomic<uint64_t> codel_interval_time_ns_;

  std::atomic<std::chrono::milliseconds> target_delay_;
  ...
  std::atomic<bool> codel_reset_delay_;

  std::atomic<bool> overloaded_;
  TimeSource& time_source_;
};

using CodelPtr = std::unique_ptr<Codel>;
