 ...
 * @note  Sizes are not compared, so ensure the src is of size sizeof(*(dst)) before proceeding to call safeMemcpyUnsafeSrc
 */
#define safeMemcpyUnsafeSrc(dst, src) memcpy(dst, src, sizeof(*(dst)));

/**
 * @brief Copies src to dst based on the size of src
 * @note  Sizes are not compared, so ensure the dst is of size sizeof(*(src)) before proceeding to call safeMemcpyUnsafeDst
 */
