...
CompiledGoogleReMatcher::CompiledGoogleReMatcher(const std::string& regex, bool do_program_size_check)
    ...
    if (do_program_size_check) {
      ...
      if (runtime) {
        ...
        Stats::StatNameManagedStorage warn_count_stat_name("re2.exceeded_warn_level",
                                                           root_scope.symbolTable());
        Stats::Counter& warn_count =
            root_scope.counterFromStatName(warn_count_stat_name.statName());
...
CompiledGoogleReMatcher::CompiledGoogleReMatcher(const envoy::type::matcher::v3::RegexMatcher& config)
