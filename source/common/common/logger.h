...
extern void updateFancyLogger(const char* file, spdlog::logger* logger, int* local_epoch);
...
#define FANCY_LOG(LEVEL, ...)                                                                \
  do {                                                                                       \
    ...
    static spdlog::logger flogger(__FILE__, sink);                  \
    if (!global_epoch__ || local_epoch < global_epoch__->load()) {                           \
