...
TokenBucketOriginalImpl::TokenBucketOriginalImpl(uint64_t max_tokens, TimeSource& time_source,
                                                 double fill_rate)
...
uint64_t TokenBucketOriginalImpl::consume(uint64_t tokens, bool allow_partial) {
...
std::chrono::milliseconds TokenBucketOriginalImpl::nextTokenAvailable() {
...
void TokenBucketOriginalImpl::maybeReset(uint64_t num_tokens) {
