...
using envoy::config::filter::network::http_connection_manager::v3alpha::ScopedRoutes;
...
public:
  ScopedRouteInfo(envoy::api::v3alpha::ScopedRouteConfiguration&& config_proto,
  ...
  const envoy::api::v3alpha::ScopedRouteConfiguration& configProto() const { return config_proto_; }
  const std::string& scopeName() const { return config_proto_.name(); }

private:
  envoy::api::v3alpha::ScopedRouteConfiguration config_proto_;
  ScopeKey scope_key_;
