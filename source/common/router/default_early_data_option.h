...
#include "envoy/config/route/v3/default_early_data_option.pb.h"
...
public:
  std::string name() const override { return "envoy.route.early_data_options.default"; }

  EarlyDataOptionPtr createEarlyDataOption(const Protobuf::Message& config) override {
    auto& early_data_config =
        dynamic_cast<const envoy::config::route::v3::DefaultEarlyDataOption&>(config);
