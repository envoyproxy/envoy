...
Http::FilterHeadersStatus Filter::decodeHeaders(Http::RequestHeaderMap& headers, bool end_stream) {
  ...
  on_host_selected_ = ([this, cluster, end_stream,
    ...
    continueDecodeHeaders(cluster, *downstream_headers_, end_stream, modify_headers,
                          &should_continue, std::move(host));
    // continueDecodeHeaders can itself send a local reply, in which case should_continue should be
    // false. If this is not the case, we can continue the filter chain due to successful
...
Filter::continueDecodeHeaders(Upstream::ThreadLocalCluster* cluster,
                              Http::RequestHeaderMap& headers, bool end_stream,
                              std::function<void(Http::ResponseHeaderMap&)> modify_headers,
                              bool* should_continue, Upstream::HostConstSharedPtr&& selected_host) {
  const StreamInfo::FilterStateSharedPtr& filter_state = callbacks_->streamInfo().filterState();
  const DebugConfig* debug_config = filter_state->getDataReadOnly<DebugConfig>(DebugConfig::key());

  std::unique_ptr<GenericConnPool> generic_conn_pool;
  if (selected_host) {
    generic_conn_pool = createConnPool(*cluster, selected_host);
  }
