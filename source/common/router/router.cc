...
Http::FilterHeadersStatus Filter::decodeHeaders(Http::HeaderMap& headers, bool end_stream) {
  ...
  // Contains all fallthru route names we hit while routing this request
  std::string fallthru_route_names("");
  // if this is the fallthru route then keep on picking routes till we find non-fallthru route or
  // run out of the routes
  while (route_ && route_->routeEntry() && route_->routeEntry()->fallthru()) {
    if (route_->routeEntry()->addRouteNameToStreamInfo()) {
      fallthru_route_names.append(route_->routeEntry()->routeName() + ";");
    }
    route_ = callbacks_->route();
  }

  ENVOY_STREAM_LOG(trace, "fallthru route names={}", *callbacks_, fallthru_route_names);

  // Add to access logs if we have encountered some fallthru routes
  if (fallthru_route_names.length() != 0) {
    callbacks_->streamInfo().setFallthruRouteNames(fallthru_route_names);
  }
