...
void rebuildRouteConfigVirtualHosts(
    const std::map<std::string, envoy::config::route::v3::VirtualHost>&
        rds_vhosts,
    const std::map<std::string, envoy::config::route::v3::VirtualHost>&
        vhds_vhosts,
...
bool RouteConfigUpdateReceiverImpl::onRdsUpdate(
  ...
  const uint64_t new_vhds_config_hash =
      rc.has_vhds() ? MessageUtil::hash(rc.vhds()) : 0ul;
  std::map<std::string, envoy::config::route::v3::VirtualHost>
      rds_virtual_hosts;
  ...
  rebuildRouteConfigVirtualHosts(rds_virtual_hosts, *vhds_virtual_hosts_,
                                 new_route_config);
  auto new_config = std::make_shared<ConfigImpl>(
      new_route_config, optional_http_filters_, factory_context_,
      factory_context_.messageValidationContext().dynamicValidationVisitor(),
      false);
