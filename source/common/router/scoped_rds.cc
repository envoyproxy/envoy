...
ConfigProviderPtr create(
  ...
  case envoy::config::filter::network::http_connection_manager::v3alpha::ScopedRoutes::
      ConfigSpecifierCase::kScopedRouteConfigurationsList: {
    const envoy::config::filter::network::http_connection_manager::v3alpha::
    ...
    return scoped_routes_config_provider_manager.createStaticConfigProvider(
        RepeatedPtrUtil::convertToConstMessagePtrContainer<
            envoy::api::v3alpha::ScopedRouteConfiguration, ProtobufTypes::ConstMessagePtrVector>(
...
InlineScopedRoutesConfigProvider::InlineScopedRoutesConfigProvider(
    ...
    envoy::config::filter::network::http_connection_manager::v3alpha::ScopedRoutes::ScopeKeyBuilder
        scope_key_builder)
...
bool ScopedRdsConfigSubscription::addOrUpdateScopes(
  ...
  for (const auto& resource : resources) {
    ...
    try {
      scoped_route_config = MessageUtil::anyConvert<envoy::api::v3alpha::ScopedRouteConfiguration>(
          resource.resource());
...
void ScopedRdsConfigSubscription::onRdsConfigUpdate(const std::string& scope_name,
  ...
  auto new_scoped_route_info = std::make_shared<ScopedRouteInfo>(
      envoy::api::v3alpha::ScopedRouteConfiguration(iter->second->configProto()),
...
void ScopedRdsConfigSubscription::onConfigUpdate(
    ...
    const std::string& version_info) {
  absl::flat_hash_map<std::string, envoy::api::v3alpha::ScopedRouteConfiguration> scoped_routes;
  ...
  for (const auto& resource_any : resources) {
    ...
    auto scoped_route =
        MessageUtil::anyConvert<envoy::api::v3alpha::ScopedRouteConfiguration>(resource_any);
    ...
    const envoy::api::v3alpha::ScopedRouteConfiguration& scoped_route_config =
...
ProtobufTypes::MessagePtr ScopedRoutesConfigProviderManager::dumpConfigs() const {
  auto config_dump = std::make_unique<envoy::admin::v3alpha::ScopedRoutesConfigDump>();
  ...
  for (const auto& provider : immutableConfigProviders(ConfigProviderInstanceType::Inline)) {
    const auto protos_info =
        provider->configProtoInfoVector<envoy::api::v3alpha::ScopedRouteConfiguration>();
          ...
          [&config_source_proto, &factory_context, &stat_prefix,
           &typed_optarg](const uint64_t manager_identifier,
            ...
            const auto& scoped_rds_config_source = dynamic_cast<
                const envoy::config::filter::network::http_connection_manager::v3alpha::ScopedRds&>(
                config_source_proto);
