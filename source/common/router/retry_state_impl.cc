...
void RetryStateImpl::enableBackoffTimer() {
  ...
  if (ratelimited_backoff_strategy_ != nullptr) {
    ...
    cluster_.upstreamStats()->upstream_rq_retry_backoff_ratelimited_.inc();

  } else {
    // Otherwise we use a fully jittered exponential backoff algorithm.
    retry_timer_->enableTimer(std::chrono::milliseconds(backoff_strategy_->nextBackOffMs()));

    cluster_.upstreamStats()->upstream_rq_retry_backoff_exponential_.inc();
  }
}
...
RetryStatus RetryStateImpl::shouldRetry(RetryDecision would_retry, DoRetryCallback callback) {
  ...
  if ((backoff_callback_ || next_loop_callback_) && would_retry == RetryDecision::NoRetry) {
    cluster_.upstreamStats()->upstream_rq_retry_success_.inc();
    if (vcluster_) {
  ...
  if (retries_remaining_ == 0) {
    cluster_.upstreamStats()->upstream_rq_retry_limit_exceeded_.inc();
    if (vcluster_) {
  ...
  if (!cluster_.resourceManager(priority_).retries().canCreate()) {
    cluster_.upstreamStats()->upstream_rq_retry_overflow_.inc();
    if (vcluster_) {
  ...
  cluster_.upstreamStats()->upstream_rq_retry_.inc();
  if (vcluster_) {
