...
Rds::ConfigConstSharedPtr StaticRouteConfigProviderImpl::config() const {
  ...
  return route_config ? route_config : base_.config();
}

const std::optional<RouteConfigProvider::ConfigInfo>&
StaticRouteConfigProviderImpl::configInfo() const {
  // With VHDS this is unset until a route configuration has actually been published, i.e. until the
  // initial VHDS fetch has landed.
  if (vhds_context_) {
...
SystemTime StaticRouteConfigProviderImpl::lastUpdated() const {
  // Zero until a route configuration has been published, for the same reason as configInfo().
  if (vhds_context_) {
