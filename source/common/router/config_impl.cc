...
RetryPolicyImpl::RetryPolicyImpl(const envoy::config::route::v3::RetryPolicy& retry_policy,
  ...
  if (!retry_header.name().empty()) {
    auto& factory =
        Envoy::Config::Utility::getAndCheckFactory<Upstream::RetryHeaderFactory>(retry_header);
    retry_header_config_ =
        std::make_pair(&factory, Envoy::Config::Utility::translateToFactoryConfig(
                                     retry_header, validation_visitor, factory));
  }
...
Upstream::RetryHeaderSharedPtr RetryPolicyImpl::retryHeader() const {
  if (retry_header_config_.first == nullptr) {
    return nullptr;
  }
