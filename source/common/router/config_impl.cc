...
RouteEntryImplBase::RouteEntryImplBase(const VirtualHostImpl& vhost,
    ...
    : case_sensitive_(PROTOBUF_GET_WRAPPED_OR_DEFAULT(route.match(), case_sensitive, true)),
      ...
      use_grpc_timeout_(route.route().use_grpc_timeout()), runtime_(loadRuntimeData(route.match())),
      loader_(factory_context.runtime()), host_redirect_(route.redirect().host_redirect()),
