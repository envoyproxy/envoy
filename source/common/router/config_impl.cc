...
RouteSpecificFilterConfigConstSharedPtr PerFilterConfigs::createRouteSpecificFilterConfig(
    ...
    Server::Configuration::ServerFactoryContext& factory_context,
    ProtobufMessage::ValidationVisitor& validator) {
  // TODO(wbpcode): new optional support will be add in the
  // https://github.com/envoyproxy/envoy/pull/27210
...
PerFilterConfigs::PerFilterConfigs(
  ...
  for (const auto& it : typed_configs) {
    ...
    auto object = createRouteSpecificFilterConfig(name, it.second, factory_context, validator);
    if (object != nullptr) {
