...
void UpstreamCodecFilter::onBelowWriteBufferLowWatermark() {
  callbacks_->clusterInfo()->upstreamStats()->upstream_flow_control_resumed_reading_total_.inc();
  callbacks_->upstreamCallbacks()->upstream()->readDisable(false);
}

void UpstreamCodecFilter::onAboveWriteBufferHighWatermark() {
  callbacks_->clusterInfo()->upstreamStats()->upstream_flow_control_paused_reading_total_.inc();
