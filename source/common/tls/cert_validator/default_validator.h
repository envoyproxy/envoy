...
struct SharedCrlList {
  std::vector<bssl::UniquePtr<X509_CRL>> crls;
};
using SharedCrlListSharedPtr = std::shared_ptr<SharedCrlList>;
...
//     TLS context referencing it is torn down (for example after an xDS update).
...
class CrlCache : public Singleton::Instance {
public:
  ...
  absl::StatusOr<SharedCrlListSharedPtr> getOrCreate(const std::string& crl_pem,
...
private:
  // Keyed by CRL PEM content; stores a weak_ptr so entries do not outlive the
  // contexts that use them.
  absl::flat_hash_map<std::string, std::weak_ptr<SharedCrlList>> cache_;
};
...
private:
  ...
  // Process-wide CRL cache, held to keep it alive while this validator uses it.
  std::shared_ptr<CrlCache> crl_cache_;
  // The parsed CRLs shared with other TLS contexts that reference the same CRL.
  SharedCrlListSharedPtr shared_crl_;
