...
Tracing::SpanPtr Tracer::startSpan(const Tracing::Config&, Tracing::TraceContext& trace_context,
  ...
  const bool ignore_envoy_decision = span.trace_segment().sampling_decision().has_value();
  if (!ignore_envoy_decision && !tracing_decision.traced) {
    // TODO(wbpcode): use USER_KEEP to indicate that the trace should be kept if the
    // Envoy is telling us to keep the trace. By this way, this will be consistent with
    // the setDecision() method.
    span.trace_segment().override_sampling_priority(
        int(datadog::tracing::SamplingPriority::USER_DROP));
  }

  return std::make_unique<Span>(std::move(span), ignore_envoy_decision);
}
