...
public:
   ...
   */
  SamplingRequest(const std::string& host_name, const std::string& http_method,
...
public:
   ...
   */
  SamplingRule()
      : host_(), service_name_(), http_method_(), url_path_(), fixed_target_(-1), rate_(-1.0f) {}

  /**
   * Constructor for default sampling rule.
   */
  SamplingRule(int fixed_target, float rate, Reservoir& reservoir);
   ...
   * @param fixed_target The number of traces per any given second for which the sampling rule will
   * sample.
  ...
  SamplingRule(std::string& host_name, std::string& service_name, std::string& http_method,
               std::string& url_path, int fixed_target, float rate, Reservoir& reservoir);
  ...
  /**
   * Sets the reservoir (copy semantics).
   * @param reservoir
   */
  void setReservoir(Reservoir& reservoir) { reservoir_ = reservoir; }
  ...
  /**
   * @return the copy of the const reservoir.
   */
  const Reservoir& reservoir() const { return reservoir_.value(); }

  /**
   * @return the copy of the reservoir.
   */
  Reservoir& reservoir() { return reservoir_.value(); }

  /**
   * @return true if the reservoir attribute is set, or false otherwise.
   */
  bool isSetReservoir() const { return reservoir_.has_value(); }

  /**
   * Determines whether or not this sampling rule applies to the incoming request based on some of
   * the request's parameters. Any null parameters provided will be considered an implicit match.
   * For example, {@code appliesTo(null, null, null)} will always return {@code true}, for any rule.
   *
...
private:
  ...
  int fixed_target_ = -1;
  float rate_ = -1.0f;
  absl::optional<Reservoir> reservoir_;
};

/**
 * SamplingRuleManifest represents a full sampling ruleset, with a list of custom rules and default
 * values for incoming requests.
 */
...
public:
   ...
   * @param rules
   */
  SamplingRuleManifest(int version, SamplingRule& default_rule, std::vector<SamplingRule>& rules)
      : version_(version), default_rule_(default_rule), rules_(rules) {}
  ...
  /**
   * Given a json file contains sampling rules and then serialize it.
   * @param json_file
   */
  void deSerialize(std::string json_file);

  /**
   * The version of sampling rule json file.
   */
...
public:
  ...
  /**
   * ShouldTrace consults sampling rule set to determine if the given request should be traced or
   * not.
   */
...
/**
 * LocalizedSamplingStrategy implements SamplingStrategy class and performs sampling for traces
 * based on sampling rules json file customer provided or default one.
 */
...
public:
   ...
   * @param json_file
   */
  LocalizedSamplingStrategy(std::string json_file, TimeSource& time_source);
   ...
   */
  SamplingRuleManifest& customRuleManifest() { return custom_rule_manifest_.value(); }

  /**
   * @return true if the custom rule manifest attribute is set, or false otherwise.
   */
  bool isSetCustomRuleManifest() const { return custom_rule_manifest_.has_value(); }

  /**
   * shouldTrace takes sampling request and then determines whether the request will be traced or
   * not.
...
private:
  ...
  absl::optional<SamplingRuleManifest> custom_rule_manifest_;
