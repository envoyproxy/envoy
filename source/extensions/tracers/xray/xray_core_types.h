...
public:
   ...
   */
  ChildSpan(TimeSource& time_source) : name_(), id_(0), start_time_(0), time_source_(time_source) {}
...
public:
   ...
   */
  Span(TimeSource& time_source)
      : trace_id_(), name_(), id_(0), sampled_(false), start_time_(0), time_source_(time_source) {}
   ...
   */
  void addChildSpan(const ChildSpan& child) { child_span_.push_back(child); }
  ...
  /**
   * Marks a successful end of the span. This method will: invoke the tracer's reportSpan() method
   * if a tracer has been associated with the span.
   */
  void finish();
