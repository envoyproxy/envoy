...
void XRaySpan::finishSpan() { span_.finish(); }
...
Driver::Driver(const envoy::config::trace::v2::XRayConfig& xray_config,
  ...
  if (xray_config.json_file().size() > 0) {
    json_file = xray_config.json_file();
  }
  tls_->set([this, daemon_endpoint, span_name, json_file,
             &random_generator](Event::Dispatcher&) -> ThreadLocal::ThreadLocalObjectSharedPtr {
    LocalizedSamplingStrategy localized_sampling_strategy =
        LocalizedSamplingStrategy(json_file, time_source_);
    TracerPtr tracer(
        new Tracer(span_name, random_generator, localized_sampling_strategy, time_source_));
    Network::Address::InstanceConstSharedPtr server_address_ =
        Network::Utility::parseInternetAddressAndPort(daemon_endpoint, false);
...
Tracing::SpanPtr Driver::startSpan(const Tracing::Config& config, Http::HeaderMap& request_headers,
  ...
  if (!xray_header.sampleDecision().empty()) {
    ...
    SamplingRequest sampling_request = SamplingRequest(request_headers.Host()->value().c_str(),
                                                       request_headers.Method()->value().c_str(),
  ...
  if (!xray_header.traceId().empty() || !xray_header.parentId().empty()) {
    ...
    if (!xray_header.parentId().empty() &&
        !StringUtil::atoull(xray_header.parentId().c_str(), parent_id, 16)) {
  ...
  std::string sample_decision = new_xray_span->sampled() ? XRayCoreConstants::get().SAMPLED
                                                         : XRayCoreConstants::get().NOT_SAMPLED;
  std::string xray_header_string = XRayCoreConstants::get().ROOT_PREFIX + new_xray_span->traceId() +
                                   "; " + XRayCoreConstants::get().PARENT_PREFIX +
                                   new_xray_span->childSpans()[0].idAsHexString() + "; " +
