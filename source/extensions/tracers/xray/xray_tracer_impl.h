...
public:
   ...
   */
  Driver(const envoy::config::trace::v2::XRayConfig& xray_config, ThreadLocal::SlotAllocator& tls,
         Runtime::Loader& runtime, const LocalInfo::LocalInfo& localinfo,
         Runtime::RandomGenerator& random_generator, TimeSource& time_source);

  /**
   * This function is inherited from the abstract Driver class.
   *
   * It starts a new xray span. Depending on the request headers, it can create a root span or a
   * shared-context span.
   */
