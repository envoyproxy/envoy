...
void callSampler(SamplerPtr sampler, absl::StatusOr<SpanContext> span_context, Span& new_span,
  ...
  if (sampler) {
    ...
    auto modified_tracestate =
        sampler->modifyTracestate(new_span.spanId(), sampling_result.tracestate);
