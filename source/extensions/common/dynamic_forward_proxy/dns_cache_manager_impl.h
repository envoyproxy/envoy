...
public:
  ...
  DnsCacheSharedPtr getCache(
      const envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig& config) override;

private:
  struct ActiveCache {
    ActiveCache(const envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig& config,
                DnsCacheSharedPtr cache)
        : config_(config), cache_(cache) {}

    const envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig config_;
