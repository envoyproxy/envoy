...
absl::StatusOr<std::shared_ptr<DnsCacheImpl>> DnsCacheImpl::createDnsCacheImpl(
  ...
  return std::shared_ptr<DnsCacheImpl>(new DnsCacheImpl(
      context, config, std::move(*resolver_or_error), std::move(deny_address_matcher)));
}
...
DnsCacheImpl::DnsCacheImpl(
    ...
    : main_thread_dispatcher_(context.serverFactoryContext().mainThreadDispatcher()),
      ...
      deny_address_matcher_(std::move(deny_address_matcher)) {
...
void DnsCacheImpl::finishResolve(const std::string& host,
  ...
  // Filter out addresses matching deny_address_matcher.
  if (deny_address_matcher_ != nullptr && !response.empty()) {
    ...
    response.remove_if([this](const Network::DnsResponse& dns_resp) {
      ...
      return deny_address_matcher_->match(*address);
    });
    const size_t denied_count = original_size - response.size();
    if (denied_count > 0) {
      stats_.dns_address_denied_.add(denied_count);
      ENVOY_LOG(debug, "denied {} address(es) for host '{}' matching deny_address_matcher",
                denied_count, host);
      if (response.empty()) {
