...
void DnsCacheImpl::loadCacheEntries(
  ...
  KeyValueStore::ConstIterateCb load = [this](const std::string& key, const std::string& value) {
    ...
    ENVOY_LOG_EVENT(debug, "dns_cache_load_finished",
                    "persistent dns cache load complete for host '{}': {}", key,
                    accumulateToString<Network::DnsResponse>(responses, [](const auto& dns_response) {
                      return dns_response.addrInfo().address_->asString();
                    }));
