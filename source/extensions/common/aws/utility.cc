...
Utility::canonicalizeHeaders(const Http::RequestHeaderMap& headers,
  ...
  headers.iterate(
      [&out, &excluded_headers, &included_headers](const Http::HeaderEntry& entry) -> Http::HeaderMap::Iterate {
        // Skip empty headers
        ...
        const bool is_required_header = absl::StartsWith(key_lower, "x-amz-") ||
        ...
        ENVOY_LOG(trace, "included_headers.empty() = {}, included_headers.size() = {}, "
