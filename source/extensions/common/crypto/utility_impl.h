...
public:
  ...
  std::vector<uint8_t> getSha256Hmac(const std::vector<uint8_t>& key, absl::string_view message) override;
...
std::unique_ptr<Crypto::ScopedUtilitySingleton> utility_ = std::make_unique<Crypto::ScopedUtilitySingleton>(std::make_unique<Crypto::UtilityImpl>());
