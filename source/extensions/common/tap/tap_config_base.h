...
protected:
  TapConfigBaseImpl(envoy::config::tap::v3alpha::TapConfig&& proto_config,
...
private:
  ...
  Sink* sink_to_use_;
  SinkPtr sink_;
  envoy::config::tap::v3alpha::OutputSink::Format sink_format_;
...
public:
  FilePerTapSink(const envoy::config::tap::v3alpha::FilePerTapSink& config) : config_(config) {}
...
private:
  ...
  const envoy::config::tap::v3alpha::FilePerTapSink config_;
};
