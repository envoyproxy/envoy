...
bool Utility::addBufferToProtoBytes(envoy::data::tap::v3alpha::Body& output_body,
...
TapConfigBaseImpl::TapConfigBaseImpl(envoy::config::tap::v3alpha::TapConfig&& proto_config,
  ...
  case envoy::config::tap::v3alpha::OutputSink::OutputSinkTypeCase::kStreamingAdmin:
    ...
    RELEASE_ASSERT(sink_format_ == envoy::config::tap::v3alpha::OutputSink::JSON_BODY_AS_BYTES ||
                       sink_format_ == envoy::config::tap::v3alpha::OutputSink::JSON_BODY_AS_STRING,
                   "admin output only supports JSON formats");
    sink_to_use_ = admin_streamer;
    break;
  case envoy::config::tap::v3alpha::OutputSink::OutputSinkTypeCase::kFilePerTap:
    sink_ =
...
namespace {
void swapBytesToString(envoy::data::tap::v3alpha::Body& body) {
  body.set_allocated_as_string(body.release_as_bytes());
}
} // namespace

void Utility::bodyBytesToString(envoy::data::tap::v3alpha::TraceWrapper& trace,
                                envoy::config::tap::v3alpha::OutputSink::Format sink_format) {
  ...
  if (sink_format != envoy::config::tap::v3alpha::OutputSink::JSON_BODY_AS_STRING) {
    return;
  }

  switch (trace.trace_case()) {
  case envoy::data::tap::v3alpha::TraceWrapper::TraceCase::kHttpBufferedTrace: {
  ...
  case envoy::data::tap::v3alpha::TraceWrapper::TraceCase::kHttpStreamedTraceSegment: {
  ...
  case envoy::data::tap::v3alpha::TraceWrapper::TraceCase::kSocketBufferedTrace: {
  ...
  case envoy::data::tap::v3alpha::TraceWrapper::TraceCase::kSocketStreamedTraceSegment: {
  ...
  case envoy::data::tap::v3alpha::TraceWrapper::TraceCase::TRACE_NOT_SET:
    NOT_REACHED_GCOVR_EXCL_LINE;
  }
}
...
void FilePerTapSink::FilePerTapSinkHandle::submitTrace(
  ...
  if (!output_file_.is_open()) {
    ...
    switch (format) {
    case envoy::config::tap::v3alpha::OutputSink::PROTO_BINARY:
      path += MessageUtil::FileExtensions::get().ProtoBinary;
      break;
    case envoy::config::tap::v3alpha::OutputSink::PROTO_BINARY_LENGTH_DELIMITED:
      path += MessageUtil::FileExtensions::get().ProtoBinaryLengthDelimited;
      break;
    case envoy::config::tap::v3alpha::OutputSink::PROTO_TEXT:
