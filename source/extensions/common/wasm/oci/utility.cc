...
absl::Status parseImageURI(const std::string& uri, std::string& registry, std::string& image_name,
  ...
  if (!absl::StartsWith(uri, prefix)) {
    return {absl::StatusCode::kInvalidArgument, "Failed to parse image URI - unsupported scheme"};
  }
  ...
  if (slash_pos == std::string::npos) {
    return {absl::StatusCode::kInvalidArgument,
            "Failed to parse image name - URI does not include '/'"};
  }
  ...
  if (colon_pos == std::string::npos || colon_pos < slash_pos + 1) {
    return {absl::StatusCode::kInvalidArgument,
            "Failed to parse image tag - URI does not include ':'"};
  }
