...
absl::StatusOr<std::string> prepareAuthorizationHeader(std::string image_pull_secret_raw, std::string registry) {
  ...
  try {
    Json::ObjectSharedPtr image_pull_secret = THROW_OR_RETURN_VALUE(Json::Factory::loadFromString(image_pull_secret_raw), Json::ObjectSharedPtr);
  } catch (EnvoyException& e) {
    return absl::InvalidArgumentError(absl::StrCat("Failed to parse image pull secret: ", e.what()));
  }
  ...
  try {
    ...
    return absl::InvalidArgumentError(absl::StrCat("Did not find 'auths' key in the image pull secret: ", e.what()));
  }
  ...
  try {
    ...
    return absl::InvalidArgumentError(absl::StrCat("Did not find 'auths.", registry, "' key in the image pull secret: ", e.what()));
  }

  auto auth = registry_object->getString("auth");
  if (!auth.ok()) {
    return absl::InvalidArgumentError(absl::StrCat("Did not find 'auths.", registry, ".auth' key in the image pull secret"));
  }
  ...
  // ECR uses basic auth and the "auth" key in the image pull secret should contain base64-encoded AWS:<password>
  // so it ca be passed as is to the request.
