...
public:
  RequestChunker(uint32_t max_dp, uint32_t max_rm) : max_dp_(max_dp), max_rm_(max_rm) {}
...
private:
  ...
  MetricsExportRequestPtr current_request_ = std::make_unique<MetricsExportRequest>();
      ...
      for (const auto& metric : sm.metrics()) {
        ...
        case opentelemetry::proto::metrics::v1::Metric::DataCase::kGauge:
          for (const auto& dp : metric.gauge().data_points()) {
            chunker.appendDataPoint(rm, sm, metric, [&](auto* m) { *m->mutable_gauge()->add_data_points() = dp; });
          }
          break;
        case opentelemetry::proto::metrics::v1::Metric::DataCase::kSum:
          for (const auto& dp : metric.sum().data_points()) {
            chunker.appendDataPoint(rm, sm, metric, [&](auto* m) { *m->mutable_sum()->add_data_points() = dp; });
          }
          break;
        case opentelemetry::proto::metrics::v1::Metric::DataCase::kHistogram:
          for (const auto& dp : metric.histogram().data_points()) {
            chunker.appendDataPoint(rm, sm, metric, [&](auto* m) { *m->mutable_histogram()->add_data_points() = dp; });
          }
          break;
        case opentelemetry::proto::metrics::v1::Metric::DataCase::kExponentialHistogram:
          for (const auto& dp : metric.exponential_histogram().data_points()) {
            chunker.appendDataPoint(rm, sm, metric, [&](auto* m) { *m->mutable_exponential_histogram()->add_data_points() = dp; });
          }
          break;
        case opentelemetry::proto::metrics::v1::Metric::DataCase::kSummary:
          for (const auto& dp : metric.summary().data_points()) {
            chunker.appendDataPoint(rm, sm, metric, [&](auto* m) { *m->mutable_summary()->add_data_points() = dp; });
          }
          break;
        default:
          break;
