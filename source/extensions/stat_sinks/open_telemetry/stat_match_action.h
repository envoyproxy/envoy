...
class OnMatchAction
    : public Matcher::ActionBase<envoy::extensions::stat_sinks::open_telemetry::
                                     v3::SinkConfig::ConversionAction> {
 public:
  explicit OnMatchAction(envoy::extensions::stat_sinks::open_telemetry::v3::
                             SinkConfig::ConversionAction config)
      : config_(config) {}

  const envoy::extensions::stat_sinks::open_telemetry::v3::SinkConfig::
      ConversionAction*
      config() const {
    return &config_;
  }
...
class ActionValidationVisitor
 ...
 public:
  absl::Status performDataInputValidation(
      const Matcher::DataInputFactory<Stats::StatMatchingData>&,
 ...
 public:
  Matcher::ActionConstSharedPtr createAction(
    ...
    const auto& action_config = MessageUtil::downcastAndValidate<
        const envoy::extensions::stat_sinks::open_telemetry::v3::SinkConfig::
            ConversionAction&>(config, validation_visitor);
    return std::make_shared<OnMatchAction>(action_config);
  }

  std::string name() const override {
    return "otlp_metric_conversion_action_factory";
  }

  ProtobufTypes::MessagePtr createEmptyConfigProto() override {
    return std::make_unique<envoy::extensions::stat_sinks::open_telemetry::v3::
