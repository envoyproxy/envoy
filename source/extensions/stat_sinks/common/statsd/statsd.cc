...
void UdpStatsdSink::onHistogramComplete(const Stats::Histogram& histogram, uint64_t value) {
  // TODO: for statsd histograms are all timers in milliseconds, Envoy histograms are however not
  // necessarily timers in milliseconds, for Envoy histograms suffixed with their corresponding SI
  // unit symbol this is acceptable, but for histograms without a suffix, especially those which are
  // timers but record in units other than milliseconds, it may make sense to scale the value to
...
void TcpStatsdSink::TlsSink::onTimespanComplete(const std::string& name,
  ...
  // TODO: see the comment at UdpStatsdSink::onHistogramComplete.
  ASSERT(current_slice_mem_ == nullptr);
