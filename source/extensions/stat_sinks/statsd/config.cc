...
Stats::SinkPtr StatsdSinkFactory::createStatsSink(const Protobuf::Message& config,
  ...
  const auto& statsd_sink =
      MessageUtil::downcastAndValidate<const envoy::config::metrics::v3alpha::StatsdSink&>(
          config, server.messageValidationContext().staticValidationVisitor());
  switch (statsd_sink.statsd_specifier_case()) {
  case envoy::config::metrics::v3alpha::StatsdSink::StatsdSpecifierCase::kAddress: {
  ...
  case envoy::config::metrics::v3alpha::StatsdSink::StatsdSpecifierCase::kTcpClusterName:
...
ProtobufTypes::MessagePtr StatsdSinkFactory::createEmptyConfigProto() {
  return std::make_unique<envoy::config::metrics::v3alpha::StatsdSink>();
}
