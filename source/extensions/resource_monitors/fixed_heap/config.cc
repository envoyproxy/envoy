...
FixedHeapMonitorFactory::createResourceMonitorFromProtoTyped(
  ...
  if ((config.max_heap_size_bytes() > 0) && config.has_max_heap_size_bytes_runtime()) {
    return absl::InvalidArgumentError("fixed_heap: exactly one of max_heap_size_bytes or "
  ...
  if (config.has_max_heap_size_bytes_runtime()) {
    ...
    if (runtime_uint64.value() == 0) {
      return absl::InvalidArgumentError("fixed_heap: max heap size must be greater than 0");
    }
