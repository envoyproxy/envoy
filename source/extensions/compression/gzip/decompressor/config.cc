...
GzipDecompressorFactory::GzipDecompressorFactory(
    ...
    : scope_(scope), window_bits_(PROTOBUF_GET_WRAPPED_OR_DEFAULT(gzip, window_bits, DefaultWindowBits) |
                   GzipHeaderValue),
...
Envoy::Compression::Decompressor::DecompressorPtr GzipDecompressorFactory::createDecompressor() {
  auto decompressor = std::make_unique<ZlibDecompressorImpl>(scope_, chunk_size_);
...
GzipDecompressorLibraryFactory::createDecompressorFactoryFromProtoTyped(
    const envoy::extensions::compression::gzip::decompressor::v3::Gzip& proto_config, Server::Configuration::FactoryContext& context) {
