...
envoy_dynamic_module_on_header_formatter_config_new(
  ...
  return failClosed(
      "envoy_dynamic_module_on_header_formatter_config_new", nullptr,
      [&]() -> envoy_dynamic_module_type_header_formatter_config_module_ptr {
        ...
        if (!formatter_config) {
          DYM_LOG((*config_handle), LogLevel::Warn,
                  "Failed to create header formatter for name: {}", name_view);
          return nullptr;
        }
...
void envoy_dynamic_module_on_header_formatter_config_destroy(
  ...
  failClosedVoid("envoy_dynamic_module_on_header_formatter_config_destroy", [&]() {
...
envoy_dynamic_module_type_header_formatter_module_ptr envoy_dynamic_module_on_header_formatter_new(
  ...
  return failClosed("envoy_dynamic_module_on_header_formatter_new", nullptr,
                    [&]() -> envoy_dynamic_module_type_header_formatter_module_ptr {
                      auto* wrapper =
                          unwrapPointer<HeaderFormatterConfigWrapper>(config_module_ptr);
                      if (wrapper == nullptr || !wrapper->config_) {
                        // A null formatter makes Envoy fall back to the default header casing for
                        // this message.
                        return nullptr;
                      }

                      // The handle is created before the formatter so it can be handed to it, and
                      // outlives every formatter hook, which is why the formatter may keep the
                      // reference.
                      auto handle =
                          std::make_unique<HeaderFormatterHandleImpl>(formatter_envoy_ptr);
...
void envoy_dynamic_module_on_header_formatter_destroy(
  ...
  failClosedVoid("envoy_dynamic_module_on_header_formatter_destroy",
...
void envoy_dynamic_module_on_header_formatter_process_key(
  ...
  failClosedVoid("envoy_dynamic_module_on_header_formatter_process_key", [&]() {
...
bool envoy_dynamic_module_on_header_formatter_format(
  ...
  return failClosed("envoy_dynamic_module_on_header_formatter_format", false, [&]() {
    auto* wrapper = unwrapPointer<HeaderFormatterWrapper>(formatter_module_ptr);
    if (wrapper == nullptr || !wrapper->formatter_) {
      return false;
    }
