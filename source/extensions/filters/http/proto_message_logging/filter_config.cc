...
FilterConfig::FilterConfig(const ProtoMessageLoggingConfig& proto_config,
  ...
  type_helper_ = std::make_unique<const TypeHelper>(
...
const Extractor* FilterConfig::findExtractor(
    absl::string_view proto_path) const {
  ...
  for (const auto& it : proto_config_.logging_by_method()) {
    ...
    auto extractor = extractor_factory.createExtractor(
        *type_helper_, *type_helper_->Info(), *type_finder_,
        ...
        it.second);
    if (!extractor.ok()) {
      ENVOY_LOG_MISC(debug, "couldn't init extractor for method `{}`: {}",
                     it.first, extractor.status().message());
      return;
    }

    ENVOY_LOG_MISC(debug, "registered field extraction for gRPC method `{}`",
                   it.first);
...
void FilterConfig::initDescriptorPool(Api::Api& api) {
  ...
  const ::envoy::config::core::v3::DataSource& descriptor_config =
      proto_config_.data_source();
  ...
  switch (descriptor_config.specifier_case()) {
    case envoy::config::core::v3::DataSource::SpecifierCase::kFilename: {
      ...
      if (!file_or_error.status().ok() ||
          !descriptor_set.ParseFromString(file_or_error.value())) {
    ...
    case envoy::config::core::v3::DataSource::SpecifierCase::kInlineBytes: {
      if (!descriptor_set.ParseFromString(descriptor_config.inline_bytes())) {
        ENVOY_LOG_MISC(debug,
                       "unable to parse proto descriptor from inline bytes: {}",
