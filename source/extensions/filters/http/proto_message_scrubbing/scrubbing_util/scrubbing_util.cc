...
// Singleton mapping of string to ScrubbedMessageDirective.
const absl::flat_hash_map<std::string, ScrubbedMessageDirective>& StringToDirectiveMap() {
  static auto* string_to_directive_map =
      new absl::flat_hash_map<std::string, ScrubbedMessageDirective>({
          {kScrubRedact, ScrubbedMessageDirective::SCRUB_REDACT},
          {kScrub, ScrubbedMessageDirective::SCRUB},
      });
  return *string_to_directive_map;
}

absl::optional<ScrubbedMessageDirective>
ScrubbedMessageDirectiveFromString(absl::string_view directive) {
