...
// The filter prefixes.
const char kRcDetailFilterProtoMessageScrubbing[] = "proto_message_scrubbing";
...
Envoy::Http::FilterHeadersStatus Filter::decodeHeaders(Envoy::Http::RequestHeaderMap& headers,
                                                       bool) {
  ENVOY_STREAM_LOG(debug, "Called Proto Message Scrubbing Filter : {}", *decoder_callbacks_,
                   __func__);
  ...
  if (!extractor) {
    ENVOY_STREAM_LOG(debug, "gRPC method `{}` isn't configured for proto message scrubbing filter ",
...
Filter::HandleDataStatus Filter::handleDecodeData(Envoy::Buffer::Instance& data, bool end_stream) {
  RELEASE_ASSERT(extractor_ && request_msg_converter_,
                 ...
                 "initiated when scrubbing proto messages");
  ...
  for (size_t msg_idx = 0; msg_idx < buffering->size(); ++msg_idx) {
    ...
    // Set the request_scrubbing_done_ to true since we have received a message for scrubbing.
...
Envoy::Http::FilterHeadersStatus Filter::encodeHeaders(Envoy::Http::ResponseHeaderMap& headers,
                                                       bool end_stream) {
  ENVOY_STREAM_LOG(debug, "Called Proto Message scrubbing Filter : {}", *encoder_callbacks_,
                   __func__);
...
Filter::HandleDataStatus Filter::handleEncodeData(Envoy::Buffer::Instance& data, bool end_stream) {
  ...
  for (size_t msg_idx = 0; msg_idx < buffering->size(); ++msg_idx) {
    ...
    // Set the response_scrubbing_done_ to true since we have received a message for scrubbing.
    response_scrubbing_done_ = true;
...
void Filter::handleRequestScrubbingResult(const std::vector<ScrubbedMessageMetadata>& result) {
  ...
  auto addResultToMetadata = [&](const std::string& category, const std::string& key,
                                 const ScrubbedMessageMetadata& metadata) {
