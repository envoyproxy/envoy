...
using VirtualHostOptConstRef = absl::optional<std::reference_wrapper<const Router::VirtualHost>>;
...
public:
  ...
  /**
   * @return VirtualHostOptConstRef the value of virtual host inside the lua filter scope of current
   * route entry.
   */
  virtual VirtualHostOptConstRef virtualHost() const PURE;
...
private:
  struct DecoderCallbacks : public FilterCallbacks {
    ...
    VirtualHostOptConstRef virtualHost() const override;
