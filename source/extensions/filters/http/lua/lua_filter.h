...
public:
   ...
   * @return a failed status if responding is not supported in this direction. The caller turns it
   *         into a Lua error once it has no live C++ objects; see DECLARE_LUA_FUNCTION_EX().
   */
  virtual absl::Status respond(Http::ResponseHeaderMapPtr&& headers, Buffer::Instance* body,
...
private:
  ...
  absl::StatusOr<int> doHttpCall(lua_State* state, const HttpCallOptions& options);
  ...
  // Overload for the paths that call one of the lua*() functions directly from C++ rather than
  // through the thunk generated by DECLARE_LUA_FUNCTION(). There is no enclosing lua_resume() on
  // the stack there, so a failure cannot be raised as a Lua error; it becomes the coroutine
  // status instead, which the filter already reports.
  absl::Status resumeCoroutine(absl::StatusOr<int> num_args,
                               const Filters::Common::Lua::YieldCallback& yield_callback) {
    RETURN_IF_NOT_OK_REF(num_args.status());
    return resumeCoroutine(*num_args, yield_callback);
  }
...
private:
  struct DecoderCallbacks : public FilterCallbacks {
    ...
    absl::Status respond(Http::ResponseHeaderMapPtr&& headers, Buffer::Instance* body,
