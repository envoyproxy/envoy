...
std::string SignerImpl::createContentHash(Http::Message& message, bool sign_body) const {
  ...
  const auto content_hash =
      message.body()
          ? Hex::encode(Envoy::Extensions::TransportSockets::Tls::Crypto::Utility::getSha256Digest(
                *message.body()))
...
std::string SignerImpl::createSignature(absl::string_view secret_access_key,
  ...
  const auto date_key = Envoy::Extensions::TransportSockets::Tls::Crypto::Utility::getSha256Hmac(
      std::vector<uint8_t>(secret_key.begin(), secret_key.end()), short_date);
  const auto region_key =
      Envoy::Extensions::TransportSockets::Tls::Crypto::Utility::getSha256Hmac(date_key, region_);
  const auto service_key = Envoy::Extensions::TransportSockets::Tls::Crypto::Utility::getSha256Hmac(
      region_key, service_name_);
  const auto signing_key = Envoy::Extensions::TransportSockets::Tls::Crypto::Utility::getSha256Hmac(
      service_key, SignatureConstants::get().Aws4Request);
  return Hex::encode(Envoy::Extensions::TransportSockets::Tls::Crypto::Utility::getSha256Hmac(
