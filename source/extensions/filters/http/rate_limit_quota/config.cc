...
Http::FilterFactoryCb RateLimitQuotaFilterFactory::createFilterFactoryFromProtoTyped(
  ...
  Grpc::GrpcServiceConfigWithHashKey config_with_hash_key = Grpc::GrpcServiceConfigWithHashKey(config->rlqs_server());
  return [config = std::move(config), &context, bucket_cache = std::move(bucket_cache), config_with_hash_key](
