...
class LocalRateLimitClientImpl
 ...
 private:
  inline std::shared_ptr<GlobalRateLimitClientImpl> getGlobalClient() {
    return (global_client_tls_.get().has_value())
               ? global_client_tls_.get()->global_client
               : nullptr;
  }
  inline std::shared_ptr<BucketsCache> getBucketsCache() {
    return (buckets_cache_tls_.get().has_value())
               ? buckets_cache_tls_.get()->quota_buckets_
               : nullptr;
  }
...
inline std::unique_ptr<RateLimitClient> createLocalRateLimitClient(
    ThreadLocal::TypedSlot<ThreadLocalGlobalRateLimitClientImpl>&
        global_client_tls,
    ThreadLocal::TypedSlot<ThreadLocalBucketsCache>& buckets_cache_tls_) {
  return std::make_unique<LocalRateLimitClientImpl>(
      global_client_tls, buckets_cache_tls_);
}
