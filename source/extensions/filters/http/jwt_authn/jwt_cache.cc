...
public:
  JwtCacheImpl(bool enable_cache, const JwtCacheConfig& config, TimeSource& time_source)
    ...
    if (enable_cache) {
      ...
      jwt_lru_cache_ =
          std::make_unique<SimpleLRUCache<std::string, ::Envoy::JwtVerify::Jwt>>(cache_size);
  ...
  ::Envoy::JwtVerify::Jwt* lookup(const std::string& token) override {
    if (!jwt_lru_cache_) {
      return nullptr;
    }
    SimpleLRUCache<std::string, ::Envoy::JwtVerify::Jwt>::ScopedLookup lookup(
        jwt_lru_cache_.get(), token);
    if (lookup.found()) {
      ::Envoy::JwtVerify::Jwt* const found_jwt = lookup.value();
      ASSERT(found_jwt != nullptr);
      if (found_jwt->verifyTimeConstraint(DateUtil::nowToSeconds(time_source_)) !=
          ::Envoy::JwtVerify::Status::JwtExpired) {
        return found_jwt;
      } else {
  ...
  void insert(const std::string& token, std::unique_ptr<::Envoy::JwtVerify::Jwt>&& jwt) override {
...
private:
  std::unique_ptr<SimpleLRUCache<std::string, ::Envoy::JwtVerify::Jwt>> jwt_lru_cache_;
