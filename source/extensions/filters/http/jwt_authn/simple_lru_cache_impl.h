...
#include "extensions/filters/http/jwt_authn/simple_lru_cache.h"
...
public:
  ...
  // The default is true.
  bool updateEvictionOrder() const { return update_eviction_order_; }
  void setUpdateEvictionOrder(bool v) { update_eviction_order_ = v; }
...
public:
  ...
  // The output of this function is not suitable for parsing by `borgmon`.
...
protected:
  ...
  virtual void debugIterator([[maybe_unused]] const Key& k, const Value* value, int pin_count,
                             int64_t last_timestamp, bool is_deferred, std::string* output) const {
    std::stringstream ss;
...
Value* SimpleLRUCacheBase<Key, Value, MapType, EQ>::lookupWithOptions(
  ...
  if (iter != table_.end()) {
    ...
    if (e->pin == 0) {
      ...
      if (lru_ && options.updateEvictionOrder())
        e->unlink();
    }
    e->pin++;
    return e->value;
...
protected:
  virtual void removeElement([[maybe_unused]] const Key& k, Value* value) { delete value; }
