...
using ::Envoy::JwtVerify::StructUtils;
...
private:
  ...
  // The JWT object.
  std::unique_ptr<::Envoy::JwtVerify::Jwt> owned_jwt_;
...
void AuthenticatorImpl::startVerify() {
  ...
  if (!use_jwt_cache) {
    ...
    owned_jwt_ = std::make_unique<::Envoy::JwtVerify::Jwt>();
  ...
  uint64_t clock_skew_seconds = ::Envoy::JwtVerify::kClockSkewInSecond;
...
void AuthenticatorImpl::verifyKey() {
  const Status status =
      ::Envoy::JwtVerify::verifyJwtWithoutTimeChecking(*jwt_, *jwks_data_->getJwksObj());
...
void AuthenticatorImpl::doneWithStatus(const Status& status) {
  ...
  if (Status::Ok != status) {
    ...
    ENVOY_LOG(debug, "status is: {}", ::Envoy::JwtVerify::getStatusString(status));
