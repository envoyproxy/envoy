 ...
 */
void validateJwtConfig(const JwtAuthentication& proto_config, Filesystem::Instance& file_system) {
  for (const auto& it : proto_config.providers()) {
    const auto& provider = it.second;
    const auto inline_jwks = Config::DataSource::read(provider.local_jwks(), true, file_system);
...
FilterFactory::createFilterFactoryFromProtoTyped(const JwtAuthentication& proto_config,
  ...
  validateJwtConfig(proto_config, context.fileSystem());
