...
Http::FilterFactoryCb AdaptiveConcurrencyFilterFactory::createFilterFactoryFromProtoTyped(
  ...
  std::shared_ptr<ConcurrencyController::ConcurrencyController> concurrency_controller;
  switch (config.gradient_controller_config_case()) {
    case envoy::config::filter::http::adaptive_concurrency::v2alpha::AdaptiveConcurrency::kGradientControllerConfig:
      concurrency_controller = std::make_shared<ConcurrencyController::GradientController>(
        context.dispatcher(), context.runtime(), stats_prefix, context.timeSource());
      break;
    default:
  ...
  return [filter_config, concurrency_controller](Http::FilterChainFactoryCallbacks& callbacks) -> void {
    callbacks.addStreamFilter(std::make_shared<AdaptiveConcurrencyFilter>(filter_config, concurrency_controller));
  };  
}
