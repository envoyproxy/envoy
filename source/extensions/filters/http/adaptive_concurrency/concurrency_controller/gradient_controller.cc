...
GradientControllerConfig::GradientControllerConfig(
    ...
    : runtime_(runtime),
      ...
      min_concurrency_(PROTOBUF_GET_WRAPPED_OR_DEFAULT(proto_config.min_rtt_calc_params(),
                                                       min_concurrency, 3)),
      min_rtt_buffer_pct_(PROTOBUF_PERCENT_TO_DOUBLE_OR_DEFAULT(proto_config.min_rtt_calc_params(), buffer, 10)){}
...
uint32_t GradientController::calculateNewLimit() {
  ...
  auto buffered_min_rtt = min_rtt_.count() + min_rtt_.count() * config_.minRTTBufferPercent();
