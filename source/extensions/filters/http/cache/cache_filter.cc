...
void CacheFilter::processSuccessfulValidation(Http::ResponseHeaderMap& response_headers) {
  ...
  lookup_result_->headers_->removeInline(CacheCustomHeaders::get().age_handle.handle());
...
bool CacheFilter::shouldUpdateCachedEntry(const Http::ResponseHeaderMap& response_headers) const {
  ...
  const Http::HeaderEntry* response_etag =
      response_headers.getInline(CacheCustomHeaders::get().etag_handle.handle());
  const Http::HeaderEntry* cached_etag =
      lookup_result_->headers_->getInline(CacheCustomHeaders::get().etag_handle.handle());
...
void CacheFilter::injectValidationHeaders(Http::RequestHeaderMap& request_headers) {
  ...
  const Http::HeaderEntry* last_modified_header =
      lookup_result_->headers_->getInline(CacheCustomHeaders::get().last_modified_handle.handle());

  if (etag_header) {
    absl::string_view etag = etag_header->value().getStringView();
    request_headers.setInline(CacheCustomHeaders::get().if_none_match_handle.handle(), etag);
  }
  if (DateUtil::timePointValid(CacheHeadersUtils::httpTime(last_modified_header))) {
    ...
    request_headers.setInline(CacheCustomHeaders::get().if_modified_since_handle.handle(),
                              last_modified);
  } else {
    ...
    request_headers.setInline(CacheCustomHeaders::get().if_modified_since_handle.handle(), date);
  }
}
