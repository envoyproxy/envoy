...
absl::StatusOr<std::string> FindSingularLastValue(const Field* field,
  ...
  while (FieldExtractor::SearchField(*field, input_stream)) {
    ...
    if (field->kind() == Field::TYPE_STRING) {
      static_cast<void>(WireFormatLite::ReadString(input_stream, &resource));
    }
  }
  return resource;
}
    ...
    if (field->kind() != Field::TYPE_STRING) {
      ...
      while ((tag = input_stream->ReadTag()) != 0) {
        if (field->number() == WireFormatLite::GetTagFieldNumber(tag)) {
          uint32_t length;
          static_cast<void>(input_stream->ReadVarint32(&length));
          static_cast<void>(input_stream->ReadString(&result, length));
        } else {
          static_cast<void>(WireFormatLite::SkipField(input_stream, tag));
        }
      }
