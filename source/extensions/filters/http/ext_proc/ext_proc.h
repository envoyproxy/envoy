...
public:
  ...
  ProtobufTypes::MessagePtr serializeAsProto() const override {
    ...
    if (inbound.header_stats_) {
      (*struct_msg->mutable_fields())["inbound_header_latency_us"].set_number_value(
          static_cast<double>(inbound.header_stats_->latency_.count()));
      (*struct_msg->mutable_fields())["inbound_header_status"].set_number_value(
    ...
    if (inbound.body_stats_) {
      (*struct_msg->mutable_fields())["inbound_body_calls"].set_number_value(
          static_cast<double>(inbound.body_stats_->call_count_));
      (*struct_msg->mutable_fields())["inbound_body_total_latency_us"].set_number_value(
      ...
      (*struct_msg->mutable_fields())["inbound_body_last_status"].set_number_value(
          static_cast<double>(static_cast<int>(inbound.body_stats_->last_call_status_)));
    }
    ...
    if (outbound.header_stats_) {
      (*struct_msg->mutable_fields())["outbound_header_latency_us"].set_number_value(
          static_cast<double>(outbound.header_stats_->latency_.count()));
      (*struct_msg->mutable_fields())["outbound_header_status"].set_number_value(
          static_cast<double>(static_cast<int>(outbound.header_stats_->call_status_)));
    }
    if (outbound.body_stats_) {
      (*struct_msg->mutable_fields())["outbound_body_calls"].set_number_value(
          static_cast<double>(outbound.body_stats_->call_count_));
      (*struct_msg->mutable_fields())["outbound_body_total_latency_us"].set_number_value(
          static_cast<double>(outbound.body_stats_->total_latency_.count()));
      (*struct_msg->mutable_fields())["outbound_body_max_latency_us"].set_number_value(
          static_cast<double>(outbound.body_stats_->max_latency_.count()));
      (*struct_msg->mutable_fields())["outbound_body_last_status"].set_number_value(
          static_cast<double>(static_cast<int>(outbound.body_stats_->last_call_status_)));
    }
    if (outbound.trailer_stats_) {
      (*struct_msg->mutable_fields())["outbound_trailer_latency_us"].set_number_value(
          static_cast<double>(outbound.trailer_stats_->latency_.count()));
      (*struct_msg->mutable_fields())["outbound_trailer_status"].set_number_value(
          static_cast<double>(static_cast<int>(outbound.trailer_stats_->call_status_)));
    }
    (*struct_msg->mutable_fields())["bytes_sent"].set_number_value(
  ...
  absl::optional<std::string> serializeAsString() const override {
    ...
    if (inbound.header_stats_) {
      parts.push_back(absl::StrCat("ih:", inbound.header_stats_->latency_.count(), ":",
                                   static_cast<int>(inbound.header_stats_->call_status_)));
    }
    if (inbound.body_stats_) {
      parts.push_back(absl::StrCat("ib:", inbound.body_stats_->call_count_, ":",
                                   inbound.body_stats_->total_latency_.count(), ":",
                                   static_cast<int>(inbound.body_stats_->last_call_status_)));
    }
    if (inbound.trailer_stats_) {
      parts.push_back(absl::StrCat("it:", inbound.trailer_stats_->latency_.count(), ":",
                                   static_cast<int>(inbound.trailer_stats_->call_status_)));
    }
    if (outbound.header_stats_) {
      parts.push_back(absl::StrCat("oh:", outbound.header_stats_->latency_.count(), ":",
                                   static_cast<int>(outbound.header_stats_->call_status_)));
    }
    if (outbound.body_stats_) {
      parts.push_back(absl::StrCat("ob:", outbound.body_stats_->call_count_, ":",
                                   outbound.body_stats_->total_latency_.count(), ":",
                                   static_cast<int>(outbound.body_stats_->last_call_status_)));
    }
    if (outbound.trailer_stats_) {
      parts.push_back(absl::StrCat("ot:", outbound.trailer_stats_->latency_.count(), ":",
                                   static_cast<int>(outbound.trailer_stats_->call_status_)));
    }
  ...
  FieldType getField(absl::string_view field_name) const override {
    ...
    if (field_name == "inbound_header_status" && inbound.header_stats_) {
      return static_cast<int64_t>(inbound.header_stats_->call_status_);
    }
    if (field_name == "inbound_body_calls" && inbound.body_stats_) {
      return static_cast<int64_t>(inbound.body_stats_->call_count_);
    }
    if (field_name == "inbound_body_total_latency_us" && inbound.body_stats_) {
      return static_cast<int64_t>(inbound.body_stats_->total_latency_.count());
    }
    if (field_name == "inbound_body_max_latency_us" && inbound.body_stats_) {
      return static_cast<int64_t>(inbound.body_stats_->max_latency_.count());
    }
    if (field_name == "inbound_body_last_status" && inbound.body_stats_) {
      return static_cast<int64_t>(inbound.body_stats_->last_call_status_);
    }
    if (field_name == "inbound_trailer_latency_us" && inbound.trailer_stats_) {
      return static_cast<int64_t>(inbound.trailer_stats_->latency_.count());
    }
    if (field_name == "inbound_trailer_status" && inbound.trailer_stats_) {
      return static_cast<int64_t>(inbound.trailer_stats_->call_status_);
    }
    if (field_name == "outbound_header_latency_us" && outbound.header_stats_) {
      return static_cast<int64_t>(outbound.header_stats_->latency_.count());
    }
    if (field_name == "outbound_header_status" && outbound.header_stats_) {
      return static_cast<int64_t>(outbound.header_stats_->call_status_);
    }
    if (field_name == "outbound_body_calls" && outbound.body_stats_) {
      return static_cast<int64_t>(outbound.body_stats_->call_count_);
    }
    if (field_name == "outbound_body_total_latency_us" && outbound.body_stats_) {
      return static_cast<int64_t>(outbound.body_stats_->total_latency_.count());
    }
    if (field_name == "outbound_body_max_latency_us" && outbound.body_stats_) {
      return static_cast<int64_t>(outbound.body_stats_->max_latency_.count());
    }
    if (field_name == "outbound_body_last_status" && outbound.body_stats_) {
      return static_cast<int64_t>(outbound.body_stats_->last_call_status_);
    }
    if (field_name == "outbound_trailer_latency_us" && outbound.trailer_stats_) {
      return static_cast<int64_t>(outbound.trailer_stats_->latency_.count());
    }
    if (field_name == "outbound_trailer_status" && outbound.trailer_stats_) {
      return static_cast<int64_t>(outbound.trailer_stats_->call_status_);
    }
