...
IpTagsProvider::IpTagsProvider(const envoy::config::core::v3::DataSource& ip_tags_datasource,
  ...
  ip_tags_reload_timer_ = main_dispatcher.createTimer([this]() -> void {
    try {
      ...
      ENVOY_LOG(error, "IpTagsProvider timer callback exception: {}", e.what());
      ...
      ENVOY_LOG(error, "IpTagsProvider timer callback unknown exception");
      // DO NOT re-throw exceptions from timer callbacks - they cause std::terminate!
      if (reload_error_cb_) {
        try {
          reload_error_cb_();
    ...
    try {
      ...
      ENVOY_LOG(error, "Failed to re-enable timer");
    }
  });
  ip_tags_reload_timer_->enableTimer(ip_tags_refresh_interval_ms_);
}
...
const std::string& IpTagsLoader::getDataSourceData() {
  try {
    ...
    ENVOY_LOG(error, "IpTagsLoader::getDataSourceData() bad_variant_access: {}", e.what());
    data_ = "";
    return data_;
  } catch (const std::exception& e) {
    ENVOY_LOG(error, "IpTagsLoader::getDataSourceData() exception: {}", e.what());
    throw;
  }
}
