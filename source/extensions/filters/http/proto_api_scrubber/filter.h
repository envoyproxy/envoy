...
private:
  ...
  // Creates and returns an instance of `ProtoScubber` which can be used for request scrubbing.
  std::unique_ptr<ProtoScrubber> createAndReturnRequestProtoScrubber();

  const ProtoApiScrubberFilterConfig& filter_config_;

  // Stores the full gRPC method name in dotted format eg, `my.package.service.method`.
