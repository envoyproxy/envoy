...
RoleBasedAccessControlFilterConfig::RoleBasedAccessControlFilterConfig(
    ...
    : stats_(Filters::Common::RBAC::generateStats(stats_prefix, scope)),
      ...
      matcher_(std::make_unique<Filters::Common::Matcher::RequestMatcher>(proto_config.matcher())) {
}
...
RoleBasedAccessControlFilter::decodeHeaders(Http::RequestHeaderMap& headers, bool) {
  ...
  auto matcher = config_->matcher();
  ENVOY_LOG(debug, "matching status: {}",
            matcher->match(*callbacks_->connection(), callbacks_->streamInfo(), headers));
