...
class Oauth2ClientCredentialsGrantCredentialRequestImpl : public CredentialSource::Request,
                                                          public FilterCallbacks {
public:
  Oauth2ClientCredentialsGrantCredentialRequestImpl(CredentialSource::Callbacks& callbacks,
                                                    std::unique_ptr<OAuth2Client>&& oauth_client,
                                                    absl::string_view client_id,
                                                    absl::string_view client_secret)
      : callbacks_(callbacks), oauth_client_(std::move(oauth_client)), client_id_(client_id),
  ...
  // FilterCallbacks
  void onGetAccessTokenSuccess(const std::string& access_code, const std::string&,
  ...
  void sendUnauthorizedResponse() override { callbacks_.onFailure(); }
...
class ThreadLocalOauth2ClientCredentialsGrantCredentialSource
    : public ThreadLocal::ThreadLocalObject {
public:
  ThreadLocalOauth2ClientCredentialsGrantCredentialSource(absl::string_view client_id,
                                                          absl::string_view client_secret)
...
using ThreadLocalOauth2ClientCredentialsGrantCredentialSourceSharedPtr =
    std::shared_ptr<ThreadLocalOauth2ClientCredentialsGrantCredentialSource>;

/**
 * Source of OAuth2 Client Credentials Grant credential.
 */
class Oauth2ClientCredentialsGrantCredentialSource : public CredentialSource,
                                                     public SecretReader::Callbacks {
public:
  virtual ~Oauth2ClientCredentialsGrantCredentialSource() = default;

  Oauth2ClientCredentialsGrantCredentialSource(
  ...
  CredentialSource::RequestPtr requestCredential(CredentialSource::Callbacks& callbacks) override {
    ...
    return std::make_unique<Oauth2ClientCredentialsGrantCredentialRequestImpl>(
        callbacks, std::move(oauth_client), tls.client_id(), tls.client_secret());
  }
...
private:
  ...
  const envoy::extensions::filters::http::credential_injector::v3::OAuth2Credential proto_config_;
