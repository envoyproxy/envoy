...
public:
  ...
  const Extensions::Filters::Common::LocalRateLimit::LocalRateLimiterImpl& value() const {
    return rate_limiter_;
  }

private:
  Extensions::Filters::Common::LocalRateLimit::LocalRateLimiterImpl rate_limiter_;
};
...
public:
  ...
  ~FilterConfig() override {
    ...
    auto shared_ptr_wrapper = std::make_shared<
        std::unique_ptr<Extensions::Filters::Common::LocalRateLimit::LocalRateLimiterImpl>>(
  ...
  requestAllowed(absl::Span<const Envoy::RateLimit::Descriptor> request_descriptors) const;
  bool enabled() const;
  bool enforced() const;
  ...
  void populateDescriptors(
      ...
      Extensions::Filters::Common::RateLimit::RateLimitDescriptors& descriptors) const {
    ASSERT(rate_limit_config_ != nullptr);
...
private:
  ...
  Extensions::Filters::Common::LocalRateLimit::ShareProviderManagerSharedPtr
      share_provider_manager_;
  std::unique_ptr<Extensions::Filters::Common::LocalRateLimit::LocalRateLimiterImpl> rate_limiter_;
