...
struct MethodInfo {
  ...
  std::vector<const Envoy::ProtobufWkt::Field*> response_body_field_path;
};
...
class TranscoderImpl : public Transcoder {
 public:
  explicit TranscoderImpl(
      std::unique_ptr<RequestTranslator> request_translator,
  ...
  TranscoderInputStream* RequestOutput() override {
    return request_stream_.get();
  }

  absl::Status RequestStatus() override {
    return request_translator_->Status();
  }

  Protobuf::io::ZeroCopyInputStream* ResponseOutput() override {
    return response_stream_.get();
  }

  absl::Status ResponseStatus() override {
    return response_message_stream_.Status();
  }
...
class GrpcJsonReverseTranscoderConfig
 ...
 public:
  GrpcJsonReverseTranscoderConfig(
      const envoy::extensions::filters::http::grpc_json_reverse_transcoder::v3::GrpcJsonReverseTranscoder& transcoder_config, Api::Api& api);

  absl::Status CreateTranscoder(std::string_view path,
 ...
 private:
  absl::Status ExtractHttpAnnotationValues(
      const Envoy::Protobuf::MethodDescriptor* method_descriptor,
