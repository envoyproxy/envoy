...
class TranscoderInputStreamImpl
    : public Buffer::ZeroCopyInputStreamImpl,
...
class GrpcJsonReverseTranscoderFilter
 ...
 public:
  ...
  Http::FilterDataStatus decodeData(Buffer::Instance&,
                                           bool) override;

  Http::FilterTrailersStatus decodeTrailers(
      Http::RequestTrailerMap&) override {
  ...
  Http::FilterHeadersStatus encodeHeaders(
      Http::ResponseHeaderMap&, bool) override;

  Http::FilterDataStatus encodeData(Buffer::Instance&,
                                           bool) override;

  Http::FilterTrailersStatus encodeTrailers(
      Http::ResponseTrailerMap&) override;

  Http::FilterMetadataStatus encodeMetadata(
      Http::MetadataMap&) override {
  ...
  GrpcJsonReverseTranscoderFilter(const GrpcJsonReverseTranscoderFilter&) =
      delete;
  GrpcJsonReverseTranscoderFilter& operator=(
      const GrpcJsonReverseTranscoderFilter&) = delete;
 ...
 private:
  ...
  Grpc::Status::GrpcStatus GrpcStatusFromHeaders(
  ...
  void ReplaceAPIVersionInPath(const Http::RequestHeaderMap& headers,
