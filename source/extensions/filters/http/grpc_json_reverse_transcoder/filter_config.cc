...
using ::envoy::extensions::filters::http::grpc_json_reverse_transcoder::v3::GrpcJsonReverseTranscoder;
...
GrpcJsonReverseTranscoderConfig::GrpcJsonReverseTranscoderConfig(
  ...
  switch (transcoder_config.descriptor_set_case()) {
    case GrpcJsonReverseTranscoder::DescriptorSetCase::kDescriptorPath: {
      auto file_or_error =
          api.fileSystem().fileReadToEnd(transcoder_config.descriptor_path());
    ...
    case GrpcJsonReverseTranscoder::DescriptorSetCase::kDescriptorBinary:
      if (!descriptor_set.ParseFromString(
              transcoder_config.descriptor_binary())) {
        throw EnvoyException("Unable to parse proto descriptor binary");
      }
      break;
    case GrpcJsonReverseTranscoder::DescriptorSetCase::
  ...
  max_request_body_size_ =
      transcoder_config.has_max_request_body_size()
          ? std::make_optional(
                transcoder_config.max_request_body_size().value())
          : std::nullopt;
  max_response_body_size_ =
      transcoder_config.has_max_response_body_size()
          ? std::make_optional(
                transcoder_config.max_response_body_size().value())
          : std::nullopt;
...
absl::Status GrpcJsonReverseTranscoderConfig::ExtractHttpAnnotationValues(
  ...
  if (!method_descriptor->options().HasExtension(google::api::http)) {
    ...
    return absl::InvalidArgumentError(
        absl::StrCat("Method, ", method_descriptor->full_name(),
                     ", is missing the google.api.http option."));
  }

  HttpRule http_rule =
      method_descriptor->options().GetExtension(google::api::http);
  switch (http_rule.pattern_case()) {
    ...
    case HttpRule::PatternCase::kPatch:
      ...
      break;
    default:
      ENVOY_LOG(error, "Method, {}, doesn't map to a valid HTTP verb.",
                method_descriptor->full_name());
      return absl::InvalidArgumentError(
          absl::StrCat("Method, ", method_descriptor->full_name(),
                       ", doesn't map to a valid HTTP method."));
  }
...
absl::Status GrpcJsonReverseTranscoderConfig::CreateTranscoder(
    ...
    std::unique_ptr<Transcoder>& transcoder, HttpRequestParams& request_params,
  ...
  if (method_descriptor == nullptr) {
    ...
    return absl::NotFoundError(
        absl::StrCat("Couldn't find the gRPC method: ", grpc_method));
  }

  method_info.is_request_http_body =
      method_descriptor->input_type()->full_name() ==
      google::api::HttpBody::descriptor()->full_name();
  method_info.is_response_http_body =
      method_descriptor->output_type()->full_name() ==
      google::api::HttpBody::descriptor()->full_name();

  absl::Status status = ExtractHttpAnnotationValues(
      method_descriptor, request_params.http_rule_path,
  ...
  if (request_params.http_body_field != "*") {
    ...
    status = type_helper_->ResolveFieldPath(*request_type,
                                            request_params.http_body_field,
                                            &request_body_field_path);
    if (!status.ok()) {
      return status;
    }
    const Envoy::ProtobufWkt::Type* request_body_type =
        type_helper_->Info()->GetTypeByTypeUrl(
            request_body_field_path.back()->type_url());
    method_info.is_request_nested_http_body =
        request_body_type != nullptr &&
        request_body_type->name() ==
            google::api::HttpBody::descriptor()->full_name();
  }

  auto request_translator = std::make_unique<RequestTranslator>(
      type_helper_->Resolver(), request_type_url, false, &request_input);

  ResponseInfo response_info;
  std::string response_type_url =
      Grpc::Common::typeUrl(method_descriptor->output_type()->full_name());
  response_info.message_type =
      type_helper_->Info()->GetTypeByTypeUrl(response_type_url);
  if (response_info.message_type == nullptr) {
    return absl::NotFoundError(
        absl::StrCat("Couldn't resolve type: ",
                     method_descriptor->output_type()->full_name()));
  }
  response_info.body_field_path = "*";

  status = type_helper_->ResolveFieldPath(
      *response_info.message_type, "", &method_info.response_body_field_path);
  if (!status.ok()) {
    return status;
  }

  auto response_translator = std::make_unique<ResponseTranslator>(
      type_helper_->Resolver(), &response_input, std::move(response_info),
      false, true);
