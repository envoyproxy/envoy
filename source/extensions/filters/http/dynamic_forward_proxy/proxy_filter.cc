...
auto applyFilterStateOverrides = [](absl::string_view& host, uint32_t& port,
                                    const bool allow_dynamic_from_filter_state,
                                    Http::StreamDecoderFilterCallbacks* decoder_callbacks) {
  if (!allow_dynamic_from_filter_state) {
    return;
  }
...
Http::FilterHeadersStatus ProxyFilter::decodeHeaders(Http::RequestHeaderMap& headers, bool) {
  ...
  applyFilterStateOverrides(host, port_u32, config_->allowDynamicHostFromFilterState(),
                            decoder_callbacks_);
  port = port_u32;
...
ProxyFilter::loadDynamicCluster(const Common::DynamicForwardProxy::DfpClusterSharedPtr& cluster,
  ...
  // Parse host and port from headers (preserving original parsing method)
  ...
  uint32_t port_u32 = port;
  applyFilterStateOverrides(host_view, port_u32, config_->allowDynamicHostFromFilterState(),
                            decoder_callbacks_);
