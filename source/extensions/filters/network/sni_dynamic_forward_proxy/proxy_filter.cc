...
ProxyFilterConfig::ProxyFilterConfig(
    ...
    Upstream::ClusterManager&, ProtobufMessage::ValidationVisitor& validation_visitor,
    absl::Status& creation_status)
    : port_(static_cast<uint16_t>(proto_config.port_value())),
      dns_cache_manager_(cache_manager_factory.get()),
      save_upstream_address_(proto_config.save_upstream_address()) {
  auto cache_or_error =
      dns_cache_manager_->getCache(validation_visitor, proto_config.dns_cache_config());
