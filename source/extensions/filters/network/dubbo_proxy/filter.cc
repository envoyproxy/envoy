...
Network::FilterStatus Filter::onData(Buffer::Instance& data, bool) {
  ...
  try {
    if (!request_decoder_) {
      request_decoder_ = createDecoder(*this, *this);
    }
...
Network::FilterStatus Filter::onWrite(Buffer::Instance& data, bool) {
  ...
  try {
    if (!response_decoder_) {
      response_decoder_ = createDecoder(*this, *this);
    }
...
DecoderPtr Filter::createDecoder(ProtocolCallbacks& prot_callback,
                                 SerializationCallbacks& seri_callback) {
  auto parser = createProtocol(prot_callback);
  auto serializer = createSerializer(seri_callback);
  return std::make_unique<Decoder>(std::move(parser), std::move(serializer));
}
  ...
  default:
    throw EnvoyException(
        fmt::format("unsupported protocol type, value is : {}",
                    static_cast<std::underlying_type<SerializationType>::type>(protocol_type_)));
  }
}

SerializationPtr Filter::createSerializer(SerializationCallbacks& callback) {
  using Type = envoy::extensions::filters::network::dubbo_proxy::v2alpha1::DubboProxy;
  switch (serialization_type_) {
  case Type::Hessian2:
    return std::make_unique<HessianSerializerImpl>(callback);
  default:
    throw EnvoyException(fmt::format(
        "unsupported serialization type, value is: {}",
        static_cast<std::underlying_type<SerializationType>::type>(serialization_type_)));
  }
}
