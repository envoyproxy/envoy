...
public:
  RouteEntryImplBase(const envoy::config::filter::network::dubbo_proxy::v3alpha::Route& route);
...
private:
  ...
  public:
    using WeightedCluster = envoy::api::v3alpha::route::WeightedCluster::ClusterWeight;
...
public:
  ParameterRouteEntryImpl(const envoy::config::filter::network::dubbo_proxy::v3alpha::Route& route);
  ~ParameterRouteEntryImpl() override;

  struct ParameterData {
    using ParameterMatchSpecifier =
        envoy::config::filter::network::dubbo_proxy::v3alpha::MethodMatch::ParameterMatchSpecifier;
    ParameterData(uint32_t index, const ParameterMatchSpecifier& config);

    Http::HeaderUtility::HeaderMatchType match_type_;
    std::string value_;
    envoy::type::v3alpha::Int64Range range_;
    uint32_t index_;
  };
...
public:
  MethodRouteEntryImpl(const envoy::config::filter::network::dubbo_proxy::v3alpha::Route& route);
...
public:
  using RouteConfig = envoy::config::filter::network::dubbo_proxy::v3alpha::RouteConfiguration;
