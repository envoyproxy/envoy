...
RouteEntryImplBase::RouteEntryImplBase(
    const envoy::config::filter::network::dubbo_proxy::v3alpha::Route& route)
    : cluster_name_(route.route().cluster()),
      config_headers_(Http::HeaderUtility::buildHeaderDataVector(route.match().headers())) {
  if (route.route().cluster_specifier_case() ==
      envoy::config::filter::network::dubbo_proxy::v3alpha::RouteAction::ClusterSpecifierCase::
          kWeightedClusters) {
...
SingleRouteMatcherImpl::SingleRouteMatcherImpl(const RouteConfig& config,
  ...
  using envoy::config::filter::network::dubbo_proxy::v3alpha::RouteMatch;
