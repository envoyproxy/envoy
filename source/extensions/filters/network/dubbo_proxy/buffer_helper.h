...
public:
  ...
  /**
   * Reads an uint8_t from the buffer at the given offset.
   * @param buffer Buffer::Instance containing data to decode
   * @param offset offset into buffer to peek at
   * @return the uint8_t at offset in buffer
   */
  static uint8_t peekU8(Buffer::Instance& buffer, uint64_t offset = 0);
