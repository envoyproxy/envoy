...
bool SubscriptionRegistry::reissueSsubscribe(const std::string& channel) {
  ...
  // Placement: the RECORD decides, resolving the slot AT MOST once per reissue.
  Upstream::HostConstSharedPtr target;
  {
    // A retry is NOT a move: if the RECORD is still valid, re-target the recorded home; only a
    // channel with no valid record is re-placed. The connection-loss path (doResubscribe) arrives
    // with its record intact, so it usually retries; the topology path (onClusterTopologyChange)
    // erased the record just before calling, so it always resolves a fresh home below.
    auto it = channel_hosts_.find(channel);
    Upstream::HostConstSharedPtr resolved;
    // Validity here is checked health-aware (FailureDriven): a still-valid but unhealthy owner is
    // re-placed onto a healthy sibling rather than retried forever. A topology reroute reaches
    // this code with its record already erased, so the check short-circuits and it re-places
    // below — TopologyEvent validity itself (in the sweep) stays health-agnostic.
    if (it != channel_hosts_.end() &&
      ...
      // No valid record: re-place. ``resolved`` is non-null in exactly one case — the
      // FailureDriven UNHEALTHY-owner escape, where recordedOwnerValid already dry-resolved a
      // healthier home and hands it back so this path does not resolve a second time. Every other
      // invalidity (owner fell out of the policy's feasible set, or the channel has no record at
      // all) leaves ``resolved`` null, so resolve fresh for those.
...
void SubscriptionRegistry::onClusterTopologyChange() {
  ...
  for (const auto& [channel, _] : subscriptions_) {
    auto it = channel_hosts_.find(channel);
    Upstream::HostConstSharedPtr resolved;
    // Record still valid — the recorded home is still inside the read policy's feasible set for
    // the channel's slot, OR the shard snapshot is TRANSIENTLY absent (slot-migration window /
    // momentary topology gap — a genuine ownership loss arrives separately as Redis's unsolicited
    // SUNSUBSCRIBE). Leave it untouched: re-issuing SUNSUBSCRIBE+SSUBSCRIBE on an unmoved channel
    // wastes upstream control traffic AND opens a needless message gap / duplicate window. A
    // slot-only rebalance moves only some slots, so most active channels take this skip.
    // (``resolved`` stays null in TopologyEvent mode — recordedOwnerValid's hand-back exists only
    // for the FailureDriven unhealthy-owner escape.)
    if (it != channel_hosts_.end() &&
        recordedOwnerValid(channel, it->second.host, resolved, ValidityMode::TopologyEvent)) {
      continue;
    }
    // Record invalid (owner no longer in the policy's feasible set for its slot) or owner-less:
    // (re)place the channel. A LIVE old owner is dropped + sent SUNSUBSCRIBE BEFORE reissue so any
    // subscription-mode cleanup along the send path observes the up-to-date map and tears down
    // push_callbacks_ on the old client. The record was just erased, so reissueSsubscribe (with no
    // pre-resolved host — TopologyEvent validity never dry-resolves) resolves the new home through
    // the read policy exactly once, inside the send path.
    if (it != channel_hosts_.end()) {
      forgetOwnerAndSunsubscribe(channel);
    }
    any_reissued = true;
    ...
    // silently-lost SSUBSCRIBE ack is then retried.
    if (!reissueSsubscribe(channel)) {
...
bool SubscriptionRegistry::collectPreservedSubscribeAcks(
  ...
  if (collected) {
    // Drop exactly this subscriber's now-collected entries and erase the bucket if it empties;
    // other subscribers on the same channel stay pending. Erasing the bucket does NOT touch the
    // shared ack scheduler: its (now-stale) deadline entry stays behind and is pruned lazily on a
    // token miss — one no-op wakeup at worst — the same teardown contract failPendingSubscribers
    // documents (the eager pruneAndRearm is reserved for the ack hot path).
