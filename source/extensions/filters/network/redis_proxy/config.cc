...
Network::FilterFactoryCb RedisProxyFilterConfigFactory::createFilterFactoryFromProtoTyped(
  ...
  auto redis_command_stats = Common::Redis::RedisCommandStats::createRedisCommandStats(
        context.scope().symbolTable(), "upstream_commands", proto_config.settings().enable_command_stats());
  ...
  for (auto& cluster : unique_clusters) {
    ...
    upstreams.emplace(cluster, std::make_shared<ConnPool::InstanceImpl>(
                                   ...
                                   std::move(stats_scope), std::move(redis_command_stats))); // TODO: Don't use move?
  }
