...
GrpcExternalAuthClient::GrpcExternalAuthClient(const Grpc::RawAsyncClientSharedPtr& async_client,
...
void GrpcExternalAuthClient::cancel() {
    // Make sure the callback and the pending redis request are not null before cancelling the request.
...
// Authenticate method. Called to initiate an asynchronous request to the external authentication server.
void GrpcExternalAuthClient::authenticateExternal(AuthenticateCallback& callback, CommandSplitter::SplitCallbacks& pending_request, const StreamInfo::StreamInfo& stream_info, std::string username, std::string password) {
    ...
    request_ = async_client_->send(service_method_, req, *this, Tracing::NullSpan::instance(), options);
}

// Callback method called when the request is successful.
void GrpcExternalAuthClient::onSuccess(std::unique_ptr<envoy::service::redis_auth::v3::RedisProxyExternalAuthResponse> &&response, Tracing::Span &span) {
    ENVOY_LOG(trace, "Received response for external Redis authentication. Status: {}", response->status().code());

    AuthenticateResponsePtr auth_response = std::make_unique<AuthenticateResponse>(AuthenticateResponse{});
    ...
    } else if (response->status().code() == Grpc::Status::WellKnownGrpcStatus::PermissionDenied || response->status().code() == Grpc::Status::WellKnownGrpcStatus::Unauthenticated) {
...
void GrpcExternalAuthClient::onFailure(Grpc::Status::GrpcStatus status, const std::string &message, Tracing::Span &span) {
