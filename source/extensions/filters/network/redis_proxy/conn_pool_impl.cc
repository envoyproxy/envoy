...
InstanceImpl::ThreadLocalPool::makeRequest(const std::string& key, RespVariant&& request,
  ...
  if (config_->protocol() == Common::Redis::Protocol::Memcached) {
    lb_context = new Clusters::Redis::TwemLoadBalancerContextImpl(key);
  } else {
    lb_context = new Clusters::Redis::RedisLoadBalancerContextImpl(key, config_->enableHashtagging(),
                        is_redis_cluster_, getRequest(request), config_->readPolicy());
  }
