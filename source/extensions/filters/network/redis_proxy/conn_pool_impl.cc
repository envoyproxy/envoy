...
Common::Redis::Client::PoolRequest*
InstanceImpl::ThreadLocalPool::makeRequest(Upstream::LoadBalancerContext* lb_context,
...
InstanceImpl::ThreadLocalPool::makeRequest(const std::string& key, RespVariant&& request,
  ...
  auto result = makeRequest(&lb_context, getRequest(request), callbacks, transaction);
  if (!result) {
