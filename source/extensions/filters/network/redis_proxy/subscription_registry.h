...
public:
  ...
  /**
   * Whether ``host`` is still inside the configured read policy's FEASIBLE SET for ``channel``'s
   * hash-slot shard — the record-validity check. Under ``MASTER`` only the slot primary is
   * feasible; under strict ``REPLICA`` the primary is excluded (unless it is the shard's sole
   * member); the remaining policies accept any shard member. Health-AGNOSTIC, so a
   * momentarily-unhealthy member keeps its subscriptions rather than churning them on a flap.
   * @return true when ``host`` is feasible, AND true when the slot has no shard snapshot yet
   * (transient — keep the record; every policy shares this null-resolution tolerance).
   ...
   * placement target itself (chooseUpstreamHostForChannel, which routes through the configured
   * read policy) and hands it here, so a fresh subscribe, a connection-loss reissue, and a topology
   * reroute all send
...
public:
  ...
  // pub/sub tuning knobs, threaded from ConnPoolSettings.pubsub_settings by the conn pool. All
  // default to the historical values so unit tests (and any caller that omits them) keep the
  // original behavior. Channel homing carries no knob: placement and record validity are delegated
  // to ``upstream_callbacks_``, which route through the conn pool's read policy.
...
private:
  ...
  // judged by upstream_callbacks_.hostServesChannelSlot, which encodes the policy's FEASIBLE SET:
  // under ``MASTER`` the only valid home is the slot's current primary (a moved primary re-homes
  // the channel); under strict ``REPLICA`` the primary is excluded (a promoted owner re-homes onto
  // a replica, sole-member shards excepted); under the remaining replica-capable policies validity
  // is health-AGNOSTIC shard membership, so a momentarily-unhealthy member keeps its channels
  // rather than churning on a flap.
  //
  ...
  // BOTH register the generation identically — the topology path erases the record just before
  // calling, so it always takes the re-place branch. Returns whether the upstream send succeeded.
  ...
  //
  // ORDERING PRECONDITION: every caller records AFTER its successful send (the fresh-subscribe
  // path parks its downstream ack bucket after the send too). That order is safe because an
  // SSUBSCRIBE ack can never be observed synchronously inside the send: an ack is a socket READ,
  // delivered by a later dispatcher read event — the send path's buffer write cannot re-enter
  // onPushMessage — so attempt and bucket are always in place before any ack can arrive.
  // Recording only after a successful send also keeps generation bookkeeping free of un-stamping
  // on a failed send.
