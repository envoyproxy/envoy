...
absl::optional<OpCodes> DecoderImpl::decodeOnData(Buffer::Instance& data, uint64_t& offset) {
...
absl::optional<OpCodes> DecoderImpl::decodeOnWrite(Buffer::Instance& data, uint64_t& offset) {
...
void DecoderImpl::decode(Buffer::Instance& data, DecodeType dtype, uint64_t full_packets_len) {
  ...
  TRY_NEEDS_AUDIT {
    while (offset < full_packets_len) {
      ...
      absl::optional<OpCodes> opcode;
      switch (dtype) {
      case DecodeType::READ:
        opcode = decodeOnData(data, offset);
        callbacks_.onRequestBytes(opcode, offset - current);
        break;
      case DecodeType::WRITE:
        opcode = decodeOnWrite(data, offset);
        callbacks_.onResponseBytes(opcode, offset - current);
        break;
      }
    }
  }
