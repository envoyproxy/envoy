...
public:
  ...
  virtual void onDecodeError(const std::optional<OpCodes> opcode) PURE;
  virtual void onRequestBytes(const std::optional<OpCodes> opcode, const uint64_t bytes) PURE;
  ...
  virtual void onResponseBytes(const std::optional<OpCodes> opcode, const uint64_t bytes) PURE;
...
private:
  ...
  // decodeOnData and decodeOnWrite return ZooKeeper opcode or std::nullopt.
  // std::nullopt indicates WATCH_XID, which is generated by the server and has no corresponding
  // opcode.
  absl::StatusOr<std::optional<OpCodes>> decodeOnData(Buffer::Instance& data, uint64_t& offset);
  absl::StatusOr<std::optional<OpCodes>> decodeOnWrite(Buffer::Instance& data, uint64_t& offset);
