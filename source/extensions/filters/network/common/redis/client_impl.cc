...
ClientImpl::ClientImpl(Upstream::HostConstSharedPtr host, Event::Dispatcher& dispatcher,
  ...
  host->cluster().upstreamStats()->upstream_cx_total_.inc();
  host->stats().cx_total_.inc();
  host->cluster().upstreamStats()->upstream_cx_active_.inc();
...
ClientImpl::~ClientImpl() {
  ...
  host_->cluster().upstreamStats()->upstream_cx_active_.dec();
...
void ClientImpl::onConnectOrOpTimeout() {
  ...
  if (connected_) {
    host_->cluster().upstreamStats()->upstream_rq_timeout_.inc();
    host_->stats().rq_timeout_.inc();
  } else {
    host_->cluster().upstreamStats()->upstream_cx_connect_timeout_.inc();
...
ClientImpl::PendingRequest::PendingRequest(ClientImpl& parent, ClientCallbacks& callbacks,
  ...
  parent.host_->cluster().upstreamStats()->upstream_rq_total_.inc();
  parent.host_->stats().rq_total_.inc();
  parent.host_->cluster().upstreamStats()->upstream_rq_active_.inc();
  parent.host_->stats().rq_active_.inc();
}

ClientImpl::PendingRequest::~PendingRequest() {
  parent_.host_->cluster().upstreamStats()->upstream_rq_active_.dec();
