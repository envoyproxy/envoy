...
const envoy::config::core::v3::Http1ProtocolOptions& getHttpOptions(const envoy::extensions::filters::network::http_connection_manager::v3::HttpProtocolOptions& options) {
...
const envoy::config::core::v3::Http2ProtocolOptions& getHttp2Options(const envoy::extensions::filters::network::http_connection_manager::v3::HttpProtocolOptions& options) {
    if (options.has_explicit_http_config()) {
...
ProtocolOptionsConfigImpl::ProtocolOptionsConfigImpl(
  ...
  : http1_settings_(Http::Utility::parseHttp1Settings(getHttpOptions(options))),
  ...
  upstream_http_protocol_options_(options.has_upstream_http_protocol_options() ? absl::make_optional<envoy::config::core::v3::UpstreamHttpProtocolOptions>(options.upstream_http_protocol_options()) : absl::nullopt){
    if (options.has_explicit_http_config() &&
    ...
    if (options.has_alpn_config()) {
      if (options.alpn_config().has_http2_protocol_options()) {
        use_http2_ = true;
      }
      use_alpn_ = true;
    }
  }
ProtocolOptionsConfigImpl::ProtocolOptionsConfigImpl(const envoy::config::core::v3::Http1ProtocolOptions& http1_settings,
  ...
  : http1_settings_(Http::Utility::parseHttp1Settings(http1_settings)),
  http2_options_(Http2::Utility::initializeAndValidateOptions(http2_options)),
  common_http_protocol_options_(common_options), upstream_http_protocol_options_(upstream_options),
use_alpn_(use_alpn), use_downstream_protocol_(use_downstream_protocol), use_http2_(use_http2) {}
