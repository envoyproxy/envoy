...
struct WireHeader {
  WireHeader(size_t header_length, size_t header_addr_length, size_t addr_lengh, size_t extensions_length)
      : header_length_(header_length), header_addr_length_(header_addr_length), addr_lengh_(addr_lengh), extensions_length_(extensions_length), protocol_version_(Network::Address::IpVersion::v4),
        remote_address_(nullptr), local_address_(nullptr), local_command_(true) {}
  WireHeader(size_t header_length, size_t header_addr_length, size_t addr_lengh, size_t extensions_length, Network::Address::IpVersion protocol_version,
             Network::Address::InstanceConstSharedPtr remote_address,
             Network::Address::InstanceConstSharedPtr local_address)
      : header_length_(header_length), header_addr_length_(header_addr_length), addr_lengh_(addr_lengh), extensions_length_(extensions_length), protocol_version_(protocol_version),
        remote_address_(remote_address), local_address_(local_address), local_command_(false) {

    ASSERT(extensions_length_ <= 65535);
  }

  size_t wholeHeaderLength() {
    return header_length_ + header_addr_length_;
  }

  size_t headerLengthWithoutExtension() {
    return header_length_ + addr_lengh_;
  }
