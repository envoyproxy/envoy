...
UdpProxyFilter::ClusterInfo::createSession(Network::UdpRecvData::LocalPeerAddresses&& addresses,
  ...
  if (!cluster_.info()
    ...
    cluster_.info()->upstreamStats()->upstream_cx_overflow_.inc();
    return nullptr;
  }
  ...
  if (host == nullptr) {
    ...
    cluster_.info()->upstreamStats()->upstream_cx_none_healthy_.inc();
    return nullptr;
  }
...
void UdpProxyFilter::ActiveSession::write(const Buffer::Instance& buffer) {
  ...
  if (!rc.ok()) {
    ...
    cluster_.cluster_.info()->upstreamStats()->upstream_cx_tx_bytes_total_.add(buffer_length);
  }
}
...
void UdpProxyFilter::ActiveSession::processPacket(Network::Address::InstanceConstSharedPtr,
  ...
  cluster_.cluster_.info()->upstreamStats()->upstream_cx_rx_bytes_total_.add(buffer_length);
