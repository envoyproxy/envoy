...
bool validate(
    ...
    const std::vector<envoy::extensions::filters::common::matcher::v3::MatchPredicate::RuleCase>&
        allowed) {
  auto predicate =
      [&allowed](const envoy::extensions::filters::common::matcher::v3::MatchPredicate& config) {
        return validate(config, allowed);
      };

  const auto rule_case = match_config.rule_case();
  switch (rule_case) {
  case envoy::extensions::filters::common::matcher::v3::MatchPredicate::RuleCase::kOrMatch: {
    const auto& rules = match_config.or_match().rules();
    return std::all_of(rules.begin(), rules.end(), predicate);
  }
  case envoy::extensions::filters::common::matcher::v3::MatchPredicate::RuleCase::kAndMatch: {
    const auto& rules = match_config.or_match().rules();
    return std::all_of(rules.begin(), rules.end(), predicate);
  ...
  case envoy::extensions::filters::common::matcher::v3::MatchPredicate::RuleCase::kNotMatch:
    return validate(match_config.not_match(), allowed);
  case envoy::extensions::filters::common::matcher::v3::MatchPredicate::RuleCase::kAnyMatch:
    return true;
  default:
    return std::any_of(
        ...
        [&rule_case](
            envoy::extensions::filters::common::matcher::v3::MatchPredicate::RuleCase allow) {
          return allow == rule_case;
        });
  }
}
...
NotMatcher::NotMatcher(
    const envoy::extensions::filters::common::matcher::v3::MatchPredicate& config,
    std::vector<MatcherPtr>& matchers)
...
HttpGenericBodyMatcher::HttpGenericBodyMatcher(
  ...
  for (const auto& i : config.patterns()) {
    ...
    case envoy::extensions::filters::common::matcher::v3::HttpGenericBodyMatch::GenericTextMatch::
        kBinaryMatch: {
