...
FaultDelayConfig::FaultDelayConfig(
    const envoy::config::filter::fault::v3alpha::FaultDelay& delay_config)
    : percentage_(delay_config.percentage()) {
  switch (delay_config.fault_delay_secifier_case()) {
  case envoy::config::filter::fault::v3alpha::FaultDelay::FaultDelaySecifierCase::kFixedDelay:
    provider_ = std::make_unique<FixedDelayProvider>(
        std::chrono::milliseconds(PROTOBUF_GET_MS_REQUIRED(delay_config, fixed_delay)));
    break;
  case envoy::config::filter::fault::v3alpha::FaultDelay::FaultDelaySecifierCase::kHeaderDelay:
    provider_ = std::make_unique<HeaderDelayProvider>();
    break;
  case envoy::config::filter::fault::v3alpha::FaultDelay::FaultDelaySecifierCase::
...
FaultRateLimitConfig::FaultRateLimitConfig(
  ...
  case envoy::config::filter::fault::v3alpha::FaultRateLimit::LimitTypeCase::kFixedLimit:
    provider_ =
        std::make_unique<FixedRateLimitProvider>(rate_limit_config.fixed_limit().limit_kbps());
    break;
  case envoy::config::filter::fault::v3alpha::FaultRateLimit::LimitTypeCase::kHeaderLimit:
    provider_ = std::make_unique<HeaderRateLimitProvider>();
    break;
  case envoy::config::filter::fault::v3alpha::FaultRateLimit::LimitTypeCase::LIMIT_TYPE_NOT_SET:
    NOT_REACHED_GCOVR_EXCL_LINE;
  }
}
