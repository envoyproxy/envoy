...
bool ReverseConnectionIOHandle::initiateOneReverseConnection(const std::string& cluster_name,
  ...
  // Stamp the episode on the first dial: initiation time and setup-time span, reused across
  // handshake retries until the host reaches target connection count.
  ...
  if (closed || (!error.empty() && !is_success)) {
    ...
    if (host_it != host_to_conn_info_map_.end()) {
      ...
      if (host_it->second.connection_keys.size() >= host_it->second.target_connection_count) {
        ...
        if (host_it->second.setup_time_span != nullptr) {
...
ReverseConnectionIOHandle::HostConnectionInfo::HostConnectionInfo(absl::string_view host_address,
