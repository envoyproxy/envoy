    ...
    if (bytes_read == 1) {
      ...
      if (!established_connections_.empty()) {
        ...
        ENVOY_LOG(info, "reverse_tunnel: RAII IoHandle created with duplicated socket for node_id: {}"
                         "and protection enabled.", config_.src_node_id);
...
void ReverseConnectionIOHandle::maintainClusterConnections(
  ...
  for (const auto& [host_address, host] : *host_map_ptr) {
    ...
    ENVOY_LOG(info,
              "reverse_tunnel: Number of reverse connections to host {} of cluster {} from source node: {}: "
              "Current: {}, Pending: {}, Required: {}",
              host_address, cluster_name, config_.src_node_id, current_connections, pending_connections,
