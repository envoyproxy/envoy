...
EnvoyQuicConnectionIdGeneratorContextPtr Factory::createQuicConnectionIdGeneratorContext() {
  return std::make_unique<Context>(*tls_slot_);
}

QuicConnectionIdGeneratorPtr Context::createQuicConnectionIdGenerator(uint32_t worker_id) {
  return std::make_unique<QuicLbConnectionIdGenerator>(tls_slot_, worker_id);
}

absl::StatusOr<Network::Socket::OptionConstSharedPtr>
Context::createCompatibleLinuxBpfSocketOption(uint32_t concurrency) {
...
Context::getCompatibleConnectionIdWorkerSelector(uint32_t concurrency) {
