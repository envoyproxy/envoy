...
// AggregateRetryLoadBalancerContext wraps the load balancer context to provide retry-aware
// cluster selection while delegating most operations to the underlying context.
class AggregateRetryLoadBalancerContext : public Upstream::LoadBalancerContextBase {
public:
  AggregateRetryLoadBalancerContext(Upstream::LoadBalancerContext* context,
  ...
  const Upstream::HealthyAndDegradedLoad& determinePriorityLoad(
    ...
    // Delegate priority load determination to the wrapped context. The selected cluster
    // maintains its own priority handling.
  ...
  // Get the selected cluster index for this retry attempt.
