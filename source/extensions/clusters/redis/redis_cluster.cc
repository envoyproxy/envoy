...
RedisCluster::RedisDiscoverySession::RedisDiscoverySession(
    ...
    : parent_(parent), dispatcher_(parent.dispatcher_),
      ...
      redis_command_stats_(NetworkFilters::Common::Redis::RedisCommandStats::createRedisCommandStats(
          parent_.info()->statsScope().symbolTable(), "upstream_commands", false)) {}
...
void RedisCluster::RedisDiscoverySession::startResolveRedis() {
  ...
  if (!client) {
    ...
    client->client_ =
        client_factory_.create(host, dispatcher_, *this, std::move(redis_command_stats_), parent_.info()->statsScope());
