...
newDynamicModuleClusterConfig(const absl::string_view cluster_name,
  ...
  auto on_cluster_config_new =
      config->dynamic_module_->getFunctionPointer<decltype(&envoy_dynamic_module_on_cluster_config_new)>(
          "envoy_dynamic_module_on_cluster_config_new");
  RETURN_IF_NOT_OK_REF(on_cluster_config_new.status());

  auto on_cluster_config_destroy = config->dynamic_module_->getFunctionPointer<OnClusterConfigDestroyType>(
      "envoy_dynamic_module_on_cluster_config_destroy");
  RETURN_IF_NOT_OK_REF(on_cluster_config_destroy.status());

  auto on_cluster_new =
      config->dynamic_module_->getFunctionPointer<OnClusterNewType>(
          "envoy_dynamic_module_on_cluster_new");
  RETURN_IF_NOT_OK_REF(on_cluster_new.status());

  auto on_cluster_destroy =
      config->dynamic_module_->getFunctionPointer<OnClusterDestroyType>(
          "envoy_dynamic_module_on_cluster_destroy");
  RETURN_IF_NOT_OK_REF(on_cluster_destroy.status());

  auto on_cluster_init =
      config->dynamic_module_->getFunctionPointer<OnClusterInitType>(
          "envoy_dynamic_module_on_cluster_init");
  RETURN_IF_NOT_OK_REF(on_cluster_init.status());

  auto on_cluster_cleanup =
      config->dynamic_module_->getFunctionPointer<OnClusterCleanupType>(
          "envoy_dynamic_module_on_cluster_cleanup");
  RETURN_IF_NOT_OK_REF(on_cluster_cleanup.status());

  auto on_load_balancer_new =
      config->dynamic_module_->getFunctionPointer<OnLoadBalancerNewType>(
  ...
  auto on_host_set_change =
      config->dynamic_module_->getFunctionPointer<OnHostSetChangeType>(
          "envoy_dynamic_module_on_host_set_change");
  RETURN_IF_NOT_OK_REF(on_host_set_change.status());

  auto on_host_health_change =
      config->dynamic_module_->getFunctionPointer<OnHostHealthChangeType>(
