  ...
  if (proto_config.has_cluster_config()) {
    ...
    if (config_any.type_url() == "type.googleapis.com/google.protobuf.StringValue") {
      ...
      if (config_any.UnpackTo(&struct_config)) {
        ...
        // For unknown types, use deterministic hash.
        const auto hash_bytes = MessageUtil::hash(config_any);
        cluster_config_str =
            Hex::encode(reinterpret_cast<const uint8_t*>(&hash_bytes), sizeof(hash_bytes));
      }
    }
  }
