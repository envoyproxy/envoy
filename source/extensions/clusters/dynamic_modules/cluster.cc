...
absl::StatusOr<Upstream::HostSharedPtr> DynamicModuleCluster::addHost(const std::string& address,
                                                                       uint32_t port,
                                                                       uint32_t weight) {
  // Validate weight.
  ...
  if (host_map_.size() >= max_hosts_) {
    return absl::ResourceExhaustedError(
        absl::StrCat("Maximum hosts limit reached: ", max_hosts_));
  }
  ...
  auto host_or_error = Upstream::HostImpl::create(
      info(), "", address_instance, nullptr, nullptr, weight, locality, health_check_config, 0,
  ...
  priority_set_.updateHosts(
      0,
      Upstream::HostSetImpl::partitionHosts(all_hosts, empty_hosts_per_locality),
...
void DynamicModuleCluster::onHostSetChange(const Upstream::HostVector& hosts_added,
  ...
  config_->on_host_set_change_(thisAsVoidPtr(), in_module_cluster_,
                                added_ptrs.empty() ? nullptr : added_ptrs.data(), added_ptrs.size(),
                                removed_ptrs.empty() ? nullptr : removed_ptrs.data(),
                                removed_ptrs.size());
}
...
Upstream::LoadBalancerFactorySharedPtr
DynamicModuleCluster::ThreadAwareLoadBalancer::factory() {
...
DynamicModuleLoadBalancer::chooseHost(Upstream::LoadBalancerContext* context) {
  ...
  return {std::shared_ptr<Upstream::Host>(static_cast<Upstream::Host*>(host_ptr),
                                          [](Upstream::Host*) {
