  ...
  if (context) {
    ...
    if (dst_host) {
      ...
      if (dst_ip) {
        ...
        HostSharedPtr host(std::shared_ptr<HostImpl>(THROW_OR_RETURN_VALUE(HostImpl::create(
            info, info->name() + dst_addr.asString(), std::move(host_ip_port), nullptr, nullptr, 1,
            envoy::config::core::v3::Locality().default_instance(),
            envoy::config::endpoint::v3::Endpoint::HealthCheckConfig().default_instance(), 0,
            envoy::config::core::v3::UNKNOWN, parent_->cluster_->time_source_), std::unique_ptr<HostImpl>)));
