 ...
 * When given equivalent provider configs, the singleton returns pointers to the same driver/provider.
 * When given different configs, the singleton returns different provider instances.
 */
class DriverSingleton : public Envoy::Singleton::Instance {
  ...
  private:
    ...
    // that config of the provider. Each provider stores shared_ptrs to this singleton, which keeps the
    // singleton from being destroyed unless it's no longer keeping track of any providers.
