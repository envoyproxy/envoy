...
#include "envoy/config/accesslog/v3alpha/als.pb.validate.h"
...
HttpGrpcAccessLogFactory::createAccessLogInstance(const Protobuf::Message& config,
  ...
  const auto& proto_config = MessageUtil::downcastAndValidate<
      const envoy::config::accesslog::v3alpha::HttpGrpcAccessLogConfig&>(
...
ProtobufTypes::MessagePtr HttpGrpcAccessLogFactory::createEmptyConfigProto() {
  return std::make_unique<envoy::config::accesslog::v3alpha::HttpGrpcAccessLogConfig>();
}
