...
OtGrpcAccessLog::ThreadLocalLogger::ThreadLocalLogger(GrpcCommon::GrpcAccessLoggerSharedPtr logger)
...
void OtGrpcAccessLog::emitLog(const Http::RequestHeaderMap& request_headers,
  ...
  // tls_slot_->getTyped<ThreadLocalLogger>().logger_->log(formatter.format(std::move(log_entry),
  // config));
}
