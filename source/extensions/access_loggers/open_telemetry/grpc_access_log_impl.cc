...
GrpcAccessLoggerImpl::GrpcAccessLoggerImpl(const Grpc::RawAsyncClientSharedPtr& client,
                                           std::string log_name, uint64_t max_buffer_size_bytes,
                                           const LocalInfo::LocalInfo& local_info,
                                           Stats::Scope& scope)
    : GrpcAccessLogger(client, max_buffer_size_bytes, scope, GRPC_LOG_STATS_PREFIX,
...
GrpcAccessLoggerImpl::SharedPtr GrpcAccessLoggerCacheImpl::createLogger(
  ...
  auto logger = std::make_shared<GrpcAccessLoggerImpl>(client, config.log_name(),
                                                       max_buffer_size_bytes, local_info_, scope);
