...
#include "envoy/config/accesslog/v3alpha/file.pb.validate.h"
...
FileAccessLogFactory::createAccessLogInstance(const Protobuf::Message& config,
  ...
  const auto& fal_config =
      MessageUtil::downcastAndValidate<const envoy::config::accesslog::v3alpha::FileAccessLog&>(
          config, context.messageValidationVisitor());
  AccessLog::FormatterPtr formatter;

  if (fal_config.access_log_format_case() ==
          envoy::config::accesslog::v3alpha::FileAccessLog::AccessLogFormatCase::kFormat ||
      fal_config.access_log_format_case() == envoy::config::accesslog::v3alpha::FileAccessLog::
    ...
    formatter = std::make_unique<AccessLog::JsonFormatterImpl>(json_format_map, false);
             ...
             envoy::config::accesslog::v3alpha::FileAccessLog::AccessLogFormatCase::
                 kTypedJsonFormat) {
...
ProtobufTypes::MessagePtr FileAccessLogFactory::createEmptyConfigProto() {
  return ProtobufTypes::MessagePtr{new envoy::config::accesslog::v3alpha::FileAccessLog()};
}
