...
#include "envoy/extensions/access_loggers/file/v3alpha/file.pb.validate.h"
...
FileAccessLogFactory::createAccessLogInstance(const Protobuf::Message& config,
  ...
  const auto& fal_config = MessageUtil::downcastAndValidate<
      const envoy::extensions::access_loggers::file::v3alpha::FileAccessLog&>(
      config, context.messageValidationVisitor());
  AccessLog::FormatterPtr formatter;

  if (fal_config.access_log_format_case() == envoy::extensions::access_loggers::file::v3alpha::
...
ProtobufTypes::MessagePtr FileAccessLogFactory::createEmptyConfigProto() {
  return ProtobufTypes::MessagePtr{
      new envoy::extensions::access_loggers::file::v3alpha::FileAccessLog()};
}
