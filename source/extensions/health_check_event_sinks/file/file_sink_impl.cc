...
void HealthCheckEventFileSink::log(
...
HealthCheckEventFileSinkFactory::createHealthCheckEventSink(
  ...
  const auto& validator_config = Envoy::MessageUtil::anyConvertAndValidate<
      envoy::extensions::health_check_event_sinks::file::v3::
          HealthCheckEventFileSink>(config, context.messageValidationVisitor());
  return std::make_unique<HealthCheckEventFileSink>(validator_config,
