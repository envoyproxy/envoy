...
ReqWithoutQuery::format(const Envoy::Formatter::HttpFormatterContext& context,
...
ProtobufWkt::Value
ReqWithoutQuery::formatValue(const Envoy::Formatter::HttpFormatterContext& context,
                             const StreamInfo::StreamInfo&) const {
  const Http::HeaderEntry* header = findHeader(context.requestHeaders());
  if (!header) {
