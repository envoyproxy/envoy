...
ContextImpl::ContextImpl(Stats::Scope& scope, const Envoy::Ssl::ContextConfig& config,
  ...
  // Cipher suite names, obtained via:
  // curl https://raw.githubusercontent.com/openssl/openssl/master/include/openssl/tls1.h 2>&1
  //     grep "#\ define TLS1_.*\".*\"" | grep -o "\".*\"$" | sort | uniq
