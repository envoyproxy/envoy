...
ssl_verify_result_t ContextImpl::verifyCallback(SSL* ssl, uint8_t* out_alert) {
  ...
  if (impl->verify_trusted_ca_) {
    ...
    return impl->doVerifyCertChain(store_ctx.get(), ssl_extended_info, *cert.get(),
                                   transport_socket_options, out_alert);
  } else {
    return impl->doVerifyCertChain(nullptr, ssl_extended_info, *cert.get(),
                                   transport_socket_options, out_alert);
  }
}

ssl_verify_result_t ContextImpl::doVerifyCertChain(
  ...
  if (allow_untrusted_certificate_ || validated != Envoy::Ssl::ClientValidationStatus::Failed) {
    return ssl_verify_ok;
  } else {
    return ssl_verify_invalid;
  }
...
bool ContextImpl::verifyCertChain(X509& leaf_cert, STACK_OF(X509) & intermediates,
  ...
  bssl::ScopedX509_STORE_CTX store_ctx;
  if (!X509_STORE_CTX_init(store_ctx.get(), tls_contexts_[0].x509_store_.get(), &leaf_cert,
                           &intermediates)) {
    error_details = "Failed to verify certificate chain: X509_STORE_CTX_init";
    return false;
  }
  auto ret = doVerifyCertChain(store_ctx.get(), nullptr, leaf_cert, nullptr, nullptr);
  if (ret != ssl_verify_ok) {
    const int n = X509_STORE_CTX_get_error(store_ctx.get());
    const int depth = X509_STORE_CTX_get_error_depth(store_ctx.get());
