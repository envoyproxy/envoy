...
void RedisHttpCacheLookupContext::getHeaders(LookupHeadersCallback&& cb) {
  ...
  if (!tls_slot_->send(
          cluster_name_, {"get", fmt::format(RedisCacheHeadersEntry, stableHashKey(lookup_.key()))},
          [this, weak](bool connected, bool success,
            ...
            if (!connected) {
              ...
              lookup_headers_callback_(std::move(lookup_result),
                                       /* end_stream (ignored) = */ false);
              return;
            }
            if (!success) {
              // Entry was not found.
            ...
            lookup_headers_callback_(lookup_.makeLookupResult(std::move(headers),
                                                              metadataFromHeaderProto(header),
                                                              body_size),
                                     stream_end);
          })) {
...
void RedisHttpCacheLookupContext::getBody(const AdjustedByteRange& range, LookupBodyCallback&& cb) {
  ...
  std::weak_ptr<bool> weak = alive_;
  if (!tls_slot_->send(cluster_name_,
                       {"getrange", fmt::format(RedisCacheBodyEntry, stableHashKey(lookup_.key())),
                        fmt::format("{}", range.begin()),
                        fmt::format("{}", range.begin() + range.length() - 1)},
                       [this, weak](bool connected, bool success,
                                    absl::optional<std::string> redis_value) mutable {
                         // Session was destructed during the call to Redis.
                         // Do nothing. Do not call callback because its context is gone.
                         ...
                         lookup_body_callback_(std::move(buf), !has_trailers_);
                       })) {
    // Callback must be executed on filter's thread.
    dispatcher_.post([this]() { lookup_body_callback_(nullptr, true); });
  }
}
...
void RedisHttpCacheLookupContext::getTrailers(LookupTrailersCallback&& cb) {
  ...
  std::weak_ptr<bool> weak = alive_;
  if (!tls_slot_->send(cluster_name_,
                       {"get", fmt::format(RedisCacheTrailersEntry, stableHashKey(lookup_.key()))},
                       [this, weak](bool connected, bool success,
                                    absl::optional<std::string> redis_value) mutable {
                         // Session was destructed during the call to Redis.
                         // Do nothing. Do not call callback because its context is gone.
    ...
    // Callback must be executed on filter's thread.
    dispatcher_.post([this]() { lookup_trailers_callback_(nullptr); });
  }
}
