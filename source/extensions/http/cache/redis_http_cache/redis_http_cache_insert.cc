...
void RedisHttpCacheInsertContext::insertHeaders(const Http::ResponseHeaderMap& response_headers,
  ...
  // Make proto with received headers. It will be sent to Redis as the last item (after body and
  // trailers).
  header_proto_ = makeCacheFileHeaderProto(lookup_->key(), response_headers, metadata);

  std::weak_ptr<bool> weak = alive_;
  if (!tls_slot_->send(lookup_->clusterName(),
                       {"set", fmt::format(RedisCacheHeadersEntry, stableHashKey(lookup_->key())),
                        "", "NX", "EX", "30"},
                       [this, weak, end_stream](bool connected, bool success,
                                                absl::optional<std::string> /*redis_value*/) {
                         ...
                         if (!success) {
                           // Error writing to Redis (connection was OK).
                           // Entry containing headers exists. The entry was added most likely by
                           // other Envoy or by other thread.
                           (insert_callback_)(false);
                         }
                         ...
                         (insert_callback_)(true);
                       }))

  {
    // Callback must be executed on filter's thread.
    lookup_->dispatcher()->post([this]() { (insert_callback_)(false); });
  }
}
...
void RedisHttpCacheInsertContext::insertBody(const Buffer::Instance& chunk,
  ...
  if (first_body_chunk_) {
    success =
        tls_slot_->send(lookup_->clusterName(),
                        {"set", fmt::format(RedisCacheBodyEntry, stableHashKey(lookup_->key())),
                         chunk.toString(), "EX", "30"},
                        std::move(result_callback));
  } else {
    success =
        tls_slot_->send(lookup_->clusterName(),
                        {"append", fmt::format(RedisCacheBodyEntry, stableHashKey(lookup_->key())),
                         chunk.toString()},
                        std::move(result_callback));
  }
...
void RedisHttpCacheInsertContext::insertTrailers(const Http::ResponseTrailerMap& trailers,
  ...
  std::weak_ptr<bool> weak = alive_;
  if (!tls_slot_->send(lookup_->clusterName(),
                       {"set", fmt::format(RedisCacheTrailersEntry, stableHashKey(lookup_->key())),
                        trailers_proto.SerializeAsString(), "EX", "30"},
                       [this, weak](bool connected, bool success,
                                    absl::optional<std::string> /*redis_value*/) mutable {
                         // Session was destructed during the call to Redis.
                         // Do nothing. Do not call callback because its context is gone.
                         if (weak.expired()) {
                           return;
...
void RedisHttpCacheInsertContext::onStreamEnd() {
  ...
  if (!tls_slot_->send(lookup_->clusterName(),
                       {"expire", fmt::format(RedisCacheBodyEntry, stableHashKey(lookup_->key())),
                        fmt::format("{}", cache_for)},
                       [](bool /* connected */, bool /*success*/,
                          absl::optional<std::string> /*redis_value*/) {})) {
    return;
  }
  ...
  if (!tls_slot_->send(lookup_->clusterName(),
                       {"expire",
                        fmt::format(RedisCacheTrailersEntry, stableHashKey(lookup_->key())),
                        fmt::format("{}", cache_for)},
                       [](bool /* connected */, bool /*success*/,
                          absl::optional<std::string> /*redis_value*/) {})) {
    return;
  ...
  tls_slot_->send(
      lookup_->clusterName(),
      {"set", fmt::format(RedisCacheHeadersEntry, stableHashKey(lookup_->key())), serialized_proto,
       "XX", "EX", fmt::format("{}", cache_for)},
