...
GlobalHostStats::GlobalHostStats(Upstream::HostConstSharedPtr host, Stats::Scope& scope)
    : pool_(scope.symbolTable()),
      cost_stat_(
          scope.gaugeFromStatName(pool_.add("peak_ewma." + host->address()->asString() + ".cost"),
                                  Stats::Gauge::ImportMode::NeverImport)),
      ewma_rtt_stat_(scope.gaugeFromStatName(
          pool_.add("peak_ewma." + host->address()->asString() + ".ewma_rtt_ms"),
          Stats::Gauge::ImportMode::NeverImport)),
      active_requests_stat_(scope.gaugeFromStatName(
          pool_.add("peak_ewma." + host->address()->asString() + ".active_requests"),
          Stats::Gauge::ImportMode::NeverImport)),
      host_(host) {}
