...
public:
  explicit PatternTemplatePredicate(std::string url_pattern, std::string url_rewrite_pattern)
      : Router::PatternTemplatePredicate(url_pattern, url_rewrite_pattern),
        matching_pattern_regex_(RE2(PatternTemplatePredicate::convertURLPatternSyntaxToRegex(url_pattern).value())) {}
  ...
  static absl::Status is_valid_rewrite_pattern(std::string match_pattern, std::string rewrite_pattern);
...
private:
  ...
  static absl::StatusOr<std::string> convertURLPatternSyntaxToRegex(absl::string_view url_pattern);
  ...
  // use it.
  static absl::StatusOr<std::vector<RewritePatternSegment>> parseRewritePatternHelper(absl::string_view pattern);
  ...
  static absl::StatusOr<envoy::extensions::url_template::v3::RouteUrlRewritePattern>
  ...
  static absl::Status isValidPathTemplateMatchPattern(const std::string& path_template_match);

  // Returns if provided rewrite pattern is valid
  static absl::Status isValidPathTemplateRewritePattern(const std::string& path_template_rewrite);

  // Returns if path_template and rewrite_template have valid variables
  static absl::Status isValidSharedVariableSet(const std::string& path_template_rewrite,
