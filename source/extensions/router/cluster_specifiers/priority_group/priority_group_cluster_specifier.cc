...
uint64_t PriorityGroupClusterSpecifierPlugin::randomValue(
    const Envoy::Router::RouteEntryAndRoute& parent, const Http::RequestHeaderMap& headers,
  ...
  if (!random_value_header_.get().empty()) {
    ...
    // cluster picking throughout the process because different values are used to compute the
    // selected value. So, we treat multi-valued header as invalid input and fall back to use
    // internally generated random number.
    if (header_value.size() == 1) {
      uint64_t value = 0;
      if (absl::SimpleAtoi(header_value[0]->value().getStringView(), &value)) {
        return value;
      }
    }
    ENVOY_LOG(debug, "priority group cluster specifier: the random value can not be found from the "
                     "header and it will fall back to the internally generated random value");
    return random;
  }

  // Use the hash policies of the route to generate the random value if it's enabled.
  if (use_hash_policy_) {
    if (const auto* hash_policy = parent.hashPolicy(); hash_policy != nullptr) {
      const auto hash =
          hash_policy->generateHash(OptRef<const Http::RequestHeaderMap>(headers),
                                    OptRef<const StreamInfo::StreamInfo>(stream_info), nullptr);
      if (hash.has_value()) {
        return hash.value();
      }
    }
    ENVOY_LOG(debug, "priority group cluster specifier: the random value can not be generated by "
                     "the hash policies and it will fall back to the internally generated random "
                     "value");
  }
...
Envoy::Router::RouteConstSharedPtr PriorityGroupClusterSpecifierPlugin::route(
    ...
    const StreamInfo::StreamInfo& stream_info, uint64_t random) const {
  // The random value is only computed once for the request and the same value is reused for all
  // the attempts of the request to keep the cluster picking stable.
  const uint64_t random_value = randomValue(*parent, headers, stream_info, random);
  std::string cluster_name = selectCluster(stream_info, random_value);
  return std::make_shared<PriorityGroupRouteEntry>(std::move(parent), shared_from_this(),
                                                   std::move(cluster_name), random_value);
}
