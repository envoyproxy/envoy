...
//
// 1) If the config has typed_dns_resolver_config, copy it into typed_dns_resolver_config and use it.
//
...
// 2.3) if config is DnsFilterConfig, pack an empty CaresDnsResolverConfig into typed_dns_resolver_config.
// 2.4) For all others, copy config.use_tcp_for_dns_lookups into
//      CaresDnsResolverConfig.dns_resolver_options.use_tcp_for_dns_lookups
// 2.5) For ClusterConfig, one extra thing is to copy dns_resolvers into CaresDnsResolverConfig.resolvers,
// 2.6) Then pack CaresDnsResolverConfig into typed_dns_resolver_config.
// Note, to make cares DNS library to work,
// In file: source/extensions/extensions_build_config.bzl, cares DNS extension need to be enabled:
...
// 3.1) In file: source/extensions/extensions_build_config.bzl, enable Apple DNS extension by adding below line
//      "envoy.dns_resolver.apple":                "//source/extensions/network/dns_resolver/apple:apple_dns_lib",
// 3.2) In running configuration file, add apple DNS as a typed extension in *typed_dns_resolver_config* field.
// For example, to add such configuration in bootstrap (or other configurations like cluster, dynamic_forward_proxy):
...
template <class T>
static inline
    void makeDnsResolverConfig(const T& config,
