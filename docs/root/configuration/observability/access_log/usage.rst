...
%GRPC_STATUS(X)%
  `gRPC status code <https://github.com/googleapis/googleapis/blob/master/google/rpc/code.proto>` formatted according to the optional parameter X, which can be ``CAMEL_STRING``, ``SNAKE_STRING`` and ``NUMBER``.
