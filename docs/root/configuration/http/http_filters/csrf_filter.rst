...
<envoy_api_msg_config.filter.http.csrf.v2.CsrfPolicy>` field.
...
:ref:`shadow_enabled <envoy_api_msg_config.filter.http.csrf.v2.CsrfPolicy>` field. When enabled in
shadow-only node, the filter will evaluate the request's *Origin* and *Destination* to determine if
