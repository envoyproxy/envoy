...
:ref:`config_http_filters_router_x-envoy-upstream-rq-timeout-ms`) or it is ignored, but
will not be ignored if the global route timeout is disabled (set to 0). This allows a
