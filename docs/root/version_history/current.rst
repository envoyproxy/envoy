...
* http3: add :ref:`early_data_allows_safe_requests <envoy_v3_api_field_config.route.v3.RouteAction.early_data_allows_safe_requests>` and :ref:`early_data_options <envoy_v3_api_field_config.route.v3.RouteAction.early_data_options>` to allow upstream HTTP/3 sending requests over early data. If either of them is configured, HTTP/3 pool will send early data to the host if the pool already cached 0-RTT credentials of that host. If :ref:`early_data_allows_safe_requests <envoy_v3_api_field_config.route.v3.RouteAction.early_data_allows_safe_requests>` is configured true, only safe request will be sent over early data, and if those requests fail and the underlying connection pool supports TCP fallback, the request may be retried automatically. If non-safe request is configured to be allowed over early data via :ref:`early_data_options <envoy_v3_api_field_config.route.v3.RouteAction.early_data_options>`, the Envoy will not automatically retry them. If desired, config their retry manually. This feature requires both ``envoy.reloadable_features.conn_pool_new_stream_with_early_data_and_http3`` and ``"envoy.reloadable_features.http3_sends_early_data`` to be turned on.
