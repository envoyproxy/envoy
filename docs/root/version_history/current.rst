...
* dynamic forward proxy: added configurable :ref:`circuit breakers <dns_cache_circuit_breakers>` for resolver on DNS cache. This behavior can be temporarily disabled by the runtime feature `envoy.reloadable_features.enable_dns_cache_circuit_breakers`. If this runtime feature is disabled, we will use circuit breakers for upstream connection as default even if we set :ref:`DNS Cache circuit breakers <dns_cache_circuit_breakers>`.
...
* To use cluster circuit breakers for DNS Cache is now deprecated. This behavior can be temporarily disabled by setting `envoy.reloadable_features.enable_dns_cache_circuit_breakers` to `false`.