...
* lua: added ``header:getAtIndex()`` and ``header:getValueSize()`` methods to :ref:`header object <config_http_filters_lua_header_wrapper>` for retrieving the value of a header at certain index and get the value size of a given header.
