...
* http: http3 is now enabled/disabled via build option :option:`--define http3=disabled` rather than the extension framework. The behavior is the same, but builds may be affected for platforms or build configurations where http3 is not supported.
