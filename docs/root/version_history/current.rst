...
* http2: added initial implementation for deferring processing of backed up http2 streams by buffering in the receiving codec. This is guarded by ``envoy.reloadable_features.defer_processing_backedup_streams``, but is disabled by default. Trailers and bodies are buffered if a stream is backed up. Body data is processed in chunks up to the size of the receiving codec's buffer high watermark. Fairness is implemented by prioritizing streams that haven't written to the connection recently to process deferred data when we transition from network connection backed up to network connection low watermark. Defer processing applies to both upstream and downstream connections.
