...
* listener: convert the IPv4 mapped IPv6 address as IPv4 address for new connection's local address when the listener is listening on IPv4-mapped IPv6 address with ``ipv4_compat`` flag. This behavior matches the case of the listener is listening on IPv6 any address with ``ipv4_compat`` flag. The runtime flag ``envoy.reloadable_features.convert_ipv4_mapped_address_to_ipv4_address`` can be used to revert this behavior.
