...
* http3: supports upstream HTTP/3 sending early data via 0-RTT safe requests. HTTP/3 pool will send early data to the host if the pool already cached 0-RTT credentials of that host. If those requests fail and the underlying connection pool supports TCP fallback, the request will be retried automatically. This feature requires both ``envoy.reloadable_features.conn_pool_new_stream_with_early_data_and_http3`` and ``"envoy.reloadable_features.http3_sends_early_data`` to be turned on.
