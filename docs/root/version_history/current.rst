...
* jwt_authn filter: allows Jwt time constraint verification with a clock skew (default to 60 seconds) and added a filter config field :ref:`clock_skew_seconds <envoy_v3_api_msg_extensions.filters.http.jwt_authn.v3.Provider.clock_skew_seconds>` to configure it.
