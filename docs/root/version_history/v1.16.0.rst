...
* build: added visibility rules for upstream. If these cause visibility related breakage, see notes in `BUILD <https://github.com/envoyproxy/envoy/blob/release/v1.16/BUILD>`_.
...
* stats: the fake symbol table implementation has been removed from the binary, and the option ``--use-fake-symbol-table`` is now a no-op with a warning.
...
* listener: fixed crash at listener in-place update when connection load balancer is set.
...
* http: removed legacy header sanitizing and the runtime guard ``envoy.reloadable_features.strict_header_validation``.
...
* http: removed the connection header sanitizing runtime guard ``envoy.reloadable_features.connection_header_sanitization``.
...
* watchdog: support randomizing the watchdog's kill timeout to prevent synchronized kills via a maximum jitter parameter :ref:`max_kill_timeout_jitter <v1.16:envoy_v3_api_field_config.bootstrap.v3.Watchdog.max_kill_timeout_jitter>`.
