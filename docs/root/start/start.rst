...
默认地，Docker 镜像将以构建时创建的 ``envoy`` 用户来运行。
...
默认地，``envoy`` 镜像会把应用程序的日志发送到 ``/dev/stdout`` 和 ``/dev/stderr`` ，这样就可以在容器日志中看到了。
...
我们已经创建了一些使用 Docker 组件的沙箱环境，来设定不同的环境以进行 Envoy 特性的测试和样例配置的展示。随着我们对人们兴趣的判定，我们将添加更多的沙箱来验证不同的特性，如下是一些可用的沙箱：
