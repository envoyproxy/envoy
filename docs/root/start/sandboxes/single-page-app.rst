...
.. warning::
   ...
   Upstreams are excluded if passed through by the
...
Change to the ``examples/single-page-app`` directory, and create the HMAC secret.
...
Step 3: Browse to the dev app and login
***************************************
...
Step 5: Live reload code changes
********************************
...
.. code-block:: console
   ...
   $ sed -i s/Envoy\ single\ page\ app\ example/DEV\ APP/g ui/index.html

The page should automatically refresh.

Likewise any changes to the app components in ``ui/src/...`` should automatically reload in
the browser.
...
Step 8: Browse to the production server
***************************************
...
Step 9: Setup Github OAuth/API access
*************************************
...
Step 10: Update Envoy's configuration to use Github
***************************************************
...
For the OAuth configuration in ``envoy.yml`` set the `Github provided client secret <github-oauth-credentials_>`__:
...
.. code-block:: console
   ...
   $ sed -i s@authorization_endpoint:\ http://localhost:7000/authorize@authorization_endpoint:\ https://github.com/login/oauth/authorize@g envoy.yml
...
.. code-block:: console
   ...
   $ sed -i s@uri:\ http://myhub:7000/authenticate@uri:\ https://github.com/login/oauth/access_token@g envoy.yml
...
Step 11: Update the app configuration to use Github
***************************************************
...
`Vite <vite_>`__ app environment settings are read from ``ui/.env*``:
...
Step 12: Rebuild the app and restart Envoy
******************************************
