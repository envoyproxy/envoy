...
and then shipped upstream with all of the normal Envoy request sanitizing taking place.

.. warning::
  Note that HTTP connection manager sanitizing such as clearing untrusted headers will only be
