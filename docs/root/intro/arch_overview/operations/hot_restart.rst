...
* 新进程在它从老进程拷贝监听套接字之前要完全的初始化自己（如加载配置，执行初始化服务发现和健康检查）。然后新进程开始监听，
  并且通知老进程开始消退（draining）。
* 在消退（draining）阶段，老进程会尝试优雅的关闭已经已有链接。具体如何关闭这取决于过滤器的配置。消退（draining）时间可以
  通过:option:`--drain-time-s`来配置，并且随着时间的推移，消退（draining）会变得更强硬。
* 在老进程消退（draining）之后，新的 Envoy 进程通知老进程停止自己。这个时间可以通过:option:`--parent-shutdown-time-s` 选项来配置。
* Envoy 的热重启设计使得即便新 Envoy 进程和老 Envoy 进程运行在不同的容器内也能正确的工作。新老进程之间的通信只使用 unix 域套接字。
