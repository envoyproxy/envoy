...
负载管理是一个用于保护Envoy服务器避免因太多的客户端连接或请求而导致各种资源(例如内存、CPU或文件描述符)负载太重的可扩展组件。注意它与 :ref:`熔断器 <arch_overview_circuit_break>` 不同，后者的主要目的是用于保护上游服务。

负载管理是可以 :ref:`配置 <config_overload_manager>` 的，可以配置一组被监视的资源，并可以定义当资源的压力超过阈值时该如何操作。
...
当一个触发器改变了状态，值就会被发送给注册好的操作，而操作就会影响有多少连接和请求能被处理。每个操作都可以解析传来的不同状态值，有些操作的行为可能是完全忽略*scaling*状态，只有超过*saturated*的时候才有效果。