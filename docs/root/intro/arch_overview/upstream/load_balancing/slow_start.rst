...
Slow start mode is most effective for cases where few new endpoints come up e.g. scale event in Kubernetes and is not effective when all endpoints are relatively new e.g. new deployment in Kubernetes.
