...
is authorized or not. The filter can be either configured either as a
:ref:`network filter <config_network_filters_ext_authz>`, as a
...
.. tip::
  ...
  that requests are authorized prior to rest of the filters running through the request.

The external authorization service cluster may be either statically configured or via
:ref:`Cluster Discovery Service <config_cluster_manager_cds>`. If the external service is not
...
:ref:`HTTP filter <envoy_api_msg_config.filter.http.ext_authz.v2alpha.ExtAuthz>`. If it is set to
