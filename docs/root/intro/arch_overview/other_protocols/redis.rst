...
re-places the channel onto a current member without disturbing the downstream subscriber. Strict
``REPLICA`` additionally re-homes a member that is promoted to primary — reads stop using a
promoted member at that instant, and its subscriptions follow onto a replica — unless it is the
shard's only member, in which case the subscription keeps delivering from the sole member until a
replica appears.

On a standalone (non-Redis-cluster) upstream the same rule degrades naturally: delivery depends on
``SSUBSCRIBE`` and ``SPUBLISH`` hashing to the same node, so the cluster must use a hash-based load
balancer (ring hash or Maglev — the requirement keyed commands already carry), and a subscription
follows that ring: when a rebalance remaps its channel, the proxy re-places the subscription onto
the node publishes now reach.
