...
2. Vulnerabilities must be reported to **both** the Envoy project and via https://bughunters.google.com/report.
   ...
   — or, as a fallback for reporters without a GitHub account, email envoy-security@googlegroups.com
   (if you have a GitHub account, please include your GitHub username in the email so we can add
   you to the corresponding Security Advisory later).
