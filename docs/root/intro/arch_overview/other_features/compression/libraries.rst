...
.. note::
  ...
  Envoy is built using `zlib-ng <https://github.com/zlib-ng/zlib-ng>`_ instead of regular
  `zlib <http://zlib.net>`_, but it could be linked to `zlib <http://zlib.net>`_ by using ``--define zlib=original``
