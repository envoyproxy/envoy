...
* The `operation_name` field in :ref:`HTTP connection manager
  ...
  specified.
* The `use_http2` field in :ref:`HTTP health checker <envoy_api_field_core.HealthCheck.HttpHealthCheck>` has been deprecated in favor of the `codec_client_type` field. The latter allows configuring HTTP3.
