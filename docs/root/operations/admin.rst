...
.. http:get:: /stats
  ...
  represents the summary since last flush. If :ref:`stats_flush_on_admin <envoy_v3_api_field_config.bootstrap.v3.Bootstrap.stats_flush_on_admin>`
  is specified, stats are flushed when this endpoint is queried. By default, a timer is setup to flush in intervals
  defined by :ref:`stats_flush_interval <envoy_v3_api_field_config.bootstrap.v3.Bootstrap.stats_flush_interval>`,
  defaulting to 5 seconds. The cumulative value represents the summary since the start of Envoy instance.
  "No recorded values" in the histogram output indicates that it has not been updated with a value.
