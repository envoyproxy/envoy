...
.. option:: --stats-tag
  ...
  *(optional)* This flag provides a universal tag for all stats generated by Envoy. The format is ``tag:value``.
  This flag can be repeated multiple times to set multiple universal tags. However, multiple stats with different values
  are not allowed.
