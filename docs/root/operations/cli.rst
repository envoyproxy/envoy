...
.. option:: --mode <string>
  ...
  * ``load``: Load only the provided JSON configuration file and then exit, printing either an "OK"
    ...
    other resources are not available. No network traffic is generated, and the hot restart process
    is not performed, so no other Envoy process on the machine will be disturbed.
