...
def check_comment(checker, offset, comment):
  ...
  comment, found = mask_with_regex(comment, RST_LINK, 2)
  if not found:
...
def extract_comments(lines):
  ...
  # Filter out comments that are part of RST code block directives.
  result = []
  n = 0
  nc = len(comments)
  while n < nc:
    if comments[n].text.strip().startswith(RST_CODE_BLOCK):
      # Start of a code block.
      indent = len(INDENT.search(comments[n].text).group(1))
