...
class ProtoFormatVisitor(visitor.Visitor):
  ...
  """
    ...
    def visit_file(self, file_proto, type_context, services, msgs, enums):
        ...
        pkg_version_status_exempt = file_proto.name.startswith(
            'envoy/annotations') or empty_file
        ...
        if existing_pkg_version_status == status_pb2.UNKNOWN and not pkg_version_status_exempt:
            raise ProtoXformError('package_version_status must be set in %s' %
                                  file_proto.name)
        ...
        if self._active_or_frozen and not pkg_version_status_exempt:
            ...
            if self._freeze and typedb.next_version_protos.get(
                    output_proto.name, None):
            ...
            output_proto.options.Extensions[
                status_pb2.
                file_status].package_version_status = target_pkg_version_status
...
def main():
    plugin.plugin([
        ...
        plugin.OutputDescriptor('.next_major_version_candidate.proto',
                                ...
                                functools.partial(migrate.version_upgrade_xform,
                                                  2, False),
                                want_params=True),
