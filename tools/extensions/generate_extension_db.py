...
def num_read_filters_fuzzed():
    ...
    return len(
        re.findall('NetworkFilterNames::get()',
...
def num_robust_to_downstream_network_filters(db):
    ...
    return len([
        ext for ext, data in db.items() if 'network' in ext and
        data['security_posture'] == 'robust_to_untrusted_downstream'
    ])
...
def get_extension_metadata(target):
    ...
    r = subprocess.run([
        BUILDOZER_PATH, '-stdout',
        'print security_posture status undocumented category', target
    ],
    ...
    return {
        'security_posture':
            security_posture,
        'undocumented':
            False if is_missing(undocumented) else bool(undocumented),
        'status':
            'stable' if is_missing(status) else status,
        'categories':
            categories,
    }
...
if __name__ == '__main__':
    ...
    if num_robust_to_downstream_network_filters(
            extension_db) != num_read_filters_fuzzed():
        raise ExtensionDbError(
    ...
    extension_db[
        'envoy.upstreams.http.http_protocol_options'] = get_extension_metadata(
