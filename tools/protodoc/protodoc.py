...
from tools.api_proto_plugin import (
    annotations, plugin, traverse, type_context as api_type_context, visitor)
...
PRETTY_TYPE_NAMES = {
    field_proto.TYPE_DOUBLE: 'double',
    field_proto.TYPE_FLOAT: 'float',
    field_proto.TYPE_INT32: 'int32',
    field_proto.TYPE_SFIXED32: 'int32',
    field_proto.TYPE_SINT32: 'int32',
    field_proto.TYPE_FIXED32: 'uint32',
    field_proto.TYPE_UINT32: 'uint32',
    field_proto.TYPE_INT64: 'int64',
    field_proto.TYPE_SFIXED64: 'int64',
    field_proto.TYPE_SINT64: 'int64',
    field_proto.TYPE_FIXED64: 'uint64',
    field_proto.TYPE_UINT64: 'uint64',
    field_proto.TYPE_BOOL: 'bool',
    field_proto.TYPE_STRING: 'string',
    field_proto.TYPE_BYTES: 'bytes'
}

PRETTY_LABEL_NAMES = {field_proto.LABEL_OPTIONAL: '', field_proto.LABEL_REPEATED: '**repeated** '}

import contextlib


@contextlib.contextmanager
def debug(name, threshold=None):
    import time
    start = time.time()
    yield
    total = time.time() - start
    if total > (threshold if threshold is not None else 0.1):
        logger.warning(f"{name}: {total}s")
...
class RstFormatVisitor(visitor.Visitor):
    ...
    def envoy_prefixes(self) -> Set[str]:
        return set([ENVOY_PREFIX, ENVOY_API_NAMESPACE_PREFIX, CNCF_PREFIX])
    ...
    def visit_message(self, msg_proto, ctx, nested_msgs: Iterable, nested_enums: Iterable) -> str:
        ...
        return self.tpl_content.render(
            ...
            body=self.tpl_message.render(
                pretty_label_names=PRETTY_LABEL_NAMES,
    ...
    def _enum(self, index, ctx, enum_value) -> Dict:
        ...
        comment = default_comment + UNICODE_INVISIBLE_SEPARATOR + self._comment(ctx.leading_comment)
        return dict(
    ...
    def _extension(self, extension):
        ...
        return self.tpl_extension.render(
            extension=extension,
            contrib=contrib,
            status=status,
    ...
    def _field_type(self, typenames, field_type, type_name):
        ...
        """
        if type_name.startswith(WKT_NAMESPACE_PREFIX):
            ...
            return type_name
        if field_type in PRETTY_TYPE_NAMES:
            return format_external_link(
                PRETTY_TYPE_NAMES[field_type],
    ...
    def _field_type_envoy(self, typenames, field_type, type_name):
        ...
        if field_type == field_proto.TYPE_MESSAGE:
            ...
            return format_internal_link(type_name, message_cross_ref_label(normal_type_name))
    ...
    def _security_option(self, name, untrusted_upstream, untrusted_downstream):
        # logger.warning(f"SEC OPT: {name} {untrusted_upstream} {untrusted_downstream}")
...
class TraversalAdapter:
    _debug_threshold = None

    def __call__(self, file_proto, visitor):
        with debug(f"FILE: {file_proto.name}", threshold=.2):
            return self.file(file_proto, visitor)

    def file(self, file_proto, visitor) -> str:
        # logger.warning(f"TRAVERSER: {file_proto.name}")
        source_code_info = api_type_context.SourceCodeInfo(
            file_proto.name, file_proto.source_code_info)
        package_type_context = api_type_context.TypeContext(source_code_info, file_proto.package)
        return visitor.visit_file(
            file_proto, package_type_context,
            self.services(package_type_context.extend_service, file_proto.service, visitor),
            self.messages(package_type_context.extend_message, file_proto.message_type, visitor),
            self.enums(package_type_context.extend_enum, file_proto.enum_type, visitor))

    def enums(self, extend, enums, visitor) -> Iterator[str]:
        for index, enum in enumerate(enums):
            with debug(f"ENUM: {enum.name}", threshold=self._debug_threshold):
                yield traverse.traverse_enum(
                    extend(index, enum.name, enum.options.deprecated), enum, visitor)

    def message(self, ctx, msg_proto, visitor) -> str:
        ctx.map_typenames = {
            f"{ctx.name}.{nested_msg.name}": (nested_msg.field[0], nested_msg.field[1])
            for nested_msg in msg_proto.nested_type
            if nested_msg.options.map_entry
        }
        return visitor.visit_message(
            msg_proto, ctx,
            self.messages(ctx.extend_nested_message, msg_proto.nested_type, visitor),
            self.enums(ctx.extend_nested_enum, msg_proto.enum_type, visitor))

    def messages(self, extend, msgs, visitor) -> Iterator[str]:
        for index, msg in enumerate(msgs):
            with debug(f"MSG: {msg.name}", threshold=self._debug_threshold):
                message = self.message(
                    extend(index, msg.name, msg.options.deprecated), msg, visitor)
                if message:
                    yield message

    def services(self, extend, services, visitor) -> Iterator[str]:
        for index, service in enumerate(services):
            with debug(f"SERVICE: {service}", threshold=self._debug_threshold):
                yield traverse.traverse_service(
                    extend(index, service.name, service.options.deprecated), service, visitor)
