...
def generate_empty_extension_docs(extension, details, api_extensions_root):
    ...
    path = pathlib.Path(api_extensions_root, extension_root, 'empty',
                        extension_root.name + '.rst')
    path.parent.mkdir(parents=True, exist_ok=True)
    description = details.get('description', '')
    reflink = ''
    if 'ref' in details:
        reflink = '%s %s.' % (details['title'],
                              protodoc.format_internal_link(
...
if __name__ == '__main__':
    ...
    empty_extensions = json.loads(
        pathlib.Path(empty_extensions_path).read_text())
