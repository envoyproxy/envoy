...
from tools.base.checker import BazelRunError
from tools.testing import all_pytests, pytest_all_pytests


def test_pytest_all_pytests():
    with patch("tools.testing.pytest_all_pytests.python_pytest") as m_pytest:
        pytest_all_pytests.main("arg1", "arg2", "arg3")
...
def test_all_pytests_pytest_targets():
    checker = all_pytests.PytestChecker("path1", "path2", "path3")
    bazel_mock = patch("tools.testing.all_pytests.checker.BazelChecker.bazel_query")
...
def test_all_pytests_check_pytests(patches):
    checker = all_pytests.PytestChecker("path1", "path2", "path3")
    patched = patches(
        "checker.BazelChecker.bazel_run",
        "PytestChecker.error",
    ...
    with patched as (m_bazel, m_error, m_succeed, m_targets):
        m_targets.return_value = check_runs.keys()
        m_bazel.side_effect = _run_bazel
        checker.check_pytests()

    assert (
        list(list(c) for c in m_bazel.call_args_list)
        == [[('check1',), {}],
            [('check2',), {}],
            [('check3',), {}],
            [('check4',), {}],
...
def test_all_pytests_on_checks_complete(patches, cov_html):
    ...
    if cov_html:
        ...
        assert not m_bazel.called


def test_python_checker_main():
    class_mock = patch("tools.testing.all_pytests.PytestChecker")

    with class_mock as m_class:
        assert (
            all_pytests.main("arg0", "arg1", "arg2")
