...
class PytestChecker(checker.BazelChecker):
    ...
    def pytest_targets(self) -> set:
        return set(target for target in self.bazel_query("tools/...") if ":pytest_" in target)
    ...
    def check_pytests(self) -> int:
        for target in self.pytest_targets:
            try:
                self.bazel_run(target, *self.pytest_bazel_args)
                self.succeed("pytest", [target])
            except checker.BazelRunError:
...
def main(*args: list) -> None:
    return PytestChecker(*args).run_checks()
