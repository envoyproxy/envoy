...
def replace_includes(mock_name):
  ...
  for filename in filenames:
    ...
    bazelname = "//test/mocks/{}:"+filename.split('/')[-1].replace('.h','')+'_mocks'.format(mock_name)
  ...
  for i in p.glob('**/*.cc'):
    ...
    if flag:
      with i.open(mode='w') as f:
        f.write(content.replace('#include "test/mocks/{}/mocks.h"'.format(mock_name),replace_includes))
