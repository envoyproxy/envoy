...
def deprecate_proto():
    grep_output = subprocess.check_output('grep -r "deprecated = true" api/*',
                                          shell=True)
    ...
    exiting_deprecated_regex = re.compile(
        r'.*"envoy.deprecated_features.(.*):(.*)",.*')
    with open('source/common/runtime/runtime_features.cc', 'r') as features:
        for line in features.readlines():
            ...
            if match:
                filenames_and_fields.discard(
                    tuple([match.group(1), match.group(2)]))
    ...
    for (filename, field) in filenames_and_fields:
        code_snippets.append('    "envoy.deprecated_features.' + filename +
                             ':' + field + '",\n')
...
if not input('Apply relevant runtime changes? [yN] ').strip().lower() in (
        'y', 'yes'):
    exit(1)

for line in fileinput.FileInput('source/common/runtime/runtime_features.cc',
                                inplace=1):
