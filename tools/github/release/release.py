...
class GithubRelease:
    ...
    async def create(self, assets: Optional[List[pathlib.Path]] = None) -> Dict[str, Union[List[Dict[str, Union[str, pathlib.Path]]], Dict]]:
    ...
    async def fetch(
        ...
        if not response["errors"]:
            self.log.success(f"Assets downloaded for release version: {self.version_name} -> {path}")
        return response
    ...
    async def push(self, artefacts: Iterable[pathlib.Path]) -> Dict[str, List[Dict[str, Union[str, pathlib.Path]]]]:
