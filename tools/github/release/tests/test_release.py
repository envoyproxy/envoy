...
async def test_release_release_id(patches):
    ...
    with patched as (m_release, ):
        _release = AsyncMock()
        m_release.side_effect = _release
        assert await release.release_id == _release.return_value.__getitem__.return_value
...
async def test_release_upload_url(patches):
    ...
    with patched as (m_release, ):
        ...
        assert (
            ...
            == _release.return_value.__getitem__.return_value.split.return_value.__getitem__.return_value)

    assert (
        list(_release.return_value.__getitem__.call_args)
        == [('upload_url',), {}])
    assert (
        list(_release.return_value.__getitem__.return_value.split.call_args)
        == [('{',), {}])
    assert (
        list(_release.return_value.__getitem__.return_value.split.return_value.__getitem__.call_args)
        == [(0,), {}])
