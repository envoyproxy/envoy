...
class GuardDeprecationRunner(runner.Runner):
    ...
    async def deprecation_issues(self) -> list[str]:
        return [issue.title async for issue in self.repo.issues.search("label:deprecation")]
    ...
    async def features_to_flip(self) -> list[tuple[str, int, git.Commit]]:
        ...
        async for commit, runtime_guard in self.guard_lines:
            ...
            _features_to_flip.extend([to_flip] if (
                to_flip := self._should_flip(commit, runtime_guard)) else [])
    ...
    @async_property
    async def guard_lines(self) -> AsyncIterator[tuple[str, str]]:
    ...
    async def runtime_features_blame(self) -> list[list[git.Commit, list[str]]]:
        return await asyncio.to_thread(
            self.git_repo.blame, rev="HEAD", file=RUNTIME_FEATURES_PATH, **dict(BLAME_KWARGS))
    ...
    async def check_labels_exist(self) -> None:
        labels = [label async for label in self.repo.labels if label.name in self.labels]
        if len(labels) != len(self.labels):
            raise DeprecateGuardsError(f"Missing labels (expected {self.labels}, got {labels})")

    async def create_issue(self, title: str, body: str, login: str) -> None:
        repo = (self.staging_repo or self.repo)
            ...
            if login:
                if not self.dry_run:
                    await repo.issues.create(
                        title, body=body, assignees=[login], labels=self.labels)
                    self.log.notice(f"Created issue '{title}' for {login}")
                else:
    ...
    @runner.catches((gidgethub.BadRequest, DeprecateGuardsError))
    ...
    def _get_pr_from(self, commit: git.Commit) -> tuple[int, datetime.date]:
        ...
        return (int(pr_num.group(1)) if pr_num else None, date.fromtimestamp(commit.committed_date))

    async def _issue_info(self, runtime_guard: str, pr: int | None,
                          commit: git.Commit) -> tuple[str, str, str, str]:
        ...
        if pr and (pull := await self._get_pr(pr)):
            ...
            login = (await self._login_for(commit) or commit.author.email)
    ...
    def _should_flip(self, commit: git.Commit,
        ...
        self.log.debug(f"Flag {runtime_guard} added at {str(pr_date)} {log_message}")
        return result
