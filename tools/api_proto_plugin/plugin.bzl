...
def api_proto_plugin_impl(target, ctx, output_group, mnemonic, output_suffixes, output_dir = ""):
    ...
    proto_sources = [
        ...
        if ((f.path.startswith("external/envoy_api") and not f.path.startswith("external/envoy_api++"))
            or f.path.startswith("tools/testdata/protoxform/envoy")
            or f.path.startswith("external/xds/xds")
            or f.path.startswith("external/xds+"))
    ]
