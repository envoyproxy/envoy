...
def ProtoFileCanonicalFromLabel(label):
  ...
  """
  assert (label.startswith('@envoy_api_published//'))
  return label[len('@envoy_api_published//'):].replace(':', '/')
...
def BazelBinPathForOutputArtifact(label, suffix, root=''):
  ...
  Returns:
    Path in bazel-bin/external/envoy_api_published for label output with given suffix.
  """
  ...
  glob_pattern = os.path.join(
      root, 'bazel-bin/external/envoy_api_published/**/%s%s' %
