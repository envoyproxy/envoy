...
declare -r PROTO_TARGETS=$(bazel query "labels(srcs, labels(deps, @envoy_api_published//docs:protos))")
...
# the @envoy_api_published//docs:protos target in a few places as a result. This is not
...
# here.
bazel build ${BAZEL_BUILD_OPTIONS} --//tools/api_proto_plugin:default_type_db_target=@envoy_api_published//docs:protos \
  @envoy_api_published//docs:protos --aspects //tools/protoxform:protoxform.bzl%protoxform_aspect --output_groups=proto \
