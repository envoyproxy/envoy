...
if [[ $# -gt 1 ]]; then
  shift
  TEST_TARGETS=$*
  COVERAGE_TEST_TARGETS="$TEST_TARGETS"
else
  COVERAGE_TEST_TARGETS="//test/..."
  TEST_TARGETS="${COVERAGE_TEST_TARGETS} $(bazel query ${BAZEL_QUERY_OPTIONS} "attr('tags', 'test_included', '@com_googlesource_quiche//:all')" | grep "^@com_googlesource_quiche")"
fi
