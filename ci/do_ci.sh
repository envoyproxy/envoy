...
if [[ "$CI_TARGET" == "bazel.release" ]]; then
  ...
  # TODO(dio): To resolve the conflicted zlib=ng vs. zlib=ng_with_optimizations. However, it should
  # be safe to only build zlib=ng_with_optimizations for now, since these two compile options use
  # the same source and most likely to build with "ng_with_optimizations" is what people want.
  COMPILE_TIME_OPTIONS="\
    ...
    --define zlib=ng_with_optimizations \
  "
