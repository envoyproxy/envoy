...
case $CI_TARGET in
    ...
    deps)
        ...
        export TODAY_DATE
        bazel run "${BAZEL_BUILD_OPTIONS[@]}" @envoy//tools/dependency:check \
              --@envoy//tools/dependency:preload_cve_data \
        ...
        bazel run "${BAZEL_BUILD_OPTIONS[@]}" \
              @envoy//tools/dependency:dependatool
