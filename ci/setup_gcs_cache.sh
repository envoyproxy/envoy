...
if [[ ! -z "${BAZEL_REMOTE_CACHE}" ]]; then
  if [[ ! -z "${GCP_SERVICE_ACCOUNT_KEY}" ]]; then
    ...
    echo "Set up bazel read/write cache at ${BAZEL_REMOTE_CACHE}."
  else
    export BAZEL_BUILD_EXTRA_OPTIONS="${BAZEL_BUILD_EXTRA_OPTIONS} \
      --remote_cache=${BAZEL_REMOTE_CACHE} --noremote_upload_local_results"
    echo "Set up bazel read only cache at ${BAZEL_REMOTE_CACHE}."
  fi
else
