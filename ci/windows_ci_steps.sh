...
BAZEL_TEST_EXTRA_OPTIONS="--test_output=errors --test_summary=terse"

BAZEL_STARTUP_OPTIONS="--output_base=c:/_eb"
BAZEL_BUILD_OPTIONS="-c opt --config=msvc-cl --show_task_finish --verbose_failures \
  ${BAZEL_BUILD_EXTRA_OPTIONS} ${BAZEL_TEST_EXTRA_OPTIONS}"


bazel ${BAZEL_STARTUP_OPTIONS} build ${BAZEL_BUILD_OPTIONS} //source/exe:envoy-static --build_tag_filters=-skip_on_windows

# Test compilation of known MSVC-compatible test sources before test invocation
bazel ${BAZEL_STARTUP_OPTIONS} build --jobs=2 ${BAZEL_BUILD_OPTIONS} //test/... --test_tag_filters=-skip_on_windows --build_tests_only
