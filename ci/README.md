...
* `registry` &mdash; update the Envoy bazel-registry hash in `.bazelrc`, `api/.bazelrc`, and `bazel/tests/external/.bazelrc`, then regenerate all `MODULE.bazel.lock` files. Set `ENVOY_REGISTRY_HASH` to use a specific hash, `ENVOY_REGISTRY_REPO` to override the repository used for `git ls-remote`, or `ENVOY_REGISTRY_BRANCH` to override the branch used for the bump and checked by `deps`/`registry.check`. Set `ENVOY_REGISTRY_CHECK_SKIP` to a non-empty value to skip `registry.check` validation during `registry` bumps only (for testing with an arbitrary hash).
