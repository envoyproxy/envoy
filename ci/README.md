...
* `check_proto_format`&mdash; run `clang-format` and `buildifier` on entire source tree.
* `fix_proto_format`&mdash; run and enforce `clang-format` and `buildifier` on entire source tree.
* `format`&mdash; run validation and linting tools.
