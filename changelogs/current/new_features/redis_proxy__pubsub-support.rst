...
``pubsub_settings`` unset simply applies those defaults — presence or absence of the message does
not itself toggle the sharded ``SUBSCRIBE`` rewrite, which stays enabled on a RESP3 listener
unless ``sharded_subscription_mode`` is explicitly set to ``DISABLED``. For a
RESP3-speaking upstream that does
