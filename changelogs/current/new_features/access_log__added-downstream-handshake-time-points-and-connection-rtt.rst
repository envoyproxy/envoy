Added the ``DX_HS_BEG`` (downstream TLS handshake begin, i.e. when the ClientHello was received) and
``DX_HS_END`` (downstream TLS handshake end) time points to the :ref:`%COMMON_DURATION%
