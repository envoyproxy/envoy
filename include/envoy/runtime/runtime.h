...
public:
  ...
  /**
   * Fetch an integer runtime key using the numerator of a FractionalPercent. Values larger
   * than ~2^53 may not be accurately converted into integers and will return default_value.
   * @param key supplies the key to fetch.
   * @param default_value supplies the default value that will be used if either the feature key
   *        does not exist or it is not a fractional percent.
   * @return uint64_t the runtime value or the default value.
   */
  virtual uint64_t getIntegerNumeratorOfFractionalPercent(
      absl::string_view key, const envoy::type::v3::FractionalPercent& default_value) const PURE;
