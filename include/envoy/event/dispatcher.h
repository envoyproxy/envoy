...
public:
  ...
  public:
     ...
     * watchdog at least once per touch interval. Dispatcher implementations may choose to touch
     * more often to avoid spurious miss events when processing long callback queues.
     ...
     * dispatcher.
     * @param resolvers supplies the addresses of DNS resolvers that this resolver should use. If
     * left empty, it will not use any specific resolvers, but use defaults (/etc/resolv.conf)
    ...
    virtual Network::ListenerPtr createListener(Network::SocketSharedPtr&& socket,
                                                ...
                                                bool bind_to_port, uint32_t backlog_size) PURE;
    ...
    /**
     * Runs the event loop. This will not return until exit() is called either from within a
     * callback or from a different thread.
