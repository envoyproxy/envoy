...
public:
  ...
  */
   ...
   * TODO: Remove dynamic metadata and change dependent code to use dynamic stream info.
   */
  virtual void setDynamicMetadata(const std::string& name, const ProtobufWkt::Struct& value) PURE;

  /**
   * StreamInfo contains a fixed set of attributes generated by Envoy core,
   * pertaining to a TCP connection or HTTP stream. Filters can add
   * additional information to a StreamInfo object through
   * dynamicAttributes. Dynamic attributes are typed objects with one of
   * the following semantics:
   *
   * Singleton data objects where only one filter can produce a named data object.
   *
   * List data objects that can be updated by multiple filters (append only).
   * 
   * Both object types can be consumed by multiple filters.
   *  
   * @return the per-request state associated with this request.
   */
  virtual FilterState& dynamicAttributes() PURE;
  virtual const FilterState& dynamicAttributes() const PURE;

  /**
   * Deprecated. Use dynamicAttributes instead
   */
