...
TEST(SubstitutionFormatterTest, plainStringFormatter) {
  PlainStringFormatter<HttpFormatterContext> formatter("plain");
  StreamInfo::MockStreamInfo stream_info;

  EXPECT_EQ("plain", formatter.format(HttpFormatterContext(), stream_info));
  EXPECT_THAT(formatter.formatValue(HttpFormatterContext(), stream_info),
              ProtoEq(ValueUtil::stringValue("plain")));
}

TEST(SubstitutionFormatterTest, plainNumberFormatter) {
  PlainNumberFormatter<HttpFormatterContext> formatter(400);
  StreamInfo::MockStreamInfo stream_info;

  EXPECT_EQ("400", formatter.format(HttpFormatterContext(), stream_info));
  EXPECT_THAT(formatter.formatValue(HttpFormatterContext(), stream_info),
...
TEST(SubstitutionFormatterTest, inFlightDuration) {
...

  {
    ...
    StreamInfoFormatter<HttpFormatterContext> duration_format("DURATION");
    EXPECT_EQ("100", duration_format.format(HttpFormatterContext(), stream_info));
  }

  {
    time_system.setMonotonicTime(MonotonicTime(std::chrono::milliseconds(200)));
    StreamInfoFormatter<HttpFormatterContext> duration_format("DURATION");
    EXPECT_EQ("200", duration_format.format(HttpFormatterContext(), stream_info));

    time_system.setMonotonicTime(MonotonicTime(std::chrono::milliseconds(300)));
    EXPECT_THAT(duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(300.0)));
  }
}

TEST(SubstitutionFormatterTest, StreamInfoFormatter) {
  EXPECT_THROW(StreamInfoFormatter<HttpFormatterContext> formatter("unknown_field"),
               EnvoyException);
  ...
  {
    StreamInfoFormatter<HttpFormatterContext> request_duration_format("REQUEST_DURATION");
    EXPECT_EQ(absl::nullopt, request_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(request_duration_format.formatValue(HttpFormatterContext(), stream_info),
...

  {
    ...
    EXPECT_EQ("5", request_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(request_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(5.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> request_tx_duration_format("REQUEST_TX_DURATION");
    EXPECT_EQ(absl::nullopt,
              request_tx_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(request_tx_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    ...
    EXPECT_EQ("15", request_tx_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(request_tx_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(15.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> response_duration_format("RESPONSE_DURATION");
    EXPECT_EQ(absl::nullopt, response_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    ...
    EXPECT_EQ("10", response_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(10.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> ttlb_duration_format("RESPONSE_TX_DURATION");

    EXPECT_EQ(absl::nullopt, ttlb_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(ttlb_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    ...
    EXPECT_EQ("15", ttlb_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(ttlb_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(15.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> handshake_duration_format(
        "DOWNSTREAM_HANDSHAKE_DURATION");

    EXPECT_EQ(absl::nullopt, handshake_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(handshake_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    ...
    EXPECT_EQ("25", handshake_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(handshake_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(25.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> roundtrip_duration_format("ROUNDTRIP_DURATION");

    EXPECT_EQ(absl::nullopt, roundtrip_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(roundtrip_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    ...
    EXPECT_EQ("25", roundtrip_duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(roundtrip_duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(25.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> bytes_retransmitted_format("BYTES_RETRANSMITTED");
    EXPECT_CALL(stream_info, bytesRetransmitted()).WillRepeatedly(Return(1));
    EXPECT_EQ("1", bytes_retransmitted_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(bytes_retransmitted_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(1.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> packets_retransmitted_format("PACKETS_RETRANSMITTED");
    EXPECT_CALL(stream_info, packetsRetransmitted()).WillRepeatedly(Return(1));
    EXPECT_EQ("1", packets_retransmitted_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(packets_retransmitted_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(1.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> bytes_received_format("BYTES_RECEIVED");
    EXPECT_CALL(stream_info, bytesReceived()).WillRepeatedly(Return(1));
    EXPECT_EQ("1", bytes_received_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(bytes_received_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(1.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> attempt_count_format(
        "UPSTREAM_REQUEST_ATTEMPT_COUNT");
    absl::optional<uint32_t> attempt_count{3};
    EXPECT_CALL(stream_info, attemptCount()).WillRepeatedly(Return(attempt_count));
    EXPECT_EQ("3", attempt_count_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(attempt_count_format.formatValue(HttpFormatterContext(), stream_info),
...

  {
    ...
    EXPECT_CALL(stream_info, attemptCount()).WillRepeatedly(Return(attempt_count));
    EXPECT_EQ("0", attempt_count_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(attempt_count_format.formatValue(HttpFormatterContext(), stream_info),
...

  {
    ...
    StreamInfoFormatter<HttpFormatterContext> wire_bytes_received_format(
        "UPSTREAM_WIRE_BYTES_RECEIVED");
    EXPECT_CALL(stream_info, getUpstreamBytesMeter())
        .WillRepeatedly(ReturnRef(upstream_bytes_meter));
    EXPECT_EQ("1", wire_bytes_received_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(wire_bytes_received_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(1.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> protocol_format("PROTOCOL");
    absl::optional<Http::Protocol> protocol = Http::Protocol::Http11;
    EXPECT_CALL(stream_info, protocol()).WillRepeatedly(Return(protocol));
    EXPECT_EQ("HTTP/1.1", protocol_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(protocol_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("HTTP/1.1")));
  }
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> protocol_format("UPSTREAM_PROTOCOL");
    EXPECT_CALL(stream_info, upstreamInfo()).WillRepeatedly(Return(nullptr));

    EXPECT_EQ(absl::nullopt, protocol_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(protocol_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }
  {
  ...
  }
  {
    ...
    EXPECT_EQ("HTTP/2", protocol_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(protocol_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("HTTP/2")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> response_format("RESPONSE_CODE");
    absl::optional<uint32_t> response_code{200};
    EXPECT_CALL(stream_info, responseCode()).WillRepeatedly(Return(response_code));
    EXPECT_EQ("200", response_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(200.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> response_code_format("RESPONSE_CODE");
    absl::optional<uint32_t> response_code;
    EXPECT_CALL(stream_info, responseCode()).WillRepeatedly(Return(response_code));
    EXPECT_EQ("0", response_code_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_code_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(0.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> response_format("RESPONSE_CODE_DETAILS");
    absl::optional<std::string> rc_details;
    EXPECT_CALL(stream_info, responseCodeDetails()).WillRepeatedly(ReturnRef(rc_details));
    EXPECT_EQ(absl::nullopt, response_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> response_code_format("RESPONSE_CODE_DETAILS");
    absl::optional<std::string> rc_details{"via_upstream"};
    EXPECT_CALL(stream_info, responseCodeDetails()).WillRepeatedly(ReturnRef(rc_details));
    EXPECT_EQ("via_upstream", response_code_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_code_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("via_upstream")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> termination_details_format(
    ...
    EXPECT_EQ(absl::nullopt,
              termination_details_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(termination_details_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }
  ...
  {
    StreamInfoFormatter<HttpFormatterContext> bytes_sent_format("BYTES_SENT");
    EXPECT_CALL(stream_info, bytesSent()).WillRepeatedly(Return(1));
    EXPECT_EQ("1", bytes_sent_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(bytes_sent_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(1.0)));
  }

  {
    ...
    StreamInfoFormatter<HttpFormatterContext> wire_bytes_sent_format("UPSTREAM_WIRE_BYTES_SENT");
    EXPECT_CALL(stream_info, getUpstreamBytesMeter())
        .WillRepeatedly(ReturnRef(upstream_bytes_meter));
    EXPECT_EQ("1", wire_bytes_sent_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(wire_bytes_sent_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(1.0)));
  }

  {
    ...
    EXPECT_EQ("15", duration_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(duration_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(15.0)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> response_flags_format("RESPONSE_FLAGS");
    ON_CALL(stream_info, hasResponseFlag(StreamInfo::ResponseFlag::LocalReset))
        .WillByDefault(Return(true));
    EXPECT_EQ("LR", response_flags_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_flags_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("LR")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> response_flags_format("RESPONSE_FLAGS_LONG");
    ON_CALL(stream_info, hasResponseFlag(StreamInfo::ResponseFlag::LocalReset))
        .WillByDefault(Return(true));
    EXPECT_EQ("LocalReset", response_flags_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(response_flags_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("LocalReset")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_LOCAL_ADDRESS");
    ...
    EXPECT_EQ("127.1.2.3:18443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
    ...
    stream_info.upstreamInfo()->setUpstreamLocalAddress(address);
    EXPECT_EQ("[::1]:19443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
    ...
    stream_info.upstreamInfo()->setUpstreamLocalAddress(address);
    EXPECT_EQ("/foo", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("/foo")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format(
        "UPSTREAM_LOCAL_ADDRESS_WITHOUT_PORT");
    auto address = Network::Address::InstanceConstSharedPtr{
        new Network::Address::Ipv4Instance("127.0.0.3", 18443)};
    stream_info.upstreamInfo()->setUpstreamLocalAddress(address);
    EXPECT_EQ("127.0.0.3", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("127.0.0.3")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_LOCAL_PORT");

    // Validate for IPv4 address
    auto address = Network::Address::InstanceConstSharedPtr{
        new Network::Address::Ipv4Instance("127.1.2.3", 18443)};
    stream_info.upstreamInfo()->setUpstreamLocalAddress(address);
    EXPECT_EQ("18443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
    ...
    address =
        Network::Address::InstanceConstSharedPtr{new Network::Address::Ipv6Instance("::1", 19443)};
    stream_info.upstreamInfo()->setUpstreamLocalAddress(address);
    EXPECT_EQ("19443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
    ...
    address = Network::Address::InstanceConstSharedPtr{new Network::Address::PipeInstance("/foo")};
    stream_info.upstreamInfo()->setUpstreamLocalAddress(address);
    EXPECT_EQ("", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_HOST");
    EXPECT_EQ("10.0.0.1:443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("10.0.0.1:443")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_REMOTE_ADDRESS");
    EXPECT_EQ("10.0.0.1:443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("10.0.0.1:443")));
  }
  {
    ...
    EXPECT_EQ("10.0.0.1", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("10.0.0.1")));
  }
  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_REMOTE_PORT");
    EXPECT_EQ("443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_CLUSTER");
    ...
    EXPECT_EQ("observability_name", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
...

  {
    ...
    EXPECT_CALL(stream_info, upstreamClusterInfo()).WillRepeatedly(Return(cluster_info));
    EXPECT_EQ(absl::nullopt, upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }
...

  {
    ...
    StreamInfoFormatter<HttpFormatterContext> upstream_format("HOSTNAME");
    EXPECT_EQ(absl::nullopt, upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    ...
    StreamInfoFormatter<HttpFormatterContext> upstream_format("HOSTNAME");
    EXPECT_EQ("myhostname", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("myhostname")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_LOCAL_ADDRESS");
    EXPECT_EQ("127.0.0.2:0", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
...

  {
    ...
    EXPECT_EQ("127.0.0.2", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("127.0.0.2")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_LOCAL_PORT");

    // Validate for IPv4 address
    auto address = Network::Address::InstanceConstSharedPtr{
        new Network::Address::Ipv4Instance("127.1.2.3", 8443)};
    stream_info.downstream_connection_info_provider_->setLocalAddress(address);
    EXPECT_EQ("8443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
    ...
    stream_info.downstream_connection_info_provider_->setLocalAddress(address);
    EXPECT_EQ("9443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
...

  {
    ...
    EXPECT_EQ("127.0.0.1", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("127.0.0.1")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_REMOTE_ADDRESS");
    EXPECT_EQ("127.0.0.1:0", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("127.0.0.1:0")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_REMOTE_PORT");
    EXPECT_EQ("0", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_DIRECT_REMOTE_ADDRESS");
    EXPECT_EQ("127.0.0.3:63443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("127.0.0.3:63443")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_DIRECT_REMOTE_PORT");
    EXPECT_EQ("63443", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("CONNECTION_ID");
    uint64_t id = 123;
    stream_info.downstream_connection_info_provider_->setConnectionID(id);
    EXPECT_EQ("123", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::numberValue(id)));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("STREAM_ID");
    ...
    EXPECT_EQ("ffffffff-0012-0110-00ff-0c00400600ff",
              upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue("ffffffff-0012-0110-00ff-0c00400600ff")));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("REQUESTED_SERVER_NAME");
    ...
    EXPECT_EQ("stub_server", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    StreamInfoFormatter<HttpFormatterContext> listener_format(
    ...
    EXPECT_EQ("TLS_error", listener_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(listener_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  }
  {
    ...
    stream_info.setDownstreamTransportFailureReason(downstream_transport_failure_reason);
    EXPECT_EQ(absl::nullopt, listener_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(listener_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  }

  {
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_TRANSPORT_FAILURE_REASON");
    std::string upstream_transport_failure_reason = "SSL error";
    stream_info.upstreamInfo()->setUpstreamTransportFailureReason(
        upstream_transport_failure_reason);
    EXPECT_EQ("SSL_error", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
...
TEST(SubstitutionFormatterTest, StreamInfoFormatterWithSsl) {
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("VIRTUAL_CLUSTER_NAME");
    std::string virtual_cluster_name = "authN";
    stream_info.setVirtualClusterName(virtual_cluster_name);
    EXPECT_EQ("authN", upstream_format.format(HttpFormatterContext(), stream_info));
  }
...

  {
    ...
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_URI_SAN");
    ...
    EXPECT_EQ("san", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
...

  {
    ...
    EXPECT_CALL(*connection_info, uriSanPeerCertificate()).WillRepeatedly(Return(sans));
    stream_info.downstream_connection_info_provider_->setSslConnection(connection_info);
    EXPECT_EQ("san1,san2", upstream_format.format(HttpFormatterContext(), stream_info));
  }
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_URI_SAN");
  ...
  }
  {
    // Use a local stream info for these tests as as setSslConnection can only be called once.
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_DNS_SAN");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    const std::vector<std::string> sans{"san"};
  ...
  }
  {
    // Use a local stream info for these tests as as setSslConnection can only be called once.
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_IP_SAN");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    const std::vector<std::string> sans{"san"};
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_LOCAL_URI_SAN");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    const std::vector<std::string> sans{"san"};
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_LOCAL_DNS_SAN");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    const std::vector<std::string> sans{"san"};
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_LOCAL_IP_SAN");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    const std::vector<std::string> sans{"san"};
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_LOCAL_SUBJECT");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    ...
    stream_info.downstream_connection_info_provider_->setSslConnection(connection_info);
    EXPECT_EQ("subject", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_SUBJECT");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_TLS_SESSION_ID");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    const std::string session_id = "deadbeef";
    EXPECT_CALL(*connection_info, sessionId()).WillRepeatedly(ReturnRef(session_id));
    stream_info.downstream_connection_info_provider_->setSslConnection(connection_info);
    EXPECT_EQ("deadbeef", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_TLS_CIPHER");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_TLS_VERSION");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    std::string tlsVersion = "TLSv1.2";
    EXPECT_CALL(*connection_info, tlsVersion()).WillRepeatedly(ReturnRef(tlsVersion));
    stream_info.downstream_connection_info_provider_->setSslConnection(connection_info);
    EXPECT_EQ("TLSv1.2", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_FINGERPRINT_256");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    ...
    stream_info.downstream_connection_info_provider_->setSslConnection(connection_info);
    EXPECT_EQ(expected_sha, upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_FINGERPRINT_1");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    std::string expected_sha = "685a2db593d5f86d346cb1a297009c3b467ad77f1944aa799039a2fb3d531f3f";
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_SERIAL");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    ...
    stream_info.downstream_connection_info_provider_->setSslConnection(connection_info);
    EXPECT_EQ("b8b5ecc898f2124a", upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_ISSUER");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("DOWNSTREAM_PEER_CERT");
    auto connection_info = std::make_shared<Ssl::MockConnectionInfo>();
    ...
    stream_info.downstream_connection_info_provider_->setSslConnection(connection_info);
    EXPECT_EQ(expected_cert, upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_TLS_SESSION_ID");
    EXPECT_CALL(stream_info, upstreamInfo()).WillRepeatedly(Return(nullptr));
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_TLS_CIPHER");
    EXPECT_CALL(stream_info, upstreamInfo()).WillRepeatedly(Return(nullptr));
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_TLS_VERSION");
    EXPECT_CALL(stream_info, upstreamInfo()).WillRepeatedly(Return(nullptr));
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_PEER_ISSUER");
    EXPECT_CALL(stream_info, upstreamInfo()).WillRepeatedly(Return(nullptr));
  ...
  }
  {
    ...
    EXPECT_CALL(*connection_info, issuerPeerCertificate()).WillRepeatedly(ReturnRef(issuer_peer));
    stream_info.upstreamInfo()->setUpstreamSslConnection(connection_info);
    EXPECT_EQ(issuer_peer, upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::stringValue(issuer_peer)));
  }
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_PEER_CERT");
    EXPECT_CALL(stream_info, upstreamInfo()).WillRepeatedly(Return(nullptr));
    EXPECT_EQ(absl::nullopt, upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
  ...
  {
    NiceMock<StreamInfo::MockStreamInfo> stream_info;
    StreamInfoFormatter<HttpFormatterContext> upstream_format("UPSTREAM_PEER_SUBJECT");
    EXPECT_CALL(stream_info, upstreamInfo()).WillRepeatedly(Return(nullptr));
    EXPECT_EQ(absl::nullopt, upstream_format.format(HttpFormatterContext(), stream_info));
    EXPECT_THAT(upstream_format.formatValue(HttpFormatterContext(), stream_info),
                ProtoEq(ValueUtil::nullValue()));
...
TEST(SubstitutionFormatterTest, StructFormatterPlainStringTest) {
  ...
  verifyStructOutput(
      formatter.format(HttpFormatterContext(request_header, response_header, response_trailer, body,
                                            AccessLog::AccessLogType::NotSet),
                       stream_info),
      expected_json_map);
}
...
TEST(SubstitutionFormatterTest, StructFormatterClusterMetadataNoClusterInfoTest) {
  ...
  // Empty optional (absl::nullopt)
  {
    ...
    verifyStructOutput(
        formatter.format(HttpFormatterContext(request_header, response_header, response_trailer,
                                              body, AccessLog::AccessLogType::NotSet),
                         stream_info),
        expected_json_map);
  }
...
TEST(SubstitutionFormatterTest, StructFormatterFilterStateTest) {
  ...
  verifyStructOutput(
      formatter.format(HttpFormatterContext(request_headers, response_headers, response_trailers,
                                            body, AccessLog::AccessLogType::NotSet),
                       stream_info),
      expected_json_map);
}
...
TEST(SubstitutionFormatterTest, CompositeFormatterSuccess) {
...

  {
    ...
    EXPECT_EQ(format, formatter.format(HttpFormatterContext(request_header, response_header,
                                                            response_trailer, body,
                                                            AccessLog::AccessLogType::NotSet),
                                       stream_info));
  }
...
#if !defined(WIN32) && !defined(__APPLE__)
  {
    ...
    EXPECT_EQ("%E4n", formatter.format(HttpFormatterContext(request_header, response_header,
                                                            response_trailer, body,
                                                            AccessLog::AccessLogType::NotSet),
                                       stream_info));
  }
...
TEST(SubstitutionFormatterTest, CompositeFormatterEmpty) {
...

  {
    ...
    EXPECT_EQ("-|-|-|-|-|-", formatter.format(HttpFormatterContext(
                                                  request_header, response_header, response_trailer,
                                                  body, AccessLog::AccessLogType::NotSet),
                                              stream_info));
  }

  {
    ...
    EXPECT_EQ("||||", formatter.format(HttpFormatterContext(request_header, response_header,
                                                            response_trailer, body,
                                                            AccessLog::AccessLogType::NotSet),
                                       stream_info));
  }
...
TEST(SubstitutionFormatterTest, EmptyFormatParse) {
  ...
  EXPECT_EQ("", providers[0]->format(HttpFormatterContext(), stream_info));
}
...
TEST(SubstitutionFormatterTest, EscapingFormatParse) {
  ...
  EXPECT_EQ("%", providers[0]->format(HttpFormatterContext(request_headers, response_headers,
...
TEST(SubstitutionFormatterTest, FormatterExtension) {
  ...
  EXPECT_EQ("TestFormatter",
            providers[1]->format(HttpFormatterContext(request_headers, response_headers,
                                                      response_trailers, body,
                                                      AccessLog::AccessLogType::NotSet),
                                 stream_info));
}
...
TEST(SubstitutionFormatterTest, PercentEscapingEdgeCase) {
  ...
  EXPECT_EQ("myhostname",
            providers[0]->format(HttpFormatterContext(request_headers, response_headers,
                                                      response_trailers, body,
                                                      AccessLog::AccessLogType::NotSet),
                                 stream_info));
  EXPECT_EQ("HTTP/1.1", providers[1]->format(HttpFormatterContext(request_headers, response_headers,
                                                                  response_trailers, body,
                                                                  AccessLog::AccessLogType::NotSet),
                                             stream_info));
}
...
TEST(SubstitutionFormatterTest, EnvironmentFormatterTest) {
...

  {
    ...
    EXPECT_EQ("test", providers[0]->format(HttpFormatterContext(request_headers, response_headers,
                                                                response_trailers, body,
                                                                AccessLog::AccessLogType::NotSet),
                                           stream_info));
  }

  {
    ...
    EXPECT_EQ("te", providers[0]->format(HttpFormatterContext(request_headers, response_headers,
                                                              response_trailers, body,
                                                              AccessLog::AccessLogType::NotSet),
                                         stream_info));
  }
