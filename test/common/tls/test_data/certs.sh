    ...
    # Use a single redirect for the entire block
    {
        ...
        for (( i=0; i<${#trust_domains[@]}; i+=3 )); do
            ...
            for ca_cert_file in "${ca_cert_files[@]}"; do
                ...
                modulus=$(openssl x509 -in "$ca_cert_file" -noout -modulus | cut -d'=' -f2 | tr -d '\n' | perl -ne 'print pack("H*", $_)' | base64 | tr -d "=\n" | tr '/+' '_-')
...
# $1=<number of days in the past>
# Prints a [CC]YYMMDDHHMMSSZ timestamp for openssl's -not_before/-not_after.
days_ago() {
    if date -u -d "-1 days" > /dev/null 2>&1; then
        date -u -d "-$1 days" +%Y%m%d%H%M%SZ  # GNU coreutils
    else
        date -u -v"-$1"d +%Y%m%d%H%M%SZ       # BSD/macOS
    fi
}

# $1=<certificate name> $2=<CA name> $3=[days]
generate_x509_cert() {
    local days extra_args=() validity_args=()
    days="${3:-${DEFAULT_VALIDITY_DAYS}}"
    if [[ -f "${1}_password.txt" ]]; then
        extra_args=(-passin "file:${1}_password.txt")
    fi
    if [[ "$days" -lt 0 ]]; then
        # Generate an already-expired certificate. openssl rejects a non-positive
        # "-days", so place the whole validity window in the past explicitly.
        validity_args=(-not_before "$(days_ago 2)" -not_after "$(days_ago 1)")
    else
        validity_args=(-days "$days")
    fi
    openssl req -new -key "${1}_key.pem" -out "${1}_cert.csr" -config "${1}_cert.cfg" -batch -sha256 "${extra_args[@]}"
    openssl x509 -req "${validity_args[@]}" -in "${1}_cert.csr" -sha256 -CA "${2}_cert.pem" -CAkey \
