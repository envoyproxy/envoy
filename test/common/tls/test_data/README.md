...
To build the fixtures and look at them:
...
The set of fixtures is described by [certs.spec](certs.spec). To add a new one,
add a `[cert <name>]` stanza naming the private key, the `.cfg` file that
supplies the subject and the v3 extensions, and the issuer (`self` or the name
of another fixture), then add the file names it produces to the `CERTS` list in
[BUILD](BUILD). Chains, CRLs, PKCS#12 bundles and SPIFFE trust bundles are
described by `[concat]`, `[crl]`, `[p12]` and `[trust_bundle]` stanzas.

Certificates are stamped with a validity window that starts on Jan 1 of the
year the build runs in (`STABLE_CERT_EPOCH_YEAR`, see
`bazel/get_workspace_status`), so they never age out of validity. Fixtures that
must be expired use `validity = expired`, which pins them to 2020. Serial
numbers are derived from the fixture name, so they are stable across builds.

`no_extension_cert.pem` and the `bad_rsa_key_usage_*` fixtures are hand-crafted
and are not generated; they are checked in as-is.
