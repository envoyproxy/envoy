...
TEST_F(HandshakerFactoryTest, SetMockFunctionCb) {
  ...
  EXPECT_CALL(mock_factory_ctx.server_factory_context_.api_, processContext())
...
TEST_F(HandshakerFactoryTest, HandshakerContextProvidesObjectsFromParentContext) {
  ...
  EXPECT_CALL(mock_factory_cb, Call)
      .WillOnce(WithArg<1>([&](Ssl::HandshakerFactoryContext& context) {
        ...
        EXPECT_THAT(context.api(), Ref(mock_factory_ctx.server_factory_context_.api_));
        EXPECT_THAT(context.options(), Ref(mock_factory_ctx.server_factory_context_.options_));
        EXPECT_THAT(context.lifecycleNotifier(),
                    Ref(mock_factory_ctx.server_factory_context_.lifecycle_notifier_));
      }));
