...
public:
  TestPendingStream(ConnPoolImplBase& parent, AttachContext& context, bool has_early_data)
      : PendingStream(parent, has_early_data), context_(context) {}
...
public:
  ...
  ConnectionPool::Cancellable* newPendingStream(AttachContext& context,
    ...
    auto entry = std::make_unique<TestPendingStream>(*this, context, has_early_data);
...
public:
  ...
  void newConnectingClient() {
    ...
    pool_.newStreamImpl(context_, /*has_early_data=*/false);
...
TEST_F(ConnPoolImplBaseTest, BasicPreconnect) {
  ...
  auto cancelable = pool_.newStreamImpl(context_, /*has_early_data=*/false);
