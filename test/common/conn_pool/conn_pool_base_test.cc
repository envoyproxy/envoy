...
TEST_F(ConnPoolImplDispatcherBaseTest, MaxConnectionDurationBusy) {
  ...
  EXPECT_EQ(0, pool_.host()->cluster().upstreamStats()->upstream_cx_max_duration_reached_.value());
  ...
  advanceTimeAndRun(2);
  EXPECT_EQ(1, pool_.host()->cluster().upstreamStats()->upstream_cx_max_duration_reached_.value());
...
TEST_F(ConnPoolImplDispatcherBaseTest, ConnectedZeroRttSendsEarlyData) {
  ...
  EXPECT_EQ(1, pool_.host()->cluster().upstreamStats()->upstream_rq_0rtt_.value());
...
TEST_F(ConnPoolImplDispatcherBaseTest, EarlyDataStreamsReachConcurrentStreamLimit) {
  ...
  EXPECT_EQ(2, pool_.host()->cluster().upstreamStats()->upstream_rq_0rtt_.value());
  EXPECT_EQ(ActiveClient::State::Busy, clients_.back()->state());
