...
constexpr absl::string_view kAlternativeParent = "projects/cloudesf-testing";
...
std::vector<CreateApiKeyRequest> CreateRequestMessages(int num_requests,
                                                       int message_size,
                                                       bool is_modified) {
  ...
  for (int i = 0; i < num_requests; i++) {
    ...
    if (is_modified) {
      ...
      requests.back().mutable_key()->set_location(
          std::string(message_size, 'z'));
    } else {
      requests.back().set_parent(CreateOriginalRequestParent(i));
      requests.back().mutable_key()->set_current_key(
          std::string(message_size, 'A'));
    }
  }
  return requests;
}
...
// per request message.
std::vector<Envoy::Buffer::InstancePtr> CreateOneFramePerBuffer(
  ...
  for (auto& request_message : request_messages) {
    buffers.emplace_back(
        Envoy::Grpc::Common::serializeToGrpcFrame(request_message));
  }
  return buffers;
}
...
std::vector<Envoy::Buffer::InstancePtr> ConsolidateBuffers(
    const std::vector<Envoy::Buffer::InstancePtr> request_buffers) {
  Envoy::Buffer::InstancePtr consolidated_buffer =
      std::make_unique<Envoy::Buffer::OwnedImpl>();
...
std::vector<Envoy::Buffer::InstancePtr> ConsolidateBuffersIntoSplitFrames(
    const std::vector<Envoy::Buffer::InstancePtr> request_buffers,
    int num_splits) {
  std::vector<Envoy::Buffer::InstancePtr> buffers;
...
ConsolidateBuffersSlicePerRequestExtraAlloc(
    const std::vector<Envoy::Buffer::InstancePtr> request_buffers) {
  Envoy::Buffer::InstancePtr consolidated_buffer =
      std::make_unique<Envoy::Buffer::OwnedImpl>();
  ...
  for (auto& single_slice_request_data : request_buffers) {
    ...
    memcpy(dst, single_slice_request_data->frontSlice().mem_,
           single_slice_request_data->length());
...
void MutateMessage(StreamMessage& message_data, int index, int message_size) {
  ...
  parsed_request.mutable_key()->set_location(
      std::string(message_size / 2, 'X'));
...
void BatchMutateStreamMessages(
    const std::vector<std::unique_ptr<StreamMessage>>& message_datas,
    int message_size) {
  for (unsigned long i = 0; i < message_datas.size(); i++) {
    ASSERT_NE(message_datas[i], nullptr);
...
TEST_F(MessageConverterReadOnlyBehavior, SingleMessageSingleFrame) {
 ...
 EXPECT_TRUE(MessageDifferencer::Equals(ParseFromStreamMessage(*message_data),request));
  EXPECT_TRUE(message_data->is_first_message());
  EXPECT_TRUE(message_data->is_final_message());

  // Function under test.
  ASSERT_OK_AND_ASSIGN(auto final_data,
...
TEST_F(MessageConverterReadOnlyBehavior, SingleMessageSplitFrame) {
  ...
  MessageConverter converter(Factory());
  ASSERT_OK_AND_ASSIGN(auto message_data,
                       converter.AccumulateMessage(start_request_data, false));
  EXPECT_EQ(message_data, nullptr);
  EXPECT_EQ(start_request_data.length(), 0);

  ASSERT_OK_AND_ASSIGN(message_data,
                       converter.AccumulateMessage(middle_request_data, false));
  EXPECT_EQ(message_data, nullptr);
  EXPECT_EQ(middle_request_data.length(), 0);

  // Last call has the remaining message.
  ASSERT_OK_AND_ASSIGN(message_data,
                       converter.AccumulateMessage(end_request_data, true));
  ASSERT_NE(message_data, nullptr);
...
TEST_F(MessageConverterReadOnlyBehavior, MultipleMessagesSingleFrame) {
  ...
  Envoy::Buffer::InstancePtr request_data1 =
      Envoy::Grpc::Common::serializeToGrpcFrame(request1);
  Envoy::Buffer::InstancePtr request_data2 =
      Envoy::Grpc::Common::serializeToGrpcFrame(request2);
  ...
  ASSERT_OK_AND_ASSIGN(message_data,
                       converter.AccumulateMessage(request_in, true));
  ASSERT_NE(message_data, nullptr);
  EXPECT_EQ(request_in.length(), 0);
  EXPECT_TRUE(MessageDifferencer::Equals(ParseFromStreamMessage(*message_data),request2));
  EXPECT_FALSE(message_data->is_first_message());
  EXPECT_TRUE(message_data->is_final_message());
  ASSERT_OK_AND_ASSIGN(converted_buffer,
                       converter.ConvertBackToBuffer(std::move(message_data)));
  ASSERT_NE(converted_buffer, nullptr);
  final_data.move(*converted_buffer);
...
// MessageConverter.
TEST_F(MessageConverterReadOnlyBehavior,
       MultipleMessagesSingleFrameEndStreamAfterData) {
  CreateApiKeyRequest request1 = GetCreateApiKeyRequest() ;
  request1.set_parent(kAlternativeParent);
  CreateApiKeyRequest request2 = GetCreateApiKeyRequest() ;
  Envoy::Buffer::InstancePtr request_data1 =
...
TEST_F(MessageConverterReadOnlyBehavior, EmptyFrame) {
  ...
  EXPECT_EQ(final_data->toString(),
            std::string(kZeroSizeFrame, kGrpcDelimiterByteSize));
}
...
TEST_F(MessageConverterReadOnlyBehavior, OutOfOrderConversionSucceeds) {
  ...
  ASSERT_OK_AND_ASSIGN(auto message_data1,
                       converter.AccumulateMessage(request_in, false));
  ASSERT_OK_AND_ASSIGN(auto message_data2,
                       converter.AccumulateMessage(request_in, true));
...
TEST_F(MessageConverterReadOnlyBehavior, OutOfOrderCallsFail) {
  ...
  ASSERT_OK_AND_ASSIGN(auto message_data,
                       converter1.AccumulateMessage(*request_data, false));
...
TEST_F(MessageConverterReadOnlyBehavior,
       MultipleMessagesOutOfOrderDrainingWorks) {
  CreateApiKeyRequest request = GetCreateApiKeyRequest() ;
  std::string kAlternativeParent1(16 * 1024, '*');
  request.set_parent(kAlternativeParent1);
  request.mutable_key()->set_current_key(kAlternativeParent1);
  ...
  // Convert the first message.
  ASSERT_OK_AND_ASSIGN(auto message_data,
                       converter.AccumulateMessage(*request_data1, false));
  ASSERT_OK_AND_ASSIGN(auto converted_buffer1,
                       converter.ConvertBackToBuffer(std::move(message_data)));

  // Convert the second message.
  ASSERT_OK_AND_ASSIGN(message_data,
                       converter.AccumulateMessage(*request_data2, true));
  ASSERT_OK_AND_ASSIGN(auto converted_buffer2,
                       converter.ConvertBackToBuffer(std::move(message_data)));
...
TEST_F(MessageConverterMutableBehavior, SingleMessageEmptyMutation) {
  ...
  // Emulate mutation by ESF wrapper, where it fully drains the DataBuffer.
...
// mutation.
void RunConversionOnMessage(
    MessageConverter& converter,
    const std::vector<Envoy::Buffer::InstancePtr>& input_buffers,
    const std::vector<CreateApiKeyRequest>&,
    int message_size) {
  ASSERT_FALSE(input_buffers.empty());
  ASSERT_OK_AND_ASSIGN(auto message_data,
                       converter.AccumulateMessage(*input_buffers[0], true));
...
void RunOutOfScope(const std::vector<Envoy::Buffer::InstancePtr>& input_buffers,
                   ...
                   int num_conversions, int message_size) {
  ...
  for (int i = 0; i < num_conversions; i++) {
    ...
    MessageConverter converter(Factory());
    RunConversionOnMessage(converter, input_buffers, input_requests,
                           message_size);
  }
...
std::string RunChainedParamsString(
    const ::testing::TestParamInfo<RunChained::ParamType>& info) {
  return absl::StrCat(std::get<0>(info.param) ? "InScope_" : "OutOfScope_",
                      "NumConverters_", std::get<1>(info.param),
                      "_MessageSize_", std::get<2>(info.param));
}
...
TEST_P(RunChained, ConsolidatedBuffer) {
  ...
  auto input_requests = CreateRequestMessages(1, message_size, false);
  auto input_buffers =
      ConsolidateBuffers(CreateOneFramePerBuffer(input_requests));
...
TEST_P(RunChained, ConsolidateBuffersSingleSliceExactAlloc) {
  ...
  auto input_requests = CreateRequestMessages(1, message_size, false);
  auto input_buffers = ConsolidateBuffersSingleSliceExactAlloc(
      CreateOneFramePerBuffer(input_requests));
...
TEST_P(RunChained, ConsolidateBuffersSlicePerRequestExtraAlloc) {
  ...
  auto input_requests = CreateRequestMessages(1, message_size, false);
  auto input_buffers = ConsolidateBuffersSlicePerRequestExtraAlloc(
      CreateOneFramePerBuffer(input_requests));
...
void RunStreaming(const std::vector<Envoy::Buffer::InstancePtr>& input_buffers,
  ...
  auto output_requests =
      CreateRequestMessages(num_messages, message_size, true);
  ...
  for (auto& input_buffer : input_buffers) {
    ASSERT_OK_AND_ASSIGN(auto message_datas,
                         converter.AccumulateMessages(*input_buffer, false));
    ...
    for (auto& message_data : message_datas) {
      ...
      ASSERT_OK_AND_ASSIGN(auto converted_buffer, converter.ConvertBackToBuffer(
                                                      std::move(message_data)));
      ASSERT_NE(converted_buffer, nullptr);
      EXPECT_GT(converter.BytesBuffered(), 0);
      CheckSerializedData<CreateApiKeyRequest>(
          *converted_buffer, {output_requests[request_index]});
      request_index++;
    }
  }
  ...
  ASSERT_OK_AND_ASSIGN(auto final_messages,
                       converter.AccumulateMessages(empty_buffer, true));
...
void RunBatch(const std::vector<Envoy::Buffer::InstancePtr>& input_buffers,
  ...
  std::vector<std::unique_ptr<StreamMessage>> message_datas;
  {
    ...
    MessageConverter converter(Factory());
    for (auto& input_buffer : input_buffers) {
      ASSERT_OK_AND_ASSIGN(auto intermediates,
                           converter.AccumulateMessages(*input_buffer, false));
      message_datas.insert(message_datas.end(),
                           std::make_move_iterator(intermediates.begin()),
...
void RunBatchAndVerifyInOrder(
  ...
  // Run test.
  RunBatch(input_buffers, input_requests, output_requests, message_size,
           output_data);
...
std::string NoSplitsParamsString(
    const ::testing::TestParamInfo<RunNoSplits::ParamType>& info) {
  return absl::StrCat(std::get<0>(info.param) ? "Streaming_" : "Batch_",
                      "NumMessages_", std::get<1>(info.param), "_MessageSize_",
...
TEST_P(RunNoSplits, OneFramePerBuffer) {
  ...
  auto input_requests =
      CreateRequestMessages(num_messages, message_size, false);
  auto input_buffers = CreateOneFramePerBuffer(input_requests);
...
std::string WithSplitsParamsString(
    const ::testing::TestParamInfo<RunWithSplits::ParamType>& info) {
  return absl::StrCat(std::get<0>(info.param) ? "Streaming_" : "Batch_",
                      "NumMessages_", std::get<1>(info.param), "_MessageSize_",
                      std::get<2>(info.param), "_NumSplits_",
                      std::get<3>(info.param));
}

TEST_P(RunWithSplits, ConsolidateBuffersIntoSplitFrames) {
  auto [is_streaming_enabled, num_messages, message_size, num_splits] =
      GetParam();
...
// 2) Number of request messages
// 3) Per-message size
class RunWithIncomingStreamingData
    : public ::testing::TestWithParam<std::tuple<int, int, int>> {
 public:
  RunWithIncomingStreamingData() {}
};

std::string IncomingWithStreamingDataParamsString(
    const ::testing::TestParamInfo<RunWithIncomingStreamingData::ParamType>&
        info) {
  return absl::StrCat("NumWaitIterations_", std::get<0>(info.param),
                      "_NumMessages_", std::get<1>(info.param), "_MessageSize_",
                      std::get<2>(info.param));
}
...
void RunIncomingStreamingData(
    const std::vector<Envoy::Buffer::InstancePtr>& input_buffers,
    const std::vector<CreateApiKeyRequest>& input_requests, int message_size,
    int num_wait_iterations) {
  ASSERT_FALSE(input_buffers.empty());
  ...
  for (int iterations = 0; request_index < num_messages; iterations++) {
    ...
    ASSERT_OK_AND_ASSIGN(auto message_data, converter.AccumulateMessage(
                                                first_half_requests, false));
    ...
    ASSERT_OK_AND_ASSIGN(auto converted_buffer, converter.ConvertBackToBuffer(
                                                    std::move(message_data)));
    ASSERT_NE(converted_buffer, nullptr);
    CheckSerializedData<CreateApiKeyRequest>(*converted_buffer,
                                             {output_requests[request_index]});
    request_index++;
  }
...
TEST_P(RunWithIncomingStreamingData, ConsolidatedBuffer) {
  ...
  RunIncomingStreamingData(input_buffers, input_requests, message_size,
                           num_wait_iterations);
}

// Runs the mutable lifetime tests via out of order draining (of the converted
// back) buffers. This tests the implementation of
// go/cloud-esf-convergence-data-conversion-drain-algorithm.
//
// Parameters are the following:
// 1) Number of request messages
// 2) Per-message size
class RunOutOfOrderDrain
...
std::string RunOutOfOrderDrainParamsString(
  ...
  return absl::StrCat("Batch_", "NumMessages_", std::get<0>(info.param),
                      "_MessageSize_", std::get<1>(info.param));
}

void RunBatchAndDrainBackwards(
    const std::vector<Envoy::Buffer::InstancePtr>& input_buffers,
    const std::vector<CreateApiKeyRequest>& input_requests, int message_size) {
  auto num_messages = input_requests.size();
  auto output_requests =
...
INSTANTIATE_TEST_SUITE_P(MessageConverterDataIntegrity,
                         RunWithIncomingStreamingData,
