...
public:
  ExtAuthzFilterTest() {
    ...
    Envoy::Config::FilterJson::translateTcpExtAuthzFilter(*json_config, proto_config);
    ...
    crg_ = new NiceMock<MockCheckRequestGen>();
  ...
  NiceMock<MockCheckRequestGen>* crg_;
...
TEST_F(ExtAuthzFilterTest, BadExtAuthzConfig) {
  ...
  EXPECT_THROW(Envoy::Config::FilterJson::translateTcpExtAuthzFilter(*json_config, proto_config),
               Json::Exception);
}
...
TEST_F(ExtAuthzFilterTest, OK) {
  ...
  EXPECT_CALL(*crg_, createTcpCheck(_, _));
