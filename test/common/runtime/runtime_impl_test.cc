...
public:
    ...
    *config.add_layers()->mutable_static_layer() =
    ...
    ON_CALL(cm_.subscription_factory_, subscriptionFromConfigSource(_, _, _, _))
        .WillByDefault(testing::Invoke(
            [this](const envoy::api::v3alpha::core::ConfigSource&, absl::string_view, Stats::Scope&,
  ...
  void doOnConfigUpdateVerifyNoThrow(const envoy::service::discovery::v3alpha::Runtime& runtime,
  ...
  void
  doDeltaOnConfigUpdateVerifyNoThrow(const envoy::service::discovery::v3alpha::Runtime& runtime) {
    Protobuf::RepeatedPtrField<envoy::api::v3alpha::Resource> resources;
...
TEST_F(RtdsLoaderImplTest, WrongResourceName) {
  ...
  auto runtime = TestUtility::parseYaml<envoy::service::discovery::v3alpha::Runtime>(R"EOF(
    name: other_resource
    layer:
      foo: bar
      baz: meh
...
TEST_F(RtdsLoaderImplTest, OnConfigUpdateSuccess) {
  ...
  runtime = TestUtility::parseYaml<envoy::service::discovery::v3alpha::Runtime>(R"EOF(
    name: some_resource
    layer:
      baz: saz
  )EOF");
