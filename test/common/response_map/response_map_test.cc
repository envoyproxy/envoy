#include "envoy/config/response_map/v3/response_map.pb.h"
#include "envoy/config/response_map/v3/response_map.pb.validate.h"
...
class ResponseMapTest : public testing::Test {
public:
  ResponseMapTest() : stream_info_(time_system_.timeSystem()) { resetData(TestInitCode); }
  ...
  envoy::config::response_map::v3::ResponseMap config_;
  NiceMock<Server::Configuration::MockFactoryContext> context_;
};

TEST_F(ResponseMapTest, TestEmptyConfig) {
  // Empty ResponseMap config.
  auto response_map = Factory::create(config_, context_, context_.messageValidationVisitor());

  response_map->rewrite(nullptr, response_headers_, stream_info_, code_, body_, content_type_);
...
TEST_F(ResponseMapTest, TestDefaultResponseMap) {
  // Default ResponseMap should be the same as empty config.
  auto response_map = Factory::createDefault();
...
TEST_F(ResponseMapTest, TestInvalidConfigEmptyFilter) {
...
TEST_F(ResponseMapTest, TestInvalidConfigStatusCode) {
...
TEST_F(ResponseMapTest, TestDefaultTextFormatter) {
...
TEST_F(ResponseMapTest, TestDefaultJsonFormatter) {
...
TEST_F(ResponseMapTest, TestMapperRewrite) {
...
TEST_F(ResponseMapTest, TestMapperFormat) {
  // Match with response_code, and rewrite the code and body.
  const std::string yaml = R"(
    mappers:
    - filter:
...
TEST_F(ResponseMapTest, TestHeaderAddition) {
  // Default text formatter without any mappers
  const std::string yaml = R"(
    mappers:
    - filter:
...
TEST_F(ResponseMapTest, TestMapperWithContentType) {
  // Match with response_code, and rewrite the code and body.
  const std::string yaml = R"(
    mappers:
    - filter:
