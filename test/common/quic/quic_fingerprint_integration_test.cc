...
public:
  ...
  void SetUp() override {
    ...
    // Create factory.
    factory_ = std::make_unique<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting>();
...
protected:
  ...
  std::unique_ptr<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting> factory_;
...
TEST_F(QuicFingerprintIntegrationTest, FactoryConfigurationFlow) {
  ...
  EXPECT_EQ("envoy.quic.crypto_stream.server.fingerprinting", factory_->name());
...
TEST_F(QuicFingerprintIntegrationTest, ConfigurationVariations) {
  // Test``JA3``only.
  {
    ...
    auto ja3_only_factory =
        std::make_unique<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting>();
...
TEST_F(QuicFingerprintIntegrationTest, FactoryRegistrationAndName) {
  ...
  EXPECT_TRUE(factory_->name().find("fingerprinting") != std::string::npos);
...
TEST_F(QuicFingerprintIntegrationTest, ConfigurationLifecycle) {
  ...
  auto factory1 = std::make_unique<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting>();
  auto factory2 = std::make_unique<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting>();
...
TEST_F(QuicFingerprintIntegrationTest, FactoryLifecycleManagement) {
  ...
  for (int cycle = 0; cycle < 3; ++cycle) {
    auto factory = std::make_unique<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting>();

    EXPECT_EQ("envoy.quic.crypto_stream.server.fingerprinting", factory->name());
...
TEST_F(QuicFingerprintIntegrationTest, ConfigurationIsolationTest) {
  ...
  std::vector<std::unique_ptr<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting>> factories;
  ...
  for (int i = 0; i < num_factories; ++i) {
    ...
    EXPECT_EQ(i % 3 == 0, PROTOBUF_GET_WRAPPED_OR_DEFAULT(factories[i]->getQuicConfig().value(),
                                                          enable_ja4_fingerprinting, false));
    EXPECT_EQ("envoy.quic.crypto_stream.server.fingerprinting", factories[i]->name());
  }
}
...
TEST_F(QuicFingerprintIntegrationTest, ResourceManagementTest) {
  ...
  for (int i = 0; i < num_iterations; ++i) {
    ...
    auto temp_factory = std::make_unique<EnvoyQuicCryptoServerStreamFactoryWithFingerprinting>();
