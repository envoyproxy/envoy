...
static uint64_t clamp_u64(uint64_t in) { return in & ((1ULL << 62) - 1); }
...
public:
  QuicPacketizer(const quic::ParsedQuicVersion& quic_version,
      ...
      : quic_version_(quic_version), connection_helper_(connection_helper), packet_number_(0),
        idx_(0), destination_connection_id_(quic::test::TestConnectionId()),
  ...
  void serializePacket(const QuicFrame& frame) {
    ...
    case QuicFrame::kCrypto: {
      ...
      crypto_frame = std::make_unique<quic::QuicCryptoFrame>(
          quic::EncryptionLevel::ENCRYPTION_INITIAL, clamp_u64(f.offset()), h3_packet, len);
    ...
    case QuicFrame::kAck: {
      ...
      if (f.has_ecn_counters()) {
        ...
        ack_frame.ecn_counters =
            quic::QuicEcnCounts(clamp_u64(c.ect0()), clamp_u64(c.ect1()), clamp_u64(c.ce()));
      }
      PUSH(&ack_frame);
    } break;
    ...
    case QuicFrame::kWindowUpdate: {
      ...
      PUSH(quic::QuicWindowUpdateFrame(f.control_frame_id(), f.stream_id(),
                                       clamp_u64(f.max_data())));
    } break;
    ...
    case QuicFrame::kAckFrequency: {
      ...
      ack_frequency = std::make_unique<quic::QuicAckFrequencyFrame>(
          f.control_frame_id(), clamp_u64(f.sequence_number()), clamp_u64(f.packet_tolerance()),
          delta);
...
public:
  ...
  void OnCoalescedPacketSent(const quic::QuicCoalescedPacket& /*coalesced_packet*/,
      ...
      void OnPacketHeader(const quic::QuicPacketHeader& /*header*/, quic::QuicTime /*receive_time*/,
      ...
      void OnConnectionCloseFrame(const quic::QuicConnectionCloseFrame& /*frame*/) LOG_VISIT
      ...
      void OnNewConnectionIdFrame(const quic::QuicNewConnectionIdFrame& /*frame*/) LOG_VISIT

      // Called when a RetireConnectionIdFrame has been parsed.
      void OnRetireConnectionIdFrame(const quic::QuicRetireConnectionIdFrame& /*frame*/) LOG_VISIT
      ...
      void OnSuccessfulVersionNegotiation(const quic::ParsedQuicVersion& /*version*/) LOG_VISIT
...
public:
  ...
  void OnControlStreamCreated(quic::QuicStreamId /*stream_id*/) LOG_VISIT_H3
      ...
      void OnPriorityUpdateFrameReceived(const quic::PriorityUpdateFrame& /*frame*/) LOG_VISIT_H3
      ...
      void OnHeadersFrameReceived(quic::QuicStreamId /*stream_id*/,
      ...
      void OnUnknownFrameReceived(quic::QuicStreamId /*stream_id*/, uint64_t /*frame_type*/,
                                  quic::QuicByteCount /*payload_length*/) LOG_VISIT_H3
  ...
  void OnPriorityUpdateFrameSent(const quic::PriorityUpdateFrame& /*frame*/){};

  // Outgoing HTTP/3 frames on request or push streams.
  void OnDataFrameSent(quic::QuicStreamId /*stream_id*/, quic::QuicByteCount /*payload_length*/){};
...
public:
  FuzzDispatcher(quic::ParsedQuicVersion quic_version, quic::QuicVersionManager* version_manager,
      ...
      : quic::QuicDispatcher(
        ...
        http3_options_(http3_settings()), quic_version_(quic_version),
