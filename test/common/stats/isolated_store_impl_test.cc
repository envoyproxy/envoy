...
TEST_F(StatsIsolatedStoreImplTest, All) {
  ...
  EXPECT_EQ(0, c1.tags().size());
  CounterOptRef opt_counter = scope1->findCounter(c2.statName());
  ...
  GaugeOptRef opt_gauge = scope1->findGauge(g2.statName());
  ...
  h1.recordValue(200);
  h2.recordValue(200);
  HistogramOptRef opt_histogram = scope1->findHistogram(h2.statName());
