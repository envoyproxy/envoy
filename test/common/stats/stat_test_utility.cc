...
std::string TestScope::statNameWithTags(StatName name, absl::optional<StatNameTagSpan> name_tags,
  ...
  TagUtility::TagStatNameJoiner joiner(prefix(), {}, prefix(), name,
                                       name_tags.value_or(StatNameTagSpan{}), tagged_name,
                                       symbolTable());
  return symbolTable().toString(joiner.nameWithTags());
}

void TestScope::verifyConsistency(StatName ref_stat_name, StatName name,
...
Counter& TestScope::counterFromStatName(StatName name, absl::optional<StatNameTagSpan> name_tags,
                                        StatName tagged_name) {
  std::string flat_name = statNameWithTags(name, name_tags, tagged_name);
  Counter*& counter_ref = store_.counter_map_[flat_name];
  if (counter_ref == nullptr) {
    counter_ref = &IsolatedScopeImpl::counterFromStatName(name, name_tags, tagged_name);
  } else {
    verifyConsistency(counter_ref->statName(), name, name_tags, tagged_name);
  }
  return *counter_ref;
}

Gauge& TestScope::gaugeFromStatName(StatName name, absl::optional<StatNameTagSpan> name_tags,
                                    StatName tagged_name, Gauge::ImportMode import_mode) {
  std::string flat_name = statNameWithTags(name, name_tags, tagged_name);
  Gauge*& gauge_ref = store_.gauge_map_[flat_name];
  if (gauge_ref == nullptr) {
    gauge_ref = &IsolatedScopeImpl::gaugeFromStatName(name, name_tags, tagged_name, import_mode);
  } else {
    verifyConsistency(gauge_ref->statName(), name, name_tags, tagged_name);
  }
  return *gauge_ref;
}

Histogram& TestScope::histogramFromStatName(StatName name,
  ...
  if (histogram_ref == nullptr) {
    histogram_ref = &IsolatedScopeImpl::histogramFromStatName(name, name_tags, tagged_name, unit);
  } else {
    verifyConsistency(histogram_ref->statName(), name, name_tags, tagged_name);
  }
  return *histogram_ref;
}
