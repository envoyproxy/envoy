...
TEST(SourceImplTest, Caching) {
  ...
  std::vector<TextReadoutSharedPtr> stored_readouts;
  ...
  ON_CALL(store, textReadouts()).WillByDefault(ReturnPointee(&stored_readouts));
  ...
  stored_readouts.push_back(std::make_shared<MockTextReadout>());
  EXPECT_EQ(source.cachedTextReadouts(), stored_readouts);
  stored_readouts.push_back(std::make_shared<MockTextReadout>());
  EXPECT_NE(source.cachedTextReadouts(), stored_readouts);
