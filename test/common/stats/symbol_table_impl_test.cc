...
TEST(SymbolTableTest, Memory) {
  ...
  // to any requests.
  const std::vector<std::string> other_stats(
  ...
  // Calls a function for every stat name, which is expanded from the lists
  // above based on patterns observed running Envoy.
  auto foreach_stat = [&cluster_stats, &other_stats](std::function<void(absl::string_view)> fn) {
    for (int cluster = 0; cluster <= 1000; ++cluster) {
  ...
  auto test_memory_usage = [&foreach_stat](std::function<void(absl::string_view)> fn) -> size_t {
