...
TEST_F(StatTestUtilityTest, Counters) {
  test_store_.counterFromStatName(dynamic_.add("dynamic.stat"), absl::nullopt).inc();
  test_store_.counterFromStatName(symbolic_.add("symbolic.stat"), absl::nullopt).inc();
