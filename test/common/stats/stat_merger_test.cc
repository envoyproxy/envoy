...
TEST_F(StatMergerThreadLocalTest, DontFilterOutUninitializedGauges) {
  ...
  std::vector<Gauge*> gauges = Utility::gaugesMainThread(store_);
  ASSERT_EQ(2, gauges.size());
  auto gauge1_it = std::find_if(gauges.begin(), gauges.end(), [](const Gauge* gauge) {
    return gauge->name().compare("newgauge1") == 0;
  });
  ASSERT_TRUE(gauge1_it != gauges.end());
  EXPECT_EQ(&g1, *gauge1_it);
  auto gauge2_it = std::find_if(gauges.begin(), gauges.end(), [](const Gauge* gauge) {
    return gauge->name().compare("newgauge2") == 0;
  });
  ASSERT_TRUE(gauge2_it != gauges.end());
  EXPECT_EQ(&g2, *gauge2_it);
