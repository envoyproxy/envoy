...
TEST_F(HeapStatsThreadLocalStoreTest, MemoryWithoutTls) {
  ...
  EXPECT_LT(end_mem - start_mem, 25 * million); // actual value: 24068784 as of Oct 25, 2018
}
...
TEST_F(HeapStatsThreadLocalStoreTest, MemoryWithTls) {
  ...
  const size_t end_mem = Memory::Stats::totalCurrentlyAllocated();
  EXPECT_LT(start_mem, end_mem);
  EXPECT_LT(end_mem - start_mem, 29 * million); // actual value: 28301056 as of Oct 25, 2018
}
