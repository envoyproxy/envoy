...
TEST_F(StatsThreadLocalStoreTest, NoTls) {
  ...
  EXPECT_CALL(sink_, onTimespanComplete(Ref(t1), std::chrono::milliseconds(200)));
  t1.recordDuration(std::chrono::milliseconds(200));
  EXPECT_CALL(sink_, onTimespanComplete(Ref(t1), std::chrono::milliseconds(100)));
...
TEST_F(StatsThreadLocalStoreTest, BasicScope) {
  ...
  EXPECT_CALL(sink_, onTimespanComplete(Ref(t1), std::chrono::milliseconds(100)));
  t1.recordDuration(std::chrono::milliseconds(100));
  EXPECT_CALL(sink_, onTimespanComplete(Ref(t2), std::chrono::milliseconds(200)));
  ...
  scope1->deliverTimingToSinks(t1, std::chrono::milliseconds(200));
  tls_.shutdownThread();
