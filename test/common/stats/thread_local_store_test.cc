...
TEST_F(StatsThreadLocalStoreTest, NoTls) {
  ...
  EXPECT_EQ(&g1, &store_->gauge("g1", Gauge::ImportMode::NeverImport));
...
TEST_F(StatsThreadLocalStoreTest, NestedScopes) {
  ...
  Gauge& g1 = scope2->gauge("some_gauge", Gauge::ImportMode::NeverImport);
...
TEST_F(StatsThreadLocalStoreTest, OverlappingScopes) {
  ...
  Gauge& g1 = scope1->gauge("g", Gauge::ImportMode::NeverImport);
  Gauge& g2 = scope2->gauge("g", Gauge::ImportMode::NeverImport);
  EXPECT_NE(&g1, &g2);
  g1.set(5);
...
TEST_F(StatsMatcherTLSTest, TestNoOpStatImpls) {
  ...
  // Gauge
  Gauge& noop_gauge = store_->gauge("noop_gauge", Gauge::ImportMode::NeverImport);
  ...
  noop_gauge.sub(2);
  EXPECT_EQ(noop_gauge.value(), 0);
  Gauge& noop_gauge_2 = store_->gauge("noop_gauge_2", Gauge::ImportMode::NeverImport);
...
TEST_F(StatsMatcherTLSTest, TestExclusionRegex) {
  ...
  Gauge& lowercase_gauge = store_->gauge("lowercase_gauge", Gauge::ImportMode::NeverImport);
  ...
  Gauge& uppercase_gauge = store_->gauge("uppercase_GAUGE", Gauge::ImportMode::NeverImport);
  ...
  Gauge& valid_gauge = store_->gauge("valid_gauge", Gauge::ImportMode::NeverImport);
  valid_gauge.set(2);
  EXPECT_EQ(valid_gauge.value(), 2);

  Gauge& invalid_gauge_1 = store_->gauge("invalid_gauge", Gauge::ImportMode::NeverImport);
  invalid_gauge_1.inc();
  EXPECT_EQ(invalid_gauge_1.value(), 0);

  Gauge& invalid_gauge_2 = store_->gauge("also_INVALID_gauge", Gauge::ImportMode::NeverImport);
  invalid_gauge_2.inc();
...
TEST_F(StatsThreadLocalStoreTest, RemoveRejectedStats) {
  ...
  Gauge& gauge = store_->gauge("g1", Gauge::ImportMode::NeverImport);
...
TEST_F(StatsThreadLocalStoreTest, ShuttingDown) {
  ...
  store_->counter("c1");
  store_->gauge("g1", Gauge::ImportMode::NeverImport);
  store_->shutdownThreading();
  store_->counter("c2");
  store_->gauge("g2", Gauge::ImportMode::NeverImport);
