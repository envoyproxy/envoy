...
TEST_P(OwnedImplTest, DrainTracking) {
  if (GetParam() == BufferImplementation::Old) {
    return;
  }
...
TEST_P(OwnedImplTest, MoveDrainTrackersWhenTransferingSlices) {
  if (GetParam() == BufferImplementation::Old) {
    return;
  }
  testing::InSequence s;
...
TEST_P(OwnedImplTest, MoveDrainTrackersWhenCopying) {
  if (GetParam() == BufferImplementation::Old) {
    return;
  }
  testing::InSequence s;
...
TEST_P(OwnedImplTest, PartialMoveDrainTrackers) {
  if (GetParam() == BufferImplementation::Old) {
    return;
  }
  testing::InSequence s;
...
TEST_P(OwnedImplTest, DrainTrackingOnDestruction) {
  if (GetParam() == BufferImplementation::Old) {
    return;
  }
  testing::InSequence s;
...
TEST_P(OwnedImplTest, Linearize) {
  Buffer::OwnedImpl buffer;
...
TEST_P(OwnedImplTest, LinearizeEmptyBuffer) {
  Buffer::OwnedImpl buffer;
  EXPECT_EQ(nullptr, buffer.linearize(0));
}

TEST_P(OwnedImplTest, LinearizeSingleSlice) {
  auto buffer = std::make_unique<Buffer::OwnedImpl>();
...
TEST_P(OwnedImplTest, LinearizeDrainTracking) {
  if (GetParam() == BufferImplementation::Old) {
    return;
  }
