...
// and time, making the test fail in some settings (such as CI)
  ...
  const uint32_t segment_denominator = 32;
  const uint32_t big_segment_len = std::numeric_limits<uint32_t>::max() / segment_denominator;
  ...
  EXPECT_EQ(1, high_watermark_buffer1);
  EXPECT_EQ(0, overflow_watermark_buffer1);
  EXPECT_EQ(segment_denominator * big_segment_len + static_cast<uint64_t>(10), buffer1.length());
#endif
}
