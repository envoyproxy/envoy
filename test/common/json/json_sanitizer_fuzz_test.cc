...
DEFINE_FUZZER(const uint8_t* buf, size_t len) {
  {
    ...
    while (provider.remaining_bytes() != 0) {
      ...
      if (Envoy::Json::TestUtil::isProtoSerializableUtf8(input)) {
        buffer2 = MessageUtil::getJsonStringFromMessageOrError(ValueUtil::stringValue(input), false,
                                                               true);
...

  {
    ...
    while (provider.remaining_bytes() != 0) {
      std::string input = provider.ConsumeRandomLengthString(provider.remaining_bytes());
      absl::string_view sanitized = Envoy::Json::sanitizeExceptionFree(buffer1, input);
