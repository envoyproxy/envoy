...
RouterTestBase::RouterTestBase(bool start_child_span, bool suppress_envoy_headers,
    ...
    : pool_(stats_store_.symbolTable()), http_context_(stats_store_.symbolTable()),
      ...
      config_(pool_.add("test"), local_info_, *stats_store_.rootScope(), cm_, runtime_, random_,
              ...
              false, suppress_grpc_request_failure_code_stats, flush_upstream_log_on_new_request,
              std::move(strict_headers_to_check), test_time_.timeSystem(), http_context_,
              router_context_), router_(std::make_unique<RouterTestFilter>(config_, config_.default_stats_)) {
