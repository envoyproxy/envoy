...
public:
  ...
  void verifyRetryWithRemoteResponse(Http::RequestHeaderMap& request_headers,
    ...
    EXPECT_EQ(1UL, cluster_.upstreamStats()->upstream_rq_retry_limit_exceeded_.value());
    EXPECT_EQ(1UL, virtual_cluster_.stats().upstream_rq_retry_limit_exceeded_.value());
    EXPECT_EQ(1UL, cluster_.upstreamStats()->upstream_rq_retry_.value());
...
TEST_F(RouterRetryStateImplTest, NoRetryUponTooEarlyStatusCodeWithDownstreamEarlyData) {
  ...
  EXPECT_EQ(0UL, cluster_.upstreamStats()->upstream_rq_retry_.value());
...
TEST_F(RouterRetryStateImplTest, NoAvailableRetries) {
  ...
  EXPECT_EQ(
      ...
      state_->shouldRetryReset(connect_failure_, RetryState::Http3Used::Unknown, reset_callback_));
  EXPECT_EQ(1UL, cluster_.upstreamStats()->upstream_rq_retry_overflow_.value());
...
TEST_F(RouterRetryStateImplTest, MaxRetriesHeader) {
  ...
  EXPECT_EQ(3UL, cluster_.upstreamStats()->upstream_rq_retry_.value());
  EXPECT_EQ(0UL, cluster_.upstreamStats()->upstream_rq_retry_success_.value());
  EXPECT_EQ(1UL, cluster_.upstreamStats()->upstream_rq_retry_limit_exceeded_.value());
...
TEST_F(RouterRetryStateImplTest, Backoff) {
  ...
  EXPECT_EQ(5UL, cluster_.upstreamStats()->upstream_rq_retry_.value());
  EXPECT_EQ(1UL, cluster_.upstreamStats()->upstream_rq_retry_success_.value());
...
TEST_F(RouterRetryStateImplTest, RateLimitedRetryBackoffStrategy) {
  ...
  EXPECT_EQ(2UL, cluster_.upstreamStats()->upstream_rq_retry_backoff_ratelimited_.value());
  EXPECT_EQ(2UL, cluster_.upstreamStats()->upstream_rq_retry_backoff_exponential_.value());
}
...
TEST_F(RouterRetryStateImplTest, NoPreferredOverLimitExceeded) {
  ...
  EXPECT_EQ(0UL, cluster_.upstreamStats()->upstream_rq_retry_limit_exceeded_.value());
