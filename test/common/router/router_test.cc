...
TEST_F(RouterTest, Redirect) {
  ...
  EXPECT_CALL(direct_response, formatBody(_, _, _, _)).WillOnce(ReturnRef(EMPTY_STRING));
...
TEST_F(RouterTest, DirectResponse) {
  ...
  EXPECT_CALL(direct_response, formatBody(_, _, _, _)).WillRepeatedly(ReturnRef(EMPTY_STRING));
  EXPECT_CALL(*callbacks_.route_, directResponseEntry()).WillRepeatedly(Return(&direct_response));
...
TEST_F(RouterTest, DirectResponseWithBody) {
  ...
  EXPECT_CALL(direct_response, formatBody(_, _, _, _)).WillRepeatedly(ReturnRef(response_body));
  EXPECT_CALL(*callbacks_.route_, directResponseEntry()).WillRepeatedly(Return(&direct_response));
