    ...
    EXPECT_CALL(cm_.conn_pool_, newStream(_, _))
        .WillOnce(Invoke(
            [&](Http::ResponseDecoder& decoder,
              ...
              callbacks.onPoolReady(encoder1, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
              return nullptr;
            }));
    expectResponseTimerCreate();
    ...
    EXPECT_CALL(cm_.conn_pool_, newStream(_, _))
        .WillOnce(Invoke(
            [&](Http::ResponseDecoder& decoder,
              ...
              callbacks.onPoolReady(original_encoder_, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
              return nullptr;
            }));
  ...
  EXPECT_CALL(cm_.conn_pool_, newStream(_, _))
      .WillOnce(Invoke(
          [&](Http::ResponseDecoder& decoder,
              Http::ConnectionPool::Callbacks& callbacks) -> Http::ConnectionPool::Cancellable* {
            response_decoder = &decoder;
            callbacks.onPoolReady(encoder, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
            return nullptr;
          }));
  expectResponseTimerCreate();
  ...
  EXPECT_CALL(cm_.conn_pool_, newStream(_, _))
      .WillOnce(Invoke(
          [&](Http::ResponseDecoder& decoder,
              Http::ConnectionPool::Callbacks& callbacks) -> Http::ConnectionPool::Cancellable* {
            response_decoder = &decoder;
            callbacks.onPoolReady(encoder2, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
            return nullptr;
          }));
...
TEST_F(RouterTest, UpstreamPerTryTimeoutDelayedPoolReady) {
  ...
  EXPECT_EQ(0U,
            callbacks_.route_->route_entry_.virtual_cluster_.stats().upstream_rq_total_.value());
  pool_callbacks->onPoolReady(encoder, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
  EXPECT_EQ(1U,
            callbacks_.route_->route_entry_.virtual_cluster_.stats().upstream_rq_total_.value());
  ...
  EXPECT_CALL(cm_.conn_pool_, newStream(_, _))
      .WillOnce(Invoke(
          [&](Http::ResponseDecoder& decoder,
            ...
            EXPECT_CALL(*router_.retry_state_, onHostAttempted(_));
            callbacks.onPoolReady(encoder3, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
            return nullptr;
          }));
    ...
    EXPECT_CALL(cm_.conn_pool_, newStream(_, _))
        .WillOnce(Invoke(
            [&](Http::ResponseDecoder& decoder,
              ...
              if (pool_ready) {
                callbacks.onPoolReady(encoder_, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
              }
              return nullptr;
            }));
...
TEST_F(WatermarkTest, FilterWatermarks) {
  ...
  pool_callbacks_->onPoolReady(encoder_, cm_.conn_pool_.host_, upstream_stream_info_, Http::Protocol::Http10);
  EXPECT_EQ(1U, cm_.thread_local_cluster_.cluster_.info_->stats_store_
                    .counter("upstream_flow_control_drained_total")
                    .value());
