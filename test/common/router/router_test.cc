...
TEST_F(RouterTest, Redirect) {
  ...
  EXPECT_CALL(direct_response, formatBody(_, _, _)).WillOnce(Return(EMPTY_STRING));
...
TEST_F(RouterTest, DirectResponse) {
  ...
  EXPECT_CALL(direct_response, formatBody(_, _, _)).WillRepeatedly(Return(EMPTY_STRING));
  EXPECT_CALL(*callbacks_.route_, directResponseEntry()).WillRepeatedly(Return(&direct_response));
