...
scoped_routes:
  ...
  auto config = parseHttpConnectionManagerFromYaml(absl::Substitute(hcm_config, "foo-scoped-routes"));
  Envoy::Config::ConfigProviderPtr provider = ScopedRoutesConfigProviderUtil::create(
      config,
      server_factory_context_, context_init_manager_, "foo.", *config_provider_manager_);
  Envoy::Router::ScopeKeyBuilderPtr scope_key_builder = ScopedRoutesConfigProviderUtil::createScopeKeyBuilder(config);
  ASSERT_THAT(provider->config<ScopedConfigImpl>(), Not(IsNull()));
  EXPECT_EQ(provider->config<ScopedConfigImpl>()
                ->getRouteConfig(scope_key_builder->computeScopeKey(TestRequestHeaderMapImpl{{"addr", "foo-key"}}))
                ->name(),
            "foo");
  EXPECT_EQ(provider->config<ScopedConfigImpl>()
                ->getRouteConfig(scope_key_builder->computeScopeKey(TestRequestHeaderMapImpl{{"addr", "foo-key-2"}}))
                ->name(),
            "foo2");
  EXPECT_EQ(provider->config<ScopedConfigImpl>()
                ->getRouteConfig(scope_key_builder->computeScopeKey(TestRequestHeaderMapImpl{{"addr", "foo-key,foo-key-2"}}))
                ->name(),
            "foo");
}
...
key:
  ...
  EXPECT_EQ(getScopedRdsProvider()
                ...
                ->getRouteConfig(
                    scope_key_builder_->computeScopeKey(TestRequestHeaderMapImpl{{"Addr", "x-foo-key;x-foo-key"}}))
                ->name(),
            "");
  EXPECT_EQ(getScopedRdsProvider()
                ->config<ScopedConfigImpl>()
                ->getRouteConfig(scope_key_builder_->computeScopeKey(TestRequestHeaderMapImpl{{"Addr", "x-foo-key;x-bar-key"}}))
                ->name(),
            "");
  ...
  EXPECT_EQ(getScopedRdsProvider()
                ->config<ScopedConfigImpl>()
                ->getRouteConfig(scope_key_builder_->computeScopeKey(TestRequestHeaderMapImpl{{"Addr", "x-foo-key;x-foo-key"}}))
                ->name(),
            "foo_routes");
  EXPECT_EQ(getScopedRdsProvider()
                ->config<ScopedConfigImpl>()
  ...
  EXPECT_THAT(getScopedRdsProvider()->config<ScopedConfigImpl>()->getRouteConfig(
                  scope_key_builder_->computeScopeKey(TestRequestHeaderMapImpl{{"Addr", "x-foo-key;x-bar-key"}})),
              IsNull());
  EXPECT_EQ(getScopedRdsProvider()
                ->config<ScopedConfigImpl>()
                ->getRouteConfig(scope_key_builder_->computeScopeKey(TestRequestHeaderMapImpl{{"Addr", "x-foo-key;x-foo-key"}}))
