...
envoy::api::v3alpha::ScopedRouteConfiguration
...
protected:
  ...
  anyToResource(Protobuf::RepeatedPtrField<ProtobufWkt::Any>& resources,
    ...
    for (const auto& resource_any : resources) {
      auto config =
          TestUtility::anyConvert<envoy::api::v3alpha::ScopedRouteConfiguration>(resource_any);
...
scope_key_builder:
  ...
  void pushRdsConfig(const std::vector<std::string>& route_config_names,
    ...
    for (const std::string& name : route_config_names) {
      ...
      resources.Add()->PackFrom(TestUtility::parseYaml<envoy::api::v3alpha::RouteConfiguration>(
