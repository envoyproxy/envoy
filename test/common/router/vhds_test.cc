...
vhds:
  ...
  envoy::api::v3alpha::route::VirtualHost buildVirtualHost(const std::string& name,
                                                           const std::string& domain) {
    return TestUtility::parseYaml<envoy::api::v3alpha::route::VirtualHost>(fmt::format(R"EOF(
      name: {}
      domains: [{}]
      routes:
      - match: {{ prefix: "/" }}
  ...
  Protobuf::RepeatedPtrField<envoy::api::v3alpha::Resource> buildAddedResources(
      const std::vector<envoy::api::v3alpha::route::VirtualHost>& added_or_updated) {
    Protobuf::RepeatedPtrField<envoy::api::v3alpha::Resource> to_ret;
  ...
  RouteConfigUpdatePtr makeRouteConfigUpdate(const envoy::api::v3alpha::RouteConfiguration& rc) {
...
TEST_F(VhdsTest, VhdsInstantiationShouldSucceedWithDELTA_GRPC) {
  const auto route_config =
      TestUtility::parseYaml<envoy::api::v3alpha::RouteConfiguration>(default_vhds_config_);
...
TEST_F(VhdsTest, VhdsInstantiationShouldFailWithoutDELTA_GRPC) {
  const auto route_config = TestUtility::parseYaml<envoy::api::v3alpha::RouteConfiguration>(R"EOF(
name: my_route
vhds:
  config_source:
    api_config_source:
...
TEST_F(VhdsTest, VhdsValidatesAddedVirtualHosts) {
  ...
  auto vhost = TestUtility::parseYaml<envoy::api::v3alpha::route::VirtualHost>(R"EOF(
        name: invalid_vhost
        domains: []
        routes:
        - match: { prefix: "/" }
