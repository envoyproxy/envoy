  ...
  {
    {
      ...
      route->finalizeResponseHeaders(headers, req_headers, stream_info);
      EXPECT_EQ("route-override", headers.get_("x-global-header1"));
      EXPECT_EQ("route-override", headers.get_("x-vhost-header1"));
...
virtual_hosts:
  ...
  // Make sure weighted cluster entries call through to the parent when needed.
  {
    ...
    route_entry->finalizeResponseHeaders(response_headers, headers, stream_info);
...
virtual_hosts:
...

  {
    ...
    route->finalizeResponseHeaders(resp_headers, headers, stream_info);
...
- header:
  ...
  if (!Runtime::runtimeFeatureEnabled("envoy.reloadable_features.unified_header_formatter")) {
    EXPECT_THROW_WITH_MESSAGE(EnvoyException,
