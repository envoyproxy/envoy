...
const Http::LowerCaseString& testHeader() {
  CONSTRUCT_ON_FIRST_USE(Http::LowerCaseString, "x-route-specifier");
}

// The route a test specifier hands back: everything is delegated to the matched route except
// finalizeRequestHeaders, which appends the specifier's tag to a test header before delegating.
// Appending rather than setting lets a test read the order the specifiers ran in off the header.
...
class TagAddingRouteSpecifier : public RouteSpecifier {
public:
...
public:
  ...
  std::string name() const override { return "envoy.router.route_specifier.add_header"; }
};

envoy::config::core::v3::TypedExtensionConfig specifierConfig(const std::string& tag) {
  envoy::config::core::v3::TypedExtensionConfig config;
  config.set_name("envoy.router.route_specifier.add_header");
...
class RouteSpecifierIntegrationTest : public Envoy::HttpIntegrationTest, public testing::Test {
public:
  RouteSpecifierIntegrationTest()
      : HttpIntegrationTest(Http::CodecType::HTTP1, Network::Address::IpVersion::v4) {}

  // Adds a tagged specifier at each of the requested levels of the default route configuration.
  void initializeWithSpecifiersAt(bool config_level, bool vhost_level, bool route_level) {
    config_helper_.addConfigModifier(
        [config_level, vhost_level, route_level](HttpConnectionManager& hcm) {
          auto* route_config = hcm.mutable_route_config();
          if (config_level) {
            *route_config->add_route_specifiers() = specifierConfig("config");
          }
          auto* vhost = route_config->mutable_virtual_hosts(0);
          if (vhost_level) {
            *vhost->add_route_specifiers() = specifierConfig("vhost");
          }
          if (route_level) {
            *vhost->mutable_routes(0)->add_route_specifiers() = specifierConfig("route");
          }
        });
    initialize();
  }

  // Sends a request through the proxy and returns the value the specifiers left on the test header
  ...
  TagAddingRouteSpecifierFactory factory_;
  Registry::InjectFactory<RouteSpecifierFactory> registered_{factory_};
};

// A route configuration level specifier decorates the route of every request.
TEST_F(RouteSpecifierIntegrationTest, RouteConfigurationLevelSpecifier) {
  initializeWithSpecifiersAt(true, false, false);
  EXPECT_EQ("config", upstreamTestHeader());
}

TEST_F(RouteSpecifierIntegrationTest, VirtualHostLevelSpecifier) {
  initializeWithSpecifiersAt(false, true, false);
  EXPECT_EQ("vhost", upstreamTestHeader());
}

TEST_F(RouteSpecifierIntegrationTest, RouteLevelSpecifier) {
  initializeWithSpecifiersAt(false, false, true);
  EXPECT_EQ("route", upstreamTestHeader());
}

// With no specifier configured the route is untouched.
TEST_F(RouteSpecifierIntegrationTest, NoSpecifierLeavesTheRouteAlone) {
  initializeWithSpecifiersAt(false, false, false);
...
TEST_F(RouteSpecifierIntegrationTest, SpecifiersRunWhenNothingMatched) {
