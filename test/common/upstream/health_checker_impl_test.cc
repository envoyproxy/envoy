...
public:
  ...
  void setupServicePatternRuntimeEnabledValidationHC() {
    std::string yaml = R"EOF(
    timeout: 1s
    interval: 1s
    interval_jitter: 1s
    ...
    healthy_threshold: 2
    http_health_check:
      service_name_matcher_enabled:
        default_value: true
        runtime_key: 'health_check.pattern.runtime'
      service_name_matcher:
        safe_regex:
          google_re2: {}
          regex: 'locations-.*-.*$'
  ...
  void setupServicePatternRuntimeDisabledValidationHC() {
    std::string yaml = R"EOF(
    timeout: 1s
    interval: 1s
    interval_jitter: 1s
...
TEST_F(HttpHealthCheckerImplTest, SuccessServicePrefixPatternCheck) {
  ...
  EXPECT_CALL(runtime_.snapshot_, getBoolean("", true)).Times(1);
...
TEST_F(HttpHealthCheckerImplTest, SuccessServicePatternRuntimeEnabledCheck) {
  const std::string host = "fake_cluster";
  const std::string path = "/healthcheck";
  setupServicePatternRuntimeEnabledValidationHC();
  EXPECT_CALL(runtime_.snapshot_, getBoolean("health_check.pattern.runtime", true)).Times(1);
...
TEST_F(HttpHealthCheckerImplTest, SuccessServicePatternRuntimeDisabledCheck) {
  const std::string host = "fake_cluster";
  const std::string path = "/healthcheck";
  setupServicePatternRuntimeDisabledValidationHC();
  EXPECT_CALL(runtime_.snapshot_, getBoolean("health_check.pattern.runtime", false))
      .WillOnce(Return(true));
...
TEST_F(HttpHealthCheckerImplTest, ServicePatternDoesNotMatchFail) {
  setupServiceRegexPatternValidationHC();
  EXPECT_CALL(*event_logger_, logUnhealthy(_, _, _, true));
  EXPECT_CALL(runtime_.snapshot_, getBoolean("", true)).WillOnce(Return(true));
