...
// Validate that a successful active health clears the Outlier status during ejection period.
TEST_F(HttpHealthCheckerImplTest, SuccessServiceCheckSetsHealhtyDuringOutlierEjectionPeriod) {
  const std::string host = "fake_cluster";
  const std::string path = "/healthcheck";
  setupServiceValidationHC();
  EXPECT_CALL(runtime_.snapshot_, featureEnabled("health_check.verify_cluster", 100))
  ...
  // Mark the host health status to "FAILED_OUTLIER_CHECK" and validate that after
  // successful health check, it is cleared.
  cluster_->prioritySet().getMockHostSet(0)->hosts_[0]->healthFlagSet(
  ...
  // Last HC pass time is only updated when host transitions from unhealthy to healthy state.
  EXPECT_EQ(std::chrono::milliseconds(13000), last_hc_pass_time_ms);
  // Active health check succeded so Host's outlier check should be cleared.
