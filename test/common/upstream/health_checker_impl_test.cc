...
TEST_F(HttpHealthCheckerImplTest, SuccessStartFailedFailFirstServiceCheck) {
  ...
  EXPECT_CALL(*event_logger_, logAddHealthy(_, _, _, false));
  EXPECT_CALL(*test_sessions_[0]->interval_timer_, enableTimer(_));
  EXPECT_CALL(*test_sessions_[0]->timeout_timer_, disableTimer());
  respond(0, "200", false, false, false, health_checked_cluster);
  EXPECT_TRUE(cluster_->prioritySet().getMockHostSet(0)->hosts_[0]->healthy());
...
TEST_F(HttpHealthCheckerImplTest, SuccessStartFailedSuccessFirst) {
  ...
  EXPECT_CALL(*this, onHostStatus(_, HealthTransition::Changed)).Times(1);
  EXPECT_CALL(*event_logger_, logAddHealthy(_, _, _, true));
...
TEST(HealthCheckEventLoggerImplTest, All) {
  ...
  event_logger.logEjectUnhealthy(envoy::api::v2::core::HealthCheckerType::HTTP, host,
                                 envoy::api::v2::core::HealthCheckFailureType::ACTIVE,
  ...
  EXPECT_CALL(*file,
              write(absl::string_view{
                  "{\"health_checker_type\":\"HTTP\","
                  "\"host_address\":\"10.0.0.1:443\",\"cluster_name\":\"fake_cluster\","
                  "\"add_healthy_event\":{\"first_check\":false,\"healthy_threshold\":10}}\n"}));
  event_logger.logAddHealthy(envoy::api::v2::core::HealthCheckerType::HTTP, host, 10, false);
}
