...
protected:
  ...
  MockHostSet& host_set() { return GetParam() ? host_set_ : failover_host_set_; }
...
TEST_P(RoundRobinLoadBalancerTest, SingleHost) {
  init(false);
  host_set().healthy_hosts_ = {makeTestHost(info_, "tcp://127.0.0.1:80")};
  host_set().hosts_ = host_set().healthy_hosts_;
  EXPECT_EQ(host_set().healthy_hosts_[0], lb_->chooseHost(nullptr));
}
...
TEST_P(RoundRobinLoadBalancerTest, MaxUnhealthyPanic) {
  ...
  EXPECT_EQ(host_set().hosts_[0], lb_->chooseHost(nullptr));
  EXPECT_EQ(host_set().hosts_[1], lb_->chooseHost(nullptr));
  EXPECT_EQ(host_set().hosts_[2], lb_->chooseHost(nullptr));
  ...
  EXPECT_EQ(host_set().healthy_hosts_[3], lb_->chooseHost(nullptr));
  EXPECT_EQ(host_set().healthy_hosts_[0], lb_->chooseHost(nullptr));
...
TEST_P(RoundRobinLoadBalancerTest, ZoneAwareSmallCluster) {
  ...
  EXPECT_EQ(host_set().healthy_hosts_[0], lb_->chooseHost(nullptr));
  EXPECT_EQ(host_set().healthy_hosts_[1], lb_->chooseHost(nullptr));
  EXPECT_EQ(host_set().healthy_hosts_[2], lb_->chooseHost(nullptr));
  ...
  local_host_set_->updateHosts(hosts, hosts, hosts_per_locality, hosts_per_locality,
                               empty_host_vector_, empty_host_vector_);
  EXPECT_EQ(host_set().healthy_hosts_per_locality_[0][0], lb_->chooseHost(nullptr));
}
...
TEST_P(RoundRobinLoadBalancerTest, NoZoneAwareDifferentZoneSize) {
  ...
  host_set().healthy_hosts_ = *hosts;
  host_set().hosts_ = *hosts;
  host_set().healthy_hosts_per_locality_ = *upstream_hosts_per_locality;
...
TEST_P(RoundRobinLoadBalancerTest, ZoneAwareRoutingSmallZone) {
  ...
  host_set().healthy_hosts_ = *upstream_hosts;
  host_set().hosts_ = *upstream_hosts;
  host_set().healthy_hosts_per_locality_ = *upstream_hosts_per_locality;
  ...
  EXPECT_EQ(host_set().healthy_hosts_per_locality_[1][1], lb_->chooseHost(nullptr));
...
TEST_P(LeastRequestLoadBalancerTest, SingleHost) {
  ...
  {
    host_set().runCallbacks(empty, empty);
    EXPECT_CALL(random_, random()).WillOnce(Return(2));
    EXPECT_EQ(host_set().healthy_hosts_[0], lb_.chooseHost(nullptr));
  }

  {
    ...
    remove_hosts.push_back(host_set().hosts_[0]);
    host_set().runCallbacks(empty, remove_hosts);
...
TEST_P(LeastRequestLoadBalancerTest, Normal) {
  ...
  host_set().healthy_hosts_[0]->stats().rq_active_.set(1);
  host_set().healthy_hosts_[1]->stats().rq_active_.set(2);
  EXPECT_CALL(random_, random()).WillOnce(Return(2)).WillOnce(Return(3));
  EXPECT_EQ(host_set().healthy_hosts_[0], lb_.chooseHost(nullptr));

  host_set().healthy_hosts_[0]->stats().rq_active_.set(2);
  host_set().healthy_hosts_[1]->stats().rq_active_.set(1);
  EXPECT_CALL(random_, random()).WillOnce(Return(2)).WillOnce(Return(3));
  EXPECT_EQ(host_set().healthy_hosts_[1], lb_.chooseHost(nullptr));
}
...
TEST_P(LeastRequestLoadBalancerTest, WeightImbalanceRuntimeOff) {
  ...
  host_set().healthy_hosts_ = {makeTestHost(info_, "tcp://127.0.0.1:80", 1),
...
TEST_P(LeastRequestLoadBalancerTest, WeightImbalanceCallbacks) {
  ...
  hosts_removed.push_back(host_set().hosts_[1]);
  host_set().hosts_.erase(host_set().hosts_.begin() + 1);
  host_set().healthy_hosts_.erase(host_set().healthy_hosts_.begin() + 1);
  host_set().runCallbacks(empty, hosts_removed);
