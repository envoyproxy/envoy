...
TEST_P(StrictDnsParamTest, ImmediateResolve) {
  ...
  EXPECT_EQ(2UL, cluster.primaryHosts().hosts().size());
  EXPECT_EQ(2UL, cluster.primaryHosts().healthyHosts().size());
}
...
TEST(StrictDnsClusterImplTest, Basic) {
  ...
  EXPECT_THAT(std::list<std::string>({"127.0.0.1:11001", "127.0.0.2:11001"}),
              ContainerEq(hostListToAddresses(cluster.primaryHosts().hosts())));
  EXPECT_EQ("localhost1", cluster.primaryHosts().hosts()[0]->hostname());
  EXPECT_EQ("localhost1", cluster.primaryHosts().hosts()[1]->hostname());
  ...
  EXPECT_EQ(2UL, cluster.primaryHosts().healthyHosts().size());
  EXPECT_EQ(0UL, cluster.primaryHosts().hostsPerLocality().size());
  EXPECT_EQ(0UL, cluster.primaryHosts().healthyHostsPerLocality().size());
...
TEST(StaticClusterImplTest, EmptyHostname) {
  ...
  StaticClusterImpl cluster(parseClusterFromJson(json), runtime, stats, ssl_context_manager, cm,
                            false);
  EXPECT_EQ(1UL, cluster.primaryHosts().healthyHosts().size());
  EXPECT_EQ("", cluster.primaryHosts().hosts()[0]->hostname());
...
TEST(StaticClusterImplTest, OutlierDetector) {
  ...
  cluster.primaryHosts().hosts()[0]->outlierDetector().putHttpResponseCode(503);
  cluster.primaryHosts().hosts()[0]->healthFlagSet(Host::HealthFlag::FAILED_OUTLIER_CHECK);
  detector->runCallbacks(cluster.primaryHosts().hosts()[0]);
  EXPECT_EQ(1UL, cluster.primaryHosts().healthyHosts().size());
  EXPECT_EQ(1UL, cluster.info()->stats().membership_healthy_.value());
  EXPECT_NE(cluster.primaryHosts().healthyHosts()[0], cluster.primaryHosts().hosts()[0]);

  // Bring the host back online.
  cluster.primaryHosts().hosts()[0]->healthFlagClear(Host::HealthFlag::FAILED_OUTLIER_CHECK);
  detector->runCallbacks(cluster.primaryHosts().hosts()[0]);
  EXPECT_EQ(2UL, cluster.primaryHosts().healthyHosts().size());
  EXPECT_EQ(2UL, cluster.info()->stats().membership_healthy_.value());
}
...
TEST(StaticClusterImplTest, HealthyStat) {
  ...
  cluster.primaryHosts().hosts()[0]->healthFlagSet(Host::HealthFlag::FAILED_OUTLIER_CHECK);
  outlier_detector->runCallbacks(cluster.primaryHosts().hosts()[0]);
  EXPECT_EQ(1UL, cluster.primaryHosts().healthyHosts().size());
  EXPECT_EQ(1UL, cluster.info()->stats().membership_healthy_.value());

  cluster.primaryHosts().hosts()[0]->healthFlagSet(Host::HealthFlag::FAILED_ACTIVE_HC);
  health_checker->runCallbacks(cluster.primaryHosts().hosts()[0], true);
  EXPECT_EQ(1UL, cluster.primaryHosts().healthyHosts().size());
  EXPECT_EQ(1UL, cluster.info()->stats().membership_healthy_.value());
  ...
  cluster.primaryHosts().hosts()[0]->healthFlagClear(Host::HealthFlag::FAILED_ACTIVE_HC);
  health_checker->runCallbacks(cluster.primaryHosts().hosts()[0], true);
  EXPECT_EQ(2UL, cluster.primaryHosts().healthyHosts().size());
  EXPECT_EQ(2UL, cluster.info()->stats().membership_healthy_.value());
  ...
  cluster.primaryHosts().hosts()[1]->healthFlagSet(Host::HealthFlag::FAILED_ACTIVE_HC);
  health_checker->runCallbacks(cluster.primaryHosts().hosts()[1], true);
  EXPECT_EQ(0UL, cluster.primaryHosts().healthyHosts().size());
...
TEST(StaticClusterImplTest, UrlConfig) {
  ...
  EXPECT_EQ(0UL, cluster.primaryHosts().hostsPerLocality().size());
  EXPECT_EQ(0UL, cluster.primaryHosts().healthyHostsPerLocality().size());
  cluster.primaryHosts().hosts()[0]->healthChecker().setUnhealthy();
}
