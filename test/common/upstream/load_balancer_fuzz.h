...
public:
  ...
  void updateHealthFlagsForAHostSet(
      bool failover_host_set, uint32_t num_healthy_hosts, uint32_t num_degraded_hosts = 0,
      uint32_t num_excluded_hosts = 0); // Hostset or int, or mod the int with number of host sets
  ...
  // balancing algorithms sometimes use other components which are coupled into the algorithm
  // logically.
