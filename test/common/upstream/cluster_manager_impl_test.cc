...
TEST_F(ClusterManagerImplTest, RemoveClustersBatchDisabled) {
  TestScopedRuntime scoped_runtime;
  scoped_runtime.mergeValues({{"envoy.reloadable_features.batch_cluster_removal", "false"}});
  ...
  // Expect runOnAllThreads to be called exactly twice during removeClusters when disabled.
  EXPECT_CALL(factory_.tls_, runOnAllThreads(testing::An<std::function<void()>>()))
      .Times(2)
      .WillRepeatedly(Invoke(&factory_.tls_, &ThreadLocal::MockInstance::runOnAllThreads1));
