...
TEST_F(ClusterManagerImplTest, ConnPoolsIdleTimeout) {
  ...
  static_resources:
    ...
    - name: cluster_1
      ...
      lb_policy: ROUND_ROBIN
      type: STATIC
      connection_pool_idle_timeout: 1s
  )EOF";
...
TEST_F(PrefetchTest, PrefetchOff) {
  ...
  EXPECT_CALL(*mock_http_conn_pool, addIdlePoolTimeoutCallback);
  EXPECT_CALL(factory_, allocateConnPool_(_, _, _, _)).WillOnce(Return(mock_http_conn_pool));
  cluster_manager_->httpConnPoolForCluster("cluster_1", ResourcePriority::Default,
                                           Http::Protocol::Http11, nullptr);

  auto* mock_tcp_conn_pool = new Tcp::ConnectionPool::MockInstance();
  EXPECT_CALL(*mock_tcp_conn_pool, addIdlePoolTimeoutCallback);
