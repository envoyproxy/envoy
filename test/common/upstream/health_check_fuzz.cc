...
std::vector<uint8_t> convertToGrpcServingStatus(test::common::upstream::ServingStatus status) {
...
makeBufferListToRespondWith(test::common::upstream::GrpcRespondBytes grpc_respond_bytes) {
  std::vector<std::vector<uint8_t>> responseList;
  switch (grpc_respond_bytes.grpc_respond_bytes_selector_case()) {
  case test::common::upstream::GrpcRespondBytes::kStatus: { // TODO: Split structured responses?
    // Structured Response
    std::vector<uint8_t> response = convertToGrpcServingStatus(grpc_respond_bytes.status());
...
void GrpcHealthCheckFuzz::respond(test::common::upstream::GrpcRespond grpc_respond) {
  ...
  if (has_data && !test_sessions_[0]->interval_timer_->enabled_) {
    auto bufferList = makeBufferListToRespondWith(grpc_respond.grpc_respond_bytes());
