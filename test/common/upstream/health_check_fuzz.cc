...
// From unit tests
std::vector<uint8_t> GrpcHealthCheckFuzz::serializeResponse(
...
Buffer::OwnedImpl GrpcHealthCheckFuzz::makeBufferToRespondWith(
  ...
  case test::common::upstream::GrpcRespondBytes::kStatus: {
    ...
    case test::common::upstream::ServingStatus::SERVICE_UNKNOWN: {
      data = serializeResponse(grpc::health::v1::HealthCheckResponse::SERVICE_UNKNOWN);
      break;
    }
  ...
  case test::common::upstream::GrpcRespondBytes::kData: {
    ...
    ENVOY_LOG_MISC(trace,
                   "Responded with {}. Length (in bytes) = {}. These bytes were arbitrarily "
                   "generated and have no structure.",
...
void GrpcHealthCheckFuzz::respond(test::common::upstream::GrpcRespond grpc_respond) { //TODO: Between every single type of response, check if it called rpc complete. If it did, then you have to invoke the interval timer again. I think you could do this by simply checking if interval timer got enabled after every call
  ...
  if (grpc_respond.has_grpc_respond_bytes()) {
    ...
    case test::common::upstream::GrpcRespondBytes::kData: {
      if (grpc_respond.grpc_respond_bytes().data().length() != 0) {
        has_data = true;
      }
      break;
    }
