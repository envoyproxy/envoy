...
void GrpcHealthCheckFuzz::respondHeaders(test::common::upstream::GrpcRespondHeaders grpc_respond_headers) {
  ...
  //decodeHeaders(Http::ResponseHeaderMapPtr&& headers, bool end_stream) - same arguments as http, figure out good regression tests from unit tests
  //This will follow same flow as http - even the type of TestResponseHeaders
  std::unique_ptr<Http::TestResponseHeaderMapImpl> response_headers =
      std::make_unique<Http::TestResponseHeaderMapImpl>(
...
void GrpcHealthCheckFuzz::respondBytes(test::common::upstream::GrpcRespondBytes grpc_respond_bytes) {
  //TODO: Respond bytes arguments, and what it does logically
  //decodeData(Buffer::Instance& data, bool end_stream)
  //Two possible paths - arbitrarily generated bytes, a fuzzed response
  Buffer::OwnedImpl response;
...
void GrpcHealthCheckFuzz::respond(test::common::upstream::GrpcRespond grpc_respond) {
  ...
  switch (grpc_respond.grpc_respond_selector_case()) {
    ...
    case test::common::upstream::GrpcRespond::kGrpcRespondData: {
