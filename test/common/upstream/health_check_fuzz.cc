...
void GrpcHealthCheckFuzz::initialize(test::common::upstream::HealthCheckTestCase input) {
  ...
  ON_CALL(runtime_.snapshot_,
          getInteger("health_check.min_interval", _)) // TODO: Is this the same thing?
      .WillByDefault(testing::Return(45000));
...
// Note: the only thing that calls resetState() is onRpcComplete(), guarantees calling it

// Note: all three of these all end up calling onRpcComplete, so that codepath is taken care of
void GrpcHealthCheckFuzz::respondHeaders(
    test::common::upstream::GrpcRespondHeaders grpc_respond_headers) {
  // TODO: Respond headers arguments, and what it does logically
  // decodeHeaders(Http::ResponseHeaderMapPtr&& headers, bool end_stream) - same arguments as http,
  // figure out good regression tests from unit tests This will follow same flow as http - even the
  // type of TestResponseHeaders
  std::unique_ptr<Http::TestResponseHeaderMapImpl> response_headers =
      std::make_unique<Http::TestResponseHeaderMapImpl>(
  ...
  // When does this turn off client? This turns off client if reuse connection, or received no error
  // go away Standard if for reuse connection
...
void GrpcHealthCheckFuzz::respondBytes(
    test::common::upstream::GrpcRespondBytes grpc_respond_bytes) {
  // TODO: Two possible paths - arbitrarily generated bytes, a fuzzed response
  Buffer::OwnedImpl response;
  response.add(grpc_respond_bytes.data());
...
void GrpcHealthCheckFuzz::respondTrailers(
  ...
  std::unique_ptr<Http::TestResponseTrailerMapImpl> response_trailers =
      std::make_unique<Http::TestResponseTrailerMapImpl>(
          Fuzz::fromHeaders<Http::TestResponseTrailerMapImpl>(grpc_respond_headers.trailers(), {},
                                                              {}));
...
void GrpcHealthCheckFuzz::respond(test::common::upstream::GrpcRespond grpc_respond) {
  ...
  switch (grpc_respond.grpc_respond_selector_case()) {
  case test::common::upstream::GrpcRespond::kGrpcRespondHeaders: {
    respondHeaders(grpc_respond.grpc_respond_headers());
    break;
  }
  case test::common::upstream::GrpcRespond::kGrpcRespondBytes: {
    respondBytes(grpc_respond.grpc_respond_bytes());
    break;
  }
  case test::common::upstream::GrpcRespond::kGrpcRespondTrailers: {
    respondTrailers(grpc_respond.grpc_respond_trailers());
    break;
  }
  ...
  // All three types of responses call onRpcComplete(), which blows away client if reuse connection
  // is set to false or the health checker received a goaway fram with no error flag
  ...
  // I should honestly add log statements all around health_checker_impl.cc and figure out what
  // calls what Received_no_error_goaway_ gets set to true only when it calls onGoAway, and the go
  // away error code is no error. Note: logic interesting in ongoaway, closes client if doesnt set
...
void GrpcHealthCheckFuzz::raiseEvent(const Network::ConnectionEvent& event_type, bool last_action) {
  ...
  if (!last_action &&
      event_type !=
          Network::ConnectionEvent::Connected) { // Close events will always blow away the client
    ENVOY_LOG_MISC(trace, "Creating client and stream from close event");
    triggerIntervalTimer(
        true); // Interval timer is guaranteed to be enabled from a close event - calls
               // onResetStream which handles failure, turning interval timer on and timeout off
...
void GrpcHealthCheckFuzz::raiseGoAway(bool no_error) {
  if (no_error) {
    ...
    received_no_error_goaway_ =
        true; // Will cause other events to blow away client, because this is a "graceful" go away
  } else {
