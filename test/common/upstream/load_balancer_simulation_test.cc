...
TEST(DISABLED_LeastRequestLoadBalancerWeightTest, Weight) {
  ...
  host_set.updateHosts(updated_hosts, updated_hosts, std::make_shared<const HostVector>(),
                       updated_locality_hosts, updated_locality_hosts,
                       HostsPerLocalityImpl::empty(), {}, hosts, {}, absl::nullopt);
  ...
  void run(std::vector<uint32_t> originating_cluster, std::vector<uint32_t> all_destination_cluster,
    ...
    for (uint32_t i = 0; i < total_number_of_requests; ++i) {
      ...
      local_priority_set_->getOrCreateHostSet(0).updateHosts(
          ...
          per_zone_local_shared, per_zone_local_shared, HostsPerLocalityImpl::empty(), {},
