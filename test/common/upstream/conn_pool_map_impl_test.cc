...
TEST_F(ConnPoolMapImplTest, GetPoolHittingLimitIncrementsFailureCounter) {
  ...
  EXPECT_EQ(host_->cluster_.upstream_stats_->upstream_cx_pool_overflow_.value(), 1);
}
...
TEST_F(ConnPoolMapImplTest, GetPoolHittingLimitIncrementsFailureMultiple) {
  ...
  EXPECT_EQ(host_->cluster_.upstream_stats_->upstream_cx_pool_overflow_.value(), 3);
}
