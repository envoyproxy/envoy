...
TEST_F(OutlierDetectorImplTest, DestroyWithActive) {
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(0));
...
TEST_F(OutlierDetectorImplTest, BasicFlow5xxViaHttpCodes) {
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(9999));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
  EXPECT_FALSE(hosts_[0]->outlierDetector().lastUnejectionTime());

  // Interval that does bring the host back in.
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(30001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(40000));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::CONSECUTIVE_5XX, true));
  loadRq(hosts_[0], 1, 500);
...
TEST_F(OutlierDetectorImplTest, TimeoutWithHttpCode) {
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(50001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
...
TEST_F(OutlierDetectorImplTest, BasicFlowLocalOriginFailure) {
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(90002));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
...
TEST_F(OutlierDetectorImplTest, BasicFlowSuccessRateExternalOrigin) {
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(10000));
  ...
  // Interval that doesn't bring the host back in.
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(19999));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
  EXPECT_TRUE(hosts_[4]->healthFlagGet(Host::HealthFlag::FAILED_OUTLIER_CHECK));
  EXPECT_EQ(1UL, outlier_detection_ejections_active_.value());
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(60001));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
...
TEST_F(OutlierDetectorImplTest, EjectTimeBackoff) {
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(0));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::CONSECUTIVE_5XX, true));
  loadRq(hosts_[0], 5, 500);
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(20));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
  // Make sure that node stays ejected.
  EXPECT_TRUE(hosts_[0]->healthFlagGet(Host::HealthFlag::FAILED_OUTLIER_CHECK));
  EXPECT_EQ(1UL, outlier_detection_ejections_active_.value());
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(30));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(40));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
  EXPECT_TRUE(hosts_[0]->healthFlagGet(Host::HealthFlag::FAILED_OUTLIER_CHECK));
  EXPECT_EQ(1UL, outlier_detection_ejections_active_.value());
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(50));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
  EXPECT_TRUE(hosts_[0]->healthFlagGet(Host::HealthFlag::FAILED_OUTLIER_CHECK));
  EXPECT_EQ(1UL, outlier_detection_ejections_active_.value());
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(60));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(70));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();

  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(80));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(90));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();
  EXPECT_TRUE(hosts_[0]->healthFlagGet(Host::HealthFlag::FAILED_OUTLIER_CHECK));
  EXPECT_EQ(1UL, outlier_detection_ejections_active_.value());
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(100));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  ...
  for (auto i = 1; i <= 50; i++) {
    time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(100 + i * 10));
    EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
    interval_timer_->invokeCallback();
  }

  // Trigger ejection.
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(610));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::CONSECUTIVE_5XX, true));
  loadRq(hosts_[0], 5, 500);
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(620));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
...
TEST_F(OutlierDetectorImplTest, EjectTimeBackoffTimeBasedDetection) {
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(base_ejection_time * tick));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(base_ejection_time), _));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::FAILURE_PERCENTAGE, true));
...
TEST_F(OutlierDetectorImplTest, MaxEjectTime) {
  ...
  for (auto i = 1; i <= 30; i++) {
    ...
    for (auto j = 1; j < i; j++) {
      time_system_.setMonotonicTime(start_time_ + std::chrono::seconds(++eject_tick * 10));
      EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
      interval_timer_->invokeCallback();
      EXPECT_TRUE(hosts_[0]->healthFlagGet(Host::HealthFlag::FAILED_OUTLIER_CHECK));
      EXPECT_EQ(1UL, outlier_detection_ejections_active_.value());
...
detect_degraded_hosts: true
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(100001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(110002));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  interval_timer_->invokeCallback();

  // Degrade again - backoff was 1, will increment to 2, so use 2x base_ejection_time
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(120000));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::DEGRADED, true));
  hosts_[0]->outlierDetector().putResult(Result::ExtOriginRequestDegraded, 200);
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(180001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
...
detect_degraded_hosts: true
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(10001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(11000));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::DEGRADED, true));
  hosts_[0]->outlierDetector().putResult(Result::ExtOriginRequestDegraded, 200);

  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(31001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(32000));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::DEGRADED, true));
  hosts_[0]->outlierDetector().putResult(Result::ExtOriginRequestDegraded, 200);

  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(62001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(63000));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_, logEject(std::static_pointer_cast<const HostDescription>(hosts_[0]),
                                       _, envoy::data::cluster::v3::DEGRADED, true));
  hosts_[0]->outlierDetector().putResult(Result::ExtOriginRequestDegraded, 200);
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(93001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
...
detect_degraded_hosts: true
  ...
  time_system_.setMonotonicTime(start_time_ + std::chrono::milliseconds(33001));
  EXPECT_CALL(checker_, check(hosts_[0]));
  EXPECT_CALL(*event_logger_,
              logUneject(std::static_pointer_cast<const HostDescription>(hosts_[0])));
  EXPECT_CALL(*interval_timer_, enableTimer(std::chrono::milliseconds(10000), _));
