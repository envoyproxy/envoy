...
TEST_F(OriginalDstClusterTest, CleanupInterval) {
  ...
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).hosts().size());
...
TEST_F(OriginalDstClusterTest, NoContext) {
  ...
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).hostsPerLocality().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality().size());

  // No downstream connection => no host.
  {
    TestLoadBalancerContext lb_context(nullptr);
    OriginalDstCluster::LoadBalancer lb(cluster_->prioritySet().getHostSet(0), cluster_);
...
TEST_F(OriginalDstClusterTest, Membership) {
  ...
  HostConstSharedPtr host = lb.chooseHost(&lb_context);
  post_cb();
  auto cluster_hosts = cluster_->prioritySet().getHostSet(0).hosts();

  ASSERT_NE(host, nullptr);
  EXPECT_EQ(local_address, *host->address());

  EXPECT_EQ(1UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(1UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).hostsPerLocality().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality().size());

  EXPECT_EQ(host, cluster_->prioritySet().getHostSet(0).hosts()[0]);
  EXPECT_EQ(local_address, *cluster_->prioritySet().getHostSet(0).hosts()[0]->address());
  ...
  ASSERT_EQ(1UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(true, cluster_->prioritySet().getHostSet(0).hosts()[0]->used());
  EXPECT_CALL(*cleanup_timer_, enableTimer(_));
  cleanup_timer_->callback_();
  EXPECT_EQ(cluster_hosts,
            cluster_->prioritySet().getHostSet(0).hosts()); // hosts vector remains the same

  // host gets removed on the 2nd timeout.
  ASSERT_EQ(1UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(false, cluster_->prioritySet().getHostSet(0).hosts()[0]->used());

  EXPECT_CALL(*cleanup_timer_, enableTimer(_));
  EXPECT_CALL(membership_updated_, ready());
  cleanup_timer_->callback_();
  EXPECT_NE(cluster_hosts, cluster_->prioritySet().getHostSet(0).hosts()); // hosts vector changes

  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  cluster_hosts = cluster_->prioritySet().getHostSet(0).hosts();
  ...
  EXPECT_EQ(1UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(host3, cluster_->prioritySet().getHostSet(0).hosts()[0]);
}
...
TEST_F(OriginalDstClusterTest, Membership2) {
  ...
  EXPECT_EQ(2UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(2UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).hostsPerLocality().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality().size());

  EXPECT_EQ(host1, cluster_->prioritySet().getHostSet(0).hosts()[0]);
  EXPECT_EQ(local_address1, *cluster_->prioritySet().getHostSet(0).hosts()[0]->address());

  EXPECT_EQ(host2, cluster_->prioritySet().getHostSet(0).hosts()[1]);
  EXPECT_EQ(local_address2, *cluster_->prioritySet().getHostSet(0).hosts()[1]->address());

  auto cluster_hosts = cluster_->prioritySet().getHostSet(0).hosts();

  // Make hosts time out, no membership changes happen on the first timeout.
  ASSERT_EQ(2UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(true, cluster_->prioritySet().getHostSet(0).hosts()[0]->used());
  EXPECT_EQ(true, cluster_->prioritySet().getHostSet(0).hosts()[1]->used());
  EXPECT_CALL(*cleanup_timer_, enableTimer(_));
  cleanup_timer_->callback_();
  EXPECT_EQ(cluster_hosts,
            cluster_->prioritySet().getHostSet(0).hosts()); // hosts vector remains the same
  ...
  ASSERT_EQ(2UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(false, cluster_->prioritySet().getHostSet(0).hosts()[0]->used());
  EXPECT_EQ(false, cluster_->prioritySet().getHostSet(0).hosts()[1]->used());
...
TEST_F(OriginalDstClusterTest, MultipleClusters) {
  ...
  cluster_->prioritySet().getHostSet(0).addMemberUpdateCb(
      [&](uint32_t, const std::vector<HostSharedPtr>& added,
        ...
        HostVectorSharedPtr new_hosts(
            new std::vector<HostSharedPtr>(cluster_->prioritySet().getHostSet(0).hosts()));
  ...
  OriginalDstCluster::LoadBalancer lb1(cluster_->prioritySet().getHostSet(0), cluster_);
