...
TEST_P(LogicalDnsParamTest, ImmediateResolve) {
  ...
  EXPECT_EQ("foo.bar.com", cluster_->prioritySet().getHostSet(0).hosts()[0]->hostname());
  cluster_->prioritySet().getHostSet(0).hosts()[0]->healthChecker().setUnhealthy();
  tls_.shutdownThread();
}
...
TEST_F(LogicalDnsClusterTest, Basic) {
  ...
  EXPECT_EQ(cluster_->prioritySet().getHostSet(0).hosts()[0],
            cluster_->prioritySet().getHostSet(0).healthyHosts()[0]);
  HostSharedPtr logical_host = cluster_->prioritySet().getHostSet(0).hosts()[0];
  ...
  EXPECT_EQ(logical_host, cluster_->prioritySet().getHostSet(0).hosts()[0]);
  EXPECT_CALL(dispatcher_, createClientConnection_(
                               PointeesEq(Network::Utility::resolveUrl("tcp://127.0.0.1:443")), _))
      .WillOnce(Return(new NiceMock<Network::MockClientConnection>()));
  Host::CreateConnectionData data = logical_host->createConnection(dispatcher_);
  EXPECT_FALSE(data.host_description_->canary());
  EXPECT_EQ(&cluster_->prioritySet().getHostSet(0).hosts()[0]->cluster(),
            &data.host_description_->cluster());
  EXPECT_EQ(&cluster_->prioritySet().getHostSet(0).hosts()[0]->stats(),
