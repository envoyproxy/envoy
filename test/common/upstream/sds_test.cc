...
protected:
  ...
  HostSharedPtr findHost(const std::string& address) {
    for (HostSharedPtr host : cluster_->prioritySet().getHostSet(0).hosts()) {
  ...
  uint64_t numHealthy() {
    uint64_t healthy = 0;
    for (const HostSharedPtr& host : cluster_->prioritySet().getHostSet(0).hosts()) {
      if (host->healthy()) {
        healthy++;
      }
    }
...
TEST_F(SdsTest, NoHealthChecker) {
  ...
  cluster_->prioritySet().getHostSet(0).addMemberUpdateCb(
  ...
  EXPECT_EQ(13UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(13UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(13UL, cluster_->info()->stats().membership_healthy_.value());
  EXPECT_EQ(3UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality().size());
  EXPECT_EQ(4UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[0].size());
  EXPECT_EQ(5UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[1].size());
  EXPECT_EQ(4UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[2].size());
  ...
  EXPECT_EQ("", cluster_->prioritySet().getHostSet(0).hosts()[0]->hostname());
...
TEST_F(SdsTest, HealthChecker) {
  ...
  EXPECT_EQ("hash_5f3725fa79001155", cluster_->versionInfo());
  EXPECT_EQ(13UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(0UL, cluster_->info()->stats().membership_healthy_.value());
  EXPECT_EQ(0UL, numHealthy());
  EXPECT_EQ(3UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality().size());
  EXPECT_EQ(3UL, cluster_->prioritySet().getHostSet(0).hostsPerLocality().size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[0].size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[1].size());
  EXPECT_EQ(0UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[2].size());
  EXPECT_EQ(6860994315024339285U, cluster_->info()->stats().version_.value());
  ...
  for (size_t i = 1; i < cluster_->prioritySet().getHostSet(0).hosts().size(); i++) {
    cluster_->prioritySet().getHostSet(0).hosts()[i]->healthFlagClear(
        Host::HealthFlag::FAILED_ACTIVE_HC);
    health_checker->runCallbacks(cluster_->prioritySet().getHostSet(0).hosts()[i], true);
  }
  ...
  EXPECT_CALL(membership_updated_, ready());
  cluster_->prioritySet().getHostSet(0).hosts()[0]->healthFlagClear(
      Host::HealthFlag::FAILED_ACTIVE_HC);
  health_checker->runCallbacks(cluster_->prioritySet().getHostSet(0).hosts()[0], true);
  EXPECT_EQ("hash_5f3725fa79001155", cluster_->versionInfo());
  EXPECT_EQ(13UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(13UL, cluster_->info()->stats().membership_healthy_.value());
  EXPECT_EQ(3UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality().size());
  ...
  EXPECT_EQ(14UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(13UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(13UL, cluster_->info()->stats().membership_healthy_.value());
  ...
  EXPECT_EQ("hash_398882981ada7c10", cluster_->versionInfo());
  EXPECT_EQ(13UL, cluster_->prioritySet().getHostSet(0).hosts().size());
  EXPECT_EQ(12UL, cluster_->prioritySet().getHostSet(0).healthyHosts().size());
  EXPECT_EQ(12UL, cluster_->info()->stats().membership_healthy_.value());
  EXPECT_EQ(12UL, numHealthy());
  EXPECT_EQ(3UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality().size());
  EXPECT_EQ(3UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[0].size());
  EXPECT_EQ(5UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[1].size());
  EXPECT_EQ(4UL, cluster_->prioritySet().getHostSet(0).healthyHostsPerLocality()[2].size());
  EXPECT_EQ(4145707046791707664U, cluster_->info()->stats().version_.value());
