...
TEST_F(GrpcMuxImplTest, ResetStream) {
  ...
  grpc_mux_->grpcStreamTestOnly().onRemoteClose(Grpc::Status::GrpcStatus::Canceled, "");
...
TEST_F(GrpcMuxImplTest, TypeUrlMismatch) {
...

  {
    ...
    grpc_mux_->grpcStreamTestOnly().onReceiveMessage(std::move(response));
  }

  {
    ...
    grpc_mux_->grpcStreamTestOnly().onReceiveMessage(std::move(invalid_response));
  }
  expectSendMessage("foo", {}, "");
}
