...
public:
  ...
  void expectSendMessage(const std::string& type_url,
                         ...
                         const Protobuf::int32 error_code = Grpc::Status::GrpcStatusMapping::Ok,
...
TEST_F(GrpcMuxImplTest, ResetStream) {
  ...
  grpc_mux_->grpcStreamForTest().onRemoteClose(Grpc::Status::GrpcStatusMapping::Canceled, "");
...
TEST_F(GrpcMuxImplTest, TypeUrlMismatch) {
...

  {
    ...
    expectSendMessage(
        "foo", {"x", "y"}, "", false, "", Grpc::Status::GrpcStatusMapping::Internal,
