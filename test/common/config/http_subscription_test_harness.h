    ...
    EXPECT_CALL(cm_.async_client_, send_(_, _, _))
        .WillOnce(Invoke([this, cluster_names, version](Http::RequestMessagePtr& request,
          ...
          EXPECT_EQ(Http::Headers::get().ContentTypeValues.Json,
                    std::string(request->headers().getContentTypeValue()));
          EXPECT_EQ("eds_cluster", std::string(request->headers().getHostValue()));
          EXPECT_EQ("/v2/discovery:endpoints", std::string(request->headers().getPathValue()));
