...
#include "test/common/config/unified_delta_subscription_test_harness.h"
#include "test/common/config/unified_grpc_subscription_test_harness.h"
...
public:
  ...
  void initialize(std::chrono::milliseconds init_fetch_timeout = std::chrono::milliseconds(0)) {
    switch (GetParam()) {
    case SubscriptionType::Grpc:
      test_harness_ = std::make_unique<UnifiedMux::GrpcSubscriptionTestHarness>(init_fetch_timeout);
      break;
    case SubscriptionType::DeltaGrpc:
      test_harness_ =
          std::make_unique<UnifiedMux::DeltaSubscriptionTestHarness>(init_fetch_timeout);
      break;
    case SubscriptionType::Http:
