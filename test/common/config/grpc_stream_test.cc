...
TEST_F(GrpcStreamTest, SendMessage) {
  EXPECT_CALL(*async_client_, startRaw(_, _, _, _)).WillOnce(Return(&async_stream_));
  grpc_stream_.establishNewStream();
  envoy::api::v3alpha::DiscoveryRequest request;
...
TEST_F(GrpcStreamTest, ReceiveMessage) {
  envoy::api::v3alpha::DiscoveryResponse response_copy;
  response_copy.set_type_url("faketypeURL");
  auto response = std::make_unique<envoy::api::v3alpha::DiscoveryResponse>(response_copy);
  envoy::api::v3alpha::DiscoveryResponse received_message;
  EXPECT_CALL(callbacks_, onDiscoveryResponse(_))
      .WillOnce(
          [&received_message](std::unique_ptr<envoy::api::v3alpha::DiscoveryResponse>&& message) {
