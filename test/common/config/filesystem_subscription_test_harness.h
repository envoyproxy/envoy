...
public:
  ...
  void deliverConfigUpdate(const std::vector<std::string>& cluster_names,
    ...
    EXPECT_CALL(callbacks_, onConfigUpdate(RepeatedProtoEq(response_pb.resources()), version, control_plane))
  ...
  AssertionResult statsAre(uint32_t attempt, uint32_t success, uint32_t rejected, uint32_t failure,
                           ...
                           absl::string_view version_text, absl::string_view control_plane) override {
    ...
    return SubscriptionTestHarness::statsAre(attempt, success, rejected,
                                             ...
                                             update_time, version, version_text, control_plane);
  }
