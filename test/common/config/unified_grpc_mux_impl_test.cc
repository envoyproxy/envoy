...
public:
  ...
  void setup() {
    grpc_mux_ = std::make_unique<UnifiedMux::GrpcMuxSotw>(
  ...
  std::unique_ptr<UnifiedMux::GrpcMuxSotw> grpc_mux_;
...
TEST_F(GrpcMuxImplTest, ConfigUpdateWithAliases) {
  std::unique_ptr<UnifiedMux::GrpcMuxDelta> grpc_mux = std::make_unique<UnifiedMux::GrpcMuxDelta>(
      std::unique_ptr<Grpc::MockAsyncClient>(async_client_), dispatcher_,
      *Protobuf::DescriptorPool::generated_pool()->FindMethodByName(
          "envoy.service.discovery.v2.AggregatedDiscoveryService.StreamAggregatedResources"),
...
TEST_F(GrpcMuxImplTest, DynamicContextParameters) {
  ...
  grpc_mux_->addWatch("foo", {"x", "y"}, callbacks_, resource_decoder_,
                      std::chrono::milliseconds(0));
  grpc_mux_->addWatch("bar", {}, callbacks_, resource_decoder_, std::chrono::milliseconds(0));
  EXPECT_CALL(*async_client_, startRaw(_, _, _, _)).WillOnce(Return(&async_stream_));
