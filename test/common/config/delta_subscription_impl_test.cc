...
TEST_F(DeltaSubscriptionImplTest, PauseBlocksRequest) {
  startSubscription({"name1", "name2", "name3"});
  subscription_->pause();
...
TEST_F(DeltaSubscriptionImplTest, PauseQueuesAcks) {
  ...
  subscription_->pause();
  {
    ...
    std::string nonce = std::to_string(HashUtil::xxHash64("version1A"));
  ...
  }
  {
    ...
    std::string nonce = std::to_string(HashUtil::xxHash64("version2A"));
    message->set_nonce(nonce);
    nonce_acks_required_.push(nonce);
    subscription_->onDiscoveryResponse(std::move(message));
  }
