...
public:
  MockGrpcMuxFactory(absl::string_view name = "envoy.config_mux.grpc_mux_factory") : name_(name) {
    ON_CALL(*this, create(_, _, _, _, _, _, _, _, _, _, _))
        .WillByDefault(Invoke(
            [](std::shared_ptr<Grpc::RawAsyncClient>&&, std::shared_ptr<Grpc::RawAsyncClient>&&,
               ...
               OptRef<Config::XdsConfigTracker>,
               OptRef<Config::XdsResourcesDelegate>) -> std::shared_ptr<Config::GrpcMux> {
...
TEST_P(XdsManagerImplTest, AdsReplacementPrimaryOnly) {
  ...
  EXPECT_CALL(factory, create(_, _, _, _, _, _, _, _, _, _, _))
      .WillOnce(
...
public:
  ...
  void initialize(const std::string& bootstrap_yaml = "", bool enable_authority_a = false,
    ...
    if (enable_authority_a) {
      EXPECT_CALL(grpc_mux_factory_, create(_, _, _, _, _, _, _, _, _, _, _))
          .WillOnce(Invoke(
              [&](std::shared_ptr<Grpc::RawAsyncClient>&& primary_async_client,
