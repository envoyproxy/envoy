...
protected:
  void assertStreamInfoSize(StreamInfoImpl stream_info) {
    ASSERT_TRUE(sizeof(stream_info) == 800 || sizeof(stream_info) == 816 || sizeof(stream_info) == 824 || sizeof(stream_info) == 840 ||
...
TEST_F(StreamInfoImplTest, TimingTest) {
  ...
  info.upstreamTiming().onUpstreamHandshakeComplete(test_time_.timeSystem());
