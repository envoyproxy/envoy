  ...
  EXPECT_CALL(callbacks, encodeHeaders_(_, true))
      .WillOnce(Invoke([&](const HeaderMap& headers, bool) -> void {
        ...
        EXPECT_EQ(headers.GrpcStatus()->value().getStringView(),
                  std::to_string(enumToInt(Grpc::Status::GrpcStatusMapping::Unknown)));
  ...
  const std::string json = R"EOF(
{
  ...
  EXPECT_CALL(callbacks, encodeHeaders_(_, true))
      .WillOnce(Invoke([&](const HeaderMap& headers, bool) -> void {
        ...
        EXPECT_NE(headers.GrpcStatus(), nullptr);
        EXPECT_EQ(headers.GrpcStatus()->value().getStringView(),
                  std::to_string(enumToInt(Grpc::Status::GrpcStatusMapping::Unauthenticated)));
        EXPECT_NE(headers.GrpcMessage(), nullptr);
...
TEST(HttpUtility, RateLimitedGrpcStatus) {
  ...
  EXPECT_CALL(callbacks, encodeHeaders_(_, true))
      .WillOnce(Invoke([&](const HeaderMap& headers, bool) -> void {
        EXPECT_NE(headers.GrpcStatus(), nullptr);
        EXPECT_EQ(headers.GrpcStatus()->value().getStringView(),
                  std::to_string(enumToInt(Grpc::Status::GrpcStatusMapping::Unavailable)));
      }));
  ...
  EXPECT_CALL(callbacks, encodeHeaders_(_, true))
      .WillOnce(Invoke([&](const HeaderMap& headers, bool) -> void {
        EXPECT_NE(headers.GrpcStatus(), nullptr);
        EXPECT_EQ(headers.GrpcStatus()->value().getStringView(),
                  std::to_string(enumToInt(Grpc::Status::GrpcStatusMapping::ResourceExhausted)));
      }));
  Utility::sendLocalReply(true, callbacks, false, Http::Code::TooManyRequests, "",
                          absl::make_optional<Grpc::Status::GrpcStatus>(
                              Grpc::Status::GrpcStatusMapping::ResourceExhausted),
                          false);
}
