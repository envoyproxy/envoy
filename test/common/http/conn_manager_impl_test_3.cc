...
TEST_F(HttpConnectionManagerImplTest, HeaderValidatorRejectTrailersBeforeResponseHttp2) {
  // Disable the new behavior so this test continues to exercise the legacy stream-reset path.
  TestScopedRuntime scoped_runtime;
  scoped_runtime.mergeValues(
      {{"envoy.reloadable_features.http2_h3_send_400_for_underscored_headers", "false"}});
