...
DEFINE_PROTO_FUZZER(const test::common::http::ConnManagerImplTestCase& input) {
  ...
  ConnectionManagerImpl conn_manager(config, drain_close, random, http_context,
                                     Runtime::LoaderSingleton::get(), local_info, cluster_manager,
                                     nullptr, config.time_system_);
