...
// Test the fine-grained buffer write helper.
TEST(FineGrainedBufferWriteHelperTest, FineGrainedBufferWriteHelperTest) {
  {
    Buffer::OwnedImpl buffer;
    Http1::FineGrainedBufferWriteHelper helper(buffer);

    // No reservation and do nothing.
    helper.commitToBuffer();

    // The minimum reserved size is 4096, so 4096 bytes of space will eventually be reserved.
    helper.reserveBuffer(256);
    EXPECT_EQ(4096, helper.remainingForTest());
    // There is enough space and do nothing.
    helper.reserveBuffer(256);
    EXPECT_EQ(4096, helper.remainingForTest());

    // Write 4096 bytes to buffer safely.
    for (size_t i = 0; i < 4096; i++) {
      helper.writeToBuffer('c');
    }

    // No any byte has been committed until now.
    EXPECT_EQ(0, buffer.length());
    EXPECT_EQ(0, helper.remainingForTest());

    // No enough space then commit buffer and reserved some new.
    helper.reserveBuffer(1);
    EXPECT_EQ(4096, buffer.length());
    EXPECT_EQ(4096, helper.remainingForTest());
  }

  {
    ...
    helper.reserveBuffer(8192);
    EXPECT_EQ(8192, helper.remainingForTest());
    ...
    helper.writeToBuffer(data.data(), 1024);
    helper.writeToBuffer(data);

    // No any byte has been submitted until now.
    EXPECT_EQ(0, buffer.length());
    EXPECT_EQ(2048, helper.remainingForTest());

    // Commit buffer directly.
    helper.commitToBuffer();

    EXPECT_EQ(4096 + 2048, buffer.length());
  }

  {
    ...
    helper.reserveBuffer(4096);
    EXPECT_EQ(4096, helper.remainingForTest());

    helper.reserveBuffer(8192);
    EXPECT_EQ(8192, helper.remainingForTest());

    // No any byte be committed to buffer.
    EXPECT_EQ(0, buffer.length());
  }
  {
    Buffer::OwnedImpl buffer;
    ...
    std::string data(1024, 'c');
    helper.writeToBuffer(data.data(), 1024);
    EXPECT_EQ(1024 * 3, helper.remainingForTest());
