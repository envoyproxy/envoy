...
protected:
  Http1CodecTestBase() : http1_codec_stats_{HTTP1_CODEC_STATS(store_)} {}
...
public:
  void initialize() {
    codec_ = std::make_unique<ServerConnectionImpl>(
        connection_, http1_codec_stats_, callbacks_, codec_settings_, max_request_headers_kb_,
...
public:
  void initialize() {
    codec_ = std::make_unique<ClientConnectionImpl>(connection_, http1_codec_stats_, callbacks_,
