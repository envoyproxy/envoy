...
void Http1ServerConnectionImplTest::testTrailersExceedLimit(std::string trailer_string,
  ...
  if (enable_trailers) {
    if (many) {
      ...
      EXPECT_THAT(status.message(), testing::HasSubstr("trailers size exceeds limit"));
    } else {
      EXPECT_THROW_WITH_MESSAGE(
          ...
          EnvoyException, "trailers size exceeds limit");
    }
  } else {
...
void Http1ServerConnectionImplTest::testRequestHeadersExceedLimit(std::string header_string,
  ...
  if (many) {
    auto status = codec_->dispatch(buffer);
    EXPECT_FALSE(status.ok());
    EXPECT_THAT(status.message(), testing::HasSubstr("headers size exceeds limit"));
  } else {
    EXPECT_THROW_WITH_MESSAGE(status = codec_->dispatch(buffer), EnvoyException,
...
TEST_F(Http1ServerConnectionImplTest, IdentityEncodingNoChunked) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
      ...
      CodecProtocolException, "http/1.1 protocol error: unsupported transfer encoding");
}
...
TEST_F(Http1ServerConnectionImplTest, InvalidChunkHeader) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
      ...
      CodecProtocolException, "http/1.1 protocol error: HPE_INVALID_CHUNK_SIZE");
}
...
TEST_F(Http1ServerConnectionImplTest, FloodProtection) {
  ...
  // protection.
  {
    ...
    EXPECT_THROW_WITH_MESSAGE(
        ...
        FrameFloodException, "Too many responses queued.");
...
TEST_F(Http1ServerConnectionImplTest, HeaderNameWithUnderscoreCauseRequestRejected) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
      ...
      EnvoyException, "http/1.1 protocol error: header name contains underscores");
  EXPECT_EQ("http1.invalid_characters", response_encoder->getStream().responseDetails());
...
TEST_F(Http1ServerConnectionImplTest, HeaderEmbeddedNulRejection) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
      ...
      CodecProtocolException, "http/1.1 protocol error: HPE_INVALID_HEADER_TOKEN");
}
...
TEST_F(Http1ServerConnectionImplTest, HeaderMutateEmbeddedNul) {
  ...
  for (size_t n = 1; n < example_input.size(); ++n) {
    ...
    EXPECT_THROW_WITH_REGEX(
        ...
        CodecProtocolException, "http/1.1 protocol error:");
  }
}
...
TEST_F(Http1ServerConnectionImplTest, LargeRequestHeadersSplitRejected) {
  ...
  buffer = Buffer::OwnedImpl(fmt::format("big: {}\r\n", long_string));
  EXPECT_THROW_WITH_MESSAGE(status = status = codec_->dispatch(buffer), EnvoyException,
                            "headers size exceeds limit");
...
TEST_F(Http1ServerConnectionImplTest, ManyRequestHeadersSplitRejected) {
  ...
  status = codec_->dispatch(buffer);
  EXPECT_FALSE(status.ok());
  EXPECT_EQ("headers size exceeds limit", status.message());
}
