...
protected:
  ...
  void verifyImplementation(const Parser& parser) {
    switch (GetParam()) {
    case CodecImplementation::Old:
      ASSERT_TRUE(parser.usesOldImpl());
      break;
    case CodecImplementation::New:
      ASSERT_FALSE(parser.usesOldImpl());
      break;
    }
  }
};

class Http1ServerConnectionImplTest : public Http1CodecParamTest {
public:
  ~Http1ServerConnectionImplTest() override = default;
  void initialize() {
    codec_ =
...
INSTANTIATE_TEST_SUITE_P(Http1ServerConnectionImplTest, Http1ServerConnectionImplTest,
                         testing::ValuesIn({CodecImplementation::Old, CodecImplementation::New}));
...
TEST_P(Http1ServerConnectionImplTest, RequestWithTrailers) {
  initialize();
...
class Http1ClientConnectionImplTest : public Http1CodecParamTest {
public:
  ~Http1ClientConnectionImplTest() override = default;
  void initialize() {
