...
class Http1CodecTestBase : public testing::TestWithParam<Http::CodecHeaderValidationMode> {
protected:
...
public:
  void initialize() {
    codec_ = std::make_unique<Http1::ServerConnectionImpl>(
        ...
        max_request_headers_count_, headers_with_underscores_action_, GetParam());
  }
...
public:
  void initialize() {
    codec_ = std::make_unique<Http1::ClientConnectionImpl>(connection_, http1CodecStats(),
                                                           ...
                                                           max_response_headers_count_, GetParam());
  }
...
TEST_P(Http1ServerConnectionImplTest, Utf8Path) {
  ...
  if (GetParam() == Http::CodecHeaderValidationMode::Disabled) {
    // permissive
    EXPECT_CALL(callbacks_, newStream(_, _)).WillOnce(ReturnRef(decoder));
...
INSTANTIATE_TEST_CASE_P(Http1ServerTests, Http1ServerConnectionImplTest,
                        testing::Values(Http::CodecHeaderValidationMode::Enabled,
                                        Http::CodecHeaderValidationMode::Disabled));
...
INSTANTIATE_TEST_CASE_P(Http1ClientTests, Http1ClientConnectionImplTest,
                        testing::Values(Http::CodecHeaderValidationMode::Enabled,
                                        Http::CodecHeaderValidationMode::Disabled));
