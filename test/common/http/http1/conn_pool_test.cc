...
struct ActiveTestRequest {
  ...
  ActiveTestRequest(Http1ConnPoolImplTest& parent, size_t client_index, Type type)
    ...
    uint64_t current_rq_total = parent_.cluster_->trafficStats()->upstream_rq_total_.value();
    ...
    if (type != Type::Pending) {
      EXPECT_EQ(current_rq_total + 1, parent_.cluster_->trafficStats()->upstream_rq_total_.value());
...
TEST_F(Http1ConnPoolImplTest, ConnectTimeout) {
  ...
  EXPECT_EQ(0U, cluster_->trafficStats()->upstream_rq_total_.value());
...
TEST_F(Http1ConnPoolImplTest, ConcurrentConnections) {
  ...
  conn_pool_->expectAndRunUpstreamReady();
  r3.startRequest();
  EXPECT_EQ(3U, cluster_->trafficStats()->upstream_rq_total_.value());
