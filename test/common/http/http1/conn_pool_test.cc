...
struct ActiveTestRequest {
  ...
  ActiveTestRequest(Http1ConnPoolImplTest& parent, size_t client_index, Type type)
    ...
    uint64_t active_rq_observed = parent_.cluster_->resourceManager(Upstream::ResourcePriority::Default).requests().count();
    ...
    if (type != Type::Pending) {
      ...
      EXPECT_EQ(active_rq_observed + 1, parent_.cluster_->resourceManager(Upstream::ResourcePriority::Default).requests().count());
    }
