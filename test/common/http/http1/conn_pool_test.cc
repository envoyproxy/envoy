...
struct ActiveTestRequest {
  ...
  void startRequest() {
    EXPECT_OK(callbacks_.outer_encoder_->encodeHeaders(
...
TEST_F(Http1ConnPoolImplTest, DisconnectWhileBound) {
  ...
  EXPECT_CALL(stream_callbacks, onResetStream(StreamResetReason::RemoteConnectionTermination, _));
...
TEST_F(Http1ConnPoolImplTest, MaxConnections) {
  ...
  EXPECT_OK(callbacks.outer_encoder_->encodeHeaders(
      TestRequestHeaderMapImpl{{":path", "/"}, {":method", "GET"}}, true));
  ...
  conn_pool_->expectAndRunUpstreamReady();
  conn_pool_->expectEnableUpstreamReady();
  EXPECT_OK(callbacks2.outer_encoder_->encodeHeaders(
      TestRequestHeaderMapImpl{{":path", "/"}, {":method", "GET"}}, true));
