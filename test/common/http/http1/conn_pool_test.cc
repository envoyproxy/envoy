...
struct ActiveTestRequest {
  ...
  ActiveTestRequest(Http1ConnPoolImplTest& parent, size_t client_index, Type type)
    ...
    uint64_t active_rq_observed = parent_.cluster_->resourceManager(parent_.priority_).requests().count();
    ...
    EXPECT_EQ(active_rq_observed + 1, parent_.cluster_->resourceManager(parent_.priority_).requests().count());
