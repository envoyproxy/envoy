...
TEST_F(Http2ConnPoolImplTest, MaxConcurrentRequestsPerStream) {
  ...
  EXPECT_EQ(2U, cluster_->upstream_stats_->upstream_cx_total_.value());
}
...
TEST_F(Http2ConnPoolImplTest, RequestAndResponse) {
  ...
  EXPECT_TRUE(
      r1.callbacks_.outer_encoder_
          ->encodeHeaders(TestRequestHeaderMapImpl{{":path", "/"}, {":method", "GET"}}, true)
          .ok());
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_active_.value());
  EXPECT_CALL(r1.decoder_, decodeHeaders_(_, true));
  r1.inner_decoder_->decodeHeaders(
      ResponseHeaderMapPtr{new TestResponseHeaderMapImpl{{":status", "200"}}}, true);
  ...
  EXPECT_EQ(0U, cluster_->upstream_stats_->upstream_cx_active_.value());
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_destroy_.value());
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_destroy_remote_.value());
}
...
TEST_F(Http2ConnPoolImplTest, LocalReset) {
  ...
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_destroy_.value());
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_destroy_remote_.value());
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_rq_tx_reset_.value());
...
TEST_F(Http2ConnPoolImplTest, RemoteReset) {
  ...
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_destroy_.value());
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_destroy_remote_.value());
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_rq_rx_reset_.value());
  EXPECT_EQ(0U, cluster_->circuit_breakers_stats_.rq_open_.value());
  EXPECT_EQ(0U, cluster_->upstream_stats_->upstream_cx_active_.value());
}
...
TEST_F(Http2ConnPoolImplTest, ConnectTimeout) {
  ...
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_connect_timeout_.value());
...
TEST_F(Http2ConnPoolImplTest, GoAway) {
  ...
  EXPECT_EQ(1U, cluster_->upstream_stats_->upstream_cx_close_notify_.value());
}
