...
public:
  ...
  void initialize() {
    ...
    ON_CALL(*mockHost().cluster_.upstream_local_address_selector_, getUpstreamLocalAddress(_, _)).WillByDefault(Invoke([](const Network::Address::InstanceConstSharedPtr&,
      const Network::ConnectionSocket::OptionsSharedPtr& socket_options) -> Upstream::UpstreamLocalAddress {
...
TEST_F(Http3ConnPoolImplTest, CreationAndNewStream) {
  ...
  EXPECT_CALL(*mockHost().cluster_.upstream_local_address_selector_, getUpstreamLocalAddress(_, _)).WillOnce(Invoke([&](const Network::Address::InstanceConstSharedPtr&,
      const Network::ConnectionSocket::OptionsSharedPtr& socket_options) -> Upstream::UpstreamLocalAddress {
        Network::ConnectionSocket::OptionsSharedPtr options =
