...
TEST_F(HttpConnectionManagerImplTest, RouteShouldUseSantizedPath) {
  ...
  EXPECT_CALL(*route_config_provider_.route_config_, route(_, _, _, _))
      .WillOnce(Invoke([&](const Http::HeaderMap& header_map, const StreamInfo::StreamInfo&,
                           uint64_t, uint32_t) {
        EXPECT_EQ(normalized_path, header_map.Path()->value().getStringView());
        return route;
      }));
...
TEST_F(HttpConnectionManagerImplTest, FilterClearRouteCache) {
  ...
  EXPECT_CALL(route1->route_entry_, fallthru()).WillRepeatedly(Return(false));
  ...
  EXPECT_CALL(route2->route_entry_, fallthru()).WillRepeatedly(Return(false));
