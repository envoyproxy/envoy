...
public:
  ...
  void SetUpTest(const std::string json) {
    ...
    Config::FilterJson::translateHttpExtAuthzFilter(*json_config, proto_config);
    ...
    crg_ = new NiceMock<Envoy::ExtAuthz::MockCheckRequestGen>();
    filter_->setCheckReqGen(crg_);
  }

  const std::string filter_config_ = R"EOF(
    {
      "grpc_cluster": { "cluster_name": "ext_authz_server" },
  ...
  NiceMock<Envoy::ExtAuthz::MockCheckRequestGen>* crg_;
};
...
TEST_F(HttpExtAuthzFilterTest, BadConfig) {
  ...
  envoy::api::v2::filter::http::ExtAuthz proto_config{};
  EXPECT_THROW(Config::FilterJson::translateHttpExtAuthzFilter(*json_config, proto_config),
               Json::Exception);
}
...
TEST_F(HttpExtAuthzFilterTest, OkResponse) {
  ...
  EXPECT_CALL(*crg_, createHttpCheck(_, _, _));
