...
TEST(HeaderStringTest, All) {
    ...
    // Inline trim, only whitespace
  {
    ...
    HeaderString string;
    string.append(only_lws.data(), only_lws.size());
    EXPECT_EQ(only_lws, string.getStringView());
    string.trim();
    EXPECT_NE(only_lws, string.getStringView());
    EXPECT_TRUE(string.getStringView().empty());
  }

  // Inline trim, no leading whitespace
  {
  ...
  // Inline trim, both leading and trailing whitespace
  {
    const std::string data_without_leading_and_trailing_lws = "data";
