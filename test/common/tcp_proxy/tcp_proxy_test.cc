...
TEST_P(TcpProxyTest, UpstreamRemoteCloseWithRstPropagation) {
  ...
  // Simulate upstream RST: set detected close type to RemoteReset on the upstream connection.
...
TEST_P(TcpProxyTest, UpstreamRemoteCloseNormalCloseTypeUsesFin) {
  ...
  // RemoteClose but detected close type is Normal (graceful FIN, not RST).
  EXPECT_CALL(filter_callbacks_.connection_, close(Network::ConnectionCloseType::FlushWrite));
  upstream_callbacks_->onEvent(Network::ConnectionEvent::RemoteClose);
}
