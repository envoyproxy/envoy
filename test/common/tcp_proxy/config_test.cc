...
TEST(ConfigTest, WeightedClustersConfig) {
  ...
  EXPECT_CALL(factory_context.mock_server_context_.api_.random_, random()).WillOnce(Return(0));
  EXPECT_EQ(std::string("cluster1"), config_obj.getRouteFromEntries(connection)->clusterName());

  EXPECT_CALL(factory_context.mock_server_context_.api_.random_, random()).WillOnce(Return(2));
...
public:
  ...
  Event::TestTimeSystem& timeSystem() { return factory_context_.mock_server_context_.timeSystem(); }
