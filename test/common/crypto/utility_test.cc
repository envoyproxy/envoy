...
public:
  UtilityTest() { utility_ = Envoy::Common::Crypto::UtilitySingleton::get(); }

  Envoy::Common::Crypto::Utility utility_;
};

TEST_F(UtilityTest, TestSha256Digest) {
  const Buffer::OwnedImpl buffer("test data");
  const auto digest = utility_.getSha256Digest(buffer);
...
TEST_F(UtilityTest, TestSha256DigestGrowingBuffer) {
  ...
  auto digest = utility_.getSha256Digest(buffer);
  EXPECT_EQ("76571770bb46bdf51e1aba95b23c681fda27f6ae56a8a90898a4cb7556e19dcb",
            Hex::encode(digest));
  buffer.add("slice 2");
  digest = utility_.getSha256Digest(buffer);
...
TEST_F(UtilityTest, TestSha256Hmac) {
  ...
  auto hmac = utility_.getSha256Hmac(std::vector<uint8_t>(key.begin(), key.end()), "test data");
...
TEST_F(UtilityTest, TestVerifySignature) {
  auto key = "30820122300d06092a864886f70d01010105000382010f003082010a0282010100a7471266d01d160308d"
             "73409c06f2e8d35c531c458d3e480e9f3191847d062ec5ccff7bc51e949d5f2c3540c189a4eca1e8633a6"
             "2cf2d0923101c27e38013e71de9ae91a704849bff7fbe2ce5bf4bd666fd9731102a53193fe5a9a5a50644"
             "ff8b1183fa897646598caad22a37f9544510836372b44c58c98586fb7144629cd8c9479592d996d32ff6d"
  ...
  auto result = utility_.verifySignature(hash_func, *crypto, sig, text);

  EXPECT_EQ(true, result.result_);
  EXPECT_EQ("", result.error_message_);

  result = utility_.verifySignature("unknown", *crypto, sig, text);
  ...
  result = utility_.verifySignature(hash_func, *empty_crypto, sig, text);
  EXPECT_EQ(false, result.result_);
  EXPECT_EQ("Failed to initialize digest verify.", result.error_message_);

  data = "baddata";
  text = std::vector<uint8_t>(data, data + strlen(data));
  result = utility_.verifySignature(hash_func, *crypto, sig, text);
  EXPECT_EQ(false, result.result_);
  EXPECT_EQ("Failed to verify digest. Error code: 0", result.error_message_);

  data = "hello";
  text = std::vector<uint8_t>(data, data + strlen(data));
  result = utility_.verifySignature(hash_func, *crypto, Hex::decode("000000"), text);
  EXPECT_EQ(false, result.result_);
  EXPECT_EQ("Failed to verify digest. Error code: 0", result.error_message_);
}
