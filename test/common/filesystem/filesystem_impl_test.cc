...
TEST_F(FileSystemImplTest, OpenFifoNonBlock) {
  const std::string fifo_path = TestEnvironment::temporaryPath("fifo_reader");
  ::mkfifo(fifo_path.c_str(), 0666);
  FilePathAndType file_info{Filesystem::DestinationType::File, fifo_path};
  FilePtr reader_file = file_system_.createFile(file_info);
  FilePtr writer_file = file_system_.createFile(file_info);
  {
    const Api::IoCallBoolResult open_result = reader_file->open(FlagSet{
        (1 << Filesystem::File::Operation::Read) | (1 << Filesystem::File::Operation::NonBlock)});
    EXPECT_TRUE(open_result.return_value_) << open_result.err_->getErrorDetails();
  }

  {
    const Api::IoCallBoolResult open_result = writer_file->open(FlagSet{
        (1 << Filesystem::File::Operation::Write) | (1 << Filesystem::File::Operation::NonBlock)});
    EXPECT_TRUE(open_result.return_value_) << open_result.err_->getErrorDetails();
  }

  const std::string data = "hello!";
  const Api::IoCallSizeResult result = writer_file->write(data);
