...
std::string Base64Legacy::decode(absl::string_view input) {
  if (input.length() % 4) {
    return EMPTY_STRING;
  }
...
std::string Base64Legacy::decodeWithoutPadding(absl::string_view input) {
  if (input.empty()) {
    return EMPTY_STRING;
  }
...
std::string Base64Legacy::encode(const Buffer::Instance& buffer, uint64_t length) {
...
std::string Base64Legacy::encode(absl::string_view input) { return encode(input.data(), input.length()); }

std::string Base64Legacy::encode(const char* input, uint64_t length) {
  return encode(input, length, true);
}

std::string Base64Legacy::encode(const char* input, uint64_t length, bool add_padding) {
...
void Base64Legacy::completePadding(std::string& encoded) {
...
std::string Base64UrlLegacy::decode(absl::string_view input) {
  if (input.empty()) {
    return EMPTY_STRING;
  }
...
std::string Base64UrlLegacy::encode(const char* input, uint64_t length) {
  uint64_t output_length = (length + 2) / 3 * 4;
  std::string ret;
  ret.reserve(output_length);
