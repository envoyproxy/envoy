...
namespace Envoy {
using HexTest = TestBase;

TEST_F(HexTest, SimpleEncode) {
...
TEST_F(HexTest, BadHex) { EXPECT_EQ(0, Hex::decode("abcde").size()); }

TEST_F(HexTest, DecodeUppercase) { EXPECT_EQ(4, Hex::decode("ABCDEFAB").size()); }

TEST_F(HexTest, UIntToHex) {
