...
TEST_F(TokenBucketImplTest, Initialization) {
  TokenBucketImpl token_bucket{1, mock_time_source_, -1.0};

  EXPECT_TRUE(token_bucket.consume());
  EXPECT_CALL(mock_time_source_, monotonicTime()).WillOnce(Return(time_point{}));
  EXPECT_FALSE(token_bucket.consume());
}

// Verifies TokenBucket's maximum capacity.
TEST_F(TokenBucketImplTest, MaxBucketSize) {
  TokenBucketImpl token_bucket{3, mock_time_source_, 1};
...
TEST_F(TokenBucketImplTest, ConsumeAndRefill) {
  {
    TokenBucketImpl token_bucket{10, mock_time_source_, 1};
  ...
  ASSERT_TRUE(Mock::VerifyAndClear(&mock_time_source_));

  {
    TokenBucketImpl token_bucket{1, mock_time_source_, 0.5};
    EXPECT_TRUE(token_bucket.consume());
