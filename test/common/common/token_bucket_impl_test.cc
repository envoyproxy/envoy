...
TEST_F(TokenBucketImplTest, ConsumeAndRefill) {
  {
    TokenBucketImpl token_bucket{10, mock_time_system_, 1};
  ...
  ASSERT_TRUE(Mock::VerifyAndClear(&mock_time_system_));

  {
    TokenBucketImpl token_bucket{1, mock_time_system_, 0.5};
    EXPECT_TRUE(token_bucket.consume());
