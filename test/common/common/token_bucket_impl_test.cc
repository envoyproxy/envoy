...
public:
  TokenBucketImplTest() : time_source_(mock_system_time_, mock_monotonic_time_) {}
  using time_point = std::chrono::steady_clock::time_point;

protected:
  NiceMock<MockSystemTimeSource> mock_system_time_; // not actually used in the test.
...
TEST_F(TokenBucketImplTest, Initialization) {
  TokenBucketImpl token_bucket{1, time_source_, -1.0};

  EXPECT_TRUE(token_bucket.consume());
  EXPECT_CALL(mock_monotonic_time_, currentTime()).WillOnce(Return(time_point{}));
  EXPECT_FALSE(token_bucket.consume());
}

// Verifies TokenBucket's maximum capacity.
TEST_F(TokenBucketImplTest, MaxBucketSize) {
  TokenBucketImpl token_bucket{3, time_source_, 1};
...
TEST_F(TokenBucketImplTest, ConsumeAndRefill) {
  {
    TokenBucketImpl token_bucket{10, time_source_, 1};
  ...
  ASSERT_TRUE(Mock::VerifyAndClear(&mock_monotonic_time_));

  {
    TokenBucketImpl token_bucket{1, time_source_, 0.5};
    EXPECT_TRUE(token_bucket.consume());
