...
static void typedBmTrieLookups(benchmark::State& state, std::vector<std::string>& keys) {
  ...
  for (auto _ : state) {
    ...
    auto v = trie.Get(keys[key_selections[key_index++]]);
...
static void bmTrieHybridLookups(benchmark::State& s) {
  typedBmTrieLookups<TrieHybrid<std::string, const void*>>(s);
}

#define ADD_HEADER_TO_KEYS(name) keys.emplace_back(Http::Headers::get().name);
static void bmTrieHybridLookupsRequestHeaders(benchmark::State& s) {
  std::vector<std::string> keys;
  INLINE_REQ_HEADERS(ADD_HEADER_TO_KEYS);
  typedBmTrieLookups<TrieHybrid<std::string, const void*>>(s, keys);
}
static void bmTrieHybridLookupsResponseHeaders(benchmark::State& s) {
  std::vector<std::string> keys;
  INLINE_RESP_HEADERS(ADD_HEADER_TO_KEYS);
  typedBmTrieLookups<TrieHybrid<std::string, const void*>>(s, keys);
}

BENCHMARK(bmTrieHybridLookupsRequestHeaders);
BENCHMARK(bmTrieHybridLookupsResponseHeaders);
BENCHMARK(bmTrieHybridLookups)->ArgsProduct({{10, 100, 1000, 10000}, {0, 8, 128}});
