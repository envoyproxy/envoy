...
TEST_P(GrpcClientIntegrationTest, BasicStream) {
  ...
  stream->sendReply();
  stream->sendServerTrailers(Status::GrpcStatusMapping::Ok, "", empty_metadata_);
...
TEST_P(GrpcClientIntegrationTest, MultiStream) {
  ...
  stream_0->sendReply();
  stream_1->sendServerTrailers(Status::GrpcStatusMapping::Unavailable, "", empty_metadata_, true);
  stream_0->sendServerTrailers(Status::GrpcStatusMapping::Ok, "", empty_metadata_);
  dispatcher_helper_.runDispatcher();
}
...
TEST_P(GrpcClientIntegrationTest, HttpNon200Status) {
  ...
  for (const auto http_response_status : {400, 401, 403, 404, 429, 431}) {
    ...
    stream->expectGrpcStatus(Status::GrpcStatusMapping::Canceled);
...
TEST_P(GrpcClientIntegrationTest, GrpcStatusFallback) {
  ...
  const Http::TestHeaderMapImpl reply_headers{
      {":status", "404"},
      {"grpc-status", std::to_string(enumToInt(Status::GrpcStatusMapping::PermissionDenied))},
      {"grpc-message", "error message"}};
  stream->expectInitialMetadata(empty_metadata_);
  stream->expectTrailingMetadata(empty_metadata_);
  stream->expectGrpcStatus(Status::GrpcStatusMapping::PermissionDenied);
  stream->fake_stream_->encodeHeaders(reply_headers, true);
  dispatcher_helper_.runDispatcher();
}
...
TEST_P(GrpcClientIntegrationTest, HttpReset) {
  ...
  dispatcher_helper_.runDispatcher();
  stream->expectTrailingMetadata(empty_metadata_);
  stream->expectGrpcStatus(Status::GrpcStatusMapping::Internal);
...
TEST_P(GrpcClientIntegrationTest, OutOfRangeGrpcStatus) {
  ...
  dispatcher_helper_.setStreamEventPending();
  stream->expectGrpcStatus(Status::GrpcStatusMapping::InvalidCode);
...
TEST_P(GrpcClientIntegrationTest, MissingGrpcStatus) {
  ...
  EXPECT_CALL(*stream, onReceiveTrailingMetadata_(_)).WillExitIfNeeded();
  dispatcher_helper_.setStreamEventPending();
  stream->expectGrpcStatus(Status::GrpcStatusMapping::Unknown);
...
TEST_P(GrpcClientIntegrationTest, StreamClientInitialMetadata) {
  ...
  stream->sendServerTrailers(Status::GrpcStatusMapping::Ok, "", empty_metadata_, true);
  dispatcher_helper_.runDispatcher();
}
...
TEST_P(GrpcClientIntegrationTest, ServerTrailingMetadata) {
  ...
  stream->sendServerTrailers(Status::GrpcStatusMapping::Ok, "", trailing_metadata);
  dispatcher_helper_.runDispatcher();
}
...
TEST_P(GrpcClientIntegrationTest, ResourceExhaustedError) {
  ...
  stream->sendReply();
  dispatcher_helper_.runDispatcher();
  stream->sendServerTrailers(Status::GrpcStatusMapping::ResourceExhausted, "error message",
                             empty_metadata_);
  dispatcher_helper_.runDispatcher();
}
...
TEST_P(GrpcClientIntegrationTest, UnauthenticatedError) {
  ...
  auto stream = createStream(empty_metadata_);
  stream->sendServerInitialMetadata(empty_metadata_);
  stream->sendServerTrailers(Status::GrpcStatusMapping::Unauthenticated, "error message",
                             empty_metadata_);
  dispatcher_helper_.runDispatcher();
}
...
TEST_P(GrpcClientIntegrationTest, MaximumValidPlusOne) {
  ...
  stream->sendServerTrailers(
      static_cast<Status::GrpcStatus>(Status::GrpcStatusMapping::MaximumValid + 1), "error message",
      empty_metadata_);
  dispatcher_helper_.runDispatcher();
}
