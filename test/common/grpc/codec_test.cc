...
TEST(GrpcCodecTest, decodeSingleFrame) {
  ...
  static_cast<void>(result.ParseFromArray(frames[0].data_->linearize(frames[0].data_->length()),
                                          frames[0].data_->length()));
...
TEST(GrpcCodecTest, decodeMultipleFrame) {
  ...
  for (Frame& frame : frames) {
    ...
    helloworld::HelloRequest result;
    static_cast<void>(static_cast<void>(result.ParseFromArray(
        frame.data_->linearize(frame.data_->length()), frame.data_->length())));
    EXPECT_EQ("hello", result.name());
  }
}
