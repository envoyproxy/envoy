...
public:
  CredsUtilityTest() : file_system_(Filesystem::fileSystemForTest()) {}

  Filesystem::Instance& file_system_;
};

TEST_F(CredsUtilityTest, GetChannelCredentials) {
  EXPECT_EQ(nullptr, CredsUtility::getChannelCredentials({}, file_system_));
  envoy::api::v2::core::GrpcService::GoogleGrpc config;
  auto* creds = config.mutable_channel_credentials();
  EXPECT_EQ(nullptr, CredsUtility::getChannelCredentials(config, file_system_));
  creds->mutable_ssl_credentials();
  EXPECT_NE(nullptr, CredsUtility::getChannelCredentials(config, file_system_));
  creds->mutable_local_credentials();
  EXPECT_NE(nullptr, CredsUtility::getChannelCredentials(config, file_system_));
}

TEST_F(CredsUtilityTest, DefaultSslChannelCredentials) {
  EXPECT_NE(nullptr, CredsUtility::defaultSslChannelCredentials({}, file_system_));
  envoy::api::v2::core::GrpcService config;
  auto* creds = config.mutable_google_grpc()->mutable_channel_credentials();
  EXPECT_NE(nullptr, CredsUtility::defaultSslChannelCredentials(config, file_system_));
  creds->mutable_ssl_credentials();
  EXPECT_NE(nullptr, CredsUtility::defaultSslChannelCredentials(config, file_system_));
}
...
TEST_F(CredsUtilityTest, DefaultChannelCredentials) {
  { EXPECT_NE(nullptr, CredsUtility::defaultChannelCredentials({}, file_system_)); }
  {
    envoy::api::v2::core::GrpcService config;
    TestUtility::setTestSslGoogleGrpcConfig(config, true);
    EXPECT_NE(nullptr, CredsUtility::defaultChannelCredentials(config, file_system_));
  }
  {
    envoy::api::v2::core::GrpcService config;
    TestUtility::setTestSslGoogleGrpcConfig(config, true);
