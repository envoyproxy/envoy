        ...
        [=](DnsResolver::ResolutionStatus status, std::list<DnsResponse>&& results) -> void {
          ...
          if (expected_results) {
            ...
            for (const auto& result : results) {
              if (lookup_family == DnsLookupFamily::V4Only || lookup_family == DnsLookupFamily::V4Preferred) {
                EXPECT_NE(nullptr, result.address_->ip()->ipv4());
              } else if (lookup_family == DnsLookupFamily::V6Only || lookup_family == DnsLookupFamily::Auto) {
...
public:
  ...
  auto query =
      resolver_->resolve(hostname, dns_lookup_family,
                         [&dns_callback_executed, dns_lookup_family, address_type](DnsResolver::ResolutionStatus status,
  ...
  {
  case V4:
    reply_callback(nullptr, kDNSServiceFlagsAdd, 0,
