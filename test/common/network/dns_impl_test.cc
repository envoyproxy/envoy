...
public:
  ...
  Api::SysCallIntResult bind(IoHandle& io_handle) const override { return instance_.bind(io_handle); }
  Api::SysCallIntResult connect(IoHandle& io_handle) const override { return instance_.connect(io_handle); }
  const Address::Ip* ip() const override { return instance_.ip(); }
  IoHandle socket(Address::SocketType type) const override { return instance_.socket(type); }
