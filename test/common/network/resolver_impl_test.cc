...
TEST_F(IpResolverTest, Basic) {
  envoy::config::core::v3alpha::SocketAddress socket_address;
...
TEST_F(IpResolverTest, DisallowsNamedPort) {
  ...
  EXPECT_THROW_WITH_MESSAGE(
      ...
      fmt::format("IP resolver can't handle port specifier type {}",
                  envoy::config::core::v3alpha::SocketAddress::PortSpecifierCase::kNamedPort));
}

TEST(ResolverTest, FromProtoAddress) {
  envoy::config::core::v3alpha::Address ipv4_address;
  ...
  envoy::config::core::v3alpha::Address ipv6_address;
...
private:
  ...
  std::string getPort(const envoy::config::core::v3alpha::SocketAddress& socket_address) {
    switch (socket_address.port_specifier_case()) {
    case envoy::config::core::v3alpha::SocketAddress::PortSpecifierCase::kNamedPort:
