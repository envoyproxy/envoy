...
TEST_F(SocketOptionImplTest, HasName) {
  ...
  SocketOptionImpl socket_option{envoy::config::core::v3alpha::SocketOption::STATE_PREBIND, optname,
                                 1};
  ...
  EXPECT_LOG_CONTAINS(
      ...
      socket_option.setOption(socket_, envoy::config::core::v3alpha::SocketOption::STATE_PREBIND));
}

TEST_F(SocketOptionImplTest, SetOptionSuccessTrue) {
  SocketOptionImpl socket_option{envoy::config::core::v3alpha::SocketOption::STATE_PREBIND,
...
TEST_F(SocketOptionImplTest, GetOptionDetailsCorrectState) {
  ...
  auto result = socket_option.getOptionDetails(
      socket_, envoy::config::core::v3alpha::SocketOption::STATE_PREBIND);
...
TEST_F(SocketOptionImplTest, GetMoreOptionDetailsCorrectState) {
  SocketOptionImpl socket_option{envoy::config::core::v3alpha::SocketOption::STATE_LISTENING,
                                 ENVOY_MAKE_SOCKET_OPTION_NAME(7, 9), 5};

  auto result = socket_option.getOptionDetails(
      socket_, envoy::config::core::v3alpha::SocketOption::STATE_LISTENING);
  ASSERT_TRUE(result.has_value());
...
TEST_F(SocketOptionImplTest, GetOptionDetailsFailureWrongState) {
  ...
  auto result = socket_option.getOptionDetails(
      socket_, envoy::config::core::v3alpha::SocketOption::STATE_BOUND);
