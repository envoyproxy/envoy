...
TEST_F(NetworkFilterManagerTest, RateLimitAndTcpProxy) {
  ...
  TcpProxy::TcpProxyConfigSharedPtr tcp_proxy_config(
      new TcpProxy::TcpProxyConfig(tcp_proxy, factory_context));
  manager.addReadFilter(std::make_shared<TcpProxy::TcpProxyFilter>(
