...
public:
  ConfigTest(const OptionsImpl& options)
    ...
    // TODO(snowp): There's no way to override runtime flags per example file (since we mock out the runtime loader),
    // so temporarily enable this flag explicitly here until we flip the default. This should allow the existing
    // configuration examples to continue working despite the feature being disabled by default.
    ON_CALL(*snapshot_, runtimeFeatureEnabled("envoy.reloadable_features.experimental_matching_api"))
  ...
  std::shared_ptr<Runtime::MockSnapshot> snapshot_{std::make_shared<NiceMock<Runtime::MockSnapshot>>()};
