...
TEST_F(ZipkinDriverTest, NoB3ContextSampledTrue) {
  ...
  EXPECT_TRUE(request_headers_.get(ZipkinCoreConstants::get().X_B3_SPAN_ID.key()).empty());
  EXPECT_TRUE(request_headers_.get(ZipkinCoreConstants::get().X_B3_TRACE_ID.key()).empty());
  EXPECT_TRUE(request_headers_.get(ZipkinCoreConstants::get().X_B3_SAMPLED.key()).empty());
...
TEST_F(ZipkinDriverTest, PropagateB3NoSampleDecisionSampleTrue) {
  ...
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_TRACE_ID.key(),
                                   Hex::uint64ToHex(generateRandom64()));
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_SPAN_ID.key(),
                                   Hex::uint64ToHex(generateRandom64()));
  EXPECT_TRUE(request_headers_.get(ZipkinCoreConstants::get().X_B3_SAMPLED.key()).empty());
...
TEST_F(ZipkinDriverTest, PropagateB3NotSampled) {
  ...
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_SAMPLED.key(), NOT_SAMPLED);
  Tracing::SpanPtr span = driver_->startSpan(config_, request_headers_, stream_info_,
                                             operation_name_, {Tracing::Reason::Sampling, true});
...
TEST_F(ZipkinDriverTest, PropagateB3NotSampledWithFalse) {
  ...
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_SAMPLED.key(), sampled);
  Tracing::SpanPtr span = driver_->startSpan(config_, request_headers_, stream_info_,
                                             operation_name_, {Tracing::Reason::Sampling, true});
...
TEST_F(ZipkinDriverTest, ZipkinSpanContextFromB3HeadersTest) {
  ...
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_TRACE_ID.key(), trace_id);
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_SPAN_ID.key(), span_id);
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_PARENT_SPAN_ID.key(), parent_id);
...
TEST_F(ZipkinDriverTest, ZipkinSpanContextFromB3HeadersEmptyParentSpanTest) {
  ...
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_TRACE_ID.key(), id);
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_SPAN_ID.key(), id);
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_SAMPLED.key(), SAMPLED);

  // Set parent span id to empty string, to ensure it is ignored
  const std::string parent_span_id = "";
  request_headers_.addReferenceKey(ZipkinCoreConstants::get().X_B3_PARENT_SPAN_ID.key(),
                                   parent_span_id);
