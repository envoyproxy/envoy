...
TEST(OpenCensusTracerConfigTest, OpenCensusHttpTracerStackdriverCallCredentials) {
  NiceMock<Server::MockInstance> server;
  const std::string yaml_string = R"EOF(
  http:
    name: envoy.tracers.opencensus
    typed_config:
      ...
      stackdriver_exporter_enabled: true
      stackdriver_project_id: test_project_id
      stackdriver_call_credentials:
        sts_service:
          token_exchange_service_uri: http://localhost
          scope: test_scope
          requested_token_type: test_token_type
          subject_token_path: test_token_path
          subject_token_type: test_token_type
      incoming_trace_context: b3
      incoming_trace_context: trace_context
      incoming_trace_context: grpc_trace_bin
      incoming_trace_context: cloud_trace_context
