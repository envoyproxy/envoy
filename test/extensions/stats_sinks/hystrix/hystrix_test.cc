...
public:
  ...
  void validateResults(const std::string& data_message, uint64_t success_step, uint64_t error_step,
    ...
    EXPECT_EQ(json_data_message->getInteger("rollingCountSuccess").value(), (window_size * success_step))
...
TEST_F(HystrixSinkTest, HistogramTest) {
  ...
  Json::ObjectSharedPtr latency = Json::Factory::loadFromString(cluster_message_map[cluster1_name_]).value()
                                      ->getObject("latencyExecute").value();
