...
public:
  ...
  void setCounterReturnValues(const uint64_t i, const uint64_t success_step,
    ...
    cluster_info_->upstreamStats()->upstream_rq_timeout_.add(timeout_step);
    cluster_info_->upstreamStats()->upstream_rq_per_try_timeout_.add(timeout_retry_step);
    cluster_info_->upstreamStats()->upstream_rq_pending_overflow_.add(rejected_step);
  }
