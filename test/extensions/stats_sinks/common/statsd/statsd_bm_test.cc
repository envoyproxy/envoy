...
class StatsdSinkTest {
 public:
  StatsdSinkTest(SinkOptions sink_option, int num_stats): sink_option_(sink_option), num_stats_(num_stats) {
    if (sink_option_ == SinkOptions::TCP_COUNTER_GAUGES_SINK || sink_option_ == SinkOptions::TCP_HISTOGRAM_COMPLETE) {
    ...
    if (sink_option_ == SinkOptions::TCP_COUNTER_GAUGES_SINK || sink_option_ == SinkOptions::UDP_COUNTER_GAUGES_SINK) {
...
BENCHMARK(bmTCPStatsdSinktest)->Unit(::benchmark::kMillisecond)->RangeMultiplier(10)->Range(100, 1000000);
BENCHMARK(bmUDPStatsdSinktest)->Unit(::benchmark::kMillisecond)->RangeMultiplier(10)->Range(100, 1000000);
