...
public:
  ...
  NiceMock<Stats::MockFlushDelegate> flush_delegate_;
};
...
TEST_F(TcpStatsdSinkTest, BasicFlow) {
  ...
  counter->latch_ = 1;
  counter->used_ = true;
  flush_delegate_.counters_.push_back(counter);

  auto gauge = std::make_shared<NiceMock<Stats::MockGauge>>();
  gauge->name_ = "test_gauge";
  gauge->value_ = 2;
  gauge->used_ = true;
  flush_delegate_.gauges_.push_back(gauge);
...
TEST_F(TcpStatsdSinkTest, BufferReallocate) {
  ...
  flush_delegate_.counters_.resize(2000, counter);
