...
public:
  ...
  void initialize() override {
    config_helper_.addConfigModifier(
        [this](envoy::config::bootstrap::v3alpha::Bootstrap& bootstrap) {
          ...
          envoy::config::metrics::v3alpha::MetricsServiceConfig config;
  ...
  AssertionResult waitForMetricsRequest() {
    ...
    while (!(known_counter_exists && known_gauge_exists && known_histogram_exists)) {
      envoy::service::metrics::v3alpha::StreamMetricsMessage request_msg;
...
TEST_P(MetricsServiceIntegrationTest, BasicFlow) {
  ...
  envoy::service::metrics::v3alpha::StreamMetricsResponse response_msg;
