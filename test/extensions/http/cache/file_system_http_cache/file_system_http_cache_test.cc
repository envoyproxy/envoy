...
public:
  ...
  void initCache() { cache_ = http_cache_factory_->getCache(cacheConfig(testConfig()), context_.server_factory_context_); }
...
TEST_F(FileSystemHttpCacheTest, ExceptionOnTryingToCreateCachesWithDistinctConfigsOnSamePath) {
  ...
  EXPECT_ANY_THROW(http_cache_factory_->getCache(cacheConfig(cfg), context_.server_factory_context_));
}

TEST_F(FileSystemHttpCacheTest, IdenticalCacheConfigReturnsSameCacheInstance) {
  ConfigProto cfg = testConfig();
  auto second_cache = http_cache_factory_->getCache(cacheConfig(cfg), context_.server_factory_context_);
...
TEST_F(FileSystemHttpCacheTestWithMockFiles, InsertAbortsOnFailureToWriteEmptyHeaderBlock) {
  ...
  EXPECT_CALL(*cache_progress_receiver_, onHeadersInserted(_, _, _));
  EXPECT_CALL(*cache_progress_receiver_, onInsertFailed);
  EXPECT_CALL(*mock_async_file_manager_, createAnonymousFile(_, _, _));
  EXPECT_CALL(*mock_async_file_handle_, duplicate);
...
TEST(Registration, GetCacheFromFactory) {
  ...
  ON_CALL(factory_context.server_factory_context_.api_, threadFactory())
      .WillByDefault([]() -> Thread::ThreadFactory& { return Thread::threadFactoryForTest(); });
  TestUtility::loadFromYaml(std::string(yaml_config), cache_config);
  EXPECT_EQ(factory->getCache(cache_config, factory_context.server_factory_context_)->cacheInfo().name_,
            "envoy.extensions.http.cache.file_system_http_cache");
  auto active_cache = factory->getCache(cache_config, factory_context.server_factory_context_);
