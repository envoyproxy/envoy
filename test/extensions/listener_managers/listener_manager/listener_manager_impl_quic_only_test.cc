...
TEST_P(ListenerManagerImplQuicOnlyTest, QuicListenerFromConfig) {
  std::string yaml = getBasicConfig();
  yaml = yaml + R"EOF(
listener_filters:
- name: envoy.filters.quic_listener.test
  ...
  Network::MockListenerFilterManager filter_manager;
  Network::ListenerFilterPtr listener_filter;
  EXPECT_CALL(filter_manager, addAcceptFilter_(_, _))
  ...
  manager_->listeners()[0].get().filterChainFactory().createListenerFilterChain(filter_manager);
  ...
  EXPECT_EQ("xyz", added_filter_state->value());
}
