...
protected:
  ...
  void wipeFilters() {
    updateConfigInPlace([&](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
      ...
      static_cast<void>(hcm_filter->mutable_typed_config()->UnpackTo(&hcm_config));
      hcm_config.clear_http_filters();
      static_cast<void>(hcm_filter->mutable_typed_config()->PackFrom(hcm_config));
    });
...
bucket_action:
  ...
  updateConfigInPlace([&](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    ...
    static_cast<void>(rlqs_filter->mutable_typed_config()->UnpackTo(&rlqs_filter_config));
    ...
    static_cast<void>(on_match_action->mutable_typed_config()->UnpackTo(&on_match_settings));
    ...
    static_cast<void>(on_match_action->mutable_typed_config()->PackFrom(on_match_settings));
    static_cast<void>(rlqs_filter->mutable_typed_config()->PackFrom(rlqs_filter_config));
    static_cast<void>(hcm_filter->mutable_typed_config()->PackFrom(hcm_config));
  });
