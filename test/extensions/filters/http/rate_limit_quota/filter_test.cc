...
TEST_F(FilterTest, DenyResponseWithResourceExhaustedFlag) {
  // Create a custom config with rate_limited_as_resource_exhausted set to true
...
bucket_matchers:
  ...
  const std::string matcher_yaml_resource_exhausted = R"EOF(
  matcher_list:
    matchers:
    - predicate:
        single_predicate:
  ...
  xds::type::matcher::v3::Matcher matcher;
  TestUtility::loadFromYaml(matcher_yaml_resource_exhausted, matcher);
  addMatcherConfig(matcher);
  ...
  // Expect sendLocalReply to be called with RESOURCE_EXHAUSTED gRPC status
...
bucket_matchers:
  ...
  // Expect sendLocalReply to be called with UNAVAILABLE gRPC status (explicit)
  EXPECT_CALL(decoder_callbacks_, sendLocalReply(Http::Code::TooManyRequests, _, _, _, _))
      .WillOnce(
          Invoke([](Http::Code, absl::string_view, std::function<void(Http::ResponseHeaderMap&)>,
                    const absl::optional<Grpc::Status::GrpcStatus> grpc_status, absl::string_view) {
...
TEST_F(FilterTest, DenyResponsePrecedenceTest) {
  // Test that explicit grpc_status takes precedence over rate_limited_as_resource_exhausted
  const std::string config_yaml = R"EOF(
rlqs_server:
  google_grpc:
    target_uri: rate_limit_quota_server
...
bucket_matchers:
          ...
          typed_config:
            ...
            deny_response_settings:
              http_status:
                code: 429
              grpc_status:
                code: 14  # UNAVAILABLE - this should take precedence
                message: "Service temporarily unavailable"
              rate_limited_as_resource_exhausted: true  # This should be ignored
            expired_assignment_behavior:
              fallback_rate_limit:
                blanket_rule: ALLOW_ALL
            reporting_interval: 5s
  ...
  // Extract just the matcher portion for loading into xds::type::matcher::v3::Matcher
  const std::string matcher_yaml_precedence = R"EOF(
  matcher_list:
    matchers:
    - predicate:
        single_predicate:
  ...
  xds::type::matcher::v3::Matcher matcher;
  TestUtility::loadFromYaml(matcher_yaml_precedence, matcher);
  addMatcherConfig(matcher);
  ...
  // Should use explicit grpc_status (UNAVAILABLE), not the boolean flag (RESOURCE_EXHAUSTED)
  EXPECT_CALL(decoder_callbacks_, sendLocalReply(Http::Code::TooManyRequests, _, _, _, _))
      .WillOnce(
          Invoke([](Http::Code, absl::string_view, std::function<void(Http::ResponseHeaderMap&)>,
                    const absl::optional<Grpc::Status::GrpcStatus> grpc_status, absl::string_view) {
...
TEST_F(FilterTest, DenyResponseDefaultBehavior) {
  // Test default behavior when neither grpc_status nor rate_limited_as_resource_exhausted is set
  addMatcherConfig(MatcherConfigType::Valid);
  createFilter();
