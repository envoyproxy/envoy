...
class MockTlsCertificateConfigProvider : public TlsCertificateConfigProvider {
 public:
  ...
  MOCK_METHOD(
      ...
      (std::function<absl::Status(const envoy::extensions::transport_sockets::
                                      tls::v3::TlsCertificate&)>));
  MOCK_METHOD(Common::CallbackHandlePtr, addUpdateCallback,
              (std::function<absl::Status()>));
  MOCK_METHOD(Common::CallbackHandlePtr, addRemoveCallback,
              (std::function<absl::Status()>));
...
TEST_F(CryptoUtilsTest, GetCertificateFingerprintSuccess) {
  auto secret_provider =
      std::make_shared<NiceMock<Secret::MockTlsCertificateConfigProvider>>();
  auto secret = std::make_unique<
      envoy::extensions::transport_sockets::tls::v3::TlsCertificate>();

  std::string cert_path = TestEnvironment::runfilesPath(
  ...
  san_matchers.emplace_back(Matchers::StringMatcherImpl::createExactMatcher(
      "spiffe://lyft.com/frontend-team"));

  auto fingerprint =
      getCertificateFingerprint(secret_provider, san_matchers, *api_);
  ...
  EXPECT_EQ(fingerprint.value(),
            "wH4U/EO5x7PZLxAE%252BR06ngcdnJOlivx2tMFDA646DzQ");
}
