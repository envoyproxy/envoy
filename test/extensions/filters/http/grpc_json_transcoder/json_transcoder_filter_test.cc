...
TEST_F(GrpcJsonTranscoderFilterTest, TranscodingUnaryPost) {
  ...
  auto response_data = Grpc::ContextImpl::serializeToGrpcFrame(response);
...
TEST_F(GrpcJsonTranscoderFilterTest, ForwardUnaryPostGrpc) {
  ...
  Buffer::InstancePtr request_data = Grpc::ContextImpl::serializeToGrpcFrame(request);
  EXPECT_EQ(Http::FilterDataStatus::Continue, filter_.decodeData(*request_data, true));
  ...
  Buffer::InstancePtr response_data = Grpc::ContextImpl::serializeToGrpcFrame(response);
...
TEST_P(GrpcJsonTranscoderFilterPrintTest, PrintOptions) {
  ...
  const auto response_data = Grpc::ContextImpl::serializeToGrpcFrame(author);
  EXPECT_EQ(Http::FilterDataStatus::StopIterationAndBuffer,
            filter_->encodeData(*response_data, false));
