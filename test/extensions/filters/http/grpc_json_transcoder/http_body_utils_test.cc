...
public:
  ...
  void basicTest(const std::string& content, const std::string& content_type,
    ...
    // Parse using concrete message type.
    {
      ...
      Message message;
      static_cast<void>(message.ParseFromZeroCopyStream(&stream));
...
TEST_F(HttpBodyUtilsTest, UnknownQueryParamsAppearInExtension) {
  ...
  google::api::HttpBody http_body;
  static_cast<void>(http_body.ParseFromZeroCopyStream(&stream));
  ...
  static_cast<void>(http_body.extensions(0).UnpackTo(&unknown_vars_out));
