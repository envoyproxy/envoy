...
public:
  void setupFetcher(const std::string& config_str) {
    ...
    mock_factory_ctx_.mock_server_context_.cluster_manager_.initializeThreadLocalClusters(
        {"pubkey_cluster"});
    fetcher_ =
        JwksFetcher::create(mock_factory_ctx_.mock_server_context_.cluster_manager_, remote_jwks_);
...
TEST_F(JwksFetcherTest, TestGet400) {
  // Setup
  setupFetcher(config);
  MockUpstream mock_pubkey(mock_factory_ctx_.mock_server_context_.cluster_manager_, "400",
                           "invalid");
  MockJwksReceiver receiver;
