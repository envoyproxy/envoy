...
public:
  void setupFetcher(const std::string& config_str) {
    ...
    fetcher_ = JwksFetcher::create(mock_factory_ctx_.mock_server_context_.cluster_manager_, remote_jwks_);
...
TEST_F(JwksFetcherTest, TestGetSuccess) {
  // Setup
  setupFetcher(config);
  MockUpstream mock_pubkey(mock_factory_ctx_.mock_server_context_.cluster_manager_, "200", publicKey);
...
TEST_F(JwksFetcherTest, TestGet400) {
  // Setup
  setupFetcher(config);
  MockUpstream mock_pubkey(mock_factory_ctx_.mock_server_context_.cluster_manager_, "400", "invalid");
  MockJwksReceiver receiver;
...
TEST_F(JwksFetcherTest, TestGetNoBody) {
  // Setup
  setupFetcher(config);
  MockUpstream mock_pubkey(mock_factory_ctx_.mock_server_context_.cluster_manager_, "200", "");
  MockJwksReceiver receiver;
  EXPECT_CALL(receiver, onJwksSuccessImpl(testing::_)).Times(0);
  EXPECT_CALL(receiver, onJwksError(JwksFetcher::JwksReceiver::Failure::Network));
...
TEST_F(JwksFetcherTest, TestGetInvalidJwks) {
  // Setup
  setupFetcher(config);
  MockUpstream mock_pubkey(mock_factory_ctx_.mock_server_context_.cluster_manager_, "200", "invalid");
  MockJwksReceiver receiver;
  EXPECT_CALL(receiver, onJwksSuccessImpl(testing::_)).Times(0);
...
TEST_F(JwksFetcherTest, TestHttpFailure) {
  // Setup
  setupFetcher(config);
  MockUpstream mock_pubkey(mock_factory_ctx_.mock_server_context_.cluster_manager_,
...
TEST_F(JwksFetcherTest, TestCancel) {
  ...
  MockUpstream mock_pubkey(mock_factory_ctx_.mock_server_context_.cluster_manager_, &request);
  MockJwksReceiver receiver;
