...
public:
  void SetUp() override {
    ...
    useAccessLog("%DYNAMIC_METADATA(envoy.filters.http.proto_message_logging)%");
  ...
  std::string filter() {
    return absl::StrFormat(
        R"EOF(
name: proto_message_logging
typed_config:
  "@type": type.googleapis.com/envoy.extensions.filters.http.proto_message_logging.v3.ProtoMessageLoggingConfig
  mode: FIRST_AND_LAST
  data_source:
    filename: %s
  logging_by_method:
