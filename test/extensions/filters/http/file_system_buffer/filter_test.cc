...
TEST_F(FileSystemBufferFilterTest, BuffersEntireRequestAndReplacesContentLength) {
  ...
  EXPECT_CALL(decoder_callbacks_, injectDecodedDataToFilterChain(_, false))
      .WillOnce(Invoke([this](Buffer::Instance& out, bool) { request_sent_on_ += out.toString(); }));
...
TEST_F(FileSystemBufferFilterTest, BuffersEntireResponseAndReplacesContentLength) {
  ...
  EXPECT_CALL(encoder_callbacks_, injectEncodedDataToFilterChain(_, false))
      .WillOnce(Invoke([this](Buffer::Instance& out, bool) { response_sent_on_ += out.toString(); }));
