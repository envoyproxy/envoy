...
TEST_F(FilterTestExtractOk, UnarySingleBufferWithMultipleFields) {
  setUp(R"pb(
      ...
      scrubbing_by_method: {
        ...
        value: {
          request_scrubbing_by_field: { key: "parent" value: SCRUB }
          request_scrubbing_by_field: { key: "key.display_name" value: SCRUB }
          request_scrubbing_by_field: { key: "key.current_key" value: SCRUB }
          response_scrubbing_by_field: { key: "name" value: SCRUB }
          response_scrubbing_by_field: { key: "display_name" value: SCRUB }
          response_scrubbing_by_field: { key: "current_key" value: SCRUB }
        }
      }
    )pb");
...
scrubbing_by_method: {
  ...
  EXPECT_CALL(mock_decoder_callbacks_.stream_info_, setDynamicMetadata(_, _))
      .WillOnce(Invoke([](const std::string& ns, const ProtobufWkt::Struct& new_dynamic_metadata) {
        EXPECT_EQ(ns, "envoy.filters.http.proto_message_scrubbing");
        checkProtoStruct(new_dynamic_metadata, R"pb(
fields {
  key: "requests"
  value {
...
TEST_F(FilterTestWithScrubRedacted, UnarySingleBuffer) {
  setUp(R"pb(
      mode: FIRST_AND_LAST
      scrubbing_by_method: {
        key: "apikeys.ApiKeys.CreateApiKey"
        value: {
          ...
          request_scrubbing_by_field: { key: "key.display_name" value: SCRUB }
          response_scrubbing_by_field: { key: "expire_time" value: SCRUB_REDACT }
          response_scrubbing_by_field: { key: "kms_key" value: SCRUB }
        }
      }
    )pb");
...
TEST_F(FilterTestWithScrubRedacted, StreamingMultipleMessageSingleBuffer) {
  setUp(R"pb(
mode: FIRST_AND_LAST
scrubbing_by_method: {
  key: "apikeys.ApiKeys.CreateApiKeyInStream"
...
TEST_F(FilterTestExtractRejected, BufferLimitedExceeded) {
  ...
  EXPECT_CALL(mock_decoder_callbacks_,
              sendLocalReply(
                  ...
                  "proto_message_scrubbing_FAILED_PRECONDITION{REQUEST_BUFFER_CONVERSION_FAIL}"));
  EXPECT_EQ(Envoy::Http::FilterDataStatus::StopIterationNoBuffer,
  ...
  EXPECT_CALL(mock_encoder_callbacks_,
              sendLocalReply(
                  Http::Code::BadRequest, "Rejected because internal buffer limits are exceeded.",
                  Eq(nullptr), Eq(Envoy::Grpc::Status::FailedPrecondition),
                  "proto_message_scrubbing_FAILED_PRECONDITION{RESPONSE_BUFFER_CONVERSION_FAIL}"));
  EXPECT_EQ(Envoy::Http::FilterDataStatus::StopIterationNoBuffer,
...
TEST_F(FilterTestExtractRejected, NotEnoughData) {
  ...
  EXPECT_CALL(mock_decoder_callbacks_,
              sendLocalReply(Http::Code::BadRequest,
                             ...
                             "proto_message_scrubbing_INVALID_ARGUMENT{REQUEST_OUT_OF_DATA}"));
  ...
  EXPECT_CALL(mock_encoder_callbacks_,
              sendLocalReply(Http::Code::BadRequest,
                             "did not receive enough data to form a message.", Eq(nullptr),
                             Eq(Envoy::Grpc::Status::InvalidArgument),
                             "proto_message_scrubbing_INVALID_ARGUMENT{RESPONSE_OUT_OF_DATA}"));
...
TEST_F(FilterTestExtractRejected, RequestMisformedGrpcPath) {
  ...
  EXPECT_CALL(mock_decoder_callbacks_,
              sendLocalReply(Http::Code::BadRequest,
                             ...
                             "proto_message_scrubbing_INVALID_ARGUMENT{BAD_REQUEST}"));
...
// ScrubDirective_UNSPECIFIED is treated as SCRUB.
using FilterTestWithScrubDirectiveUnspecified = FilterTestBase;

TEST_F(FilterTestWithScrubDirectiveUnspecified, HappyPath) {
  setUp(R"pb(
      mode: FIRST_AND_LAST
      scrubbing_by_method: {
        key: "apikeys.ApiKeys.CreateApiKey"
