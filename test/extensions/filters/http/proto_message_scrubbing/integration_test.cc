...
public:
  void SetUp() override {
    ...
    useAccessLog("%DYNAMIC_METADATA(envoy.filters.http.proto_message_scrubbing)%");
  ...
  std::string filter() {
    return absl::StrFormat(
        R"EOF(
name: proto_message_scrubbing
typed_config:
  "@type": type.googleapis.com/envoy.extensions.filters.http.proto_message_scrubbing.v3.ProtoMessageScrubbingConfig
  mode: FIRST_AND_LAST
  data_source:
    filename: %s
  scrubbing_by_method:
