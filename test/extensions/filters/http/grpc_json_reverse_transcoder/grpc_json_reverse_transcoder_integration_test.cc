  ...
  void overrideConfig(const std::string& json_config) {
    ...
    ConfigHelper::HttpModifierFunction modifier =
        [per_route_config](
          ...
          static_cast<void>((*config)["grpc_json_reverse_transcoder"].PackFrom(per_route_config));
        };
...
TEST_P(GrpcJsonReverseTranscoderIntegrationTest, SimpleRequest) {
  ...
  bookstore::Book book;
  static_cast<void>(book.ParseFromString(frames[0].data_->toString()));
...
TEST_P(GrpcJsonReverseTranscoderIntegrationTest, HttpBodyRequestResponse) {
  ...
  static_cast<void>(transcoded_res.ParseFromString(frames[0].data_->toString()));
