...
// Tests with on-demand VHDS but with a static route (non-RDS).
class OnDemandVhdsStaticRouteIntegrationTest : public VhdsIntegrationTest {
public:
  void initialize() override {
    setUpstreamCount(2);
...
INSTANTIATE_TEST_SUITE_P(IpVersionsClientType, OnDemandVhdsStaticRouteIntegrationTest,
                         UNIFIED_LEGACY_GRPC_CLIENT_INTEGRATION_PARAMS);

// Validates that VHDS on-demand with a static route works.
TEST_P(OnDemandVhdsStaticRouteIntegrationTest, VhdsOnDemandUpdateWithStaticRouteConfig) {
  initialize();
  codec_client_ = makeHttpConnection(makeClientConnection((lookupPort("http"))));
  Http::TestRequestHeaderMapImpl request_headers{{":method", "GET"},
                                                 {":path", "/"},
