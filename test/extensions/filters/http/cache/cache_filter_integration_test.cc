...
TEST_P(CacheIntegrationTest, ServeHeadRequest) {
  ...
  // Send first request, and get response from upstream.
  {
    IntegrationStreamDecoderPtr response_decoder =
        codec_client_->makeHeaderOnlyRequest(request_headers);
    waitForNextUpstreamRequest();
    upstream_request_->encodeHeaders(response_headers, /*end_stream=*/true);
    response_decoder->waitForEndStream();
    EXPECT_TRUE(response_decoder->complete());
    EXPECT_THAT(response_decoder->headers(), IsSupersetOfHeaders(response_headers));
    EXPECT_TRUE(response_decoder->headers().get(Http::Headers::get().Age).empty());
  ...
  // Send second request, and get response from upstream, since the head requests are not stored in
  // cache.
  {
    IntegrationStreamDecoderPtr response_decoder =
        codec_client_->makeHeaderOnlyRequest(request_headers);
...
TEST_P(CacheIntegrationTest, ServeHeadFromCacheAfterGetRequest) {
  ...
  const Http::TestRequestHeaderMapImpl get_request_headers = {
      {":method", "GET"},
      {":path", absl::StrCat("/", protocolTestParamsToString({GetParam(), 0}))},
      {":scheme", "http"},
      {":authority", "ServeHeadFromCacheAfterGetRequest"}};
  ...
  {
    IntegrationStreamDecoderPtr response_decoder =
        codec_client_->makeHeaderOnlyRequest(get_request_headers);
    waitForNextUpstreamRequest();
    upstream_request_->encodeHeaders(response_headers, /*end_stream=*/false);
    upstream_request_->encodeData(response_body, /*end_stream=*/true);
    // Wait for the response to be read by the codec client.
  ...
  const Http::TestRequestHeaderMapImpl head_request_headers = {
      {":method", "HEAD"},
      {":path", absl::StrCat("/", protocolTestParamsToString({GetParam(), 0}))},
      {":scheme", "http"},
      {":authority", "ServeHeadFromCacheAfterGetRequest"}};
  ...
  {
    IntegrationStreamDecoderPtr response_decoder =
        codec_client_->makeHeaderOnlyRequest(head_request_headers);
    response_decoder->waitForEndStream();
    EXPECT_TRUE(response_decoder->complete());
    EXPECT_THAT(response_decoder->headers(), IsSupersetOfHeaders(response_headers));
    EXPECT_EQ(response_decoder->body().size(), 0);
...
TEST_P(CacheIntegrationTest, ServeGetFromUpstreamAfterHeadRequest) {
  ...
  const Http::TestRequestHeaderMapImpl head_request_headers = {
      ...
      {":path", absl::StrCat("/", protocolTestParamsToString({GetParam(), 0}))},
      {":scheme", "http"},
      {":authority", "ServeGetFromUpstreamAfterHeadRequest"}};
...
TEST_P(CacheIntegrationTest, ServeHead304AfterGetWithValidation) {
  useAccessLog("%RESPONSE_FLAGS% %RESPONSE_CODE_DETAILS%");
  // Set system time to cause Envoy's cached formatted time to match time on this thread.
  simTime().setSystemTime(std::chrono::hours(1));
  initializeFilter(default_config);
  ...
  const Http::TestRequestHeaderMapImpl request_headers = {
      ...
      {":path", absl::StrCat("/", protocolTestParamsToString({GetParam(), 0}))},
      {":scheme", "http"},
      {":authority", "ServeHeadAfterGetWithValidation"}};

  const std::string response_body(42, 'a');
  Http::TestResponseHeaderMapImpl response_headers = {
      {":status", "200"}, // updateHeaders function is not implemented, so returns 200
      {"date", formatter_.now(simTime())},
      {"cache-control", "max-age=10"}, // expires after 10 s
      {"content-length", std::to_string(response_body.size())},
      {"etag", "abc123"}};
...
TEST_P(CacheIntegrationTest, ServeHead200AfterGetWithValidation) {
  useAccessLog("%RESPONSE_FLAGS% %RESPONSE_CODE_DETAILS%");
  // Set system time to cause Envoy's cached formatted time to match time on this thread.
  simTime().setSystemTime(std::chrono::hours(1));
  initializeFilter(default_config);
  ...
  const Http::TestRequestHeaderMapImpl request_headers = {
      ...
      {":path", absl::StrCat("/", protocolTestParamsToString({GetParam(), 0}))},
      {":scheme", "http"},
      {":authority", "ServeHead200AfterGetWithValidation"}};
