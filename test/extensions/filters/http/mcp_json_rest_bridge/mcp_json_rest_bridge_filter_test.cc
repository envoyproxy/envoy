...
TEST_F(McpJsonRestBridgeFilterTest, LegacyMetaProtocolVersionInitializeIsNotStateless) {
  proto_config_.mutable_server_info()->mutable_max_supported_protocol_version()->set_value(
      "2026-07-28");
  ASSERT_OK(makeFilter());
  ...
  // The request metadata advertises a pre-2026-07-28 version, so the request is not stateless and
  // the legacy initialize handshake must still be served (no "resultType" is added).
  EXPECT_CALL(
      decoder_callbacks_,
      sendLocalReply(
          Eq(Http::Code::OK),
  ...
  Buffer::OwnedImpl body(
      R"json({"jsonrpc":"2.0","id":0,"method":"initialize","params":{"protocolVersion":"2025-11-25","_meta":{"io.modelcontextprotocol/protocolVersion":"2025-11-25", "io.modelcontextprotocol/clientCapabilities": {}}}})json");
  EXPECT_EQ(filter_->decodeData(body, /*end_stream=*/true),
            Http::FilterDataStatus::StopIterationNoBuffer);
}

TEST_F(McpJsonRestBridgeFilterTest,
       LegacyMetaProtocolVersionNotificationsInitializedIsNotStateless) {
  proto_config_.mutable_server_info()->mutable_max_supported_protocol_version()->set_value(
      "2026-07-28");
  ASSERT_OK(makeFilter());
  ...
  Buffer::OwnedImpl body(
      R"json({"jsonrpc":"2.0","method":"notifications/initialized","params":{"_meta":{"io.modelcontextprotocol/protocolVersion":"2025-11-25", "io.modelcontextprotocol/clientCapabilities": {}}}})json");
  EXPECT_EQ(filter_->decodeData(body, /*end_stream=*/true),
            Http::FilterDataStatus::StopIterationNoBuffer);
}
