...
static const std::string STATEFUL_SESSION_HEADER_FILTER_WITH_PACKAGES =
    R"EOF(
name: envoy.filters.http.stateful_session
typed_config:
  "@type": type.googleapis.com/envoy.extensions.filters.http.stateful_session.v3.StatefulSession
...
TEST_F(StatefulSessionIntegrationTest, StatefulSessionHeaderWithPackagesAndStrictMode) {
  initializeFilterAndRoute(
      STATEFUL_SESSION_HEADER_FILTER_WITH_PACKAGES + STATEFUL_SESSION_STRICT_MODE, "");
  ...
  const std::string encoded_address =
      Envoy::Base64::encode(fmt::format("{};origin:{}", address_string, "abcdefg"));
...
TEST_F(StatefulSessionIntegrationTest,
       DownstreamRequestWithStatefulSessionHeaderWithPackagesAndStrictMode) {
  initializeFilterAndRoute(
      STATEFUL_SESSION_HEADER_FILTER_WITH_PACKAGES + STATEFUL_SESSION_STRICT_MODE, "");
  ...
  // Upstream endpoint encoded in stateful session header points to unknown server address.
  {
    ...
    Http::TestRequestHeaderMapImpl request_headers{
        ...
        {":authority", "stateful.session.com"},
        {"session-header",
         Envoy::Base64::encode(fmt::format("{};origin:{}", "127.0.0.7:50000", "abcdefg"))}};
