...
TEST_F(InitializeFilterTest, TestWithOneClusterStandardUpstream) {
  ...
  //     std::cout << ",";
  //   }
  //   std::cout << std::endl << "{\"" << (*it)->name() << "\", \"" << (*it)->value() <<
  //   "\"}";
  // }
  // std::cout << "};" << std::endl;
...
//     EXPECT_CALL(*dns_resolver_, resolve(expected_address, _, _))
//         .WillRepeatedly(Invoke([&](const std::string&, Network::DnsLookupFamily,
//                                    Network::DnsResolver::ResolveCb cb) ->
...
//   void dnsSetup() {
//     ON_CALL(dns_resolver_factory_, createDnsResolver(_, _,
//     _)).WillByDefault(Return(dns_resolver_)); expectResolve(Network::DnsLookupFamily::V4Only,
...
//                                      "service_internal-ap-southeast-2");
