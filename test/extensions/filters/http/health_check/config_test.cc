...
#include "envoy/config/filter/http/health_check/v3alpha/health_check.pb.h"
#include "envoy/config/filter/http/health_check/v3alpha/health_check.pb.validate.h"
...
TEST(HealthCheckFilterConfig, HealthCheckFilter) {
  ...
  envoy::config::filter::http::health_check::v3alpha::HealthCheck proto_config;
...
TEST(HealthCheckFilterConfig, FailsWhenNotPassThroughButTimeoutSetProto) {
  ...
  envoy::config::filter::http::health_check::v3alpha::HealthCheck config{};
  NiceMock<Server::Configuration::MockFactoryContext> context;

  config.mutable_pass_through_mode()->set_value(false);
  config.mutable_cache_time()->set_seconds(10);
  envoy::api::v3alpha::route::HeaderMatcher& header = *config.add_headers();
...
TEST(HealthCheckFilterConfig, HealthCheckFilterWithEmptyProto) {
  ...
  NiceMock<Server::Configuration::MockFactoryContext> context;
  envoy::config::filter::http::health_check::v3alpha::HealthCheck config =
      *dynamic_cast<envoy::config::filter::http::health_check::v3alpha::HealthCheck*>(
...
void testHealthCheckHeaderMatch(
    const envoy::config::filter::http::health_check::v3alpha::HealthCheck& input_config,
  ...
  auto config = dynamic_cast<envoy::config::filter::http::health_check::v3alpha::HealthCheck*>(
      config_msg.get());
...
TEST(HealthCheckFilterConfig, HealthCheckFilterHeaderMatch) {
  ...
  envoy::api::v3alpha::route::HeaderMatcher& xheader = *config.add_headers();
  xheader.set_name("x-healthcheck");

  envoy::api::v3alpha::route::HeaderMatcher& yheader = *config.add_headers();
...
TEST(HealthCheckFilterConfig, HealthCheckFilterDuplicateMatch) {
  ...
  envoy::api::v3alpha::route::HeaderMatcher& dup_header = *config.add_headers();
