...
public:
  void setupFilter() override {
    ...
    ON_CALL(*dns_cache_manager_->dns_cache_, canCreateDnsRequest_())
        .WillByDefault(Return(new Upstream::ResourceAutoIncDec(pending_requests_)));
...
TEST_F(ProxyFilterWithFilterStateHostTest, NoFilterStatePresent) {
  ...
  ON_CALL(callbacks_.stream_info_, filterState()).WillByDefault(ReturnRef(filter_state));
...
TEST_F(ProxyFilterWithFilterStateHostDisabledTest, IgnoreFilterStateWhenDisabled) {
  Upstream::ResourceAutoIncDec* circuit_breakers_(
      new Upstream::ResourceAutoIncDec(pending_requests_));
  InSequence s;
  ...
  // Create a filter state with a dynamic host value that will be ignored.
  const std::string filter_state_host = "filter-state-host.example.com";
  auto filter_state =
      std::make_shared<StreamInfo::FilterStateImpl>(StreamInfo::FilterState::LifeSpan::FilterChain);
  filter_state->setData("envoy.upstream.dynamic_host",
  ...
  // Since filter state host is disabled, it should use the "foo" from host header
  // even though filter state is available
  Common::DynamicForwardProxy::MockLoadDnsCacheEntryHandle* handle =
      new Common::DynamicForwardProxy::MockLoadDnsCacheEntryHandle();
  EXPECT_CALL(*dns_cache_manager_->dns_cache_, loadDnsCacheEntry_(Eq("foo"), 80, _, _))
      .WillOnce(Return(
...
TEST_F(ProxyFilterWithFilterStateHostDisabledTest, IgnoresFilterStateValue) {
  Upstream::ResourceAutoIncDec* circuit_breakers_(
      new Upstream::ResourceAutoIncDec(pending_requests_));
  ...
  // Should use "foo" from host header even when filter state has a value.
  Common::DynamicForwardProxy::MockLoadDnsCacheEntryHandle* handle =
      new Common::DynamicForwardProxy::MockLoadDnsCacheEntryHandle();
  EXPECT_CALL(*dns_cache_manager_->dns_cache_, loadDnsCacheEntry_(Eq("foo"), 80, _, _))
      .WillOnce(Return(
...
TEST_F(ProxyFilterWithFilterStateHostTest, WithFilterStatePortPresent) {
  ...
  const uint32_t filter_state_port = 9999;
  auto filter_state =
      std::make_shared<StreamInfo::FilterStateImpl>(StreamInfo::FilterState::LifeSpan::FilterChain);
  filter_state->setData("envoy.upstream.dynamic_port",
...
TEST_F(ProxyFilterWithFilterStateHostDisabledTest, IgnoresFilterStatePort) {
  Upstream::ResourceAutoIncDec* circuit_breakers_(
      new Upstream::ResourceAutoIncDec(pending_requests_));
  ...
  // Should use "foo" from host header and default port 80 instead of filter state
  // values because the feature is disabled.
  Common::DynamicForwardProxy::MockLoadDnsCacheEntryHandle* handle =
      new Common::DynamicForwardProxy::MockLoadDnsCacheEntryHandle();
  EXPECT_CALL(*dns_cache_manager_->dns_cache_, loadDnsCacheEntry_(Eq("foo"), 80, _, _))
      .WillOnce(Return(
