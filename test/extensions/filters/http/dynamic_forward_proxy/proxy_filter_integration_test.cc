...
public:
  ...
  void initializeWithArgs(
      ...
      const std::string& typed_dns_resolver_config = "", bool use_sub_cluster = false,
      double dns_query_timeout = 5, bool disable_dns_refresh_on_failure = false,
...
// Test filter state host override functionality when it's enabled.
TEST_P(ProxyFilterIntegrationTest, FilterStateOverrides) {
  // Disable TLS for simplicity.
  upstream_tls_ = false;
  autonomous_upstream_ = true;

  const std::string set_filter_state_for_override_test_yaml = R"EOF(
...
typed_config:
  ...
  - object_key: "envoy.upstream.dynamic_host"
    format_string:
      text_format_source:
        inline_string: "localhost"
)EOF";

  // Initialize with the ``allow_dynamic_host_from_filter_state`` DFP option,
  // along with the custom filter for setting filter state.
  initializeWithArgs(1024, 1024, "", "", false, 5.0, false, true,
                     set_filter_state_for_override_test_yaml);
  codec_client_ = makeHttpConnection(lookupPort("http"));
  const auto response = codec_client_->makeHeaderOnlyRequest(default_request_headers_);
  ASSERT_TRUE(response->waitForEndStream());
  EXPECT_EQ("200", response->headers().getStatusValue());
  cleanupUpstreamAndDownstream();
}
