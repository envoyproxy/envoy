...
const std::string RBAC_CONFIG_PERMISSION_WITH_PATH_URI_TEMPLATE_MATCH = R"EOF(
name: rbac
typed_config:
  "@type": type.googleapis.com/envoy.extensions.filters.http.rbac.v3.RBAC
  rules:
...
TEST_P(RBACIntegrationTest, PermissionPathUriTemplateMatch) {
  config_helper_.prependFilter(RBAC_CONFIG_PERMISSION_WITH_PATH_URI_TEMPLATE_MATCH);
  initialize();
