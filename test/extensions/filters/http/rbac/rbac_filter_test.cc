...
TEST_F(RoleBasedAccessControlFilterTest, RouteLocalOverrideDynamicMetadataStats) {
  setupPolicy(envoy::config::rbac::v3::RBAC::DENY, "rbac_rules_prefix_");
  ...
  route_config.mutable_rbac()->set_rules_stat_prefix("rules_stat_prefix_");
  route_config.mutable_rbac()->set_shadow_rules_stat_prefix("shadow_rules_stat_prefix_");
  ...
  ASSERT_TRUE(filter_meta.fields().contains("shadow_rules_stat_prefix_shadow_engine_result"));
  EXPECT_EQ(
      "allowed",
      filter_meta.fields().at("shadow_rules_stat_prefix_shadow_engine_result").string_value());
  ASSERT_TRUE(filter_meta.fields().contains("shadow_rules_stat_prefix_shadow_effective_policy_id"));
  EXPECT_EQ("foobar", filter_meta.fields()
                          .at("shadow_rules_stat_prefix_shadow_effective_policy_id")
                          .string_value());
}
