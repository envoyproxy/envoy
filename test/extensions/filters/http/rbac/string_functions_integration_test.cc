...
public:
  void setUpWithConfig(const std::string& rbac_config) {
    ...
    const std::string cel_bootstrap_config = R"EOF(
      ...
      typed_config:
        "@type": type.googleapis.com/envoy.extensions.bootstrap.cel.CelEvaluatorConfig
...
INSTANTIATE_TEST_SUITE_P(Protocols, StringFunctionsIntegrationTest,
...
// Test replace() function with empty string replacement.
TEST_P(StringFunctionsIntegrationTest, ReplaceEmptyStringAllowed) {
  ...
  // The request should be allowed because "".replace("", "replaced") == "replaced".
  EXPECT_EQ("200", response->headers().getStatusValue());
}
