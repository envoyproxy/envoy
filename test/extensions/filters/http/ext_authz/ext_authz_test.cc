...
// Test per-route configuration with both base and specific having gRPC service
TEST_P(HttpFilterTestParam, PerRouteConfigurationMergingWithBothGrpcServices) {
  if (std::get<1>(GetParam())) {
    // Skip HTTP client test - gRPC service override applies to gRPC clients
    return;
  }

  // Create base configuration with gRPC service
  envoy::extensions::filters::http::ext_authz::v3::ExtAuthzPerRoute base_config;
  base_config.mutable_check_settings()
      ->mutable_grpc_service()
      ->mutable_envoy_grpc()
      ->set_cluster_name("base_cluster");

  // Create more specific configuration with different gRPC service
  envoy::extensions::filters::http::ext_authz::v3::ExtAuthzPerRoute specific_config;
  specific_config.mutable_check_settings()
      ->mutable_grpc_service()
      ->mutable_envoy_grpc()
...
// cleanup functions have been moved to the end of the test file

// Test per-route gRPC service configuration parsing and accessor methods
TEST_P(HttpFilterTestParam, PerRouteGrpcServiceConfigurationParsing) {
  if (std::get<1>(GetParam())) {
    // Skip HTTP client test - per-route gRPC service only applies to gRPC clients
    return;
  }

  // Test the per-route configuration parsing with gRPC service
  envoy::extensions::filters::http::ext_authz::v3::ExtAuthzPerRoute per_route_config;
  per_route_config.mutable_check_settings()
      ->mutable_grpc_service()
      ->mutable_envoy_grpc()
  ...
  // Verify the configuration can be created and has the gRPC service
  FilterConfigPerRoute per_route_filter_config(per_route_config);
  EXPECT_TRUE(per_route_filter_config.grpcService().has_value());
  EXPECT_TRUE(per_route_filter_config.grpcService().value().has_envoy_grpc());
  EXPECT_EQ(per_route_filter_config.grpcService().value().envoy_grpc().cluster_name(),
            "per_route_cluster");

  // Test that the configuration is properly stored
  const auto& grpc_service = per_route_filter_config.grpcService().value();
  EXPECT_EQ(grpc_service.envoy_grpc().cluster_name(), "per_route_cluster");
}
