...
#include "envoy/config/filter/http/ext_authz/v3alpha/ext_authz.pb.h"
#include "envoy/config/filter/http/ext_authz/v3alpha/ext_authz.pb.validate.h"
...
public:
  ...
  void initialize(std::string&& yaml) {
    envoy::config::filter::http::ext_authz::v3alpha::ExtAuthz proto_config{};
    if (!yaml.empty()) {
...
using CreateFilterConfigFunc = envoy::config::filter::http::ext_authz::v3alpha::ExtAuthz();
...
envoy::config::filter::http::ext_authz::v3alpha::ExtAuthz GetFilterConfig() {
...
TEST_F(HttpFilterTest, MergeConfig) {
  envoy::config::filter::http::ext_authz::v3alpha::ExtAuthzPerRoute settings;
...
TEST_F(HttpFilterTest, BadConfig) {
  ...
  EXPECT_THROW(TestUtility::downcastAndValidate<
                   const envoy::config::filter::http::ext_authz::v3alpha::ExtAuthz&>(proto_config),
...
TEST_F(HttpFilterTestParam, ContextExtensions) {
  ...
  envoy::config::filter::http::ext_authz::v3alpha::ExtAuthzPerRoute settingsvhost;
  ...
  envoy::config::filter::http::ext_authz::v3alpha::ExtAuthzPerRoute settingsroute;
