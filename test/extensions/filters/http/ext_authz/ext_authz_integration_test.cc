  ...
  void initializeConfig(GrpcInitializeConfigOpts opts = {}) {
    config_helper_.addConfigModifier([this,
      ...
      *bootstrap.mutable_node()->mutable_metadata() = metadata;
      ...
      static_cast<void>(ext_authz_filter.mutable_typed_config()->PackFrom(proto_config_));
...
TEST_P(ExtAuthzLocalReplyIntegrationTest, DeniedHeaderTest) {
  ...
  http_service:
    ...
    envoy::extensions::filters::network::http_connection_manager::v3::HttpFilter ext_authz_filter;
    ext_authz_filter.set_name("envoy.filters.http.ext_authz");
    static_cast<void>(ext_authz_filter.mutable_typed_config()->PackFrom(proto_config));
    config_helper_.prependFilter(MessageUtil::getJsonStringFromMessageOrError(ext_authz_filter));
  });
...
TEST_P(ExtAuthzGrpcIntegrationTest, ExtensionWithMatcherDynamicMetadata) {
  ...
  config_helper_.addConfigModifier([this](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    ...
    static_cast<void>(http_filter.mutable_typed_config()->PackFrom(extension_with_matcher));
