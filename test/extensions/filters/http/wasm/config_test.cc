...
TEST_P(WasmFilterConfigTest, YamlLoadFromRemoteSuccessBadcode) {
  ...
  auto h = Http::TestResponseHeaderMapImpl{{":status", "503"}};
  EXPECT_CALL(decoder_callbacks, encodeHeaders_(HeaderMapEqualRef(&h), true));
  ...
  EXPECT_EQ(context->onRequestHeaders(10, false), proxy_wasm::FilterHeadersStatus::StopAllIterationAndWatermark);
}
