...
void TestContext::onLog() {
  ...
  if (test == "headers") {
    ...
    }
    {
      ...
      std::string in;
      static_cast<void>(test_proto.SerializeToString(&in));
...
void TestRootContext::onTick() {
  if (test_ == "headers") { // NOLINT(clang-analyzer-optin.portability.UnixAPI)
    ...
    std::string function = "declare_property";
    {
      ...
      std::string in;
      static_cast<void>(args.SerializeToString(&in));
      char* out = nullptr;
      size_t out_size = 0;
    ...
    // Step 1: Create a signature using sign
    {
      ...
      std::string sign_in;
      static_cast<void>(sign_args.SerializeToString(&sign_in));
      ...
      if (WasmResult::Ok == proxy_call_foreign_function(sign_function.data(), sign_function.size(),
        ...
        if (sign_result.ParseFromString(absl::string_view(sign_out, static_cast<int>(sign_out_size))) &&
          ...
          // Step 2: Verify the signature using verify_signature
          {
            ...
            static_cast<void>(verify_args.SerializeToString(&verify_in));
