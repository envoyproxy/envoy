...
TEST_F(OriginalSrcHttpTest, DecodeHeadersIpv4AddressAddsOption) {
  ...
  for (const auto& option : *options) {
    option->setOption(socket, envoy::api::v3alpha::core::SocketOption::STATE_PREBIND);
  }
}
...
TEST_F(OriginalSrcHttpTest, FilterAddsMarkOption) {
  ...
  const auto mark_option = findOptionDetails(
      *options, ENVOY_SOCKET_SO_MARK, envoy::api::v3alpha::core::SocketOption::STATE_PREBIND);
