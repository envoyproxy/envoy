...
public:
  CorsFilterTest()
      : config_(new CorsFilterConfig("test.", *stats_.rootScope())),
        filter_(std::make_shared<CorsFilter>(config_)) {
  ...
  std::shared_ptr<CorsFilter> filter_;
...
TEST_F(CorsFilterTest, EncodeWithVaryHeader) {
  Http::TestRequestHeaderMapImpl request_headers{{"origin", "localhost"}};
  ...
  EXPECT_EQ(Http::FilterHeadersStatus::Continue, filter_->encodeHeaders(response_headers, false));
  EXPECT_EQ("localhost", response_headers.get_("access-control-allow-origin"));
  EXPECT_EQ("origin", response_headers.get_("vary"));

  EXPECT_EQ(Http::FilterDataStatus::Continue, filter_->encodeData(data_, false));
  EXPECT_EQ(Http::FilterTrailersStatus::Continue, filter_->encodeTrailers(response_trailers_));
}

TEST_F(CorsFilterTest, EncodeWithWildcardOriginNoVaryHeader) {
  Http::TestRequestHeaderMapImpl request_headers{{"origin", "localhost"}};

  // Set allow_origins to only have "*" to force the filter to use "*" as the response origin
  cors_policy_->allow_origins_.clear();
  cors_policy_->allow_origins_.emplace_back(makeExactStringMatcher("*"));
  ...
  EXPECT_EQ(Http::FilterHeadersStatus::Continue, filter_->encodeHeaders(response_headers, false));
  EXPECT_EQ("*", response_headers.get_("access-control-allow-origin"));
  EXPECT_EQ(nullptr, response_headers.get("vary"));
