...
TEST_F(LuaHttpFilterTest, ImmediateResponse) {
  ...
  SingleScriptConfig* script_config = config_->scriptConfig(GLOBAL_SCRIPT_NAME);
...
TEST_F(LuaHttpFilterTest, LuaFilterGetSourceCodeFromRouteDirectly) {
  ...
  const std::string SCRIPT_IN_ROUTE_DIRECTLY{R"EOF(
    function envoy_on_request(request_handle)
      request_handle:headers():add("script_info", "Script in route");
    end
  )EOF"};
  EXPECT_CALL(decoder_callbacks_, clearRouteCache());
  envoy::extensions::filters::http::lua::v3::Lua proto_config;
...
TEST_F(LuaHttpFilterTest, LuaFilterWithInlineCodeInBothHooks) {
  const std::string SCRIPT_IN_ROUTE_DIRECTLY{R"EOF(
    ...
    function envoy_on_response(response_handle)
      response_handle:headers():add("script_info", "Script in route by encode");
    end
  )EOF"};
  EXPECT_CALL(decoder_callbacks_, clearRouteCache());
  envoy::extensions::filters::http::lua::v3::Lua proto_config;
  ...
  Http::TestRequestHeaderMapImpl request_headers{{":path", "/"}};
  EXPECT_EQ(Http::FilterHeadersStatus::Continue, filter_->decodeHeaders(request_headers, true));
  EXPECT_EQ("Script in route by decode", request_headers.get_("script_info"));
  Http::TestResponseHeaderMapImpl response_headers{{":status", "200"}};
  EXPECT_EQ(Http::FilterHeadersStatus::Continue, filter_->encodeHeaders(response_headers, true));
  EXPECT_EQ("Script in route by encode", response_headers.get_("script_info"));
}
