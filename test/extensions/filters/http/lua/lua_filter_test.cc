...
// Test that handle:virtualHost():metadata() returns nil when no Lua metadata is available.
// This verifies that when a virtual host is matched but contains no metadata for the Lua filter,
// the metadata():get() function correctly returns nil from both request and response handles.
TEST_F(LuaHttpFilterTest, GetVirtualHostMetadataFromHandleNoLuaMetadata) {
  const std::string SCRIPT{R"EOF(
    ...
    function envoy_on_response(response_handle)
      if response_handle:virtualHost():metadata():get("foo.bar") == nil then
...
// Test that handle:virtualHost() returns nil when no virtual host matches
