...
// script touch virtualHost():metadata():get("key")
TEST_F(LuaHttpFilterTest, GetVirtualHostMetadataFromHandle) {
  const std::string SCRIPT{R"EOF(
    function envoy_on_request(request_handle)
      request_handle:logTrace(request_handle:virtualHost():metadata():get("foo.bar")["name"])
      request_handle:logTrace(request_handle:virtualHost():metadata():get("foo.bar")["prop"])
      request_handle:logTrace(request_handle:virtualHost():metadata():get("baz.bat")["name"])
      request_handle:logTrace(request_handle:virtualHost():metadata():get("baz.bat")["prop"])
    end
  )EOF"};
...
// Verifies that virtualHost():metadata():get() does not return metadata under the
...
TEST_F(LuaHttpFilterTest, GetVirtualHostMetadataFromHandleWithCanonicalName) {
  const std::string SCRIPT{R"EOF(
    function envoy_on_request(request_handle)
      if request_handle:virtualHost():metadata():get("foo.bar") == nil then
        request_handle:logTrace("ok")
      end
    end
  )EOF"};
...
// Verifies that virtualHost() returns nil when no route is available. This could happen
// when, for example, the request authority/path/headers don't match any configured routes.
TEST_F(LuaHttpFilterTest, GetVirtualHostFromHandleNoRoute) {
  const std::string SCRIPT{R"EOF(
    function envoy_on_request(request_handle)
