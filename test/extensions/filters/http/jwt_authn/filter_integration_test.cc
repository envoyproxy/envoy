...
std::string getFilterConfig(bool use_local_jwks, bool clear_route_cache = false) {
  return getAuthFilterConfig(ExampleConfig, use_local_jwks, clear_route_cache);
}
...
public:
  ...
  void initializeFilter(bool add_cluster, bool clear_route_cache = false) {
    config_helper_.prependFilter(getFilterConfig(false, clear_route_cache));
