...
public:
  ...
  void createVerifier() {
    ...
    filter_config_ = std::make_shared<FilterConfigImpl>(
        proto_config_, "", mock_factory_ctx_.server_factory_context_,
...
TEST_F(ProviderVerifierTest, TestRequiresNonexistentProvider) {
  ...
  absl::Status creation_status = absl::OkStatus();
  FilterConfigImpl filter_config(proto_config_, "", mock_factory_ctx_.server_factory_context_,
                                 makeOptRef<Init::Manager>(mock_factory_ctx_.init_manager_),
                                 creation_status);
