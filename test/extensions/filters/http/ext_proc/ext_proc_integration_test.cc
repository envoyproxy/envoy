...
TEST_P(ExtProcIntegrationTest, ResponseFromExtProcServerTooLarge) {
  ...
  proto_config_.mutable_processing_mode()->set_request_body_mode(ProcessingMode::STREAMED);
  proto_config_.mutable_processing_mode()->set_response_header_mode(ProcessingMode::SKIP);
  proto_config_.mutable_grpc_service()->mutable_envoy_grpc()->mutable_max_receive_message_length()->set_value(1024);
  initializeConfig();
  HttpIntegrationTest::initialize();
  ...
  processRequestBodyMessage(
      *grpc_upstreams_[0], true,
      [&body_str](const HttpBody& body, BodyResponse& body_resp) {
        EXPECT_TRUE(body.end_of_stream());
