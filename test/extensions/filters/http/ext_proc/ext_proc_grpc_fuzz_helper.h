...
// TODO: what namespace should these helper functions be in?
...
// use the same two functions as GET and POST but with a different method value
...
enum class HttpMethod {
  ...
  HEAD,
  PUT,
  kMaxValue = POST // TODO: update when methods supported
};
...
enum class BodySendSetting {
  ...
  Streamed,
  BufferedPartial,
  kMaxValue = Streamed // TODO: update when BufferedPartial implemented
};

// TODO: should this function be put in a standard place?
StatusCode RandomHttpStatus(FuzzedDataProvider* provider) {
  switch (provider->ConsumeIntegralInRange<uint32_t>(0,55)) {
    case 0:
      ENVOY_LOG_MISC(trace, "Selected HTTP Status Code {} (Continue)", StatusCode::Continue);
      // TODO: switch this back after bug fix
...
// TODO: should this function be put in a standard place?
grpc::StatusCode RandomGrpcStatusCode(FuzzedDataProvider* provider) {
  switch (provider->ConsumeIntegralInRange<uint32_t>(0, 16)) {
    case 0:
...
grpc::Status RandomGrpcStatusWithMessage(FuzzedDataProvider* provider) {
  grpc::StatusCode code = RandomGrpcStatusCode(provider);
  ENVOY_LOG_MISC(trace, "Closing stream with StatusCode {}", code);
  return grpc::Status(code, provider->ConsumeRandomLengthString());
}

// TODO: fix argument description once implemented
...
void RandomizeHeaderMutation(HeaderMutation*, ProcessingRequest*, FuzzedDataProvider*, bool) {
    // 1. Randomize set_headers
    /* TODO: randomly add headers
    if (provider->ConsumeBool()) {
      HeaderValueOption* set_header = msg->add_set_headers();
      set_header->mutable_header()->set_key("test");
      set_header->mutable_header()->set_value("testValue");
      set_header->mutable_append()->set_value(false);
    }
    */

    // 2. Randomize remove headers
    /* TODO: Randomly remove headers
    if (provider->ConsumeBool()) {
      msg->add_remove_headers("garbage");
    }*/
}

void RandomizeCommonResponse(CommonResponse* msg, ProcessingRequest* req, FuzzedDataProvider* provider) {
  // 1. Randomize status
  if (provider->ConsumeBool()) {
    switch (provider->ConsumeIntegralInRange<uint32_t>(0, 1)) {
      case 0:
  ...
  if (provider->ConsumeBool()) {
    ...
    RandomizeHeaderMutation(msg->mutable_header_mutation(), req, provider, false);
  }
  ...
  if (provider->ConsumeBool()) {
    ...
    if (provider->ConsumeBool()) {
      ...
      body_mutation->set_body(provider->ConsumeRandomLengthString());
    } else {
      ENVOY_LOG_MISC(trace, "CommonResponse setting body_mutation, clearing body");
      body_mutation->set_clear_body(provider->ConsumeBool());
    }
  }

  // 4. TODO: Randomize trailers - [skipping because tailers not implemented]
...
void RandomizeImmediateResponse(ImmediateResponse* msg, ProcessingRequest* req, FuzzedDataProvider* provider) {
  // 1. Randomize HTTP status (required)
  ENVOY_LOG_MISC(trace, "ImmediateResponse setting status");
  msg->mutable_status()->set_code(RandomHttpStatus(provider));

  // 2. Randomize headers
  if (provider->ConsumeBool()) {
    ENVOY_LOG_MISC(trace, "ImmediateResponse setting headers");
    RandomizeHeaderMutation(msg->mutable_headers(), req, provider, false);
  }

  // 3. Randomize body
  if (provider->ConsumeBool()) {
    ENVOY_LOG_MISC(trace, "ImmediateResponse setting body");
    msg->set_body(provider->ConsumeRandomLengthString());
  }

  // 4. Randomize grpc_status
  if (provider->ConsumeBool()) {
    ENVOY_LOG_MISC(trace, "ImmediateResponse setting grpc_status");
    msg->mutable_grpc_status()->set_status(RandomGrpcStatusCode(provider));
  }

  // 5. Randomize details
  if (provider->ConsumeBool()) {
    ENVOY_LOG_MISC(trace, "ImmediateResponse setting details");
    msg->set_details(provider->ConsumeRandomLengthString());
  }
}

void RandomizeOverrideResponse(ProcessingMode* msg, FuzzedDataProvider* provider) {
  // 1. Randomize request_header_mode
  if (provider->ConsumeBool()) {
    switch (provider->ConsumeEnum<HeaderSendSetting>()) {
  ...
  if (provider->ConsumeBool()) {
    switch (provider->ConsumeEnum<BodySendSetting>()) {
...
void RandomizeResponse(ProcessingResponse* resp, ProcessingRequest* req, FuzzedDataProvider* provider) {
  switch (provider->ConsumeEnum<ResponseType>()) {
    ...
    case ResponseType::RequestHeaders: {
      ...
      RandomizeCommonResponse(msg, req, provider);
      break;
    }
    ...
    case ResponseType::RequestTrailers: {
      ...
      RandomizeHeaderMutation(header_mutation, req, provider, true);
      break;
    }
    ...
    case ResponseType::ImmediateResponse: {
      ...
      RandomizeImmediateResponse(msg, req, provider);
      break;
    }
    default:
