  ...
  void setup(uint32_t upstream_count) {
    ...
    config_helper_.addConfigModifier([this](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
      ...
      if (cluster_lookup_test_) {
        ...
        static_cast<void>(listener_filter->typed_config().UnpackTo(&dns_filter_config));

        dns_filter_config.clear_client_config();
        static_cast<void>(listener_filter->mutable_typed_config()->PackFrom(dns_filter_config));
      }
