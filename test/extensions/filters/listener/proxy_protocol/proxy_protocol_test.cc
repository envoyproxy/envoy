...
TEST(ConfigTest, AllVersionsCannotBeDisallowed) {
  envoy::extensions::filters::listener::proxy_protocol::v3::ProxyProtocol proto_config;
  proto_config.mutable_disallowed_versions()->Add(ProxyProtocolConfig::V1);
  proto_config.mutable_disallowed_versions()->Add(ProxyProtocolConfig::V2);
  Stats::TestUtil::TestStore stats_store;
  EXPECT_THROW(ProxyProtocol::Config(*stats_store.rootScope(), proto_config),
          ...
          filter_manager.addAcceptFilter(
              nullptr, std::make_unique<Filter>(std::make_shared<Config>(
                           *stats_store_.rootScope(),
                           (nullptr != proto_config) ? *proto_config
...
TEST_P(ProxyProtocolTest, V1UnsupportedIPv4) {
  ...
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v1.error").value(), 1);
}
...
TEST_P(ProxyProtocolTest, V1Basic) {
  ...
  disconnect();
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v1.found").value(), 1);
}
...
TEST_P(ProxyProtocolTest, AllowTinyNoProxyProtocol) {
  ...
  expectData(msg);
  disconnect();
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.not_found_allowed").value(), 1);
}
...
TEST_P(ProxyProtocolTest, V2Basic) {
  ...
  disconnect();
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v2.found").value(), 1);
}
...
TEST_P(ProxyProtocolTest, V2UnsupportedIPv4) {
  ...
  write(buffer, sizeof(buffer));
  expectProxyProtoError();
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v2.error").value(), 1);
}
...
TEST_P(ProxyProtocolTest, V1TooLong) {
  ...
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.not_found_disallowed").value(), 1);
}
...
TEST_P(ProxyProtocolTest, V1TooLongWithAllowNoProxyProtocol) {
  ...
  // Not tracked as v1 due to missing /r/n at end.
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v1.error").value(), 0);
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.not_found_allowed").value(), 0);
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.not_found_disallowed").value(), 1);
}
...
TEST_P(ProxyProtocolDisallowedVersionsTest, V2DisallowedV2BasicRejected) {
  ...
  expectConnectionError();
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v2.found").value(), 1);
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v2.disallowed").value(), 1);
}
...
TEST_P(ProxyProtocolDisallowedVersionsTest, V1DisallowedV1BasicRejected) {
  ...
  expectConnectionError();
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v1.found").value(), 1);
  EXPECT_EQ(stats_store_.counter("downstream_cx_proxy_proto.versions.v1.disallowed").value(), 1);
}
