...
public:
  ...
  ~LocalRateLimitIntegrationTest() override = default;
...
filter_disabled:
  ...
  void initializeWithLocalRateLimit(bool ssl_client, const std::string& log_format,
    ...
    std::string local_ratelimit_config =
        ConfigHelper::LocalRateLimitFilter(enable_ja3_fingerprinting);
    if (listener_filter_disabled.has_value()) {
      local_ratelimit_config =
          appendMatcher(local_ratelimit_config, listener_filter_disabled.value());
    }
    config_helper_.addListenerFilter(local_ratelimit_config);
  ...
  void setupConnections(bool listener_filter_disabled, bool expect_connection_open, bool ssl_client,
                        ...
                        bool enable_ja3_fingerprinting = false) {
    initializeWithLocalRateLimit(ssl_client, log_format, listener_filter_disabled,
...
TEST_P(LocalRateLimitIntegrationTest, AllListenerFiltersAreEnabledByDefault) {
...
// The local_ratelimit is disabled. The ALPN won't be sniffed out and no filter chain is matched.
TEST_P(LocalRateLimitIntegrationTest, DisabledLocalRateLimitFailsFilterChainFind) {
...
TEST_P(LocalRateLimitIntegrationTest, ContinueOnListenerTimeout) {
  setupConnections(/*listener_filter_disabled=*/false, /*expect_connection_open=*/true,
                   /*ssl_client=*/false);
...
TEST_P(LocalRateLimitIntegrationTest, JA3FingerprintIsSet) {
