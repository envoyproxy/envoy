...
public:
  ...
  void dynamicMetadataTest(CheckStatus status, const std::string& http_status) {
    ...
    client_->check(request_callbacks_, request, parent_span_,*Http::StaticEmptyHeaders::get().request_headers, stream_info_);
...
TEST_F(ExtAuthzHttpClientTest, NoCluster) {
  ...
  EXPECT_CALL(request_callbacks_,
              onComplete_(WhenDynamicCastTo<ResponsePtr&>(AuthzErrorResponse(CheckStatus::Error))));
  client_->check(request_callbacks_, envoy::service::auth::v3::CheckRequest{}, parent_span_, *Http::StaticEmptyHeaders::get().request_headers,
                 stream_info_);
}
