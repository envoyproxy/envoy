...
public:
    MOCK_METHOD(absl::optional<ClusterConfig>, computeClusterConfigForTopic, (const std::string&), (const));
...
TEST(UpstreamKafkaFacadeTest, shouldCreateProducerOnlyOnceForTheSameCluster) {
    ...
    EXPECT_CALL(clustering_configuration, computeClusterConfigForTopic(topic1)).WillOnce(Return(cluster_config));
    EXPECT_CALL(clustering_configuration, computeClusterConfigForTopic(topic2)).WillOnce(Return(cluster_config));
    ThreadLocal::MockInstance slot_allocator;
    EXPECT_CALL(slot_allocator, allocateSlot()).WillOnce(Invoke(&slot_allocator, &ThreadLocal::MockInstance::allocateSlot_));
...
TEST(UpstreamKafkaFacadeTest, shouldCreateDifferentProducersForDifferentClusters) {
    ...
    EXPECT_CALL(clustering_configuration, computeClusterConfigForTopic(topic1)).WillOnce(Return(cluster_config1));
    const ClusterConfig cluster_config2 = { "cluster2", 1, {{"bootstrap.servers", "localhost:9092"}}};
    EXPECT_CALL(clustering_configuration, computeClusterConfigForTopic(topic2)).WillOnce(Return(cluster_config2));
    ThreadLocal::MockInstance slot_allocator;
    EXPECT_CALL(slot_allocator, allocateSlot()).WillOnce(Invoke(&slot_allocator, &ThreadLocal::MockInstance::allocateSlot_));
    Thread::ThreadFactory& thread_factory = Thread::threadFactoryForTest();
    UpstreamKafkaFacade testee = { clustering_configuration, slot_allocator, thread_factory };
