...
public:
  ...
  void setupDelayFault(bool enable_fault) {
    ...
    EXPECT_CALL(runtime_.snapshot_, featureEnabled(_, _, 100)).Times(AnyNumber());
    EXPECT_CALL(runtime_.snapshot_, featureEnabled("mongo.fault.fixed_delay.percent", 50, 100))
