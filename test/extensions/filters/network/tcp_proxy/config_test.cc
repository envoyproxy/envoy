...
TEST_P(RouteIpListConfigTest, DEPRECATED_FEATURE_TEST(TcpProxy)) {
  ...
  envoy::config::filter::network::tcp_proxy::v3alpha::TcpProxy proto_config;
...
TEST(ConfigTest, ValidateFail) {
  ...
  EXPECT_THROW(ConfigFactory().createFilterFactoryFromProto(
                   envoy::config::filter::network::tcp_proxy::v3alpha::TcpProxy(), context),
...
TEST(ConfigTest, ConfigTest) {
  NiceMock<Server::Configuration::MockFactoryContext> context;
  ConfigFactory factory;
  envoy::config::filter::network::tcp_proxy::v3alpha::TcpProxy config =
      *dynamic_cast<envoy::config::filter::network::tcp_proxy::v3alpha::TcpProxy*>(
