...
public:
  ...
  std::string encodeConnect(const bool readonly = false, const uint64_t zxid = 100,
                            const uint32_t session_timeout = 10, const uint32_t session_id = 200,
  ...
  std::string encodeBadMessage() const {
    Buffer::OwnedImpl buffer;
    // Bad length.
  ...
  std::string encodeAuth(const std::string& scheme) const {
    Buffer::OwnedImpl buffer;
  ...
  std::string encodeGetData(const std::string& path, const bool watch,
  ...
  std::string encodeCreateRequest(const std::string& path, const std::string& data,
  ...
  std::string encodeSetRequest(const std::string& path, const std::string& data,
...
TEST_F(ZooKeeperFilterTest, Connect) {
  ...
  std::string data = encodeConnect();
  Buffer::InstancePtr connect_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*connect_data, false));
...
TEST_F(ZooKeeperFilterTest, ConnectReadonly) {
  ...
  std::string data = encodeConnect(true);
  Buffer::InstancePtr connect_data(new Buffer::OwnedImpl(data));
...
TEST_F(ZooKeeperFilterTest, Fallback) {
  ...
  std::string data = encodeBadMessage();
  Buffer::InstancePtr bad_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*bad_data, false));
  EXPECT_EQ(0UL, config_->stats().connect_.value());
...
TEST_F(ZooKeeperFilterTest, PingRequest) {
  ...
  std::string data = encodePing();
  Buffer::InstancePtr ping_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*ping_data, false));
...
TEST_F(ZooKeeperFilterTest, AuthRequest) {
  ...
  std::string data = encodeAuth("digest");
  Buffer::InstancePtr auth_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*auth_data, false));
...
TEST_F(ZooKeeperFilterTest, GetDataRequest) {
  ...
  std::string data = encodeGetData("/foo", true);
  Buffer::InstancePtr get_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*get_data, false));
...
TEST_F(ZooKeeperFilterTest, CreateRequest) {
  ...
  std::string data = encodeCreateRequest("/foo", "bar", false, false);
  Buffer::InstancePtr create_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*create_data, false));
...
TEST_F(ZooKeeperFilterTest, SetRequest) {
  ...
  std::string data = encodeSetRequest("/foo", "bar", -1);
  Buffer::InstancePtr set_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*set_data, false));
...
TEST_F(ZooKeeperFilterTest, GetChildrenRequest) {
  ...
  std::string data = encodeGetData("/foo", false, true);
  Buffer::InstancePtr getchildren_data(new Buffer::OwnedImpl(data));

  EXPECT_EQ(Envoy::Network::FilterStatus::Continue, filter_->onData(*getchildren_data, false));
