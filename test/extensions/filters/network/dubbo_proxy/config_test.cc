...
resources:
  ...
  context_.mock_server_context_.cluster_manager_.subscription_factory_.callbacks_->onConfigUpdate(
      decoded_resources.refvec_, response.version_info());
  auto message_ptr =
      context_.mock_server_context_.admin_.config_tracker_.config_tracker_callbacks_["drds_routes"](
