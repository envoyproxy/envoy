...
public:
  ...
  void writeHessianErrorResponseMessage(Buffer::Instance& buffer, bool is_event,
                                        int64_t request_id) {
    uint8_t s1 = (request_id >> 56) & 0xFF;
    uint8_t s2 = (request_id >> 48) & 0xFF;
    uint8_t s3 = (request_id >> 40) & 0xFF;
    uint8_t s4 = (request_id >> 32) & 0xFF;
    uint8_t s5 = (request_id >> 24) & 0xFF;
    uint8_t s6 = (request_id >> 16) & 0xFF;
    uint8_t s7 = (request_id >> 8) & 0xFF;
    uint8_t s8 = request_id & 0xFF;
    ...
    addSeq(buffer, {
                       0xda, 0xbb, msg_type, 0x46,     // Response Message Header, SERVICE_ERROR
                       s1, s2, s3, s4, s5, s6, s7, s8, // Request Id
                       0x00, 0x00, 0x00, 0x01,         // Body Length
                   });
  }
  ...
  void writeHessianExceptionResponseMessage(Buffer::Instance& buffer, bool is_event,
    ...
    addSeq(buffer, {
                       0xda, 0xbb, msg_type, 0x14,                  // Response Message Header
                       s1,   s2,   s3,       s4,   s5,  s6, s7, s8, // Request Id
                       0x00, 0x00, 0x00,     0x06,                  // Body Length
                       0x90,                                        // return type, exception
  ...
  void writeInvalidResponseMessage(Buffer::Instance& buffer) {
    addSeq(buffer, {
                       0xda, 0xbb, 0x43, 0x14, // Response Message Header, illegal serialization id
  ...
  void writeInvalidRequestMessage(Buffer::Instance& buffer) {
    addSeq(buffer, {
                       0xda, 0xbb, 0xc3, 0x00, // Request Message Header, illegal serialization id
                       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, // Request Id
  ...
  void writePartialHessianRequestMessage(Buffer::Instance& buffer, bool is_one_way, bool is_event,
    ...
    if (start) {
      addSeq(buffer, {
                         0xda, 0xbb, msg_type, 0x00,                   // Request Message Header
                         s1,   s2,   s3,       s4,   s5,  s6,  s7, s8, // Request Id
                         0x00, 0x00, 0x00,     0x16,                   // Body Length
                         0x05, '2',  '.',      '0',  '.', '2',         // Dubbo version
                     });
