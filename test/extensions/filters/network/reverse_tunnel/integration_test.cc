...
TEST_P(ReverseTunnelFilterIntegrationTest, ValidationWithDynamicMetadataSuccess) {
  ...
  config_helper_.addConfigModifier([](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    ...
    static_cast<void>(filter.mutable_typed_config()->PackFrom(filter_config));
  ...
  config_helper_.addConfigModifier([](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    for (auto& extension : *bootstrap.mutable_bootstrap_extensions()) {
      if (extension.name() == "envoy.bootstrap.reverse_tunnel.upstream_socket_interface") {
        ...
        static_cast<void>(extension.typed_config().UnpackTo(&config));
        config.mutable_enable_tenant_isolation()->set_value(true);
        static_cast<void>(extension.mutable_typed_config()->PackFrom(config));
        break;
      }
    }
