...
DEFINE_PROTO_FUZZER(const envoy::extensions::filters::network::ext_authz::ExtAuthzTestCase& input) {
  ...
  filter_callbacks.connection_.stream_info_.downstream_connection_info_provider_->setRemoteAddress(addr);
  filter_callbacks.connection_.stream_info_.downstream_connection_info_provider_->setLocalAddress(addr);
