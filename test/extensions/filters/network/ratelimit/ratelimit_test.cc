...
stat_prefix: name
  ...
  Extensions::NetworkFilters::RateLimitFilter::ConfigSharedPtr rl_config(
      new Extensions::NetworkFilters::RateLimitFilter::Config(
          proto_config, factory_context.mock_server_context_.scope_,
  ...
  factory_context.mock_server_context_.cluster_manager_.initializeThreadLocalClusters(
      {"fake_cluster"});
  ...
  manager.addReadFilter(std::make_shared<TcpProxy::Filter>(
      tcp_proxy_config, factory_context.mock_server_context_.cluster_manager_));
  ...
  EXPECT_CALL(factory_context.mock_server_context_.cluster_manager_.thread_local_cluster_,
              tcpConnPool(_, _))
