...
TEST(RedisProxyFilterConfigFactoryTest, ValidateFail) {
  ...
  EXPECT_THROW(RedisProxyFilterConfigFactory().createFilterFactoryFromProto(
                   envoy::config::filter::network::redis_proxy::v3alpha::RedisProxy(), context),
               ProtoValidationException);
}

TEST(RedisProxyFilterConfigFactoryTest, NoUpstreamDefined) {
  envoy::config::filter::network::redis_proxy::v3alpha::RedisProxy::ConnPoolSettings settings;
  settings.mutable_op_timeout()->CopyFrom(Protobuf::util::TimeUtil::MillisecondsToDuration(20));

  envoy::config::filter::network::redis_proxy::v3alpha::RedisProxy config;
...
stat_prefix: foo
  ...
  envoy::config::filter::network::redis_proxy::v3alpha::RedisProxy proto_config;
...
settings:
  ...
  RedisProxyFilterConfigFactory factory;
  envoy::config::filter::network::redis_proxy::v3alpha::RedisProxy proto_config =
      *dynamic_cast<envoy::config::filter::network::redis_proxy::v3alpha::RedisProxy*>(
