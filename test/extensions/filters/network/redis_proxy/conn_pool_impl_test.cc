...
public:
  void setup(bool cluster_exists = true, bool hashtagging = true, uint32_t max_unknown_conns = 100,
    ...
    std::shared_ptr<InstanceImpl> conn_pool_impl = std::make_shared<InstanceImpl>(
        ...
        std::nullopt, std::nullopt, local_zone_, protocol_version);
    conn_pool_impl->init();
...
// A load balancer that ALSO implements ShardMembershipResolver, so the conn pool's read-policy
// placement callbacks (which dynamic_cast the cluster LB to that interface) can be driven against
// a controllable shard snapshot. NiceMock<MockLoadBalancer> supplies the LoadBalancer surface.
...
// Strict REPLICA excludes the slot primary from the feasible set: the LB never places there (it
// draws from the replicas-only host set), so a recorded owner that IS the primary can only be a
// member PROMOTED after placement — it is re-homed onto a replica, unless it is the shard's sole
// member, where re-placing would strand delivery in a null-resolve backoff loop. Replicas stay
// valid; a non-member never is.
TEST_F(RedisConnPoolImplTest, HostServesChannelSlotStrictReplicaExcludesPromotedPrimary) {
  FakeShardMembershipResolverLb fake_lb;
  ON_CALL(cm_.thread_local_cluster_, loadBalancer()).WillByDefault(ReturnRef(fake_lb));
  read_policy_ =
      envoy::extensions::filters::network::redis_proxy::v3::RedisProxy::ConnPoolSettings::REPLICA;
  ...
  auto replica = makeHealthHost(Upstream::Host::Health::Healthy);
  auto stranger = makeHealthHost(Upstream::Host::Health::Healthy);
  ASSERT_NE(Clusters::Redis::redisSlotForKey("chan"), Clusters::Redis::redisSlotForKey("solo"));
  fake_lb.members_.push_back(
      {Clusters::Redis::redisSlotForKey("chan"), shardMembers({primary, replica})});
  // A sole-member shard: the promoted ex-replica is all that is left.
  fake_lb.members_.push_back({Clusters::Redis::redisSlotForKey("solo"), shardMembers({primary})});

  auto& pool = threadLocalPool();
  EXPECT_FALSE(pool.hostServesChannelSlot("chan", primary)); // promoted -> re-home onto a replica
  EXPECT_TRUE(pool.hostServesChannelSlot("chan", replica));
  EXPECT_FALSE(pool.hostServesChannelSlot("chan", stranger));
  EXPECT_TRUE(pool.hostServesChannelSlot("solo", primary)); // nowhere better to go — keep

  tls_.shutdownThread();
}

// The prefer/any/zone policies include the primary in their fallback chains, so validity is plain
// health-agnostic shard MEMBERSHIP: both the primary and a replica are valid homes; a non-member
// is not.
TEST_F(RedisConnPoolImplTest, HostServesChannelSlotMembershipUnderPreferReplicaPolicy) {
  FakeShardMembershipResolverLb fake_lb;
  ON_CALL(cm_.thread_local_cluster_, loadBalancer()).WillByDefault(ReturnRef(fake_lb));
  read_policy_ = envoy::extensions::filters::network::redis_proxy::v3::RedisProxy::
      ConnPoolSettings::PREFER_REPLICA;
...
// Placement literally follows the read rule: the resolve hands the LB a context that states
// ``is_read`` EXPLICITLY (no synthetic request) with the configured policy attached, so
// readPolicy()/client zone apply exactly as on the data path.
TEST_F(RedisConnPoolImplTest, ChooseUpstreamHostForChannelUsesReadPolicyContext) {
  read_policy_ = envoy::extensions::filters::network::redis_proxy::v3::RedisProxy::
      ConnPoolSettings::PREFER_REPLICA;
  local_zone_ = "zone-a";
  setup();
      ...
      .WillByDefault(
          Invoke([&](Upstream::LoadBalancerContext* ctx) -> Upstream::HostSelectionResponse {
            ...
            if (rctx != nullptr) {
              ...
              saw_policy = rctx->readPolicy() == Common::Redis::Client::ReadPolicy::PreferReplica;
              saw_zone = rctx->clientZone() == "zone-a";
            }
            return {cm_.thread_local_cluster_.lb_.host_};
          }));
  EXPECT_CALL(*cm_.thread_local_cluster_.lb_.host_, address())
...
TEST_F(RedisConnPoolImplTest, HostServesChannelSlotFollowsPrimaryWithoutResolver) {
  ...
  // A transient null resolution keeps the record (the null-resolution tolerance every policy
  // shares).
  ON_CALL(cm_.thread_local_cluster_.lb_, chooseHost(_))
...
// A non-cluster upstream has no ShardMembershipResolver, so the topology sweep judges records via
// READ-POLICY PARITY (hostServesChannelSlot re-resolves and compares). End-to-end through the
// pool: when the cluster LB's answer for a subscribed channel flips A -> B, the executed sweep
// must send the courtesy SUNSUBSCRIBE on A's existing subscription client and re-issue the
// SSUBSCRIBE through a fresh subscription client on B — otherwise publishes (which follow the
// same hash) would land on B while the subscription sat on A, silently losing messages.
TEST_F(RedisConnPoolImplTest, NonClusterTopologySweepMovesSubscriptionWithLbAnswer) {
  setup(/*cluster_exists=*/true, /*hashtagging=*/true, /*max_unknown_conns=*/100,
        /*dns_cache=*/nullptr, /*redis_cx_rate_limit_per_sec=*/100,
        Common::Redis::RespProtocolVersion::Resp3);
  ...
  auto* client_a = new NiceMock<Common::Redis::Client::MockClient>();
  EXPECT_CALL(*this, create_(_)).WillOnce(Return(client_a));

  // Subscribe "ch" through the registry: the pool records host A and opens A's dedicated
  // subscription client.
  auto registry = conn_pool_->subscriptionRegistryShared();
  ASSERT_NE(nullptr, registry);
  EXPECT_CALL(cm_.thread_local_cluster_.lb_, chooseHost(_))
      .WillOnce(Return(Upstream::HostSelectionResponse{cm_.thread_local_cluster_.lb_.host_}));
  EXPECT_CALL(*client_a, setPushCallbacks(_));
  EXPECT_CALL(*client_a, sendCommand(_)); // the initial SSUBSCRIBE
  NiceMock<Network::MockConnection> downstream;
  auto subscriber = std::make_shared<RedisProxy::DownstreamSubscriber>(
      downstream, testDownstreamSubscriberStats());
  registry->subscribe({"ch"}, subscriber);

  // Capture the sweep posted by the member-update callback.
  std::vector<Event::PostCb> posted;
  EXPECT_CALL(tls_.dispatcher_, post(_)).WillRepeatedly([&posted](Event::PostCb cb) {
    posted.push_back(std::move(cb));
  });
  cm_.thread_local_cluster_.cluster_.prioritySet().getMockHostSet(0)->runCallbacks({}, {});
  ASSERT_EQ(1UL, posted.size());

  // Flip the LB: every subsequent resolve for the channel answers B.
  auto host_b = std::make_shared<NiceMock<Upstream::MockHost>>();
  auto address_b = *Network::Utility::resolveUrl("tcp://127.0.0.1:3001");
  EXPECT_CALL(*host_b, address()).WillRepeatedly(Return(address_b));
  EXPECT_CALL(cm_.thread_local_cluster_.lb_, chooseHost(_))
      .WillRepeatedly(
          Invoke([host_b](Upstream::LoadBalancerContext*) -> Upstream::HostSelectionResponse {
            return {host_b};
          }));

  // Executing the sweep: parity says A is stale (the LB now answers B) -> courtesy SUNSUBSCRIBE on
  // A's client, then an SSUBSCRIBE through a NEW subscription client created for B. The ORDER is
  // part of the contract (old owner dropped + sent its SUNSUBSCRIBE BEFORE the reissue, so cleanup
  // the send path observes the up-to-date map), so pin it with a sequence. A retire of A's
  // now-idle subscription connection (close + deferred delete) is incidental here.
  EXPECT_CALL(tls_.dispatcher_, deferredDelete_(_)).Times(testing::AnyNumber());
  auto* client_b = new NiceMock<Common::Redis::Client::MockClient>();
  EXPECT_CALL(*client_b, setPushCallbacks(_));
  {
    testing::InSequence ordered;
    EXPECT_CALL(*client_a, sendCommand(_)).WillOnce(Invoke([](const Common::Redis::RespValue& cmd) {
      ASSERT_EQ(Common::Redis::RespType::Array, cmd.type());
      EXPECT_EQ("SUNSUBSCRIBE", cmd.asArray()[0].asString());
    }));
    EXPECT_CALL(*this, create_(_)).WillOnce(Return(client_b));
    EXPECT_CALL(*client_b, sendCommand(_)).WillOnce(Invoke([](const Common::Redis::RespValue& cmd) {
      ASSERT_EQ(Common::Redis::RespType::Array, cmd.type());
      EXPECT_EQ("SSUBSCRIBE", cmd.asArray()[0].asString());
    }));
  }
