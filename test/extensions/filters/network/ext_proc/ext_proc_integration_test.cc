      ...
      if (typed_field.has_struct_value()) {
        ...
        for (const auto& [namespace_name, string_value] : typed_namespaces.fields()) {
          if (string_value.has_string_value()) {
            ...
            static_cast<void>(typed_value.PackFrom(string_proto));
          ...
          if (config_modifier) {
            ...
            for (auto& filter : *filters) {
              if (filter.name() == "envoy.network_ext_proc.ext_proc_filter") {
                ...
                static_cast<void>(filter.mutable_typed_config()->UnpackTo(&config));
      ...
      for (auto& filter : *filters) {
        if (filter.name() == "test.metadata_setter") {
          ...
          if (filter.has_typed_config()) {
            static_cast<void>(filter.typed_config().UnpackTo(&existing_config));
          }
          ...
          static_cast<void>(filter.mutable_typed_config()->PackFrom(existing_config));
          break;
        }
      }
    });
