...
// Validate that seting path normalization options and escape slash action at the same time throws
// exception.
TEST_F(HttpConnectionManagerConfigTest, SetPathNormalizationAndEscapeSlashesAction) {
  const std::string yaml_string = R"EOF(
  stat_prefix: ingress_http
  route_config:
    name: local_route
