...
TEST(GenericStatusCodeFormatterProviderTest, GenericStatusCodeFormatterProviderTest) {
  ...
  EXPECT_EQ(formatter.formatWithContext(Formatter::Context().setExtension(context), stream_info),
            absl::nullopt);
  EXPECT_TRUE(
      formatter.formatValueWithContext(Formatter::Context().setExtension(context), stream_info)
          .has_null_value());
  ...
  EXPECT_EQ(
      formatter.formatWithContext(Formatter::Context().setExtension(context), stream_info).value(),
      "1234");
  EXPECT_EQ(
      formatter.formatValueWithContext(Formatter::Context().setExtension(context), stream_info)
          .number_value(),
