...
public:
  ...
  void expectFinalizeUpstreamSpanAny() {
    EXPECT_CALL(*child_span_, setTag(testing::An<absl::string_view>(), _)).Times(testing::AnyNumber());
    EXPECT_CALL(*child_span_, setTag(testing::An<const Tracing::Tag&>(), _)).Times(testing::AnyNumber());
