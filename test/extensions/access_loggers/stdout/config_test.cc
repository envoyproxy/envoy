...
class StdouAccessLogTest : public testing::Test {
public:
  StdouAccessLogTest() = default;

  void runTest(const std::string& yaml, absl::string_view expected, bool is_json) {
    envoy::extensions::access_loggers::stdout::v3::StdoutAccessLog fal_config;
...
TEST_F(StdouAccessLogTest, EmptyFormat) {
  runTest(
      "{}",
...
TEST_F(StdouAccessLogTest, LogFormatText) {
  runTest(
      R"(
  log_format:
    text_format_source:
...
TEST_F(StdouAccessLogTest, LogFormatJson) {
  runTest(
      R"(
  log_format:
    json_format:
