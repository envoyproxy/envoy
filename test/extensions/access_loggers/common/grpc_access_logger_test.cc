...
TEST_F(GrpcAccessLogTest, BasicFlow) {
  ...
  logger_->log(mockHttpEntry(), false);
  ...
  logger_->log(ProtobufWkt::Empty(), false);
...
TEST_F(GrpcAccessLogTest, Batching) {
  ...
  logger_->log(std::move(big_entry), false);
  EXPECT_EQ(2, logger_->numClears());
}
...
private:
  ...
  createLogger(const envoy::extensions::access_loggers::grpc::v3::CommonGrpcAccessLogConfig& config,
    ...
    auto logger = std::make_shared<MockGrpcAccessLoggerImpl>(
        std::move(client), config, buffer_flush_interval_msec, max_buffer_size_bytes, scope,
