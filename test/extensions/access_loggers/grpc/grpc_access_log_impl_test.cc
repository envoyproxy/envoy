...
public:
  ...
  using AccessLogCallbacks =
      Grpc::AsyncStreamCallbacks<envoy::service::accesslog::v3alpha::StreamAccessLogsResponse>;
  ...
  void expectStreamMessage(MockAccessLogStream& stream, const std::string& expected_message_yaml) {
    envoy::service::accesslog::v3alpha::StreamAccessLogsMessage expected_message;
    ...
    EXPECT_CALL(stream, sendMessageRaw_(_, false))
        .WillOnce(Invoke([expected_message](Buffer::InstancePtr& request, bool) {
          envoy::service::accesslog::v3alpha::StreamAccessLogsMessage message;
...
http_logs:
  ...
  envoy::data::accesslog::v3alpha::HTTPAccessLogEntry entry;
  entry.mutable_request()->set_path("/test/path1");
  logger_->log(envoy::data::accesslog::v3alpha::HTTPAccessLogEntry(entry));
...
http_logs:
  ...
  callbacks->onReceiveMessage(
      std::make_unique<envoy::service::accesslog::v3alpha::StreamAccessLogsResponse>());
...
public:
  ...
  void expectClientCreation() {
    ...
    EXPECT_CALL(async_client_manager_, factoryForGrpcService(_, _, false))
        .WillOnce(
            Invoke([this](const envoy::config::core::v3alpha::GrpcService&, Stats::Scope&, bool) {
...
TEST_F(GrpcAccessLoggerCacheImplTest, Deduplication) {
  ...
  envoy::extensions::access_loggers::grpc::v3alpha::CommonGrpcAccessLogConfig config;
