...
#include "envoy/api/v3alpha/auth/cert.pb.validate.h"
...
public:
  ...
  void loadConfigV2(envoy::api::v3alpha::auth::DownstreamTlsContext& cfg) {
    // Must add a certificate for the config to be considered valid.
    envoy::api::v3alpha::auth::TlsCertificate* server_cert =
...
TEST_F(SslServerContextImplTicketTest, TicketKeyNone) {
  envoy::api::v3alpha::auth::DownstreamTlsContext cfg;
...
TEST_F(SslServerContextImplTicketTest, StaticTickeyKey) {
  envoy::api::v3alpha::auth::Secret secret_config;
...
TEST_F(ClientContextConfigImplTest, ValidationContextNotReady) {
  envoy::api::v3alpha::auth::UpstreamTlsContext tls_context;
  envoy::api::v3alpha::auth::TlsCertificate* client_cert =
      tls_context.mutable_common_tls_context()->add_tls_certificates();
...
TEST_F(ClientContextConfigImplTest, StaticCertificateValidationContext) {
  envoy::api::v3alpha::auth::Secret tls_certificate_secret_config;
  const std::string tls_certificate_yaml = R"EOF(
  name: "abc.com"
  tls_certificate:
    certificate_chain:
  ...
  envoy::api::v3alpha::auth::Secret certificate_validation_context_secret_config;
...
TEST_F(ServerContextConfigImplTest, MultiSdsConfig) {
  ...
  tls_context.mutable_common_tls_context()->add_tls_certificate_sds_secret_configs();
  EXPECT_THROW_WITH_REGEX(
      TestUtility::validate<envoy::api::v3alpha::auth::DownstreamTlsContext>(tls_context),
...
TEST_F(ServerContextConfigImplTest, InvalidIgnoreCertsNoCA) {
  ...
  envoy::api::v3alpha::auth::CertificateValidationContext* server_validation_ctx =
