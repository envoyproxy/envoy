...
class GrpcCollectionSubscriptionFactoryTest : public testing::Test {
protected:
  GrpcCollectionSubscriptionFactoryTest() {
...
// Validates that the creation of a GrpcCollectionSubscription is successful.
TEST_F(GrpcCollectionSubscriptionFactoryTest, CreateSucceeded) {
  DeltaGrpcCollectionConfigSubscriptionFactory factory;
  ...
  xds::core::v3::ResourceLocator collection_locator;
  collection_locator.set_scheme(xds::core::v3::ResourceLocator::XDSTP);
  collection_locator.set_resource_type("foo");
  ...
  EXPECT_NO_THROW(factory.create(data));
}
