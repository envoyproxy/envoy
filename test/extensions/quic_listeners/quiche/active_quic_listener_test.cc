...
protected:
  ...
  void configureMocks(int connection_count) {
    ...
    void sendFullCHLO(quic::QuicConnectionId connection_id) {
      ...
      quic::CryptoHandshakeMessage chlo =
          quic::test::crypto_test_utils::GenerateDefaultInchoateCHLO(
      ...
      auto send_rc =
          Network::Utility::writeToSocket(client_sockets_.back()->ioHandle(), &first_slice, 1,
...
default_value: true
  ...
  TEST_P(ActiveQuicListenerTest, FailSocketOptionUponCreation) {
    ...
    EXPECT_THROW_WITH_REGEX(std::make_unique<ActiveQuicListener>(
                                *dispatcher_, connection_handler_, listen_socket_, listener_config_,
                                quic_config_, options, enabledFlag()),
