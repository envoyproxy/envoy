...
public:
  TestGetProofCallback(bool& called, std::string leaf_cert_scts,
                       const absl::string_view cert)
      : called_(called),
        expected_leaf_certs_scts_(std::move(leaf_cert_scts)), expected_leaf_cert_(cert) {}
...
class EnvoyQuicFakeProofSourceTest : public ::testing::Test {
 public:
  EnvoyQuicFakeProofSourceTest() : server_address_(quic::QuicIpAddress::Loopback4(), 12345), client_address_(quic::QuicIpAddress::Loopback4(), 54321),
        listen_socket_(std::make_shared<Network::MockListenSocket>()),
  proof_source_(listen_socket_, filter_chain_manager_) {
  }
...
TEST_F(EnvoyQuicFakeProofSourceTest, TestGetProof) {
  bool called = false;
  auto callback = std::make_unique<TestGetProofCallback>(called, 
                                                         "Fake timestamp", quic::test::kTestCertificate);
    EXPECT_CALL(*listen_socket_, ioHandle()).Times(2);
  EXPECT_CALL(filter_chain_manager_, findFilterChain(_)).WillRepeatedly(Invoke([&](const Network::ConnectionSocket& connection_socket) {
  EXPECT_EQ(*quicAddressToEnvoyAddressInstance(server_address_), *connection_socket.localAddress());
  EXPECT_EQ(*quicAddressToEnvoyAddressInstance(client_address_), *connection_socket.remoteAddress());
...
auto server_context_config_ptr = server_context_config.get();
  ...
  EXPECT_CALL(filter_chain_, transportSocketFactory()).WillRepeatedly(ReturnRef(transport_socket_factory));

  Ssl::MockTlsCertificateConfig tls_cert_config;
  std::vector<std::reference_wrapper<const Envoy::Ssl::TlsCertificateConfig>> tls_cert_configs{std::reference_wrapper<const Envoy::Ssl::TlsCertificateConfig>(tls_cert_config)};
  EXPECT_CALL(*server_context_config_ptr, tlsCertificates()).WillRepeatedly(Return(tls_cert_configs));
 ...
 EXPECT_CALL(tls_cert_config, privateKey()).WillOnce(ReturnRef(pkey_)); 
  ...
  EXPECT_EQ(quic::QUIC_FAILURE,
            proof_verifier_.VerifyProof(hostname_, /*port=*/0, server_config_, version_, chlo_hash_,
                                        {"Fake cert", "Unexpected cert"}, "Fake timestamp", "fake signature", nullptr,
