...
TEST_F(CelBootstrapTest, CreateBootstrapExtension) {
  using envoy::extensions::bootstrap::cel::v3::CelEvaluatorConfig;

  CelEvaluatorConfig config;
  auto* profile1 = config.mutable_default_profile();
  profile1->set_enable_string_conversion(true);
  profile1->set_enable_string_concat(true);
  profile1->set_enable_string_functions(true);
  ...
  // Cast to CelBootstrapExtension to test specific methods
  auto* cel_extension = static_cast<CelBootstrapExtension*>(extension.get());

  // Test config getter
  const auto& retrieved_config = cel_extension->config();
  EXPECT_TRUE(retrieved_config.has_default_profile());
  EXPECT_TRUE(retrieved_config.default_profile().enable_string_conversion());
  EXPECT_TRUE(retrieved_config.default_profile().enable_string_concat());
  EXPECT_TRUE(retrieved_config.default_profile().enable_string_functions());

  // Test lifecycle methods (should not throw)
  EXPECT_NO_THROW(cel_extension->onServerInitialized());
  EXPECT_NO_THROW(cel_extension->onWorkerThreadInitialized());
}
...
TEST_F(CelBootstrapTest, StringConversionEnabled) {
  ...
  CelEvaluatorConfig config;
  auto* profile2 = config.mutable_default_profile();
  profile2->set_enable_string_conversion(true);
  profile2->set_enable_string_concat(false);
  profile2->set_enable_string_functions(false);
...
TEST_F(CelBootstrapTest, StringConcatEnabled) {
  ...
  CelEvaluatorConfig config;
  auto* profile3 = config.mutable_default_profile();
  profile3->set_enable_string_conversion(false);
  profile3->set_enable_string_concat(true);
  profile3->set_enable_string_functions(false);
...
TEST_F(CelBootstrapTest, StringFunctionsEnabled) {
  ...
  CelEvaluatorConfig config;
  auto* profile4 = config.mutable_default_profile();
  profile4->set_enable_string_conversion(false);
  profile4->set_enable_string_concat(false);
  profile4->set_enable_string_functions(true);
...
TEST_F(CelBootstrapTest, StringFunctionsReplace) {
  ...
  // Create config with string functions enabled
  CelEvaluatorConfig config;
  auto* profile5 = config.mutable_default_profile();
  profile5->set_enable_string_conversion(false);
  profile5->set_enable_string_concat(false);
  profile5->set_enable_string_functions(true);
...
TEST_F(CelBootstrapTest, StringFunctionsSplit) {
  ...
  // Create config with string functions enabled
  CelEvaluatorConfig config;
  auto* profile6 = config.mutable_default_profile();
  profile6->set_enable_string_conversion(false);
  profile6->set_enable_string_concat(false);
  profile6->set_enable_string_functions(true);
...
TEST_F(CelBootstrapTest, StringFunctionsDisabled) {
  ...
  CelEvaluatorConfig config;
  auto* profile7 = config.mutable_default_profile();
  profile7->set_enable_string_conversion(false);
  profile7->set_enable_string_concat(false);
  profile7->set_enable_string_functions(false);
...
TEST_F(CelBootstrapTest, AllFeaturesEnabled) {
  ...
  CelEvaluatorConfig config;
  auto* profile8 = config.mutable_default_profile();
  profile8->set_enable_string_conversion(true);
  profile8->set_enable_string_concat(true);
  profile8->set_enable_string_functions(true);
...
TEST_F(CelBootstrapTest, EmptyConfigurationTest) {
  using envoy::extensions::bootstrap::cel::v3::CelEvaluatorConfig;
  ...
  // Default profile should not be present by default
  EXPECT_FALSE(retrieved_config.has_default_profile());
  EXPECT_EQ(retrieved_config.profiles_size(), 0);
}

TEST_F(CelBootstrapTest, NamedProfilesTest) {
  using envoy::extensions::bootstrap::cel::v3::CelEvaluatorConfig;

  CelEvaluatorConfig config;

  // Add named profiles
  auto& profiles = *config.mutable_profiles();
  auto& profile1 = profiles["profile1"];
  profile1.set_enable_string_conversion(true);
  profile1.set_enable_string_concat(false);
  profile1.set_enable_string_functions(false);

  auto& profile2 = profiles["profile2"];
  profile2.set_enable_string_conversion(false);
  profile2.set_enable_string_concat(true);
  profile2.set_enable_string_functions(true);
  ...
  EXPECT_EQ(retrieved_config.profiles_size(), 2);
  EXPECT_TRUE(retrieved_config.profiles().contains("profile1"));
  EXPECT_TRUE(retrieved_config.profiles().contains("profile2"));

  const auto& p1 = retrieved_config.profiles().at("profile1");
  EXPECT_TRUE(p1.enable_string_conversion());
  EXPECT_FALSE(p1.enable_string_concat());
  EXPECT_FALSE(p1.enable_string_functions());

  const auto& p2 = retrieved_config.profiles().at("profile2");
  EXPECT_FALSE(p2.enable_string_conversion());
  EXPECT_TRUE(p2.enable_string_concat());
  EXPECT_TRUE(p2.enable_string_functions());
}

TEST_F(CelBootstrapTest, FactoryCreateEmptyConfigType) {
  CelFactory factory;

  // Test createEmptyConfigProto returns correct type
  auto empty_config = factory.createEmptyConfigProto();
  ASSERT_NE(empty_config, nullptr);

  // Should be able to cast to the correct type
  auto* cel_config =
      dynamic_cast<envoy::extensions::bootstrap::cel::v3::CelEvaluatorConfig*>(empty_config.get());
  EXPECT_NE(cel_config, nullptr);

  // Should be empty/default state
  EXPECT_FALSE(cel_config->has_default_profile());
  EXPECT_EQ(cel_config->profiles_size(), 0);
}

TEST_F(CelBootstrapTest, ConfigWithDefaultAndNamedProfiles) {
  using envoy::extensions::bootstrap::cel::v3::CelEvaluatorConfig;

  CelEvaluatorConfig config;

  // Set default profile
  auto* default_profile = config.mutable_default_profile();
  default_profile->set_enable_string_conversion(true);
  default_profile->set_enable_string_concat(true);
  default_profile->set_enable_string_functions(false);

  // Add named profile
  auto& profiles = *config.mutable_profiles();
  auto& named_profile = profiles["custom"];
  named_profile.set_enable_string_conversion(false);
  named_profile.set_enable_string_concat(false);
  named_profile.set_enable_string_functions(true);
  ...
  // Verify both default and named profiles are preserved
  EXPECT_TRUE(retrieved_config.has_default_profile());
  EXPECT_EQ(retrieved_config.profiles_size(), 1);

  const auto& default_prof = retrieved_config.default_profile();
  EXPECT_TRUE(default_prof.enable_string_conversion());
  EXPECT_TRUE(default_prof.enable_string_concat());
  EXPECT_FALSE(default_prof.enable_string_functions());

  const auto& custom_prof = retrieved_config.profiles().at("custom");
  EXPECT_FALSE(custom_prof.enable_string_conversion());
  EXPECT_FALSE(custom_prof.enable_string_concat());
  EXPECT_TRUE(custom_prof.enable_string_functions());
}
