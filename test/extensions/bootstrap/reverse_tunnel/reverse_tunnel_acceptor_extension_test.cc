...
protected:
  ReverseTunnelAcceptorExtensionTest() {
    ...
    extension_ = std::make_unique<ReverseTunnelAcceptorExtension>(
  ...
  void setupThreadLocalSlot() {
    ...
    thread_local_registry_ = std::make_shared<UpstreamSocketThreadLocal>(dispatcher_, extension_.get());
  ...
  void setupAnotherThreadLocalSlot() {
    ...
    another_thread_local_registry_ = std::make_shared<UpstreamSocketThreadLocal>(another_dispatcher_, extension_.get());
  }
...
TEST_F(ReverseTunnelAcceptorExtensionTest, InitializeWithDefaultStatPrefix) {
  ...
  auto extension_with_default = std::make_unique<ReverseTunnelAcceptorExtension>(
      *socket_interface_, context_, empty_config);
...
TEST_F(ReverseTunnelAcceptorExtensionTest, GetConnectionStatsSyncMultiThread) {
  ...
  EXPECT_TRUE(std::find(connected_nodes.begin(), connected_nodes.end(), "node1") != connected_nodes.end());
  // node2: should be present (incremented 1 time)
  EXPECT_TRUE(std::find(connected_nodes.begin(), connected_nodes.end(), "node2") != connected_nodes.end());
  // node3: should be present (incremented 1 time)
  EXPECT_TRUE(std::find(connected_nodes.begin(), connected_nodes.end(), "node3") != connected_nodes.end());
  
  // cluster1: should be present (incremented 3 times total)
  EXPECT_TRUE(std::find(accepted_connections.begin(), accepted_connections.end(), "cluster1") != accepted_connections.end());
  // cluster2: should be present (incremented 1 time)
  EXPECT_TRUE(std::find(accepted_connections.begin(), accepted_connections.end(), "cluster2") != accepted_connections.end());
  // cluster3: should be present (incremented 1 time)
  EXPECT_TRUE(std::find(accepted_connections.begin(), accepted_connections.end(), "cluster3") != accepted_connections.end());
}
...
protected:
  TestUpstreamSocketManager() {
    ...
    EXPECT_CALL(dispatcher_, createTimer_(_)).WillRepeatedly(testing::ReturnNew<NiceMock<Event::MockTimer>>());
    EXPECT_CALL(dispatcher_, createFileEvent_(_, _, _, _)).WillRepeatedly(testing::ReturnNew<NiceMock<Event::MockFileEvent>>());
  ...
  std::vector<std::string> getClusterToNodeMapping(const std::string& cluster) {
    ...
    return (it != socket_manager_->cluster_to_node_map_.end()) ? it->second : std::vector<std::string>{};
  }
  ...
  size_t getFDToNodeMapSize() {
    return socket_manager_->fd_to_node_map_.size();
  }

  size_t getNodeToClusterMapSize() {
    return socket_manager_->node_to_cluster_map_.size();
  }

  size_t getClusterToNodeMapSize() {
    return socket_manager_->cluster_to_node_map_.size();
  }

  size_t getFDToEventMapSize() {
    return socket_manager_->fd_to_event_map_.size();
  }

  size_t getFDToTimerMapSize() {
    return socket_manager_->fd_to_timer_map_.size();
  }
...
TEST_F(TestUpstreamSocketManager, AddConnectionSocketEmptyClusterId) {
  // Test adding a socket with empty cluster_id (should log error and return early without adding socket)
  ...
  socket_manager_->addConnectionSocket(node_id, cluster_id, std::move(socket), ping_interval, false);
...
TEST_F(TestUpstreamSocketManager, AddConnectionSocketEmptyNodeId) {
  // Test adding a socket with empty node_id (should log error and return early without adding socket)
...
TEST_F(TestUpstreamSocketManager, AddAndGetMultipleSocketsSameNode) {
  ...
  // Add first socket
  socket_manager_->addConnectionSocket(node_id, cluster_id, std::move(socket1), ping_interval, false);
  ...
  socket_manager_->addConnectionSocket(node_id, cluster_id, std::move(socket2), ping_interval, false);
  
  // Verify maps after second socket (should have 2 sockets for same node, but cluster maps unchanged)
  ...
  socket_manager_->addConnectionSocket(node_id, cluster_id, std::move(socket3), ping_interval, false);
...
TEST_F(TestUpstreamSocketManager, TestGetNodeID) {
  ...
  // Now call getNodeID with the cluster_id - should return the node_id that was added for this cluster
...
TEST_F(TestUpstreamSocketManager, PingConnectionsWriteSuccess) {
  ...
  auto* mock_io_handle1 = dynamic_cast<NiceMock<Network::MockIoHandle>*>(&sockets.front()->ioHandle());
  auto* mock_io_handle2 = dynamic_cast<NiceMock<Network::MockIoHandle>*>(&sockets.back()->ioHandle());
...
TEST_F(TestUpstreamSocketManager, PingConnectionsWriteFailure) {
  ...
  EXPECT_CALL(*mock_io_handle2, write(_))
    ...
    .WillRepeatedly(Invoke([](Buffer::Instance& buffer) -> Api::IoCallUint64Result {
      // Drain the buffer to simulate successful write
      buffer.drain(buffer.length());
      return Api::IoCallUint64Result{0, Network::IoSocketError::create(EAGAIN)}; // Second socket succeeds
    }));
  ...
  // Manually call pingConnections again. This should ping once socket2, fail and trigger node cleanup
  socket_manager_->pingConnections(node_id);
...
protected:
  ...
  Network::Address::InstanceConstSharedPtr createAddressWithLogicalName(const std::string& logical_name) {
    ...
    public:
      ...
      bool operator==(const Instance& rhs) const override { return logical_name_ == rhs.logicalName(); }
...
TEST_F(TestReverseTunnelAcceptor, SocketWithoutAddress) {
  ...
  auto result = socket_interface_->socket(Network::Socket::Type::Stream, 
...
TEST_F(TestReverseTunnelAcceptor, SocketWithAddressAndThreadLocalWithSockets) {
  ...
  tls_socket_manager->addConnectionSocket(node_id, cluster_id, std::move(socket), ping_interval, false);
  ...
  // Try to get another socket for the same node. This will return a default IoHandle, not an UpstreamReverseConnectionIOHandle
  auto another_io_handle = socket_interface_->socket(Network::Socket::Type::Stream, address, options);
...
protected:
  TestUpstreamReverseConnectionIOHandle() {
    ...
    io_handle_ = std::make_unique<UpstreamReverseConnectionIOHandle>(
        std::move(mock_socket_), "test-cluster");
  }
