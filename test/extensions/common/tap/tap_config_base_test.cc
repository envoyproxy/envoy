...
TEST(BodyBytesToString, All) {
  {
    ...
    trace.mutable_http_streamed_trace_segment()->mutable_request_body_chunk()->set_as_bytes(
        "hello");
    Utility::bodyBytesToString(trace, envoy::config::tap::v3alpha::OutputSink::JSON_BODY_AS_BYTES);
...

  {
    ...
    trace.mutable_http_streamed_trace_segment()->mutable_request_body_chunk()->set_as_bytes(
        "hello");
    Utility::bodyBytesToString(trace, envoy::config::tap::v3alpha::OutputSink::JSON_BODY_AS_STRING);
...
TEST(AddBufferToProtoBytes, All) {
  {
    ...
    envoy::data::tap::v3alpha::Body body;
