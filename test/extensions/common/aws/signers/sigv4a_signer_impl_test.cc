...
public:
  ...
  void ecdsaVerifyCanonicalRequest(std::string canonical_request, SigningType signing_type,
    ...
    auto ec_key_or = sigv4a_key_derivation->derivePrivateKey(
...
TEST(SigV4AKeyDerivationTest, TestKeyDerivations) {
  ...
  auto ec_key_or = sigv4a_key_derivation->derivePrivateKey(absl::string_view("akid"),
                                                           absl::string_view("secret"));

  auto ec_private_key = EC_KEY_get0_private_key(ec_key_or.value());
  auto hexkey = BN_bn2hex(ec_private_key);
  EXPECT_STREQ(hexkey, "0a56b8224b63e587ab069a15a730a103add19b45a644a197d24415ff89b993dc");
  OPENSSL_free(hexkey);
  EC_KEY_free(ec_key_or.value());
  ec_key_or = sigv4a_key_derivation->derivePrivateKey(absl::string_view("akid"),
                                                      absl::string_view("testkey2"));

  ec_private_key = EC_KEY_get0_private_key(ec_key_or.value());
...
TEST_F(SigV4ASignerImplTest, FailKeyDerivation) {
  ...
  EXPECT_CALL(*mock_ptr, derivePrivateKey(_, _))
