...
TEST(UtilityTest, CanonicalizeHeadersRemovingDefaultPortsFromHost) {
  ...
  std::vector<Matchers::StringMatcherPtr> exclusion_list,inclusion_list = {};
  const auto map_port80 = Utility::canonicalizeHeaders(headers_port80, exclusion_list,inclusion_list);
  ...
  const auto map_port443 = Utility::canonicalizeHeaders(headers_port443, exclusion_list,inclusion_list);
...
TEST(UtilityTest, IncludedHeadersTakesPrecedence) {
  ...
  included_headers.emplace_back(std::make_unique<Matchers::StringMatcherImpl>(include_config, context));
...
TEST(UtilityTest, RequiredHeadersNotExcluded) {
  ...
  // x-amz-* and content-type should still be included even when excluded, custom-header should be excluded
  const auto map = Utility::canonicalizeHeaders(headers, excluded_headers, included_headers);
...
TEST(UtilityTest, IncludedHeadersWithPrefix) {
  ...
  const auto map = Utility::canonicalizeHeaders(headers, excluded_headers, included_headers);
  EXPECT_THAT(map, ElementsAre(Pair("host", "example.com"),
                               Pair("x-custom-1", "value1"),
...
TEST(UtilityTest, XAmzHeadersCaseInsensitive) {
  ...
  const auto map = Utility::canonicalizeHeaders(headers, excluded_headers, included_headers);
  EXPECT_THAT(map, ElementsAre(Pair("custom-header", "value1"),
                               Pair("host", "example.com"),
                               Pair("x-amz-date", "20130708T220855Z"),
                               Pair("x-amz-security-token", "token123")));
}
