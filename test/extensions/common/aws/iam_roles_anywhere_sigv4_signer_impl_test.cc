...
public:
  ...
  ~SigV4SignerImplTest() override = default;
  ...
  NiceMock<MockX509CredentialsProvider>* credentials_provider_;
...
TEST_F(SigV4SignerImplTest, X509UnsignedPaylowd) {
  ...
  EXPECT_EQ(
      message_->headers().get(SignatureHeaders::get().ContentSha256)[0]->value().getStringView(),
      SignatureConstants::UnsignedPayload);
}
...
// X509 Signing with certificate using ECDSA signing algorithm. Validate we have correct headers
// added
TEST_F(SigV4SignerImplTest, FullSignNoChainECDSA) {
  ...
  std::vector<std::string> credsplit = absl::StrSplit(authsplit[1], '/');
  std::vector<std::string> idsplit = absl::StrSplit(credsplit[0], '=');
  EXPECT_EQ(idsplit[1], "5215639076998761095638506031589467414");
}

// X509 Signing with certificate using ECDSA signing algorithm and certificate chain. Validate we
// have correct headers added
TEST_F(SigV4SignerImplTest, FullSignWithChainECDSA) {
  ...
  std::vector<std::string> credsplit = absl::StrSplit(authsplit[1], '/');
  std::vector<std::string> idsplit = absl::StrSplit(credsplit[0], '=');
  EXPECT_EQ(idsplit[1], "326164854566604267642234107702204417305661869882");
}
...
public:
  ...
  ~SigV4ASignerImplTest() override = default;
