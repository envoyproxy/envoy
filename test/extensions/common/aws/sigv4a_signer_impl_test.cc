...
public:
  ...
  void expectSignHeaders(absl::string_view service_name, absl::string_view signature,
    ...
    EXPECT_EQ(fmt::format("AWS4-ECDSA-P256-SHA256 Credential=akid/20180102/{}/{}/aws4_request, "
    ...
    EXPECT_EQ(payload,
              headers.get(SigV4ASignatureHeaders::get().ContentSha256)[0]->value().getStringView());
  }
...
TEST_F(SigV4ASignerImplTest, SignAndVerify) {
  ...
  EC_KEY *ec_key = SigV4AKeyDerivation::derivePrivateKey(absl::string_view (credentials_.accessKeyId()->data(),credentials_.accessKeyId()->size()),absl::string_view (credentials_.secretAccessKey()->data(),credentials_.secretAccessKey()->size()));
...
e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855)EOF";
  ...
  std::string string_to_sign = fmt::format(fmt::runtime(SigV4ASignatureConstants::get().SigV4AStringToSignFormat), long_date,
      credential_scope,Hex::encode(crypto_util.getSha256Digest(Buffer::OwnedImpl(canonical_request))));
  ...
  EXPECT_THAT(
      ...
      testing::StartsWith("AWS4-ECDSA-P256-SHA256 Credential=akid/20180102/service/aws4_request, "
                          "SignedHeaders=host;x-amz-content-sha256;x-amz-date;x-amz-region-set, "
                          "Signature="));
  std::vector<std::string> v = absl::StrSplit(message_->headers().get(Http::CustomHeaders::get().Authorization)[0]->value().getStringView(), "Signature=");
  ...
  EXPECT_EQ(1,ECDSA_verify(0,hash.data(),hash.size(),signature.data(),signature.size(),ec_key));
}
