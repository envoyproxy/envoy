...
#include "envoy/config/common/dynamic_forward_proxy/v3alpha/dns_cache.pb.h"
...
public:
  void initialize() {
    ...
    config_.set_dns_lookup_family(envoy::api::v3alpha::Cluster::V4_ONLY);
  ...
  envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig config_;
...
TEST(DnsCacheManagerImplTest, LoadViaConfig) {
  ...
  envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig config1;
  config1.set_name("foo");

  auto cache1 = cache_manager.getCache(config1);
  EXPECT_NE(cache1, nullptr);

  envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig config2;
  config2.set_name("foo");
  EXPECT_EQ(cache1, cache_manager.getCache(config2));

  envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig config3;
  config3.set_name("bar");
  auto cache2 = cache_manager.getCache(config3);
  EXPECT_NE(cache2, nullptr);
  EXPECT_NE(cache1, cache2);

  envoy::config::common::dynamic_forward_proxy::v3alpha::DnsCacheConfig config4;
  config4.set_name("foo");
  config4.set_dns_lookup_family(envoy::api::v3alpha::Cluster::V6_ONLY);
