...
// Exercises the C ABI snapshot callbacks the module uses to read
// counters/gauges/text readouts directly from the MetricSnapshot pointer.
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetCounterCountEmpty) {
  EXPECT_EQ(0u, envoy_dynamic_module_callback_stat_sink_snapshot_get_counter_count(&snapshot_));
}
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetCounterCount) {
  ...
  EXPECT_EQ(2u, envoy_dynamic_module_callback_stat_sink_snapshot_get_counter_count(&snapshot_));
}
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetCounterIndexMany) {
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    ASSERT_TRUE(envoy_dynamic_module_callback_stat_sink_snapshot_get_counter(
        &snapshot_, i, &name_out, &value_out, &delta_out));
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetCounterOutOfRange) {
  ...
  EXPECT_FALSE(envoy_dynamic_module_callback_stat_sink_snapshot_get_counter(
      &snapshot_, 1, &name_out, &value_out, &delta_out));
  EXPECT_FALSE(envoy_dynamic_module_callback_stat_sink_snapshot_get_counter(
      &snapshot_, 42, &name_out, &value_out, &delta_out));
}
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetGaugeCountEmpty) {
  EXPECT_EQ(0u, envoy_dynamic_module_callback_stat_sink_snapshot_get_gauge_count(&snapshot_));
}
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetGaugeCount) {
  ...
  EXPECT_EQ(2u, envoy_dynamic_module_callback_stat_sink_snapshot_get_gauge_count(&snapshot_));
}
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetGaugeValid) {
  ...
  envoy_dynamic_module_type_envoy_buffer name_out{};
  uint64_t value_out = 0;
  EXPECT_TRUE(envoy_dynamic_module_callback_stat_sink_snapshot_get_gauge(&snapshot_, 0, &name_out,
                                                                         &value_out));
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetGaugeOutOfRange) {
  ...
  envoy_dynamic_module_type_envoy_buffer name_out{};
  uint64_t value_out = 0;
  EXPECT_FALSE(envoy_dynamic_module_callback_stat_sink_snapshot_get_gauge(&snapshot_, 1, &name_out,
                                                                          &value_out));
}
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetTextReadoutCountEmpty) {
  EXPECT_EQ(0u,
            envoy_dynamic_module_callback_stat_sink_snapshot_get_text_readout_count(&snapshot_));
}
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetTextReadoutValid) {
  ...
  EXPECT_TRUE(envoy_dynamic_module_callback_stat_sink_snapshot_get_text_readout(
      &snapshot_, 0, &name_out, &value_out));
...
TEST_F(DynamicModuleStatsSinkAbiTest, GetTextReadoutOutOfRange) {
  ...
  EXPECT_FALSE(envoy_dynamic_module_callback_stat_sink_snapshot_get_text_readout(
      &snapshot_, 1, &name_out, &value_out));
}
