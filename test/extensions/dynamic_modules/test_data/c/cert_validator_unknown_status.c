// Cert validator fake that returns an out-of-range detailed_status to exercise the default
// branch of the switch in DynamicModuleCertValidator::doVerifyCertChain.
#include <stddef.h>
#include <stdint.h>
...
envoy_dynamic_module_on_cert_validator_do_verify_cert_chain(
  ...
  // 9999 is not a valid client_validation_status enum value; the consumer must fall through
  // to the default branch.
  result.detailed_status = (envoy_dynamic_module_type_cert_validator_client_validation_status)9999;
  result.tls_alert = 0;
