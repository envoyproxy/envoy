// A header formatter module that implements every hook but changes nothing: format always
// declines, so Envoy serializes each key unchanged.
#include <stdbool.h>
#include <stddef.h>
#include <string.h>
...
envoy_dynamic_module_type_header_formatter_module_ptr envoy_dynamic_module_on_header_formatter_new(
  ...
  static int formatter_dummy = 0;
  return &formatter_dummy;
}
