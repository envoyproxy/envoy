...
//
// do_not_close=true: the function registry is process-wide, but registered values are raw
...
TEST_P(DynamicModulesBootstrapIntegrationTest, FunctionRegistryCrossFilterRust) {
  ...
  // function. do_not_close=true because IPv4 and IPv6 share the same process; without
  // RTLD_NODELETE the registered function pointer would be invalidated by the dlclose between
  // iterations (especially on aarch64 where the .so reloads at a different address).
...
TEST_P(DynamicModulesBootstrapIntegrationTest, FunctionRegistryCrossFilterGo) {
  ...
  // do_not_close=true: see FunctionRegistryCrossFilterRust for the rationale.
  const std::string bootstrap_yaml = R"EOF(
      name: envoy.bootstrap.dynamic_modules
      typed_config:
        "@type": type.googleapis.com/envoy.extensions.bootstrap.dynamic_modules.v3.DynamicModuleBootstrapExtension
