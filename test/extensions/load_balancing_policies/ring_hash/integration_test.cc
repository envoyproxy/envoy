...
class RingHashIntegrationTest : public testing::TestWithParam<Network::Address::IpVersion>,
                                public HttpIntegrationTest {
public:
  RingHashIntegrationTest() : HttpIntegrationTest(Http::CodecType::HTTP1, GetParam()) {
  ...
  void initializeConfig(bool legacy_api = false, bool disable_lagacy_api_conversion = false) {
    ...
    config_helper_.addConfigModifier(
        [legacy_api](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
          ...
          if (legacy_api) {
            cluster_0->set_lb_policy(envoy::config::cluster::v3::Cluster::RING_HASH);
            return;
          }
...
INSTANTIATE_TEST_SUITE_P(IpVersions, RingHashIntegrationTest,
