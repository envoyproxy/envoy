...
void configureClusterLoadBalancingPolicyWithoutLocalityWeights(
...
public:
  ...
  void runNormalLoadBalancing() {
    ...
    // applied.
    timeSystem().advanceTimeWait(Seconds(2));
...
INSTANTIATE_TEST_SUITE_P(
    IpVersions, WrrLocalityIntegrationTest,
    testing::Combine(testing::ValuesIn(TestEnvironment::getIpVersionsForTest()),
                     testing::ValuesIn<int>({1, 100, 10000})));

TEST_P(WrrLocalityIntegrationTest, NormalLoadBalancing) {
  initializeConfig();
...
class WrrLocalityXdsIntegrationTest
    : public testing::TestWithParam<std::tuple<Network::Address::IpVersion, bool>>,
      public HttpIntegrationTest {
public:
  WrrLocalityXdsIntegrationTest()
  ...
  void initialize() override {
    ...
    cluster1_ = ConfigHelper::buildStaticCluster(
        first_cluster_name_, fake_upstreams_[first_upstream_index_]->localAddress()->ip()->port(),
        Network::Test::getLoopbackAddressString(version_));
    configureClusterLoadBalancingPolicy(cluster1_);

    cluster2_ = ConfigHelper::buildStaticCluster(
        second_cluster_name_, fake_upstreams_[second_upstream_index_]->localAddress()->ip()->port(),
        Network::Test::getLoopbackAddressString(version_));
  ...
  const char* second_cluster_name_ = "cluster_2";
  // Index in fake_upstreams_
  const int first_upstream_index_ = 2;
  const int second_upstream_index_ = 3;
...
TEST_P(WrrLocalityXdsIntegrationTest, ClusterUpDownUp) {
  // Calls our initialize(), which includes establishing a listener, route, and
  // cluster.
  testRouterHeaderOnlyRequestAndResponse(nullptr, first_upstream_index_, "/cluster1");
  cleanupUpstreamAndDownstream();
...
// Tests adding a cluster, adding another, then removing and adding back the
// first.
TEST_P(WrrLocalityXdsIntegrationTest, TwoClusters) {
  // Calls our initialize(), which includes establishing a listener, route, and
  // cluster.
  testRouterHeaderOnlyRequestAndResponse(nullptr, first_upstream_index_, "/cluster1");
  cleanupUpstreamAndDownstream();
  ...
  testRouterHeaderOnlyRequestAndResponse(nullptr, second_upstream_index_, "/cluster2");
  cleanupUpstreamAndDownstream();
  ASSERT_TRUE(codec_client_->waitForDisconnect());
  ...
  EXPECT_TRUE(compareDiscoveryRequest(Config::TestTypeUrl::get().Cluster, "42", {}, {}, {}));
  sendDiscoveryResponse<envoy::config::cluster::v3::Cluster>(
      Config::TestTypeUrl::get().Cluster, {cluster2_}, {}, {first_cluster_name_}, "43");
  // We can continue the test once we're sure that Envoy's ClusterManager has
  // made use of the DiscoveryResponse that says cluster_1 is gone.
  test_server_->waitForCounterGe("cluster_manager.cluster_removed", 1);
...
public:
  ...
  void initialize() override {
    ...
    cluster1_ = ConfigHelper::buildCluster(first_cluster_name_);
    if (locality_weighted_lb_enabled_) {
      configureClusterLoadBalancingPolicy(cluster1_);
    } else {
      configureClusterLoadBalancingPolicyWithoutLocalityWeights(cluster1_);
    }
    ...
    EXPECT_TRUE(compareDiscoveryRequest(
        ...
        {first_cluster_name_}, {first_cluster_name_}, {}));
  ...
  const char* first_cluster_name_ = "cluster_1";
  // Index in fake_upstreams_
  const int first_upstream_index_ = 1;
...
INSTANTIATE_TEST_SUITE_P(
    IpVersions, WrrLocalityEdsIntegrationTest,
    testing::Combine(testing::ValuesIn(TestEnvironment::getIpVersionsForTest()), testing::Bool()));

TEST_P(WrrLocalityEdsIntegrationTest, UseLocalityWeight) {
  initialize();
  ...
  std::vector<uint64_t> initial_usage;
  sendRequestsAndTrackUpstreamUsage(first_upstream_index_, upstream_qps, 100, initial_usage);
  ENVOY_LOG(trace, "initial_usage {}", initial_usage);
  // Wait longer than blackout period to ensure that client side weights are
  // applied.
  timeSystem().advanceTimeWait(Seconds(2));
  ...
  std::vector<uint64_t> upstream_usage;
  sendRequestsAndTrackUpstreamUsage(first_upstream_index_, upstream_qps, 100, upstream_usage);
  ENVOY_LOG(trace, "upstream_usage {}", upstream_usage);
  // The upstream usage within each locality should be proportional to the
  // reported QPS.
  ...
  if (locality_weighted_lb_enabled_) {
    ...
    // The locality weights are not enabled, so the total usage should
    // be the same.
...
TEST_P(WrrLocalityEdsIntegrationTest, AddRemoveLocality) {
  ...
  for (uint32_t i = 0; i < 10; ++i) {
    ...
    // Send 100 requests to cluster1 so host weights are updated.
    std::vector<uint64_t> initial_usage;
    sendRequestsAndTrackUpstreamUsage(first_upstream_index_, upstream_qps, 10, initial_usage);
    ENVOY_LOG(trace, "initial_usage {}", initial_usage);
