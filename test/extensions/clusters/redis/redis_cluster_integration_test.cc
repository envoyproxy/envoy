  ...
  void initialize() override {
    ...
    config_helper_.addConfigModifier(
        [this](envoy::config::bootstrap::v3alpha::Bootstrap& bootstrap) {
          ...
          for (int j = 0; j < cluster_0->hidden_envoy_deprecated_hosts_size(); ++j) {
            if (cluster_0->mutable_hidden_envoy_deprecated_hosts(j)->has_socket_address()) {
              auto* host_socket_addr =
                  cluster_0->mutable_hidden_envoy_deprecated_hosts(j)->mutable_socket_address();
