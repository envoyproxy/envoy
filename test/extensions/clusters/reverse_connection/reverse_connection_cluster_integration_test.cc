...
protected:
  ...
  void configureReverseTunnelSetup(envoy::config::bootstrap::v3::Bootstrap& bootstrap,
    ...
    static_cast<void>(rt_filter->mutable_typed_config()->PackFrom(rt_config));
    ...
    if (add_lua_host_id_filter) {
      ...
      function envoy_on_request(request_handle)
        ...
        headers:add("x-computed-host-id", host_id)
                                                                              ...
                                                                              node_id));
      static_cast<void>(lua_filter->mutable_typed_config()->PackFrom(lua_config));
    }

    auto* egress_router = egress_hcm.add_http_filters();
    egress_router->set_name("envoy.filters.http.router");
    static_cast<void>(egress_router->mutable_typed_config()->PackFrom(
        envoy::extensions::filters::http::router::v3::Router()));

    static_cast<void>(egress_hcm_filter->mutable_typed_config()->PackFrom(egress_hcm));
    ...
    static_cast<void>(cluster_type->mutable_typed_config()->PackFrom(rc_config));
    ...
    static_cast<void>((*rc_cluster->mutable_typed_extension_protocol_options())
    ...
    static_cast<void>(init_router->mutable_typed_config()->PackFrom(
        envoy::extensions::filters::http::router::v3::Router()));

    static_cast<void>(init_hcm_filter->mutable_typed_config()->PackFrom(init_hcm));
...
TEST_P(ReverseConnectionClusterIntegrationTest, ReverseTunnelResiliencyTest) {
  ...
  config_helper_.addConfigModifier([cloud1_port, cloud2_port, loopback_addr, &node1_cloud1_config,
    ...
    static_cast<void>((*lds_cluster->mutable_typed_extension_protocol_options())
                          ["envoy.extensions.upstreams.http.v3.HttpProtocolOptions"]
