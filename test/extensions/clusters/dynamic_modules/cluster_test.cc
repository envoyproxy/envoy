...
TEST(DynamicModuleClusterConfigTest, ConfigCreationAndDestruction) {
  ...
  auto dynamic_module =
      Extensions::DynamicModules::newDynamicModule(testSharedObjectPath("cluster_no_op", "c"),
                                                    false);
  EXPECT_TRUE(dynamic_module.ok());

  NiceMock<Server::Configuration::MockServerFactoryContext> context;

  auto config = newDynamicModuleClusterConfig("test_cluster", "", std::move(dynamic_module.value()),
                                               context);
...
TEST(DynamicModuleClusterTest, ClusterCreationAndDestruction) {
  ...
  auto cluster = std::make_shared<DynamicModuleCluster>(cluster_proto, module_config,
                                                         config.value(), cluster_context,
                                                         creation_status);
...
TEST(DynamicModuleClusterAbiTest, LbContextDownstreamConnectionIdNullContext) {
  ...
  auto result =
      envoy_dynamic_module_callback_lb_context_get_downstream_connection_id(nullptr, &connection_id);
  EXPECT_FALSE(result);
}
