...
BAZEL_USE_LLVM_NATIVE_COVERAGE=1 GCOV=llvm-profdata bazel coverage ${BAZEL_BUILD_OPTIONS} \
    ...
    --test_arg="--log-path /dev/null" --test_arg="-l trace" --test_arg="--log-stacktrace-to-stderr 1" \
    --test_env=HEAPCHECK= \ --test_env=GTEST_SHUFFLE=1 \
    --flaky_test_attempts=5 //test/coverage:coverage_tests
