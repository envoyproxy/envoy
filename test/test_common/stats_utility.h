...
// Helper to parse varint-delimited protobuf messages from a buffer, generated by the prometheus
...
std::vector<io::prometheus::client::MetricFamily> parsePrometheusProtobuf(absl::string_view data) {
  ...
  while (true) {
    ...
    if (!coded_stream.ReadVarint64(&message_length)) {
      break;
    }

    // Set a limit for reading just this message.
    auto limit = coded_stream.PushLimit(message_length);

    // Parse the MetricFamily message.
    io::prometheus::client::MetricFamily family;
    if (!family.ParseFromCodedStream(&coded_stream)) {
      break;
    }
