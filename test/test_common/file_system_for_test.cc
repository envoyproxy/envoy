...
void MemfileInstanceImpl::renameFile(const std::string& old_name, const std::string& new_name) {
  {
    ...
    // memfile-aware.
    RELEASE_ASSERT(!use_memfiles_, "moving files not supported with memfile.  Please call setUseMemfiles(false)");
  }
#ifdef WIN32
  ...
  RELEASE_ASSERT(rc != 0,
                 fmt::format("failed to rename file from  {} to {} with error {}", old_name,
