...
private:
  ...
  SchedulableCallbackPtr schedule_ready_alarms_cb_;
};
...
void SimulatedTimeSystemHelper::Alarm::Alarm::enableHRTimer(
  ...
  armed_ = true;
  if (duration.count() == 0 && !Runtime::runtimeFeatureEnabled(
                                   "envoy.reloadable_features.activate_timers_next_event_loop")) {
    activateLockHeld();
  } else {
    time_system_.addAlarmLockHeld(*this, duration, simulated_scheduler_);
  }
}
...
template<class Condition> bool SimulatedTimeSystemHelper::awaitHelper(
  ...
  while (timeout_not_reached) {
    ...
    if (mutex.awaitWithTimeout(condition, real_time_poll_delay)) {
      return true;
    }
    ...
    mutex.unlock();
    {
      ...
      if (monotonic_time_ < end_time) {
        ...
        if (!alarms_.empty()) {
          ...
          const AlarmRegistration& alarm_reg = *(alarms_.begin());
          next_wakeup = std::min(alarm_reg.time_, next_wakeup);
        }
...
bool SimulatedTimeSystemHelper::await(const bool& condition, Thread::MutexBasicLockable& mutex,
                                      const Duration& timeout) EXCLUSIVE_LOCKS_REQUIRED(mutex) {
  return awaitHelper<const bool&>(condition, mutex, timeout);
}

bool SimulatedTimeSystemHelper::await(BoolFn check_condition, Thread::MutexBasicLockable& mutex,
                                      const Duration& timeout) EXCLUSIVE_LOCKS_REQUIRED(mutex) {
...
void SimulatedTimeSystemHelper::addAlarmLockHeld(
    ...
    SimulatedScheduler& simulated_scheduler) ABSL_NO_THREAD_SAFETY_ANALYSIS {
  ASSERT(&(alarm.timeSystem()) == this);
  ...
  if (duration.count() == 0) {
    // Force the event loop to check for timers that are ready to execute since we just added an 0
