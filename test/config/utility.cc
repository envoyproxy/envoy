...
envoy::config::route::v3::RouteConfiguration
ConfigHelper::buildRouteConfigWithVhds(const std::string& name,
                                       const bool& use_singleton_subscription) {
  API_NO_BOOST(envoy::config::route::v3::RouteConfiguration) route;
#ifdef ENVOY_ENABLE_YAML
  TestUtility::loadFromYaml(fmt::format(R"EOF(
      name: "{}"
      vhds:
        config_source:
          ads: {{}}
        use_singleton_subscription: {}
    )EOF",
                                        name, use_singleton_subscription),
                            route);
  return route;
#else
  UNREFERENCED_PARAMETER(name);
  UNREFERENCED_PARAMETER(use_singleton_subscription);
  PANIC("YAML support compiled out");
#endif
}
