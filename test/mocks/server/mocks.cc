...
MockOptions::MockOptions(const std::string& config_path) : config_path_(config_path) {
  ...
  ON_CALL(*this, allowUnknownFields()).WillByDefault(Invoke([this] {
    return allow_unknown_fields_;
  }));
  ON_CALL(*this, rejectUnknownFieldsDynamic()).WillByDefault(Invoke([this] {
    return reject_unknown_fields_dynamic_;
  }));
