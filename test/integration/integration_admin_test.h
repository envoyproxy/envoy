    ...
    for (const Json::ObjectSharedPtr& obj_ptr : array) {
      if (obj_ptr->hasObject("histograms")) {
        ...
        const std::vector<Json::ObjectSharedPtr>& computed_quantiles =
            histograms_ptr->getObjectArray("computed_quantiles").value();
        ...
        EXPECT_EQ(histograms_ptr->getObjectArray("supported_quantiles").value().size(),
                  computed_quantiles[0]->getObjectArray("values").value().size());
      }
    }
