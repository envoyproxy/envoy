...
// Test xDS matcher using SourceIP input with prefix matching on loopback.
TEST_F(TransportSocketMatcherIntegrationTest, XdsMatcherSourceIP) {
  // Override matcher to use source_ip input; map IPv4 loopback prefix to TLS, fallback to raw.
...
on_no_match:
  ...
  // Both routes should succeed; TLS selected for localhost prefix, raw for fallback.
  {
    IntegrationStreamDecoderPtr response =
        codec_client_->makeHeaderOnlyRequest(tls_request_headers_);
    ASSERT_TRUE(response->waitForEndStream());
...
// Test xDS matcher using ServerName input with auto_sni and per-request :authority.
TEST_F(TransportSocketMatcherIntegrationTest, XdsMatcherServerName) {
  // Override matcher to use server_name input and enable auto_sni on TLS socket.
  config_helper_.addConfigModifier([](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    auto* cluster = bootstrap.mutable_static_resources()->mutable_clusters(0);

    // Configure HCM upstream_http_protocol_options to set use_downstream_sni (for SNI propagation).
    for (auto& tsm : *cluster->mutable_transport_socket_matches()) {
      if (tsm.name() == "tls") {
        // No direct auto_sni in this Envoy version; we rely on HTTP upstream options.
      }
    }

    // Ensure upstream HTTP protocol options exist and enable explicit HTTP/1.1 and auto_sni.
    envoy::extensions::upstreams::http::v3::HttpProtocolOptions http_opts;
    http_opts.mutable_explicit_http_config()->mutable_http_protocol_options();
    http_opts.mutable_upstream_http_protocol_options()->set_auto_sni(true);
    Protobuf::Any any;
    any.PackFrom(http_opts);
    (*cluster->mutable_typed_extension_protocol_options())
        ["envoy.extensions.upstreams.http.v3.HttpProtocolOptions"] = any;

    // Build server_name based matcher: tls.example.com -> tls, default -> raw.
    xds::type::matcher::v3::Matcher matcher;
    const std::string matcher_yaml = R"EOF(
matcher_tree:
  input:
...
on_no_match:
  ...
  // Use per-request authority to drive SNI.
  Http::TestRequestHeaderMapImpl tls_req{{":method", "GET"},
                                         {":path", "/test"},
                                         {":scheme", "https"},
                                         {":authority", "tls.example.com"},
                                         {"x-route-type", "tls"}};
  Http::TestRequestHeaderMapImpl raw_req{{":method", "GET"},
                                         {":path", "/test"},
                                         {":scheme", "https"},
  ...
  // TLS route should succeed with TLS selected by server name.
  {
    IntegrationStreamDecoderPtr response = codec_client_->makeHeaderOnlyRequest(tls_req);
    ASSERT_TRUE(response->waitForEndStream());
    EXPECT_EQ("200", response->headers().getStatusValue());
    const auto header_entry = response->headers().get(Http::LowerCaseString{host_type_header_});
    EXPECT_FALSE(header_entry.empty());
  ...
  // Raw route should succeed with raw selected by on_no_match.
  {
    IntegrationStreamDecoderPtr response = codec_client_->makeHeaderOnlyRequest(raw_req);
    ASSERT_TRUE(response->waitForEndStream());
    EXPECT_EQ("200", response->headers().getStatusValue());
    const auto header_entry = response->headers().get(Http::LowerCaseString{host_type_header_});
    EXPECT_FALSE(header_entry.empty());
...
// Removed disabled tests; replaced with deterministic xDS input tests above.
