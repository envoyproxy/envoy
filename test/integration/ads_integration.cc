...
envoy::config::cluster::v3alpha::Cluster AdsIntegrationTest::buildCluster(const std::string& name) {
  return TestUtility::parseYaml<envoy::config::cluster::v3alpha::Cluster>(fmt::format(R"EOF(
      name: {}
      connect_timeout: 5s
      type: EDS
...
AdsIntegrationTest::buildRedisCluster(const std::string& name) {
  return TestUtility::parseYaml<envoy::config::cluster::v3alpha::Cluster>(fmt::format(R"EOF(
      name: {}
      connect_timeout: 5s
      type: EDS
...
AdsIntegrationTest::buildListener(const std::string& name, const std::string& route_config,
  ...
  return TestUtility::parseYaml<envoy::config::listener::v3alpha::Listener>(fmt::format(
      R"EOF(
      name: {}
      address:
        socket_address:
