...
export ENVOY_BIN="${TEST_SRCDIR}/${TEST_WORKSPACE}/test/integration/hotrestart_main"
...
if [[ -z "${ENVOY_IP_TEST_VERSIONS}" ]] || [[ "${ENVOY_IP_TEST_VERSIONS}" == "all" ]] \
  ...
  sed -e "s#{{ upstream_. }}#0#g" "${TEST_SRCDIR}/${TEST_WORKSPACE}/test/config/integration/server.yaml | \
    sed -e "s#{{ test_rundir }}#$TEST_SRCDIR/$TEST_WORKSPACE#" | \
...
sed -e "s#{{ socket_dir }}#${SOCKET_DIR}#" "${TEST_SRCDIR}/${TEST_WORKSPACE}/test/config/integration/server_unix_listener.yaml | \
  sed -e "s#{{ ip_loopback_address }}#127.0.0.1#" | \
  sed -e "s#{{ null_device_path }}#/dev/null#" | \
...
echo "building ${HOT_RESTART_JSON_V4} ..."
sed -e "s#{{ upstream_. }}#0#g" "${TEST_SRCDIR}/${TEST_WORKSPACE}/test/config/integration/server_multiple_addresses.yaml | \
  sed -e "s#{{ test_rundir }}#$TEST_SRCDIR/$TEST_WORKSPACE#" | \
  sed -e "s#{{ test_tmpdir }}#$TEST_TMPDIR#" | \
  sed -e "s#{{ ip_loopback_address }}#127.0.0.1#" | \
  sed -e "s#{{ enable_reuse_port }}#true#" | \
...
function run_testsuite() {
  ...
  "${TEST_SRCDIR}/${TEST_WORKSPACE}/tools/socket_passing/socket_passing "-o" "${HOT_RESTART_JSON}" "-a" "${ADMIN_ADDRESS_PATH_0}" \
  ...
  "${TEST_SRCDIR}/${TEST_WORKSPACE}/tools/socket_passing/socket_passing "-o" "${UPDATED_HOT_RESTART_JSON}" "-a" "${ADMIN_ADDRESS_PATH_1}" \
  ...
  "${TEST_SRCDIR}/${TEST_WORKSPACE}/tools/socket_passing/socket_passing "-o" "${UPDATED_HOT_RESTART_JSON}" "-a" "${ADMIN_ADDRESS_PATH_2}" \
