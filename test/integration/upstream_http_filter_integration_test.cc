...
public:
  ...
  void addStaticRouterFilter(const HttpFilterProto& config) {
    config_helper_.addConfigModifier(
        [config](
          ...
          static_cast<void>(router_filter.typed_config().UnpackTo(&router));
          *router.add_upstream_http_filters() = config;
          static_cast<void>(router_filter.mutable_typed_config()->PackFrom(router));
        });
  }
  ...
  const HttpFilterProto getAddHeaderFilterConfig(const std::string& name, const std::string& key,
    ...
    static_cast<void>(filter_config.mutable_typed_config()->PackFrom(configuration));
  ...
  void addStaticNoConfigFilter(const std::string& name) {
    HttpFilterProto filter_config;
    filter_config.set_name(name);
    static_cast<void>(filter_config.mutable_typed_config()->PackFrom(Envoy::Protobuf::Struct()));
    if (useRouterFilters()) {
  ...
  void addStaticTypedConfigFilter(const std::string& name, const ProtoConfig& config) {
    HttpFilterProto filter_config;
    filter_config.set_name(name);
    static_cast<void>(filter_config.mutable_typed_config()->PackFrom(config));
    if (useRouterFilters()) {
      addStaticRouterFilter(filter_config);
    } else {
      addStaticClusterFilter(filter_config);
...
public:
  ...
  void setDynamicFilterConfig(HttpFilterProto* filter, const std::string& name,
    ...
    if (set_default_config) {
      ...
      static_cast<void>(discovery->mutable_default_config()->PackFrom(default_configuration));
    }
  ...
  void sendLdsResponse(const std::string& version) {
    ...
    static_cast<void>(response.add_resources()->PackFrom(listener_config_));
  ...
  void sendXdsResponse(const std::string& name, const std::string& version, const std::string& key,
    ...
    static_cast<void>(typed_config.mutable_typed_config()->PackFrom(configuration));

    envoy::service::discovery::v3::Resource resource;
    resource.set_name(name);
    static_cast<void>(resource.mutable_resource()->PackFrom(typed_config));
    if (ttl) {
    ...
    static_cast<void>(response.add_resources()->PackFrom(resource));
...
TEST_P(DynamicRouterOrClusterFiltersIntegrationTest, BasicSuccessWithConfigDump) {
  ...
  static_cast<void>(config_dump.configs(2).UnpackTo(&ecds_config_dump));
  ...
  static_cast<void>(filter_config.typed_config().UnpackTo(&http_filter_config));
