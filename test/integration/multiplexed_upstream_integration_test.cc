...
TEST_P(MultiplexedUpstreamIntegrationTest, DisableUpstreamEarlyData) {
  ...
  config_helper_.addConfigModifier(
      [&](envoy::extensions::filters::network::http_connection_manager::v3::HttpConnectionManager&
        ...
        static_cast<void>(early_data_policy->mutable_typed_config()->PackFrom(config));
      });
  initialize();
  codec_client_ = makeHttpConnection(lookupPort("http"));
...
TEST_P(MultiplexedUpstreamIntegrationTest, UpstreamDisconnectDuringEarlyData) {
  ...
  static_cast<void>(crypto_stream_config->mutable_typed_config()->PackFrom(Protobuf::Struct()));
  mergeOptions(options);
