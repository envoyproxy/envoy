...
void MultiplexedUpstreamIntegrationTest::manySimultaneousRequests(uint32_t request_bytes,
  ...
  TEST_P(MultiplexedUpstreamIntegrationTest, UpstreamConnectionCloseWithManyStreams) {
    ...
    for (uint32_t i = 1; i < num_requests; ++i) {
      ...
      upstream_requests[i]->encodeHeaders(Http::TestResponseHeaderMapImpl{{":status", "200"}},
                                          false);
...
typed_config:
  ...
  upstream_log:
    ...
    config_helper_.prependFilter(
        "{ name: encoder-decoder-buffer-filter, typed_config: { \"@type\": "
  ...
  if (upstreamProtocol() != Http::CodecType::HTTP3 || !(Runtime::runtimeFeatureEnabled(Runtime::conn_pool_new_stream_with_early_data_and_http3) && Runtime::runtimeFeatureEnabled("envoy.reloadable_features.http3_sends_early_data") {
      return;
  }
  initialize();
  codec_client_ = makeHttpConnection(lookupPort("http"));
