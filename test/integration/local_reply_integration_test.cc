...
body_format:
  ...
  if (GetParam().upstream_protocol == Http::CodecType::HTTP3) {
    expected_body = R"({
      "level": "TRACE",
      "user_agent": null,
      "response_body": "upstream connect error or disconnect/reset before headers. reset reason: connection termination, transport failure reason: QUIC_NO_ERROR|from:0|Closed by application"
  })";
  } else {
    expected_body = R"({
      "level": "TRACE",
  ...
  if (GetParam().upstream_protocol == Http::CodecType::HTTP3) {
    EXPECT_EQ("220", response->headers().ContentLength()->value().getStringView());
  } else {
...
body_format:
  ...
  if (GetParam().upstream_protocol == Http::CodecType::HTTP3) {
    expected_grpc_message = R"({
      "code": 503,
      "message":"upstream connect error or disconnect/reset before headers. reset reason: connection termination, transport failure reason: QUIC_NO_ERROR|from:0|Closed by application"
})";
  } else {
    expected_grpc_message = R"({
      "code": 503,
...
body_format:
  ...
  if (GetParam().upstream_protocol == Http::CodecType::HTTP3) {
    expected_grpc_message =
        ...
        "QUIC_NO_ERROR|from:0|Closed by application:503:path=/package.service/method%0A";
  } else {
    expected_grpc_message =
        "upstream connect error or disconnect/reset before headers. reset reason:"
...
body_format:
  ...
  EXPECT_EQ("application/json", response->headers().ContentType()->value().getStringView());
  if (GetParam().upstream_protocol == Http::CodecType::HTTP3) {
    EXPECT_EQ("224", response->headers().ContentLength()->value().getStringView());
  } else {
...
mappers:
  ...
  EXPECT_EQ("text/plain", response->headers().ContentType()->value().getStringView());
  if (GetParam().upstream_protocol == Http::CodecType::HTTP3) {
    EXPECT_EQ("165", response->headers().ContentLength()->value().getStringView());
  } else {
  ...
  if (GetParam().upstream_protocol == Http::CodecType::HTTP3) {
    EXPECT_EQ(response->body(), "upstream connect error or disconnect/reset before headers. reset "
                                ...
                                "QUIC_NO_ERROR|from:0|Closed by application");
  } else {
    EXPECT_EQ(response->body(), "upstream connect error or disconnect/reset before headers. reset "
