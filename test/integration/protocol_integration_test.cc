...
bool paused(IntegrationTestServerPtr& server) {
  if (!server->counter("cluster.cluster_0.upstream_flow_control_backed_up_total") ||
      !server->counter("cluster.cluster_0.upstream_flow_control_drained_total")) {
    return false;
  }

  return server->counter("cluster.cluster_0.upstream_flow_control_backed_up_total")->value() !=
         server->counter("cluster.cluster_0.upstream_flow_control_drained_total")->value();
}

TEST_P(ProtocolIntegrationTest, RetryDuringOverrun) {
  // Set a tiny upstream buffer limit and a very large downstream limit, to
  // make it easier for the request to back up in the router filter.
  config_helper_.setBufferLimits(1024, 200000000);
  initialize();
  ...
  // Read disable, and send a large body until the router backs up.
  ASSERT_TRUE(fake_upstream_connection_->readDisable(true));
  do {
  ...
  } while (!paused(test_server_));

  // This is a bit of a hack. The router buffer may fill and drain a few times
  // before the connection really gets wedged. The loop above tries to avoid
  // catching the transient pause by sleeping when it looks wedged, and checking
  // to see that it hasn't drained yet. If that assert fails, the wait needs to
  // be longer to de-flake.
  ASSERT(paused(test_server_)) << "See test comments";

  // Send failure headers from upstream, and verify the stream is killed off.
  upstream_request_->encodeHeaders(Http::TestResponseHeaderMapImpl{{":status", "503"}}, false);
  if (fake_upstreams_[0]->httpType() == FakeHttpConnection::Type::HTTP1) {
  ...
  // Now send the end stream, and make sure it is proxied through, to verify
  // that the router causes the HCM to resume reading when the retry is set up
  // and the buffer drains.
  Buffer::OwnedImpl data(std::string(1, 'a'));
  codec_client_->sendData(encoder_decoder.first, data, true);
