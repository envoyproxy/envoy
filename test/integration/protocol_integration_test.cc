...
TEST_P(DownstreamProtocolIntegrationTest, RetryPriority) {
  ...
  static_cast<void>(retry_priority_config->mutable_typed_config()->PackFrom(config));
  config_helper_.addVirtualHost(host);
...
TEST_P(DownstreamProtocolIntegrationTest, RetryHostPredicateFilter) {
  ...
  static_cast<void>(host_predicate->mutable_typed_config()->PackFrom(config));
  config_helper_.addVirtualHost(host);
...
TEST_P(ProtocolIntegrationTest, ServerHalfCloseBeforeClientWithErrorAndBufferedResponseData) {
  ...
  if (fake_upstreams_[0]->httpType() == Http::CodecType::HTTP1) {
    ...
    if (downstreamProtocol() == Http::CodecType::HTTP1) {
      ...
      // attempts to finish receiving response. Resume reading so the response can complete.
      // The stream may terminate via end_stream (response fully received) and/or via reset
      // (STOP_SENDING). The ordering between these two events is non-deterministic, so use
      // waitForAnyTermination() to handle either case.
      request_encoder_->getStream().readDisable(false);
