...
TEST_P(InternalAddressIntegrationTest, NoUpstream) {
...
TEST_P(InternalAddressIntegrationTest, ShutdownWithOpenConnections) {
  config_helper_.addConfigModifier([&](envoy::config::bootstrap::v3::Bootstrap& bootstrap) -> void {
...
TEST_P(InternalAddressIntegrationTest, TestIdletimeoutWithNoData) {
...
TEST_P(InternalAddressIntegrationTest, DISABLED_TcpProxyDownstreamFlush) {
...
TEST_P(InternalAddressIntegrationTest, DISABLED_TcpProxyUpstreamFlush) {
  // Use a very large size to make sure it is larger than the kernel socket read buffer.
  const uint32_t size = 50 * 1024 * 1024;
...
TEST_P(InternalAddressIntegrationTest, DISABLED_TcpProxyUpstreamFlushEnvoyExit) {
  // Use a very large size to make sure it is larger than the kernel socket read buffer.
  const uint32_t size = 50 * 1024 * 1024;
  config_helper_.setBufferLimits(size, size);
  initialize();
