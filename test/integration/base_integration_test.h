  ...
  void sendSotwDiscoveryResponse(const std::string& type_url, const std::vector<T>& messages,
    ...
    for (const auto& message : messages) {
      if (!metadata.empty()) {
        ...
        static_cast<void>(resource.mutable_resource()->PackFrom(message));
        ...
        static_cast<void>(discovery_response.add_resources()->PackFrom(resource));
      } else {
        static_cast<void>(discovery_response.add_resources()->PackFrom(message));
      }
    }
