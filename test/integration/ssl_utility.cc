...
void initializeUpstreamTlsContextConfig(
  ...
  tls_context.mutable_common_tls_context()->mutable_validation_context()->mutable_trusted_ca()->set_filename("{{ test_rundir }}/test/config/integration/certs/cacert.pem");
  auto* certs = tls_context.mutable_common_tls_context()->add_tls_certificates();
  std::string chain;
  std::string key;
  if (options.client_ecdsa_cert_) {
    chain = "{{ test_rundir }}/test/config/integration/certs/client_ecdsacert.pem";
    key = "{{ test_rundir }}/test/config/integration/certs/client_ecdsakey.pem";
  } else if (options.use_expired_spiffe_cert_) {
        chain = "{{ test_rundir }}/test/extensions/transport_sockets/tls/test_data/expired_spiffe_san_cert.pem";
        key = "{{ test_rundir }}/test/extensions/transport_sockets/tls/test_data/expired_spiffe_san_key.pem";
  } else if (options.client_with_intermediate_cert_) {
        chain = "{{ test_rundir }}/test/config/integration/certs/client2_chain.pem";
        key = "{{ test_rundir }}/test/config/integration/certs/client2key.pem";
  } else {
        chain = "{{ test_rundir }}/test/config/integration/certs/clientcert.pem";
        key = "{{ test_rundir }}/test/config/integration/certs/clientkey.pem";
  }
