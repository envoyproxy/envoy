...
public:
  CustomStaticCluster(Server::Configuration::ServerFactoryContext& server_context,
                      ...
                      bool added_via_api, uint32_t priority, std::string address, uint32_t port)
...
private:
  ...
  createClusterWithConfig(Server::Configuration::ServerFactoryContext& server_context,
    ...
    auto new_cluster = std::make_shared<CustomStaticCluster>(
        server_context, cluster, context, context.runtime(), context.addedViaApi(),
        proto_config.priority(), proto_config.address(), proto_config.port_value());
