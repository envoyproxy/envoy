...
TEST_P(ConnectTerminationIntegrationTest, UpstreamRstNotPropagatedWithoutHttpGuard) {
  ...
  // Without the HTTP guard, HttpUpstream maps this to LocalClose with Normal close type,
  // so tcp_proxy should use FlushWrite (FIN), not AbortReset. The downstream behavior
  // depends on the HTTP protocol version.
  if (downstream_protocol_ == Http::CodecType::HTTP3) {
    ASSERT_TRUE(response_->waitForAnyTermination());
  } else if (downstream_protocol_ == Http::CodecType::HTTP2) {
    ASSERT_TRUE(response_->waitForAnyTermination());
