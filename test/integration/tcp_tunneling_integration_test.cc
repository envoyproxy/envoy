...
TEST_P(TcpTunnelingIntegrationTest, HeaderEvaluatorReuse) {
  ...
  // Set the "downstream-local-ip" and "downstream-remote-port".
  config_helper_.addConfigModifier([&](envoy::config::bootstrap::v3::Bootstrap& bootstrap) -> void {
    envoy::extensions::filters::network::tcp_proxy::v3::TcpProxy proxy_config;
    proxy_config.set_stat_prefix("tcp_stats");
    proxy_config.set_cluster("cluster_0");
    ...
    address_header->mutable_header()->set_key("downstream-local-ip");
    address_header->mutable_header()->set_value("%DOWNSTREAM_LOCAL_ADDRESS_WITHOUT_PORT%");

    auto port_header = proxy_config.mutable_tunneling_config()->mutable_headers_to_add()->Add();
    port_header->mutable_header()->set_key("downstream-remote-port");
    port_header->mutable_header()->set_value("%DOWNSTREAM_REMOTE_PORT%");
  ...
  // Fetch and save the remote port.
  auto port_headers_1 =
      upstream_request_->headers().get(Envoy::Http::LowerCaseString("downstream-remote-port"));
  EXPECT_EQ(1, port_headers_1.size());
  const std::string port1(port_headers_1[0]->value().getStringView());
  ...
  EXPECT_EQ(
      upstream_request_2->headers().get(Envoy::Http::LowerCaseString("downstream-local-ip")).size(),
      1);
  ...
  // The 2 downstream connections must not share the same ephemeral port. Verify that the header
  // evaluator produce the correct values.
  auto port_headers_2 =
      upstream_request_2->headers().get(Envoy::Http::LowerCaseString("downstream-remote-port"));
  EXPECT_EQ(1, port_headers_2.size());
  const std::string port2(port_headers_2[0]->value().getStringView());
