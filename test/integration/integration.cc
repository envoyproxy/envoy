...
void BaseIntegrationTest::createGeneratedApiTestServer(const std::string& bootstrap_path,
  ...
  test_server_ = IntegrationTestServer::create(
      bootstrap_path, version_, pre_worker_start_test_steps_, deterministic_, *time_system_, *api_, defer_listener_finalization_);
  if (config_helper_.bootstrap().static_resources().listeners_size() > 0 && !defer_listener_finalization_) {
...
BaseIntegrationTest::createIntegrationTestServer(const std::string& bootstrap_path,
  ...
  return IntegrationTestServer::create(bootstrap_path, version_, pre_worker_start_test_steps,
                                       deterministic_, time_system, *api_, defer_listener_finalization_);
}
