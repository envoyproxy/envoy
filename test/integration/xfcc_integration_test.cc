...
common_tls_context:
  ...
  envoy::api::v3alpha::auth::UpstreamTlsContext config;
...
void XfccIntegrationTest::initialize() {
  ...
  config_helper_.addConfigModifier(
      [&](envoy::config::bootstrap::v3alpha::Bootstrap& bootstrap) -> void {
        ...
        envoy::api::v3alpha::auth::UpstreamTlsContext context;
...
TEST_P(XfccIntegrationTest, MtlsForwardOnly) {
  fcc_ = envoy::config::filter::network::http_connection_manager::v3alpha::HttpConnectionManager::
      FORWARD_ONLY;
  initialize();
