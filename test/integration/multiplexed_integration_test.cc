...
// Test graceful GOAWAY behavior when triggered by L7 filter for HTTP/2 only.
// This tests that the graceful_goaway_timeout configuration works for filter-triggered scenarios.
// Note: Drain scenarios use shutdownNotice() which has its own adapter specific implementation
// and do not use the gracefulGoAway method.
TEST_P(MultiplexedIntegrationTest, Http2GracefulGoAwayTriggerredByFilter) {
  EXCLUDE_DOWNSTREAM_HTTP3
  if (downstreamProtocol() != Http::CodecType::HTTP2) {
    return;
  }

  // Configure graceful GOAWAY timeout - this should affect filter-triggered GOAWAY
  config_helper_.addConfigModifier(
      [&](envoy::extensions::filters::network::http_connection_manager::v3::HttpConnectionManager&
              hcm) -> void {
  ...
  // Should receive graceful GOAWAY sequence and then reset
  ASSERT_TRUE(response->waitForReset());
  ASSERT_TRUE(codec_client_->waitForDisconnect());

  // Verify graceful GOAWAY stats were incremented
  test_server_->waitForCounterGe("http2.goaway_graceful_sent", 1);
}

// Test interaction between graceful GOAWAY and connection manager drain.
// Verifies that drain-triggered GOAWAY uses shutdownNotice() and doesn't use
// graceful_goaway_timeout.
TEST_P(MultiplexedIntegrationTest, Http2GracefulGoAwayDrainInteraction) {
  EXCLUDE_DOWNSTREAM_HTTP3
  if (downstreamProtocol() != Http::CodecType::HTTP2) {
    return;
  }

  // Configure graceful GOAWAY timeout - this should NOT affect drain-triggered GOAWAY
  config_helper_.addConfigModifier(
      [&](envoy::extensions::filters::network::http_connection_manager::v3::HttpConnectionManager&
              hcm) -> void {
        auto* http2_options = hcm.mutable_http2_protocol_options();
  ...
  // Establish connection and start a request
  codec_client_ = makeHttpConnection(lookupPort("http"));
  auto response = codec_client_->makeHeaderOnlyRequest(default_request_headers_);
  ASSERT_TRUE(response->waitForEndStream());
  ...
  // Start drain sequence - this should trigger shutdownNotice() not graceful GOAWAY
  ...
  // Make requests until drain close happens - this mimics the drain test pattern
  ...
  // Connection should eventually close due to drain
  ASSERT_TRUE(codec_client_->waitForDisconnect());

  // Now check that GOAWAY was sent via shutdownNotice (not graceful GOAWAY)
  EXPECT_TRUE(codec_client_->sawGoAway());

  // Verify drain stats but graceful GOAWAY stats should NOT be incremented
  // because drain uses shutdownNotice() not graceful GOAWAY mechanism
  EXPECT_EQ(test_server_->counter("http2.goaway_graceful_sent")->value(), 0);
}

// Test that filter-triggered graceful GOAWAY works during drain phase.
// This verifies the behavior when both mechanisms could be active.
TEST_P(MultiplexedIntegrationTest, Http2FilterGracefulGoAwayDuringDrain) {
  EXCLUDE_DOWNSTREAM_HTTP3
  if (downstreamProtocol() != Http::CodecType::HTTP2) {
    return;
  }

  // Configure graceful GOAWAY timeout
  config_helper_.addConfigModifier(
      [&](envoy::extensions::filters::network::http_connection_manager::v3::HttpConnectionManager&
              hcm) -> void {
        auto* http2_options = hcm.mutable_http2_protocol_options();
        http2_options->mutable_graceful_goaway_timeout()->set_nanos(50000000); // 50ms
      });
  ...
  // Now establish connection and trigger filter-based graceful GOAWAY
  codec_client_ = makeHttpConnection(lookupPort("http"));
  auto response = codec_client_->makeHeaderOnlyRequest(
      Http::TestRequestHeaderMapImpl{{":method", "GET"},
                                     {":path", "/test/filter/goaway"},
                                     {":scheme", "http"},
                                     {":authority", "host"},
                                     {"skip-goaway", "false"}});

  // Should receive graceful GOAWAY sequence from filter, even during drain
  ASSERT_TRUE(response->waitForReset());
  ASSERT_TRUE(codec_client_->waitForDisconnect());

  // Verify filter-triggered graceful GOAWAY stats were incremented
  // This shows filter-triggered graceful GOAWAY still works during drain
  test_server_->waitForCounterGe("http2.goaway_graceful_sent", 1);
}
