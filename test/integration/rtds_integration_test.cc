...
TEST_P(RtdsIntegrationTest, RtdsReload) {
  ...
  auto some_rtds_layer = TestUtility::parseYaml<envoy::service::discovery::v3alpha::Runtime>(R"EOF(
    name: some_rtds_layer
    layer:
      foo: bar
      baz: meh
  )EOF");
  sendDiscoveryResponse<envoy::service::discovery::v3alpha::Runtime>(
  ...
  some_rtds_layer = TestUtility::parseYaml<envoy::service::discovery::v3alpha::Runtime>(R"EOF(
    name: some_rtds_layer
    layer:
      baz: saz
  )EOF");
