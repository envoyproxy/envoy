...
// Tests that a LDS adding listener works as expected receiving connections from multiple workers.
TEST_P(ListenerIntegrationTest, BasicSuccessWithMultipleClients) {
  on_server_init_function_ = [&]() {
    createLdsStream();
    sendLdsResponseV3({MessageUtil::getYamlStringFromMessage(listener_config_)}, "1");
    createRdsStream(route_table_name_);
  ...
  uint32_t port = lookupPort(listener_name_);
  codec_client_ = makeHttpConnection(port);
  const int extra_downstream_num = 10;
  for (int i = 0; i < extra_downstream_num; i++) {
    extra_codec_clients_.push_back(makeHttpConnection(port));
  }
  ...
  for (int i = 0; i <= extra_downstream_num; i++) {
    auto response = sendRequestAndWaitForResponse(
        Http::TestResponseHeaderMapImpl{
            {":method", "GET"}, {":path", "/"}, {":authority", "host"}, {":scheme", "http"}},
        request_size, response_headers, response_size, /*cluster_0*/ 0, i);
    verifyResponse(std::move(response), "200", response_headers, std::string(response_size, 'a'));
    EXPECT_TRUE(upstream_request_->complete());
    EXPECT_EQ(request_size, upstream_request_->bodyLength());
  }
}
TEST_P(ListenerIntegrationTest, BasicSuccessOnDemandFilterChainWithMultipleClients) {
  on_server_init_function_ = [&]() {
    createLdsStream();
    listener_config_2_ = listener_config_;
    listener_config_2_.set_name(listener_name_2_);
  ...
  uint32_t port = lookupPort(listener_name_2_);
  codec_client_ = makeHttpConnection(port);
  const int extra_downstream_num = 10;
  for (int i = 0; i < extra_downstream_num; i++) {
    extra_codec_clients_.push_back(makeHttpConnection(port));
  ...
  *new_filter_chain = *listener_config_2_.mutable_filter_chains(0);
  ...
  // close current connection, make a new connection, will not request rebuilding again.
  cleanupDownstream();
  port = lookupPort(listener_name_2_);
  codec_client_ = makeHttpConnection(port);
  for (int i = 0; i < extra_downstream_num; i++) {
    extra_codec_clients_.push_back(makeHttpConnection(port));
  }
