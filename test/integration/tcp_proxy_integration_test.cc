...
void TcpProxyMetadataMatchIntegrationTest::initialize() {
  ...
  config_helper_.addConfigModifier([&](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    ...
    static_cast<void>(static_resources->mutable_listeners(0)
...
TEST_P(TcpProxyIntegrationTest, UpstreamConnectModeOnDownstreamData) {
  config_helper_.addConfigModifier([](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    ...
    static_cast<void>(filter->typed_config().UnpackTo(&tcp_proxy));
    ...
    static_cast<void>(filter->mutable_typed_config()->PackFrom(tcp_proxy));
  });
