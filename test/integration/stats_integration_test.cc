...
TEST_P(ClusterMemoryTestRunner, MemoryLargeClusterSizeWithFakeSymbolTable) {
  ...
  // 2019/12/04  9227     49323       50000   router: per-cluster histograms w/ timeout budget
  ...
  EXPECT_MEMORY_EQ(m_per_cluster, 49323); // 104 bytes higher than a debug build.
...
TEST_P(ClusterMemoryTestRunner, MemoryLargeClusterSizeWithRealSymbolTable) {
  ...
  // 2019/12/04  9227     40758       42000   router: per-cluster histograms w/ timeout budget
  ...
  // https://github.com/envoyproxy/envoy/blob/master/source/docs/stats.md#stats-memory-tests
  // for details on how to fix.
  EXPECT_MEMORY_EQ(m_per_cluster, 40758); // 104 bytes higher than a debug build.
  EXPECT_MEMORY_LE(m_per_cluster, 42000);
}
