...
TEST_P(ClusterMemoryTestRunner, MemoryLargeClusterSizeWithFakeSymbolTable) {
  ...
  // 2020/01/08  8889     43464       43500   api: add UpstreamHttpProtocolOptions message
  ...
  EXPECT_MEMORY_EQ(m_per_cluster, 43464); // 104 bytes higher than a debug build.
...
TEST_P(ClusterMemoryTestRunner, MemoryLargeClusterSizeWithRealSymbolTable) {
  ...
  // 2020/01/08  8889     35598       35500   api: add UpstreamHttpProtocolOptions message
  ...
  // https://github.com/envoyproxy/envoy/blob/master/source/docs/stats.md#stats-memory-tests
  // for details on how to fix.
  EXPECT_MEMORY_EQ(m_per_cluster, 35598); // 104 bytes higher than a debug build.
