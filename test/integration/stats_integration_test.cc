...
TEST_P(ClusterMemoryTestRunner, MemoryLargeClusterSizeWithFakeSymbolTable) {
  ...
  // 2019/12/09  9227     44395       45000   router: per-cluster histograms w/ timeout budget
  ...
  EXPECT_MEMORY_EQ(m_per_cluster, 44395); // 104 bytes higher than a debug build.
...
TEST_P(ClusterMemoryTestRunner, MemoryLargeClusterSizeWithRealSymbolTable) {
  ...
  // https://github.com/envoyproxy/envoy/blob/master/source/docs/stats.md#stats-memory-tests
  // for details on how to fix.
  EXPECT_MEMORY_EQ(m_per_cluster, 36053); // 104 bytes higher than a debug build.
