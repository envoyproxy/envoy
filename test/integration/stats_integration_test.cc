...
TEST_P(StatsIntegrationTest, WithExpiringCert) {
  config_helper_.addConfigModifier([](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
    ...
    tls_certificate->set_stat_prefix("example.com");
  });
  ...
  if (GetParam() == Network::Address::IpVersion::v4) {
    test_server_->waitForGaugeGe("listener.127.0.0.1_0.tls_context.example.com.days_until_cert_expiring",
                                  days_until_expiry);
  } else {
    test_server_->waitForGaugeGe("listener.[__1]_0.tls_context.example.com.days_until_cert_expiring",
                                  days_until_expiry);
  }
}
