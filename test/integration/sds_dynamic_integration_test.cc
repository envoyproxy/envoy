...
protected:
  ...
  void sendSdsResponse(const envoy::extensions::transport_sockets::tls::v3::Secret& secret) {
    ...
    static_cast<void>(discovery_response.add_resources()->PackFrom(secret));
    ...
    if (configure_session_ticket_sds_) {
      ...
      config_helper_.addConfigModifier([this](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
        ...
        if (test_quic_) {
          ...
          static_cast<void>(ts->mutable_typed_config()->PackFrom(quic_config));
        } else {
          ...
          static_cast<void>(ts->mutable_typed_config()->PackFrom(tls_context));
        }
      });
    }
  ...
  void initialize() override {
    ...
    config_helper_.addConfigModifier([this](envoy::config::bootstrap::v3::Bootstrap& bootstrap) {
      ...
      if (test_quic_) {
        ...
        static_cast<void>(transport_socket->mutable_typed_config()->PackFrom(quic_context));
      } else {
