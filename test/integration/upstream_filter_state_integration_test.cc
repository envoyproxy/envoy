...
public:
  ...
  createTransportSocket(Network::TransportSocketOptionsConstSharedPtr options) const {
...
/*
 * Verifies that the Http Router's `upstream_log` correctly reflects the upstream fiter state data
 * set when the access log format has `UPSTREAM_FILTER_STATE` specifier.
 */
