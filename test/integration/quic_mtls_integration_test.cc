...
// Comprehensive E2E test for mTLS functionality with ProofVerifier.
TEST_P(QuicMtlsIntegrationTest, ComprehensiveMtlsE2EWithProofVerifier) {
  ...
  // Verify XFCC header is present with client certificate info.
  auto xfcc_headers =
      upstream_request_->headers().get(Http::LowerCaseString("x-forwarded-client-cert"));
  ...
  // Complete the HTTP request/response cycle.
  upstream_request_->encodeHeaders(default_response_headers_, true);
  ASSERT_TRUE(response->waitForEndStream());
  EXPECT_EQ("200", response->headers().getStatusValue());
