...
public:
  ...
  void TearDown() override {
    if (eds_connection_ != nullptr) {
      ...
      ASSERT(eds_connection_->close());
      ASSERT(eds_connection_->waitForDisconnect());
  ...
  void initialize() override {
    if (use_eds_) {
      pre_worker_start_test_steps_ = [this]() {
        ASSERT(fake_upstreams_[1]->waitForHttpConnection(*dispatcher_, &eds_connection_));
        ASSERT(eds_connection_->waitForNewStream(*dispatcher_, &eds_stream_));
        eds_stream_->startGrpcStream();

        envoy::api::v2::DiscoveryRequest discovery_request;
        ASSERT(eds_stream_->waitForGrpcMessage(*dispatcher_, discovery_request));
