...
// envoy. Sends a request with a global timeout specified, sleeps for longer than the timeout, and
// ensures that a timeout is received.
// TEST_P(HttpTimeoutIntegrationTest, GlobalTimeoutEgressIngressEnvoys) {
//  initialize();
//
//  codec_client_ = makeHttpConnection(makeClientConnection(lookupPort("http")));
//  auto encoder_decoder = codec_client_->startRequest(
...
TEST_P(HttpTimeoutIntegrationTest, GlobalTimeoutEgressIngressEnvoys2) {
  initialize();
