...
// Verifies that a 304 response returns without Transfer-Encoding nor Content-Length header
TEST_P(IntegrationTest, Response304WithoutTransferEncoding) {
  initialize();
  auto tcp_client = makeTcpConnection(lookupPort("http"));
  ...
  ASSERT_TRUE(fake_upstream_connection->waitForData(
      FakeRawConnection::waitForInexactMatch("\r\n\r\n"), &data));
  ASSERT_THAT(data, StartsWith("GET"));

  ASSERT_TRUE(fake_upstream_connection->write("HTTP/1.1 304 OK\r\netag: \"1234567890\"\r\n\r\n"));

  tcp_client->waitForData("\r\n\r\n", false);
  std::string response = tcp_client->data();

  EXPECT_THAT(response, StartsWith("HTTP/1.1 304 Not Modified\r\n"));
  EXPECT_THAT(response, Not(HasSubstr("transfer-encoding: chunked\r\n")));
  EXPECT_THAT(response, Not(HasSubstr("content-length")));
  EXPECT_THAT(response, EndsWith("\r\n\r\n"));
...
// Verifies same behavior for HEAD request with a 304 response returns without Transfer-Encoding nor
// Content-Length header
TEST_P(IntegrationTest, HeadResponse304WithoutTransferEncoding) {
  initialize();
  auto tcp_client = makeTcpConnection(lookupPort("http"));

  ASSERT_TRUE(tcp_client->write(
      "HEAD /test/long/url HTTP/1.1\r\nHost: host\r\nif-none-match: \"1234567890\"\r\n\r\n"));
  ...
  ASSERT_TRUE(fake_upstream_connection->waitForData(
      FakeRawConnection::waitForInexactMatch("\r\n\r\n"), &data));
  ASSERT_THAT(data, StartsWith("HEAD"));
...
TEST_P(IntegrationTest, Response304WithContentLength) {
  ...
  EXPECT_THAT(response, StartsWith("HTTP/1.1 304 Not Modified\r\n"));
  EXPECT_THAT(response, Not(HasSubstr("transfer-encoding: chunked\r\n")));
  EXPECT_THAT(response, HasSubstr("content-length: 123\r\n"));
  EXPECT_THAT(response, EndsWith("\r\n\r\n"));
...
// Test keepalive response with 304 and content-length
TEST_P(IntegrationTest, TestKeepAlive304Responses) {
  initialize();
  auto tcp_client = makeTcpConnection(lookupPort("http"));
  ...
  EXPECT_THAT(response, StartsWith("HTTP/1.1 304 Not Modified\r\n"));
  EXPECT_THAT(response, HasSubstr("content-length: 10\r\n"));
  EXPECT_THAT(response, HasSubstr("etag: \"0987654321\"\r\n"));
  EXPECT_THAT(response, EndsWith("\r\n\r\n"));

  tcp_client->clearData();

  // make sure subsequent request is ok after 304 response with content-length
  ASSERT_TRUE(tcp_client->write("GET /test/long/url HTTP/1.1\r\nHost: host\r\n\r\n"));
  ASSERT_TRUE(fake_upstream_connection->write(
      "HTTP/1.1 200 OK\r\ncontent-length: 10\r\netag: \"0987654321\"\r\n\r\n1234567890"));

  tcp_client->waitForData("1234567890", false);
  response = tcp_client->data();

  EXPECT_THAT(response, StartsWith("HTTP/1.1 200 OK\r\n"));
  EXPECT_THAT(response, HasSubstr("content-length: 10\r\n"));
  EXPECT_THAT(response, HasSubstr("etag: \"0987654321\"\r\n"));
  EXPECT_THAT(response, EndsWith("1234567890"));
  ASSERT_TRUE(fake_upstream_connection->close());
  ASSERT_TRUE(fake_upstream_connection->waitForDisconnect());
  tcp_client->close();
}
