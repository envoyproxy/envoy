...
public:
  ...
  void initialize() override {
    ...
    sendDiscoveryResponse<envoy::config::cluster::v3alpha::Cluster>(
        Config::TypeUrl::get().Cluster, {cluster1_}, {cluster1_}, {}, "55");
  ...
  envoy::config::cluster::v3alpha::Cluster cluster1_;
  envoy::config::cluster::v3alpha::Cluster cluster2_;
...
TEST_P(CdsIntegrationTest, CdsClusterUpDownUp) {
  ...
  sendDiscoveryResponse<envoy::config::cluster::v3alpha::Cluster>(Config::TypeUrl::get().Cluster,
                                                                  {}, {}, {ClusterName1}, "42");
  // We can continue the test once we're sure that Envoy's ClusterManager has made use of
  ...
  sendDiscoveryResponse<envoy::config::cluster::v3alpha::Cluster>(
      Config::TypeUrl::get().Cluster, {cluster1_}, {cluster1_}, {}, "413");
...
TEST_P(CdsIntegrationTest, TwoClusters) {
  ...
  EXPECT_TRUE(compareDiscoveryRequest(Config::TypeUrl::get().Cluster, "42", {}, {}, {}));
  sendDiscoveryResponse<envoy::config::cluster::v3alpha::Cluster>(
      Config::TypeUrl::get().Cluster, {cluster2_}, {}, {ClusterName1}, "42");
  // We can continue the test once we're sure that Envoy's ClusterManager has made use of
  // the DiscoveryResponse that says cluster_1 is gone.
  test_server_->waitForCounterGe("cluster_manager.cluster_removed", 1);
...
TEST_P(CdsIntegrationTest, VersionsRememberedAfterReconnect) {
  ...
  sendDeltaDiscoveryResponse<envoy::config::cluster::v3alpha::Cluster>(
      Config::TypeUrl::get().Cluster, {cluster2_}, {}, "42");
  // The '3' includes the fake CDS server.
  test_server_->waitForGaugeGe("cluster_manager.active_clusters", 3);
