...
TEST_P(ServerInstanceImplTest, InvalidLayeredBootstrapMissingName) {
  EXPECT_THROW_WITH_REGEX(
      ...
      EnvoyException, "layered_runtime.layers.name: value length must be at least 1 characters");
}
...
TEST_P(ServerInstanceImplTest, InvalidLayeredBootstrapNoLayerSpecifier) {
  EXPECT_THROW_WITH_REGEX(
      ...
      EnvoyException,
      "layered_runtime.layers.layer_specifier: exactly one field is required in oneof");
}
...
TEST_P(ServerInstanceImplTest, BootstrapClusterHealthCheckInvalidTimeout) {
  EXPECT_THROW_WITH_REGEX(
      ...
      EnvoyException,
      "static_resources.clusters.health_checks.timeout: value must be greater than 0s");
}
...
TEST_P(ServerInstanceImplTest, BootstrapClusterHealthCheckInvalidInterval) {
  EXPECT_THROW_WITH_REGEX(
      ...
      EnvoyException,
      "static_resources.clusters.health_checks.interval: value must be greater than 0s");
}
