...
TEST_F(OverloadManagerImplTest, ResetStreamConfigPropagatesToAdapter) {
  const std::string config = R"EOF(
  actions:
  ...
  // Check threshold propagated
  EXPECT_EQ(overload_manager->resetStreamAdapter()->translateToBucketsToReset(
                OverloadActionState(UnitFloat(0.69))),
            absl::nullopt);
  EXPECT_NE(overload_manager->resetStreamAdapter()->translateToBucketsToReset(
                OverloadActionState(UnitFloat(0.70))),
            absl::nullopt);
  EXPECT_NE(overload_manager->resetStreamAdapter()->translateToBucketsToReset(
                OverloadActionState(UnitFloat(0.90))),
            absl::nullopt);
}

TEST_F(OverloadManagerImplTest, ResetStreamErrorsIfLowerLimitIsGreaterThanUpperLimit) {
...
TEST_F(OverloadManagerImplTest, ResetStreamErrorsIfLowerLimitEqualsUpperLimit) {
  const std::string equal_limit_config = R"EOF(
  actions:
    - name: envoy.overload_actions.reset_streams
      typed_config:
        "@type": type.googleapis.com/envoy.config.overload.v3.ResetStreamsOverloadActionConfig
  ...
  EXPECT_THROW_WITH_REGEX(createOverloadManager(equal_limit_config), EnvoyException,
                          ".* Expected upper_limit >= lower_limit.*");
}

TEST_F(OverloadManagerImplTest, ResetStreamErrorsIfWrongTypedConfigProvided) {
  const std::string config = R"EOF(
    actions:
      - name: "envoy.overload_actions.reset_streams"
        typed_config:
  ...
  EXPECT_THROW_WITH_REGEX(createOverloadManager(config), EnvoyException,
                          "Unable to unpack as .*ResetStreamsOverloadActionConfig");
}
