...
TEST_P(AdminStatsTest, HandlerStatsPlainText) {
  ...
  auto d = std::make_shared<Stats::MockCounterGroupDescriptor>();
  EXPECT_CALL(*d, size()).WillRepeatedly(testing::Return(2));
  EXPECT_CALL(*d, nameSuffix(0)).WillRepeatedly(testing::Return("foo"));
  EXPECT_CALL(*d, nameSuffix(1)).WillRepeatedly(testing::Return("bar"));

  Stats::CounterGroup& cg = store_->counterGroupFromString("cg", d);
