...
//       counters_, gauges_, histograms_, textReadouts_, response, StatsParams(),
//       custom_namespaces);
//   EXPECT_EQ(2UL, size);
// }
...
//   // Metric with invalid prometheus namespace in the custom metric must be excluded in the
//   output. addGauge("promtest.1234abcd.test.bar", {{makeStat("tag_name"),
//   makeStat("tag-value")}});
...
//     cumulativeStatistics()).WillOnce(ReturnRef(h1_cumulative_statistics));
