...
namespace Server {
  ...
  template <class T>
  StatsRequestTestBase<T>::StatsRequestTestBase() : pool_(symbol_table_), alloc_(symbol_table_), store_(alloc_) {
  ...
  template <class T>
  std::string StatsRequestTestBase<T>::response(T& request) {
    Http::TestResponseHeaderMapImpl response_headers;
    Http::Code code = request.start(response_headers);
  ...
  template class StatsRequestTestBase<PrometheusStatsRequest>;
  template class StatsRequestTestBase<StatsRequest>;
