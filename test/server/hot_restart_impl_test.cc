...
public:
  void setup() {
    ...
    hot_restart_ = std::make_unique<HotRestartImpl>(0, 0, "@envoy_domain_socket", S_IRWXU);
...
TEST_F(HotRestartImplTest, DomainSocketAlreadyInUse) {
  ...
  EXPECT_THROW(std::make_unique<HotRestartImpl>(0, 0, "@envoy_domain_socket", S_IRWXU),
