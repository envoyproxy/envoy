...
TEST_F(ConnectionHandlerTest, CloseDuringFilterChainCreate) {
  ...
  EXPECT_CALL(*dispatcher, createListener_(_, _, _, _, _, _, _, _))
      .WillOnce(Invoke([&](Network::ConnectionHandler&, Network::ListenSocket&,
                           Network::ListenerCallbacks& cb, Stats::Store&, bool, bool, bool, size_t)
                           -> Network::Listener* {
                             listener_callbacks = &cb;
                             return listener;

                           }));
  handler.addListener(factory, socket, true, false, false, 0);
