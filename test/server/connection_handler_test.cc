...
TEST_F(ConnectionHandlerTest, RemoveListener) {
  ...
  ON_CALL(*listener, socket()).WillByDefault(ReturnRef(socket_));
  ...
  handler_->addListener(factory_, socket_, stats_store_,
  ...
  handler_->removeListener(socket_);
...
TEST_F(ConnectionHandlerTest, FindListenerByAddress) {
  ...
  handler_->addListener(factory_, socket2, stats_store_,
                        Network::ListenerOptions::listenerOptionsWithBindToPort());
