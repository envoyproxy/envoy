...
public:
  ...
  public:
    TestListener(ConnectionHandlerTest& parent, uint64_t tag, bool bind_to_port,
                 bool enable_tcp_fast_open, bool hand_off_restored_destination_connections,
                 const std::string& name)
        : parent_(parent), tag_(tag), bind_to_port_(bind_to_port),
          enable_tcp_fast_open_(enable_tcp_fast_open),
  ...
  TestListener* addListener(uint64_t tag, bool bind_to_port, bool enable_tcp_fast_open,
                            bool hand_off_restored_destination_connections,
                            const std::string& name) {
    TestListener* listener = new TestListener(*this, tag, bind_to_port, enable_tcp_fast_open,
                                              hand_off_restored_destination_connections, name);
...
TEST_F(ConnectionHandlerTest, RemoveListener) {
  ...
  EXPECT_CALL(dispatcher_, createListener_(_, _, _, _, false))
...
TEST_F(ConnectionHandlerTest, FindListenerByAddress) {
  TestListener* test_listener1 = addListener(1, true, false, true, "test_listener1");
  Network::Address::InstanceConstSharedPtr alt_address(
      new Network::Address::Ipv4Instance("127.0.0.1", 10001));

  Network::MockListener* listener = new Network::MockListener();
  EXPECT_CALL(dispatcher_, createListener_(_, _, _, _, true))
