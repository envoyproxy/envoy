...
TEST_F(ConfigValidation, SharedDnsResolver) {
  ...
  Network::DnsResolverSharedPtr dns1 = dispatcher_->createDnsResolver(resolvers, false, false);
  long use_count = dns1.use_count();
  Network::DnsResolverSharedPtr dns2 = dispatcher_->createDnsResolver(resolvers, false, false);
