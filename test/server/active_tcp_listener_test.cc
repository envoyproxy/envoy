...
public:
  ...
  //     EXPECT_CALL(listener_config_,
  //     openConnections()).WillRepeatedly(ReturnRef(resource_limit_)); auto
  //     mock_listener_will_be_moved = std::unique_ptr<Network::MockListener>(); generic_listener_ =
  //     mock_listener_will_be_moved.get(); internal_listener_ =
  //     std::make_shared<ActiveInternalListener>(
  ...
  MockTcpConnectionHandler conn_handler_;
  ...
  // std::shared_ptr<ActiveTcpListener> active_listener_;
};

TEST_F(ActiveTcpListenerTest, RedirectedRebalancer) {
  Network::MockListenerConfig listener_config1;
  Network::MockConnectionBalancer balancer1;
  ...
  auto mock_listener_will_be_moved1 = std::unique_ptr<Network::MockListener>();
  auto listener1 = mock_listener_will_be_moved1.get();
  auto active_listener1 = std::make_shared<ActiveTcpListener>(
      conn_handler_, std::move(mock_listener_will_be_moved1), listener_config1);

  Network::MockListenerConfig listener_config2;
  Network::MockConnectionBalancer balancer2;
  EXPECT_CALL(listener_config1, connectionBalancer()).WillRepeatedly(ReturnRef(balancer2));
  auto mock_listener_will_be_moved2 = std::unique_ptr<Network::MockListener>();
  auto listener2 = mock_listener_will_be_moved1.get();
  ...
  // Listener1 rebalance. Set the balance target to the the active listener itself.
  EXPECT_CALL(balancer1, pickTargetHandler(_)).WillOnce(ReturnRef(*active_listener1));

  // Listener1 has one listener filter in the listener filter chain.
  ...
  // Listener2 rebalance. Set the balance target to the the active listener itself.
  EXPECT_CALL(balancer2, pickTargetHandler(_)).WillOnce(ReturnRef(*active_listener2));
