...
protected:
  GuardDogTestBase() : dispatcher_(time_system_) {}
...
protected:
  ...
  void SetupForDeath() {
    InSequence s;
    guard_dog_ =
        std::make_unique<GuardDogImpl>(fakestats_, config_kill_, time_system_, dispatcher_);
  ...
  void SetupForMultiDeath() {
    InSequence s;
    guard_dog_ =
        std::make_unique<GuardDogImpl>(fakestats_, config_multikill_, time_system_, dispatcher_);
...
TEST_F(GuardDogAlmostDeadTest, NearDeathTest) {
  ...
  InSequence s;
  GuardDogImpl gd(fakestats_, config_multikill_, time_system_, dispatcher_);
...
TEST_F(GuardDogMissTest, MissTest) {
  ...
  GuardDogImpl gd(stats_store_, config_miss_, time_system_, dispatcher_);
...
TEST_F(GuardDogMissTest, MegaMissTest) {
  // This test checks the actual collected statistics after doing some timer
  // advances that should and shouldn't increment the counters.
  GuardDogImpl gd(stats_store_, config_mega_, time_system_, dispatcher_);
  auto unpet_dog = gd.createWatchDog(0);
  // We'd better start at 0:
...
TEST_F(GuardDogTestBase, StartStopTest) {
  ...
  GuardDogImpl gd(stats, config, time_system_, dispatcher_);
}
