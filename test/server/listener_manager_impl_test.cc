...
public:
  ...
  findFilterChain(uint16_t destination_port, const uint8_t times,
    ...
    if (times > 2) {
      EXPECT_CALL(*socket_, remoteAddress()).Times(times - 2).WillRepeatedly(ReturnRef(address_));
    }
...
TEST_F(ListenerManagerImplWithRealFiltersTest, SslContext) {
  ...
  auto filter_chain = findFilterChain(1234, 3, "127.0.0.1", true, "", true, "tls", true, {});
...
TEST_F(ListenerManagerImplWithRealFiltersTest, SingleFilterChainWithDestinationPortMatch) {
  ...
  auto filter_chain = findFilterChain(1234, 1, "127.0.0.1", false, "", false, "tls", false, {});
  EXPECT_EQ(filter_chain, nullptr);

  // IPv4 client connects to valid port - using 1st filter chain.
  filter_chain = findFilterChain(8080, 3, "127.0.0.1", true, "", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  ...
  // UDS client - no match.
  filter_chain = findFilterChain(0, 1, "/tmp/test.sock", false, "", false, "tls", false, {});
  EXPECT_EQ(filter_chain, nullptr);
}
...
TEST_F(ListenerManagerImplWithRealFiltersTest, SingleFilterChainWithDestinationIPMatch) {
  ...
  auto filter_chain = findFilterChain(1234, 2, "1.2.3.4", false, "", false, "tls", false, {});
  EXPECT_EQ(filter_chain, nullptr);

  // IPv4 client connects to valid IP - using 1st filter chain.
  filter_chain = findFilterChain(1234, 3, "127.0.0.1", true, "", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  auto transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  auto ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
  ...
  // UDS client - no match.
  filter_chain = findFilterChain(0, 2, "/tmp/test.sock", false, "", false, "tls", false, {});
  EXPECT_EQ(filter_chain, nullptr);
}
...
TEST_F(ListenerManagerImplWithRealFiltersTest, SingleFilterChainWithServerNamesMatch) {
  ...
  auto filter_chain = findFilterChain(1234, 2, "127.0.0.1", true, "", false, "tls", false, {});
  EXPECT_EQ(filter_chain, nullptr);

  // TLS client without matching SNI - no match.
  filter_chain =
      findFilterChain(1234, 2, "127.0.0.1", true, "www.example.com", false, "tls", false, {});
  EXPECT_EQ(filter_chain, nullptr);

  // TLS client with matching SNI - using 1st filter chain.
  filter_chain =
      findFilterChain(1234, 3, "127.0.0.1", true, "server1.example.com", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  auto transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  auto ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
...
TEST_F(ListenerManagerImplWithRealFiltersTest, SingleFilterChainWithTransportProtocolMatch) {
  ...
  // TCP client - no match.
  auto filter_chain =
      findFilterChain(1234, 2, "127.0.0.1", true, "", true, "raw_buffer", false, {});
  EXPECT_EQ(filter_chain, nullptr);
...
TEST_F(ListenerManagerImplWithRealFiltersTest, SingleFilterChainWithApplicationProtocolMatch) {
  ...
  auto filter_chain = findFilterChain(1234, 2, "127.0.0.1", true, "", true, "tls", true, {});
  EXPECT_EQ(filter_chain, nullptr);

  // TLS client with "http/1.1" ALPN - using 1st filter chain.
  filter_chain =
      findFilterChain(1234, 3, "127.0.0.1", true, "", true, "tls", true, {"h2", "http/1.1"});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  auto transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  auto ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
...
TEST_F(ListenerManagerImplWithRealFiltersTest, MultipleFilterChainsWithDestinationPortMatch) {
  ...
  filter_chain = findFilterChain(8081, 3, "127.0.0.1", true, "", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
  ...
  filter_chain = findFilterChain(0, 3, "/tmp/test.sock", true, "", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
...
TEST_F(ListenerManagerImplWithRealFiltersTest, MultipleFilterChainsWithDestinationIPMatch) {
  ...
  filter_chain = findFilterChain(1234, 3, "192.168.0.1", true, "", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
  ...
  filter_chain = findFilterChain(1234, 3, "192.168.1.1", true, "", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
...
TEST_F(ListenerManagerImplWithRealFiltersTest, MultipleFilterChainsWithServerNamesMatch) {
  ...
  filter_chain =
      findFilterChain(1234, 3, "127.0.0.1", true, "server2.example.com", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
  ...
  // TLS client with wildcard SNI match - using 3nd filter chain.
  filter_chain =
      findFilterChain(1234, 3, "127.0.0.1", true, "www.wildcard.com", true, "tls", true, {});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
  ssl_socket = dynamic_cast<Ssl::SslSocket*>(transport_socket.get());
...
TEST_F(ListenerManagerImplWithRealFiltersTest, MultipleFilterChainsWithTransportProtocolMatch) {
  ...
  auto filter_chain = findFilterChain(1234, 3, "127.0.0.1", true, "", true, "raw_buffer", true, {});
  ASSERT_NE(filter_chain, nullptr);
...
TEST_F(ListenerManagerImplWithRealFiltersTest, MultipleFilterChainsWithMultipleRequirementsMatch) {
  ...
  filter_chain =
      findFilterChain(1234, 2, "127.0.0.1", true, "server1.example.com", true, "tls", true, {});
  EXPECT_EQ(filter_chain, nullptr);
  ...
  filter_chain = findFilterChain(1234, 3, "127.0.0.1", true, "server1.example.com", true, "tls",
                                 true, {"h2", "http/1.1"});
  ASSERT_NE(filter_chain, nullptr);
  EXPECT_TRUE(filter_chain->transportSocketFactory().implementsSecureTransport());
  auto transport_socket = filter_chain->transportSocketFactory().createTransportSocket();
