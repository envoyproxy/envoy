...
// Regression test(issue #22678) where using server's initialized state init manager to add init
// target by the secret manager led to the assertion failure.
TEST_P(ListenerManagerImplWithRealFiltersTest, InitializedServerInitManagerAndOauthFilterWithSDS) {
  const std::string yaml = R"EOF(
name: listener
address:
  socket_address:
...
- filters:
  ...
  server_.server_factory_context_->cluster_manager_.initializeClusters(
      {"upstream_cluster", "oauth2_cluster"}, {});
  server_.cluster_manager_.initializeClusters({"upstream_cluster", "oauth2_cluster"}, {});
  std::function<void()> ready_func = []() {};
  envoy::config::core::v3::ConfigSource config_source;
  use_matcher_ = false;
  sds_api_ = std::make_unique<Secret::GenericSecretSdsApi>(
      config_source, "abc.com", subscription_factory_, time_system_, validation_visitor_, stats_,
      []() {}, dispatcher_, *api_);

  // set the server's init manager state as "initialized"
  Init::WatcherImpl watcher("test-watcher", ready_func);
  server_.transport_socket_factory_context_->init_manager_.initialize(watcher);

  EXPECT_EQ(server_.transport_socket_factory_context_->init_manager_.state(),
            Init::Manager::State::Initialized);
  EXPECT_CALL(server_.cluster_manager_.cluster_manager_factory_.secretManager(),
              findOrCreateGenericSecretProvider(_, _, _, _))
      .WillRepeatedly(Invoke(
          [this](const envoy::config::core::v3::ConfigSource& sds_config_source,
                 const std::string& config_name,
                 Server::Configuration::TransportSocketFactoryContext& secret_provider_context,
                 Init::Manager& init_manager) -> GenericSecretConfigProviderSharedPtr {
            return helperFindOrCreateGenericSecretProvider(sds_config_source, config_name,
