...
TEST_F(DrainManagerImplTest, Default) {
  InSequence s;
  DrainManagerImpl drain_manager(server_, envoy::api::v3alpha::Listener::DEFAULT);
...
TEST_F(DrainManagerImplTest, ModifyOnly) {
  InSequence s;
  DrainManagerImpl drain_manager(server_, envoy::api::v3alpha::Listener::MODIFY_ONLY);
