...
Envoy::EnvoyStreamCallbacks makeLoggingStreamCallbacks(
  envoy_status_t* status, absl::Notification* request_finished, std::vector<Envoy::Http::Protocol>* used_protocols) {
  Envoy::EnvoyStreamCallbacks stream_callbacks;
  stream_callbacks.on_headers_ = [](const Envoy::Http::ResponseHeaderMap& headers, bool /* end_stream */,
                                    envoy_stream_intel intel) {
    std::cerr << "Received headers on connection: " << intel.connection_id << "with headers:\n"
              << headers << "\n";
  };
  stream_callbacks.on_data_ = [](const Envoy::Buffer::Instance& buffer, uint64_t length, bool end_stream,
                                 envoy_stream_intel) {
  ...
  stream_callbacks.on_complete_ =
      [request_finished, used_protocols](envoy_stream_intel, envoy_final_stream_intel final_intel) {
...
int main(int argc, char** argv) {
  ...
  Envoy::Platform::EngineSharedPtr engine = Envoy::Platform::EngineBuilder()
