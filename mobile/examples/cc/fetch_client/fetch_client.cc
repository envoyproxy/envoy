...
void Fetch::fetch() {
  Thread::ThreadPtr envoy_thread = api_->threadFactory().createThread([this]() -> void { runEngine(); });
...
void Fetch::sendRequest(const absl::string_view url_string) {
  ...
  stream_prototype_->setOnComplete(
	...
	std::cerr << "Request finished after " << final_intel.stream_end_ms - final_intel.stream_start_ms << "ms\n";
	request_finished.Notify();
      });
  stream_prototype_->setOnError(
      [&request_finished](Platform::EnvoyErrorSharedPtr, envoy_stream_intel, envoy_final_stream_intel final_intel) {
	std::cerr << "Request failed after " << final_intel.stream_end_ms - final_intel.stream_start_ms << "ms\n";
	request_finished.Notify();
      });
  stream_prototype_->setOnCancel([&request_finished](envoy_stream_intel, envoy_final_stream_intel final_intel) {
    std::cerr << "Request cancelled after " << final_intel.stream_end_ms - final_intel.stream_start_ms << "ms\n";
    request_finished.Notify();
  });
