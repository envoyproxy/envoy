...
extension EnvoyHTTPFilterFactory {
  ...
  func register() {
    ...
    filter.pointee.init_filter = { context in
      ...
      let filterFactory = Unmanaged<EnvoyHTTPFilterFactory>
        .fromOpaque(c_filter.static_context)
        .takeUnretainedValue()
    ...
    filter.pointee.on_request_headers = { headers, end_stream, stream_intel, context in
      let filter = Unmanaged<EnvoyHTTPFilter>.fromOpaque(context!).takeUnretainedValue()
    ...
    filter.pointee.release_filter = { context in
      Unmanaged<EnvoyHTTPFilter>.fromOpaque(context!).release()
    }

    filter.pointee.static_context = UnsafeRawPointer(Unmanaged.passRetained(self).toOpaque())
