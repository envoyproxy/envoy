...
// Encapsulates logic responsible for resolving proxies using currently active system proxy
// settings.
...
// @param completion: A closure that's called in an async manner for cases when a proxy resolution
// cannot be
//                    performed in a sync manner. The method returns
//                    `ENVOY_PROXY_RESOLUTION_RESULT_IN_PROGRESS` to let the caller know that the
//                    resolution result will be delivered in an async manner.
