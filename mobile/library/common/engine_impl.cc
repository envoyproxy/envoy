...
envoy_stream_t EngineImpl::initStream() { return current_stream_handle_++; }

EngineImpl::EngineImpl(envoy_engine_callbacks callbacks, envoy_logger logger,
...
envoy_status_t EngineImpl::run(const std::string& config, const std::string& log_level) {
...
envoy_status_t EngineImpl::run(std::unique_ptr<Envoy::OptionsImplBase>&& options) {
  main_thread_ = std::thread(&EngineImpl::main, this, std::move(options));
  return ENVOY_SUCCESS;
}

envoy_status_t EngineImpl::main(std::unique_ptr<Envoy::OptionsImplBase>&& options) {
...
envoy_status_t EngineImpl::terminate() {
  if (terminated_) {
...
bool EngineImpl::isTerminated() const { return terminated_; }
...
envoy_status_t EngineImpl::setProxySettings(const char* hostname, const uint16_t port) {
...
envoy_status_t EngineImpl::resetConnectivityState() {
  return dispatcher_->post([&]() -> void { connectivity_manager_->resetConnectivityState(); });
}

envoy_status_t EngineImpl::setPreferredNetwork(envoy_network_t network) {
...
envoy_status_t EngineImpl::recordCounterInc(absl::string_view elements, envoy_stats_tags tags,
                                            uint64_t count) {
...
Event::ProvisionalDispatcher& EngineImpl::dispatcher() { return *dispatcher_; }
...
std::string EngineImpl::dumpStats() {
  if (!main_thread_.joinable()) {
    return "";
  }
