...
void register_test_apple_proxy_resolver(absl::string_view host, int port) {
  ...
  // Create a TestAppleSystemProxySettingsMonitor and set the test resolver to use the TestAppleSystemProxySettingsMonitor.
  test_resolver->setSettingsMonitorForTest(std::make_unique<Envoy::test::TestAppleSystemProxySettingsMonitor>(
  ...
  auto* context = static_cast<Envoy::Network::envoy_proxy_resolver_context*>(proxy_resolver->context);
