...
TEST_F(MainInterfaceTest, BasicStream) {
  ...
  std::unique_ptr<Envoy::EngineImpl> engine(new Envoy::EngineImpl(engine_cbs, {}, {}));
...
TEST(EngineTest, Logger) {
  ...
  envoy_logger logger{[](envoy_data data, const void* context) -> void {
                      ...
                      &test_context};
  std::unique_ptr<Envoy::EngineImpl> engine(new Envoy::EngineImpl(engine_cbs, logger, {}));
  engine->run(MINIMAL_TEST_CONFIG.c_str(), LEVEL_DEBUG.c_str());
  ASSERT_TRUE(test_context.on_engine_running.WaitForNotificationWithTimeout(absl::Seconds(3)));
...
TEST(EngineTest, EventTrackerRegistersAPI) {
  ...
  std::unique_ptr<Envoy::EngineImpl> engine(new Envoy::EngineImpl(engine_cbs, {}, event_tracker));
  engine->run(MINIMAL_TEST_CONFIG.c_str(), LEVEL_DEBUG.c_str());
