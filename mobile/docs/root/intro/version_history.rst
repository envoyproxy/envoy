...
- api: The ``enableGzip`` and ``enableBrotli`` APIs were renamed to ``enableGzipDecompression`` and ``enableBrotliDecompression``.
...
- api: Add support for brotli and gzip request compression.
